...
// cells using parameters from the parameter containers MdcCalPar for the
// calibration parameters and from MdcLookupGeom to map the layer number and
// cell number on Cal1 level to the motherboard number and TDC/TDC channel
// number on Raw level.
  ...
  while ((raw=(HMdcRaw *)iter->Next())!=0) {
    ...
    if (loc[3]>=0) {
      ...
      if (cal) {
        ...
        cal->setTime1(((Float_t)(raw->getTime(1)) + gRandom->Rndm() - 0.5F)
                   * cell.getSlope() + cell.getOffset());
        if (nHits==-2 || nHits==+2)
           cal->setTime2(((Float_t)(raw->getTime(2)) + gRandom->Rndm() - 0.5F)
                   * cell.getSlope() + cell.getOffset());
      }
    } else {
