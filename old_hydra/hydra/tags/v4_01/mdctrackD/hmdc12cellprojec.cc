...
Bool_t HMdcCalc12CellProjec::calcMdc12Proj(Int_t nSec, 
  ...
  if( nT < 0 ) {
   Printf("Error:  Number of targets = %i !!!!",nT);
   return kFALSE;
  }
  Double_t zF, zL;
...
//  It's the plane bitwin MDC0 and MAC1
//  par=0.0 -> plane=layer5_MDC0
//  par=1.0 -> plane=layer0_MDC1
  ...
  HGeomCompositeVolume* pComVol=fGeomPar->getRefVolume(0); //MDC0
  if(!pComVol) {
   printf("Error: GeomCompositeVolume for MDC1 is absenst!!!\n");
   return kFALSE;
  }
  HGeomVolume* pGeomVol=pComVol->getComponent(5);          // Layer 5
  if(!pGeomVol) {
    printf("Error: GeomCompositeVolume for Layer5 is absenst!!!\n");
    return kFALSE;
  }
  coordSys.setTransform(pGeomVol->getTransform());      //From layer to modul
  coordSys.transFrom(pComVol->getTransform());          //From layer to sector
  Double_t tmp=(coordSys.getRotMatrix())(8);  tmp=1./(tmp*tmp);
  Double_t tanTheta1=sqrt(tmp-1.);
  Double_t zMdc1=(coordSys.getTransVector()).getZ()*tmp;
  
  pComVol=fGeomPar->getRefVolume(1); //MDC1
  if(!pComVol) {
   printf("Error: GeomCompositeVolume for MDC1 is absenst!!!\n");
   return kFALSE;
  }
  pGeomVol=pComVol->getComponent(0); // Layer 0
  if(!pGeomVol) {
    printf("Error: GeomCompositeVolume for Layer0 is absenst!!!\n");
    return kFALSE;
  }
  coordSys.setTransform(pGeomVol->getTransform());   //From layer to modul
  coordSys.transFrom(pComVol->getTransform());       //From layer to sector
  tmp=(coordSys.getRotMatrix())(8);  tmp=1./(tmp*tmp);
  Double_t tanTheta2=sqrt(tmp-1.);
  Double_t zMdc2=(coordSys.getTransVector()).getZ()*tmp;
  ...
  Double_t newRot[9];
  newRot[0]=1; newRot[1]=newRot[2]=newRot[3]=newRot[6]=0;
