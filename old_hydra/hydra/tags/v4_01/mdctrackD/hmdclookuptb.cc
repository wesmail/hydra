...
void HMdcLookUpTbCell::addLine(Int_t nc1, Int_t nc2){
 ...
 else printf("Error in HMdcLookUpTbCell::addLine : num. of lines > %i\n", line);
}
...
void HMdcLookUpTbLayer::initCell(Int_t nce, Int_t yBinMinT, Int_t yBinMaxT){
  if((*array)[nce]) {
   delete (*array)[nce];
   printf("Delete old obj. and create new obj. HMdcLookUpTbCell: n.cell=%i\n",nce);
  }
...
//=======================================================
  ...
  closeBins[0]=-1; closeBins[1]=+1; closeBins[2]=-nBinX; closeBins[3]=+nBinX;
  closeBins[4]=-1-nBinX; closeBins[5]=-1+nBinX; closeBins[6]=1-nBinX; closeBins[7]=1+nBinX;
}
...
void HMdcLookUpTbSec::setParPlane(HGeomTransform &sys){
...
void HMdcLookUpTbSec::setCell(Short_t mod, Short_t layer, 
 ...
 if(mod<2) {
  ...
  if(pLayer.isCell(cell)) {
   ...
   else Printf("Error: Very many hits in layer !!! It's imposible!.");
  }
  else Printf("Cell %i don't exist !!!!",cell);
 }
}
...
//  if(hitplot[nc]>0 && testBiN[hitplot[nc]]>=max) {
  ...
  max=maxLocal;
  if(hitplot[nc]>0 && tablBiN[hitplot[nc]]>=max) {       // For bimtime 9.99 only (I hope)!!!
  //---New cluster---------------------------------------
...
//    Double_t wt=(Double_t)(testBiN[hitplot[nci]]-max+1);
    Double_t wt=(Double_t)(tablBiN[-hitplot[nci]]-max+1); // For bimtime 9.99 only
...
//      if(testBiN[hitplot[nBtmp]]>=max) {
      if(tablBiN[hitplot[nBtmp]]>=max) {  // For bimtime 9.99 only
       stack[stackB++]=nBtmp;
       hitplot[nBtmp]*=-1;
      }
     } 
   ...
   clst.setXmid(xmid);
   clst.setYmid(ymid);
   clst.setZmid(Dp-Ap*xmid-Bp*ymid);
   ...
   clst.setErrXmid(dXmid);
   clst.setErrYmid(dYmid);
   clst.setErrZmid( Bp*dYmid );
    ...
    for(Int_t layer=0; layer<nla[mod]; layer++) {   
     ...
     for(Int_t i=0; i<nHits[iLayer]; i++) {
      if(hitsDel[iLayer][i]>-1) {
       ...
       if( nLmax < yBinMin ) break;
      
       Int_t lMin, lMax;
       yBinMax < nLmax ? lMax=yBinMax : lMax=nLmax;
       yBinMin > nLmin ? lMin=yBinMin : lMin=nLmin;
       Bool_t indSetCell=kFALSE;
//Printf("Cell=%i lMin=%i lMax=%i",cell,lMin,lMax);
       for (Int_t nl=lMin; nl<=lMax; nl++) {
        Int_t nLind=nLmax-nl;
//Printf("    nl=%i cell=%i-%i  clust=%i-%i",nl,pCell.xBinMin[nl-yBinMin],pCell.xBinMax[nl-yBinMin],
//                               xMinCl[nLind],xMaxCl[nLind]);
...
// nClusters=clusters.Test(Ap,Bp,Dp);
  ...
  for(Int_t nClS=nClF+1; nClS<nClusters; nClS++) { 
   ...
   if(fabs(pClF.getYmid()-pClS.getYmid()) >= 1.) break;
   if(fabs(pClF.getXmid()-pClS.getXmid()) >= 3.) break;
...
//!!!!!!!!!!!!!!!!     
     if(fabs(pClT.getYmid()-pClF.getYmid()) >= 1.) break;
     if(fabs(pClF.getXmid()-pClS.getXmid()) >= 3.) break;
