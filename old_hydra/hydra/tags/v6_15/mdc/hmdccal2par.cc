...
Bool_t HMdcCal2Par::init(HParIo* inp) {
  // intitializes the container from an input
    ...
    {
      INFO_msg(10,DET_MDC,"HMdcCal2Par initialized from HMdcCal2ParSim for modules ");
      char *buf;
      buf = (char *)calloc(1,200);
      strcpy(buf,"\n");
	for(Int_t s=0; s<6; s++)
	{  //loop over sectors
	    for(Int_t m=0; m<4; m++)
	...
	for(Int_t m=0;m<sec.getSize();m++) {
		...
		for(Int_t mytype=0;mytype<2;mytype++){
		    for(Int_t myline=0;myline<10;myline++){
		        gHades->getMsg()->info(10,DET_MDC,GetName(),"%1i %1i %2i %1i",s, m, l, mytype);
			switch (mytype){
			case 0:
