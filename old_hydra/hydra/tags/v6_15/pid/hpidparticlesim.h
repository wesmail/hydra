// @(#)$Id: hpidparticlesim.h,v 1.7 2003-11-21 14:53:05 halo Exp $
...
public:
    ...
    inline Char_t          getTrackDetectors(UInt_t iPos = 0) const;
    inline UInt_t          getTrackProcess(UInt_t iPos = 0) const;
    inline UInt_t          getTrackMask(UInt_t iPos = 0) const;

    inline Short_t         getGeantTrack(UInt_t iPos = 0) const;
    inline Short_t         getTrackGeant(UInt_t iPos = 0) const;

    inline Short_t         getGeantId(UInt_t iPos = 0) const;
    inline Short_t         getTrackPid(UInt_t iPos = 0) const;

    inline Double_t        getTrackMomX(UInt_t iPos = 0) const;
    inline Double_t        getTrackMomY(UInt_t iPos = 0) const;
    inline Double_t        getTrackMomZ(UInt_t iPos = 0) const;

    inline const TVector3* getTrackMom(UInt_t iPos = 0) const;
    inline TLorentzVector  getTrackLorentzVector(UInt_t iPos = 0) const;

protected:

    void                setDefault(void);
    inline HPidCommonTrack*  getTrackFullAccess(UInt_t iPos = 0);
...
const HPidCommonTrack *p;
    ...
    return ((p = getTrack(iPos)) != NULL) ? p->getGeantId() : 0;
}
...
inline TLorentzVector HPidParticleSim::getTrackLorentzVector(UInt_t iPos) const
{
const HPidCommonTrack *p;

    if((p = getTrack(iPos)) != NULL)
        return p->getLorentzVector();
