  ...
  HPidTofRec::HPidTofRec()
{
  ...
  for (Int_t nAlg=0; nAlg<MAX_MOM_ALGS; nAlg++) // switch on all the tracking algorythms
...
Bool_t HPidTofRec::init(void){
  ...
  if (bPidParam) {
	...
	{
		Error("HPidTofRec::init()", "Cannot get parameters: %s",
			PIDALGSTANDCUTSPAR_NAME);
		return kFALSE;
	}
	if (bTrackAlg[ALG_RUNGEKUTTA]) {
		pidalg = ALG_RUNGEKUTTA;
	} else if (bTrackAlg[ALG_SPLINE]) {
		pidalg = ALG_SPLINE;
	} else if (bTrackAlg[ALG_REFT])  {
		pidalg = ALG_REFT;
	} else if (bTrackAlg[ALG_KICK123]) {
		pidalg = ALG_KICK123;
	} else if (bTrackAlg[ALG_KICK])  {
		pidalg = ALG_KICK;
	}
	if (-1111==pidalg) {
	...
	richParam->setContext(pidalg);
      ...
      cout << fRichQparams[2] << " RingAverageCharge " << fRichQparams[3] << endl;
	} // ! bPidParam
    }
...
Int_t HPidTofRec::execute(void){
  ...
  Float_t fStartTime[MAX_MOM_ALGS], fRecTof[MAX_MOM_ALGS], fRecStart[MAX_MOM_ALGS];
  Short_t nIndex[MAX_MOM_ALGS], nRecSys[MAX_MOM_ALGS],nRecPol[MAX_MOM_ALGS], nRecSec[MAX_MOM_ALGS], nRecIndex[MAX_MOM_ALGS] ;
	...
	if (bTrackAlg[nAlg]) // selection of algorythm activated for reconstruction
	  {
		...
		if (// positrons
			((pCand->getHitData()->nRingPatMat
				>richParam->getValue(2, 0, mdc_sec, 0))&&
			(pCand->getHitData()->fRingCentroid
				>=richParam->getValue(2, 0, mdc_sec, 1))&&
			(pCand->getHitData()->nRingPadNr
				>=richParam->getValue(2, 0, mdc_sec, 2))&&
			(pCand->getHitData()->nRingPatMat/pCand->getHitData()->nRingPadNr
				>=richParam->getValue(2, 0, mdc_sec, 3)))
#ifdef THIS_IS_BEAUTIFUL_BUT_OBSOLETE // positrons and electrons have same thr
		|| // electrons
			((pCand->getHitData()->nRingPatMat
				>richParam->getValue(3, 0, mdc_sec, 0))&&
			(pCand->getHitData()->fRingCentroid
				>=richParam->getValue(3, 0, mdc_sec, 1))&&
			(pCand->getHitData()->nRingPadNr
				>=richParam->getValue(3, 0, mdc_sec, 2))&&
			(pCand->getHitData()->nRingPatMat/pCand->getHitData()->nRingPadNr
				>=richParam->getValue(3, 0, mdc_sec, 3)))
