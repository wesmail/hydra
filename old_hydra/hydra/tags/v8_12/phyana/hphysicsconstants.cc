...
const int nPart = 53;  // number of particles stored permanently

// particle naming convention (adopted from Pluto++)
const char *cName[nPart] = {
...
double *HPhysicsConstants::PMass=(double*)fMass;
int *HPhysicsConstants::PCharge=(int*)fCharge;
char **HPhysicsConstants::PName = (char**)cName;

Int_t HPhysicsConstants::pid(const char *n) {
// returns integer id given a particle's name

  Int_t i;
  if (!n) return 0;
  for(i=0; i<nPart && strcmp(n,(char*)cName[i]) ; i++) { }
  if (i==nPart) {
...
const char *HPhysicsConstants::pid(const Int_t &id) {
  if (id>0 && id<nPart) return PName[id];
  else return PName[0];
}

Int_t HPhysicsConstants::charge(const int &id) {
  if (id>0 && id<nPart) return PCharge[id];
  else return -100;
}

double HPhysicsConstants::mass(const int &id) {
