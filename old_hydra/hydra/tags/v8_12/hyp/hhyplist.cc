...
{
#warning "Join darf nur im alleresrten Schritt direkt nach dem Filler benutzt werden!!!!!"
...
//  cout << "join UserValues:" << endl;
  ...
  for(Int_t i=0; ; i++){
    ...
    Float_t value;
    if(!addme->ReadNumberedUserValues( i, my_iter, valueID, value)) break;
...
Bool_t HHypList::ReadNumberedUserValues(Int_t i,Int_t &my_iter,Int_t &valueID,Float_t &value)
{
  // now the arrayed vars
  if( i<0 || i>=nvaluealg) return kFALSE;
...
Int_t HHypList::getOrder(void)
{
  ...
  for (int i = 0; i < p_hypcomb->getNcomb(); i++) {
    if (getProbAlg(i) > getProbAlg()) better++;
  }
  return better+1;
}
...
// i cannot check here if it is still in use by another analysis branch
      ...
      Error("HHypList::setMomentum","Please use initcopyMomentum BEFORE calling any setMomentum. Please update Code from CVS!");
      exit(0);
/*    
    HHypKine *HypKine = NULL;
...
//-------------------------------------------------------------------------
Float_t HHypList::getBeta(char *name, Int_t num)
{
...
// use while debuging to print HHypComb table with indexes and pids
  ...
  cout << "Index to PidPart and ProbComb and ProbAlg\n";                 // Idx table
  ...
  for (Int_t i = 0; i < getNcomb(); i++) {
    ...
    Float_t mpid;
    if (!getUserValue(FILLER_MISSING_PID, mpid,i)) mpid = -999;
    cout << "\t"<< mpid<<"\n";
  }
  cout << "\n";
  return kTRUE;
