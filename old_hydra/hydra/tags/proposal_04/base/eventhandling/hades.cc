...
Bool_t Hades::checkDatabase(void)
{
	// Check if an open ORACLE connection can be closed.
	// This is possible if:
	// first or second parameter inputs are HParOraIo,
	// and current file is last file in list of files.

	Bool_t isOracle=kFALSE;
	Int_t nFiles1=0, nFiles2=0;
	TString file1,file2;
	Bool_t isLastFile1 = kFALSE, isLastFile2 = kFALSE;

	if(enableCloseInput)
		{
			isOracle = ((rtdb->getFirstInput()  && (rtdb->getFirstInput() ->InheritsFrom("HParOraIo"))) ||
									(rtdb->getSecondInput() && (rtdb->getSecondInput()->InheritsFrom("HParOraIo"))));
			if(isOracle)
				{
					if(fDataSource)
						{
							if(fDataSource->InheritsFrom("HldFileSource"))
								{
									nFiles1 = ((HldFileSource *)fDataSource)->getListOfFiles()->GetSize();
									file1 = ((HldFileSource *)fDataSource)->getCurrentFileName();
									file2 = ((TNamed *)((HldFileSource *)fDataSource)->getListOfFiles()->At(nFiles1-1))->GetName();
									if(file1.CompareTo(file2.Data()) == 0)
										{
											isLastFile1 = kTRUE;
										}
								}
							// Currently deactivated. A ROOT file could contain events
							// from different runs!
// 							else
// 								{
// 									if(fDataSource->InheritsFrom("HRootSource"))
// 										{
// 											nFiles1 = ((TChain *)((HRootSource *)fDataSource)->getTree())->GetListOfFiles()->GetEntries();
// 											file1 = ((HRootSource *)fDataSource)->getCurrentFileName();
// 											file2 = ((TNamed *)((TChain *)((HRootSource *)fDataSource)->getTree())->GetListOfFiles()->At(nFiles1-1))->GetName();
...
// 								}
					...
					if(fSecondDataSource)
						{
							if(fSecondDataSource->InheritsFrom("HldFileSource"))
								{
									nFiles2 = ((HldFileSource *)fSecondDataSource)->getListOfFiles()->GetSize();
									file1 = ((HldFileSource *)fDataSource)->getCurrentFileName();
									file2 = ((TNamed *)((HldFileSource *)fDataSource)->getListOfFiles()->At(nFiles2-1))->GetName();
									if(file1.CompareTo(file2.Data()) == 0)
										{
											isLastFile2 = kTRUE;
										}
...
// 							else
// 								{
// 									if(fSecondDataSource->InheritsFrom("HRootSource"))
// 										{
// 											nFiles2 = ((TChain *)((HRootSource *)fSecondDataSource)->getTree())->GetListOfFiles()->GetEntries();
// 											file1 = ((HRootSource *)fSecondDataSource)->getCurrentFileName();
// 											file2 = ((TNamed *)((TChain *)((HRootSource *)fSecondDataSource)->getTree())->GetListOfFiles()->At(nFiles2-1))->GetName();
// 											if(file1.CompareTo(file2) == 0)
// 												{
// 													isLastFile2 = kTRUE;
// 												}
...
// 								}
						...
						isLastFile2 = kTRUE;
					return isLastFile1 & isLastFile2;
				}
		}
	return kFALSE;
}

Bool_t Hades::closeDatabase(void)
{
	// Close the connection to ORACLE.

	if((rtdb->getFirstInput() && rtdb->getFirstInput()->InheritsFrom("HParOraIo")))
		{
			rtdb->closeFirstInput();
			return kTRUE;
		}
	if((rtdb->getSecondInput() && rtdb->getSecondInput()->InheritsFrom("HParOraIo")))
		{
...
Int_t Hades::eventLoop(Int_t nEvents, Int_t firstEvent, Double_t timeQuota) {
  ...
  gDebuger->enterFunc("Hades::eventLoop");
	...
	while ((evN<lastEvent) && (dsCode!=kDsEndData) && (dsCode!=kDsError) 
		...
		// close ORACLE connection if last file
		if(checkDatabase())
			if(closeDatabase())
				{
				}
		...
		if (initOk) {
			...
			while ((evN<lastEvent) && timerFlag) {
				...
				if(fCounter && (evN%fCounter == 0) && !quiet) {
					dt.Set();
					msgHandler->info(10,HMessageMgr::DET_ALL,GetName()," %s  Events processed: %i",dt.AsSQLString() + 11, evN-firstEvent);
				}
