// @(#)$Id: hpidphysicsconstants.cc,v 1.2 2002-11-04 02:46:13 jaskula Exp $
//*-- Author : Dan Magestro
//*-- Created: 03/09/01
...
// Return name of the particle
    ...
    if(pid >= ARTIFICAL_PID_OFFSET)
    {
        if(pid < ARTIFICAL_PID_OFFSET + nArtPidPart)
            return cArtPidName[pid - ARTIFICAL_PID_OFFSET];
    }
    else
    {
...
// Return charge of the particle
    ...
    {
        if(pid < ARTIFICAL_PID_OFFSET + nArtPidPart)
            return fArtPidCharge[pid - ARTIFICAL_PID_OFFSET];
    }
    else
    {
        if((pid >= 0) && (pid < nPidPart))
...
// Return mass of the particle
    ...
    {
        if(pid < ARTIFICAL_PID_OFFSET + nArtPidPart)
            return fArtPidMass[pid - ARTIFICAL_PID_OFFSET];
    }
    else
    {
        if((pid >= 0) && (pid < nPidPart))
