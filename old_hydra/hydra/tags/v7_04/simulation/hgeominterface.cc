...
//*-- Modified : 08/11/2004 by Ilse Koenig
...
void HGeomInterface::addInputFile(const char* file) {
  ...
  if (setName.Contains("media")) {
    ...
    } else if (setName.Contains("fwall")) {
...
HGeomSet* HGeomInterface::findSet(const char* name) {
  ...
  else if (setName.Contains("fwall"))  pSet=(HGeomSet*)sets->At(kHGeomFWall);
...
Bool_t HGeomInterface::writeSet(HGeomSet* pSet, const char* filename) {
...
Bool_t HGeomInterface::readMedia() {
  // Reads the media
  Bool_t rc=kFALSE;
  HGeomIo* input=input=connectInput(media->getInputFile());
  if (input) {
    rc=input->read(media);
  }
  return rc;  
...
Bool_t HGeomInterface::writeMedia(const char* filename) {
...
Bool_t HGeomInterface::readAll() {
  ...
  for(Int_t i=0;i<nSets&&rc;i++) {
    ...
    if (pSet) {
      ...
      rc=readSet(pSet);
      if (rc&&i==kHGeomTof) ((HGeomTof*)pSet)->replaceTofKeepin();
    }
  }
  return rc;
}
...
Bool_t HGeomInterface::createGeometry(Bool_t withCleanup) {
  ...
  if (geoBuilder) {
	...
	cout<<"Read and create "<<pSet->GetName()<<endl;
        rc=readSet(pSet);
        if (rc) {
          if (i==kHGeomTof) ((HGeomTof*)pSet)->replaceTofKeepin();
          rc=pSet->create(geoBuilder);
          n++;
        }
        if (withCleanup) {
...
HGeomIo* HGeomInterface::connectInput(const char* filename) {
...
Bool_t HGeomInterface::connectOutput (const char* name) {
...
Bool_t HGeomInterface::readGeomConfig(const char* configFile) {
