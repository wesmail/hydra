...
// Modified by $Author: zumbruch $ on $Date: 2005-01-14 16:12:05 $  
////////////////////////////////////////////////////////////////////////////////
...
void PTools::setTitleArts( TH1* histogramm, const Char_t *xtitle, const Char_t *ytitle, TString color, int type)
{
...
void PTools::setTitleArts( TH1* histogramm, const TString xtitle, const TString ytitle, TString color, int type)
{
  // sets titles of x axis, y axis, fill color and font type for histogramm
  // with colors "black","red","bright green","bright blue","yellow","hot pink","aqua","green","blue","default"
...
void PTools::plotPolarGrid(Int_t division)
{
  ...
  TEllipse *kDeg2; 
  {
    for (int iter = 1; iter <= division; iter++) 
      {
...
void PTools::setLegendEntry(TLegend* legend,TObject* object,TString label,Int_t col,TString opt)
{
...
TLatex* PTools::plotTopLegend(char* label,Float_t x,Float_t y,Float_t size,Int_t color)
{
  // Thanks to Jochen Markert, GSI
    // coordinates in NDC!
    if(x<0||y<0)
  ...
  for (binz=0;binz<=nbinsz+1;binz++)
      ...
      for (biny=0;biny<=nbinsy+1;biny++)
	{
	  ...
	  for (binx=0;binx<=nbinsx+1;binx++)
		...
		{
		  Error("histogram %s contains negative values, cannot perform sqrt() operation ... exiting",
			hist->GetName());
		  exit(EXIT_FAILURE);
		}
	    }
...
Bool_t PTools::areHistogramsCompatible(TH1* h1, TH1 *h2)
{
  ...
  int val = checkHistogramCompatibility(h1, h2);
...
Bool_t PTools::areArraysCompatible(const TArray* array1, const TArray* array2)
{
  ...
  int arraySize1 = array1->GetSize();
  int arraySize2 = array2->GetSize();
...
Bool_t  PTools::loadMacro(const char *macro)
{
			      ...
			      Int_t x5, UInt_t x5max)
{
  ...
  unsigned int dim = 5;
  for (unsigned int iter = 1; iter < dim; iter++)
    {
      if (x[iter] < 0 )
	{
	  dim =iter;

	  for (unsigned fiter = iter-1; fiter < dim-1; fiter++)
	    {
	      x[iter] = 0;
	      xmax[fiter]=0;	      
	    }
  ...
  for (unsigned int iter = 1; iter < dim; iter++)
    {
      add = 1;
      for (unsigned int iter2 = 0; iter2 < iter; iter2++)
	{	
	  add *= xmax[iter2];
	}
      index += x[iter]*add;
...
//   for (unsigned int ai = dim-1; ai > 1 ; ai--)
//     {
...
TLatex * PTools::drawLatexWC(const char* text, Double_t x, Double_t y, 
			   Int_t color, Float_t size, Int_t align, Int_t font, Float_t angle)
{
  // Draws text as TLatex in current pad 
  // at pos x,y not in NDC but it WC
...
TLatex * PTools::drawLatexNDC(const char* text, Double_t x, Double_t y, 
			      Int_t color, Float_t size, Int_t align, Int_t font, Float_t angle)
{
  // Draws text as TLatex in current pad 
  // at pos x,y in NDC
...
TLatex * PTools::drawLatex(const char* text, Double_t x, Double_t y, 
			   Int_t color, Float_t size, Int_t align, Int_t font, Float_t angle)
{
  // Draws text as TLatex in current pad 
  // at pos x,y in NDC
...
TLatex * PTools::drawLatexPadCoords(const char* text, Double_t x, Double_t y, 
				    Int_t color, Float_t size, Int_t align, Int_t font, Float_t angle)
{
  // Draws text as TLatex in current pad 
  // at pos x,y in NDC
...
void PTools::setErrorsOfHistogram(TH1 * hist, TH1 *err)
{
  ...
  int check = checkHistogramCompatibility(hist, err);
  if (check > 0)
    {
...
////////////////////////////////////////////////////////////////////////////////
TH1D *PTools::getBinContentDistribution(TH1* hist, Int_t binning)
{
  ...
  TH1D* dist = new TH1D("binContent","binContent",binning,minimum,maximum);
...
//   int check = checkHistogramCompatibility(h1, h2);
//   if (check > 0)
//     {
...
TH2D *PTools::reBinHistX(TH2* hist,Int_t factor)
{
  ...
  TH2D* inv = new TH2D(name.Data(),name.Data(),
		       int (nbinsx/factor), hist->GetXaxis()->GetXmin(),hist->GetXaxis()->GetXmax(),
		       int (nbinsy), hist->GetYaxis()->GetXmin(),hist->GetYaxis()->GetXmax());
...
Int_t PTools::findEndOfDistribution(TH1* hist, Int_t min, Double_t threshold)
{
  ...
  if (!hist)
    {
      ::Error("cleanHistogram"," hist is NULL pointer ... return -1");
      return -1;
    }
