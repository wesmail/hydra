...
public:
 ...
 inline Int_t HBArray::getSubLocation( int *aLoc, int dim ) {
    #if DEBUG_LEVEL>2 
     Warning(" aLoc:%i \n",aLoc->operator[](0));
    #endif
    Int_t linear=aLoc[0];
 ...
 inline void HBArray::setMod( int loc, int i){
           modules[loc] = i; 
 }
 ...
 inline UInt_t  HBArray::bcount2(const size_t nsize )
 {
  ...
  // Using a double look-up on the bit integer values.
