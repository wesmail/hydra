...
//-----------------------------------------------------
Int_t HMdcTrackFitterA::minimize(Double_t threshold, Int_t iter) {
  if(fprint) {
    printf("\n  ********************************\n");
...
Int_t HMdcTrackFitterA::doMinimization(Double_t threshold) {
...
//!???????? Duzhe esli ochen' bolshoy funck. no est' minimum delat' viykid???
  ...
  if(wires.valueOfFunctional(finalParam,2) < threshold && exitFlag <= 3) {
    if(!wires.calcNGoodWiresAndChi2(finalParam)) return 0;
//    wires.calculateErrors(finalParam); //Errors calculations
    wires.calcErrorsAnalyt(finalParam); //Errors calculations
...
void HMdcTrackFitterA::printResult(const char* status,HMdcTrackParam& par) {
  printf("%s\n",status);
