...
//*-- modified : 05/03/99 by Ilse Koenig
...
Int_t HMdcParAsciIo::write(HParSet* pPar) {
  ...
  if (pFile) {
    ...
    if (strcmp(name,"MdcDigitGeomPar")==0) return writeFile((HMdcDigitGeomPar*)pPar);
  }
...
Bool_t HMdcParAsciIo::write(HDetector*) {
  return kTRUE;  // not implemented!
}


Int_t HMdcParAsciIo::writeFile(HMdcRawStruct* pPar) {
  if (pFile) {
    Text_t buf[155];
    Text_t com[]="########################################################\n"
...
Int_t HMdcParAsciIo::writeFile(HMdcMboReadout* pPar) {
  if (pFile) {
    Text_t buf[155];
    Text_t com[]="########################################################\n"
...
Int_t HMdcParAsciIo::writeFile(HMdcLookupGeom* pPar) {
  if (pFile) {
    Text_t buf[155];
    Text_t com[]="########################################################\n"
...
Int_t HMdcParAsciIo::writeFile(HMdcLookupRaw* pPar) {
  if (pFile) {
    Text_t buf[155];
    Text_t com[]="########################################################\n"
...
Int_t  HMdcParAsciIo::writeFile(HMdcDigitPar* pPar){
  if (pFile) {
    Text_t buf[155];
    Text_t com[]="########################################################\n"
    ...
    for(Int_t s=0;s<6;s++) {
      ...
      for(Int_t m=0;m<4;m++) {
        ...
        for(Int_t l=0;l<6;l++) {
          ...
          if (vel>0.0F) {
            sprintf(buf,"%3i%3i%3i%6.2f%8.4f%8.3f%8.3f%8.3f\n",
                s,m,l,vel,lay.getMdcEfficiency(),lay.getSpacePar0(),
                lay.getSpacePar1(),lay.getSpacePar2());
            pFile->write(buf,strlen(buf));
          }
        }
      }
...
Int_t  HMdcParAsciIo::writeFile(HMdcDigitGeomPar* pPar){
  if (pFile) {
    Text_t buf[155];
    Text_t com[]="########################################################\n"
    ...
    sprintf(buf,"[%s]\n",pPar->GetName());
    pFile->write(buf,strlen(buf));
    HMdcDigitGeomPar& pp = *pPar;
    for(Int_t s=0;s<6;s++) {
      HMdcDigitGeomParSec& sec= pp[s];
      for(Int_t m=0;m<4;m++) {
...
Bool_t HMdcParAsciIo::read(HMdcGeomStruct* pPar,Int_t* set) {
  if (pFile){
...
Bool_t HMdcParAsciIo::read(HMdcMboReadout* pPar,Int_t* set) {
  if (pFile) {
    const Int_t maxbuf=155;
    Text_t buf[maxbuf], mboName[10], key[20];
    Char_t c;
...
Bool_t HMdcParAsciIo::read(HMdcCalPar* pPar,Int_t* set) {
  if (pFile) {
    const Int_t maxbuf=155;
    Text_t buf[maxbuf], key[20];
    Char_t c;
...
Bool_t HMdcParAsciIo::read(HMdcDigitPar* pPar,Int_t* set) {
  if (pFile) {
    const Int_t maxbuf=155;
    Text_t buf[maxbuf], key[20];
    Char_t c;
    Int_t sector, module, layer, pos;
    Float_t driftVel, mdcEf, sR1, sR2, sR3;
    Bool_t all=kFALSE;
    while (!all && !pFile->eof()) {
      pFile->getline(buf,maxbuf);
      if (buf[0]=='[') {
        ...
        if (strstr(buf,pPar->GetName())!=0) {
          while (!all && !pFile->eof()) {
            ...
            if (c=='#' || c=='/') all=kTRUE;
              ...
              pFile->putback(c);
              pFile->getline(buf,maxbuf);
              sscanf(buf,"%i%i%i%f%f%f%f%f",
                   &sector,&module,&layer,&driftVel,&mdcEf,&sR1,&sR2,&sR3);
              pos=sector*4+module;
              if (set[pos]) {
                HMdcDigitParLayer& l=
                  (*pPar)[sector][module][layer];
                l.fill(driftVel,mdcEf,sR1,sR2,sR3);
              }
            }
          }
        }
...
Bool_t HMdcParAsciIo::read(HMdcDigitGeomPar* pPar,Int_t* set) {
  if (pFile) {
    const Int_t maxbuf=155;
    Text_t buf[maxbuf], key[20];
    Char_t c;
    ...
    if (all) {
      pPar->setInputVersion(1,inputNumber);
      pPar->setChanged();
      printf("MdcDigitGeomPar initialized from Ascii file\n");
      return kTRUE;
    }
  }
  return kFALSE;
