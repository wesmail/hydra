...
HStartCalPar::HStartCalPar() : HParSet() {
  // constructor creates an array of pointers of type
  // HStartCalParMod
  // The container name is set to "StartCalPar"
  fName="StartCalPar";
  fTitle="calibration parameters for Start";
  ...
  for (Int_t i = 0; i < nMod; i++) {
    if (det->getModule(-1,i)>0) (*array)[i] = new HStartCalParMod(nComp);
    else (*array)[i] = new HStartCalParMod(0);
  }
}
