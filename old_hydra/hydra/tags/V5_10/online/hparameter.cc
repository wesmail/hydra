...
Float_t HDefaultParameter::getValue(Int_t acat, Int_t idx, HLocation &aloc) {
  ...
  if(!strcmp(dataobj->ClassName(),"HMdcXY")) return getMdcHit(idx);
...
Float_t HDefaultParameter::getMdcHit(Int_t idx) {
  ...
  if(idx==MDC_HIT_X)    return ((HMdcXY*) dataobj) -> getX();
  if(idx==MDC_HIT_Y)    return ((HMdcXY*) dataobj) -> getY();
  if(idx==MDC_HIT_Z)    return ((HMdcXY*) dataobj) -> getZ();
  return 999999.;
}
