...
// Last update by Thomas Eberl: 00/05/12 16:31:06
//
...
// this function writes out the pedestal and calibration file
    // cout<<"bin in hrichpedestalraw::finalize"<<endl;  
    char title1[15];
    ...
    cout<<"number of rich events in file:"<<nCounter<<endl;
    Int_t xCor,yCor;
    xCor=yCor=0;
    ...
    canvas1->cd();
    pad1->cd();
    meanHisto->SetMaximum(50);
    meanHisto->Draw("colz");
    canvas1->Update();
    
    canvas2->cd();
    pad2->cd();
    sigmaHisto->SetMaximum(25);
    sigmaHisto->Draw("colz");
    canvas2->Update();
    
    canvas3->cd();
    pad3->cd();
    meanHisto1d->SetMaximum(50);
    meanHisto1d->Draw();
    canvas3->Update();
    
    canvas4->cd();
    pad4->cd();
    sigmaHisto1d->SetMaximum(25);
    sigmaHisto1d->Draw();
    canvas4->Update();
    
    canvas5->cd();
    pad5->cd();
    statHisto->SetMaximum(nCounter+10);
    statHisto->Draw("colz");
    canvas5->Update();
    // ------------------
    return kTRUE;
...
Int_t HRichPedestalRaw::execute() {
	...
	if (sec != iSec) break; // skipping pads of wrong sector, see ctor !
	...
	if (iRow < rowMax && iCol < colMax){
	    ...
	    cout<<"Warning: impossible Pad numbers: row: "<<iRow<<"  col: "<<iCol<<"   charge: "<<fCharge<<endl;
	}
    }
    if (kData) nCounter++;
    return kTRUE;
...
void HRichPedestalRaw::createPedestal(){
    // rewrite this function ! sorting is dumb !
    char cPedestal1[40];
...
void HRichPedestalRaw::createCalibration(){
	...
	for (Int_t j = 0;j<rowMax;j++){
	    ...
	    //calOut<<j<<"	"<<i<<"	1"<<"	"<<0<<endl; // zero calibration file !
	}
    }
    calOut.close();
