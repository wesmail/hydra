...
Bool_t HRichAnalysis::init() {
  ...
  // now creating pads array
    ...
    for (j = 0; j < 6; j++) {
     pPads[j] = new HRichPadSignal[((HRichGeometryPar*)fpGeomPar)->getPadsNr()];
...
Int_t HRichAnalysis::execute() {
  ...
  while((pCal = (HRichCal *)fIter->Next())) {
   ...
   if ((fPadFired[0]+fPadFired[1]+fPadFired[2]+fPadFired[3]+fPadFired[4]+fPadFired[5]) > 0)
    cout << "Event " << iEventNr << " # pads in RICH: " 
      ...
      if (hit!=NULL) {
        ...
        *hit = pRings[i];
        ...
        ((HRichGeometryPar*)fpGeomPar)->getPadsPar()->
          getPad(pRings[i].iRingX,pRings[i].iRingY)->getXY(&hit->fX, &hit->fY);
        ((HRichGeometryPar*)fpGeomPar)->getPadsPar()->
          getPad(pRings[i].iRingX,pRings[i].iRingY)->
          getXYZlab(nSec, &hit->fLabX, &hit->fLabY, &hit->fLabZ);
        hit->fPhi = ((HRichGeometryPar*)fpGeomPar)->getPadsPar()->
          getPad(pRings[i].iRingX,pRings[i].iRingY)->getPhi(nSec);
        hit->fTheta = ((HRichGeometryPar*)fpGeomPar)->getPadsPar()->
          getPad(pRings[i].iRingX,pRings[i].iRingY)->getTheta();
