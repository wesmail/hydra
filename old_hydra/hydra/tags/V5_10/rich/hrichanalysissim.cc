...
Int_t  HRichAnalysisSim::getPadsNTrack1(Int_t padx, Int_t pady,Int_t sec){
 ...
 //cout<<"Sector  "<<sec<<"X   "<<padx<<"Y   "<<pady<<endl;
...
void HRichAnalysisSim::updateHits(Int_t nSec, Int_t nEvNr) {
  ...
  for (int i = 0; i < iRingNr; i++) {
        ...
        while(hit->iRingTrack[k]){
           if (hit->track1==0) {
              ...
              (hit->weigTrack1)++;
                 ...
                 if(hit->iRingTrack[k]== hit->track1) (hit->weigTrack1)++;
                 else {
                      if(hit->track2==0) {
                         ...
                         if(hit->iRingTrack[k]== hit->track2) (hit->weigTrack2)++;
                         else {
                           if (hit->track3==0) {
	...
	if( hit->track1>1 ||   hit->track2>1)  cout<<"Track1   "<<	hit->track1<<"Track2   "<<	hit->track2<<endl;
