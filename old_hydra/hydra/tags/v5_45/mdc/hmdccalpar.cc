...
Bool_t HMdcCalPar::fillMdcCalParRaw(HMdcCalParRaw& rPar) {
  ...
  HMdcLookupRaw *lookup=
    (HMdcLookupRaw*)rtdb->getContainer("MdcLookupRaw");
  if (!lookup) {
    cerr<<"lookup table MdcLookupRaw not found"<<endl;
    return kFALSE;
  }
  for(Int_t s=0; s<6; s++) {
    for(Int_t mo=0; mo<4; mo++) {
...
Bool_t HMdcCalPar::writeline(char *buf, Int_t sec, Int_t mod, Int_t lay, Int_t cell) {
