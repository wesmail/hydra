...
Bool_t HShowerTofinoCorrelator::init() {
  ...
  // and an iterator which loops over Tofino information.
    ...
    if (!m_pTofinoCalPar) {
      m_pTofinoCalPar = new HTofinoCalPar; 
      if (m_pTofinoCalPar) rtdb->addContainer(m_pTofinoCalPar);
      else  
	return kFALSE;
    }
    m_pTofShowerMap = rtdb->getContainer("TofinoShowerMap");
    if (!m_pTofShowerMap) {
      m_pTofShowerMap = new HTofinoShowerMap;
      if (m_pTofShowerMap) rtdb->addContainer(m_pTofShowerMap);
      else
	return kFALSE;
    }
...
Int_t HShowerTofinoCorrelator::execute()
{
  ...
  while((pHit = (HShowerHit*)fHitIter->Next())) {
    ...
    nTofIndex = nSector*4 + nTofCell; 
       //  cout <<"nTofCell,nTofIndex = " <<nTofCell <<" " <<nTofIndex <<endl;

    fDriftTime = ((HTofinoCalPar*)m_pTofinoCalPar)->
      calcDriftTime(nSector, nTofCell, (Float_t)nRow);
//    cout <<"sec,cell,Row = " <<nSector <<" " <<nTofCell <<" " <<nRow <<endl;
    //cout <<"fDriftTime = " <<fDriftTime <<endl;
    ...
    if (pTofCalTab[nTofIndex]) {
      ...
      //cout <<"fTime,fAdc =" <<fTime <<" " <<fAdc <<endl;
      //cout <<"fTime-fDriftTime =" <<fTime-fDriftTime <<endl;
      if((fTime - fDriftTime) > 0.) {
	   //   fDriftTime = ((HTofinoCalPar*)m_pTofinoCalPar)->
	   //  calcDriftTime(nSector, nTofCell, (Float_t)nRow);
	   //	   if((fTime - fDriftTime)<0 &&(fTime - fDriftTime)>-10  ) printf("Time:%f DriftTime %f",fTime, fDriftTime);
	HShowerHitTof *pHitTof;
	pHitTof=(HShowerHitTof *)m_pHitTofCat->getNewSlot(m_zeroLoc);
	if (pHitTof!=NULL) {
	  pHitTof=new(pHitTof) HShowerHitTof;
  ...
  while((pHitTof = (HShowerHitTof *)fHitTofinoIter->Next())) {
    fTof = pHitTof->getTof();
    //    fAdc = pHitTof->getCharge();
    nTofIndex = pHitTof->getSector()*4 +  pHitTof->getTofinoCell();
       //         if(fTof > 0. && fAdc > 0. && nMult[nTofIndex] < 25 ) {
    //            if(fTof > 0. && nMult[nTofIndex] < 25 ) {
       // if(fTof > 0. && fAdc > 0.) {
    pHitTof->setTofinoMult(nMult[nTofIndex]);
       //  cout <<"nTofIndex,nMult[nTofIndex] =" <<nTofIndex <<" " <<nMult[nTofIndex] <<endl;
       //     cout <<"Mult. =" <<pHitTof->getTofinoMult() <<endl;
  } //while((pHitTof = (HShowerHitTof *)fHitTofinoIter->Next()))
