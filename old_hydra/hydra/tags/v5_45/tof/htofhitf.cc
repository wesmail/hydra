...
// the xposAdc cannot be calculated. Also amplL or amplR cannot be found.
// For instance when left ADC amplitude is missing amplL is not available.
// In that case it is calculated by using xpos (see TDC part).
//
/////////////////////////////////////////////////////////////////////////
    ...
    if(raw->getLeftTime() && raw->getRightTime()){
      ...
      if (hit) {
        ...
        if(raw->getLeftCharge() || raw->getRightCharge()) {
          //printf(" hups daco bolo \n");
          subCl = (raw->getLeftCharge() - cell.getPedestalL());
          subCr = (raw->getRightCharge() - cell.getPedestalR());
          HGeomVector *scintRod=fTofGeometry->getModule(hit->getSector(),hit->getModule())
                                            ->getRefVolume()->getComponent(hit->getCell())->getPoint(0);
          if(scintRod != NULL) halfLen=scintRod->getX();
          slopeAdcL =(TMath::Exp((halfLen-cell.getGainAsym())/(cell.getAttLen())))/(cell.getEdepK());
          slopeAdcR =(TMath::Exp((halfLen+cell.getGainAsym())/(cell.getAttLen())))/(cell.getEdepK());
          ...
          if(raw->getLeftCharge() && !(raw->getRightCharge())){
            flagadc=-1;
            depE=(subCl*cell.getEdepK())/(TMath::Exp((axpos-cell.getGainAsym())/(cell.getAttLen())));
            leftA=subCl/slopeAdcL;
            rightA=subCl/(slopeAdcL*TMath::Exp(2.0*axpos/cell.getAttLen()));
          }
          if(!(raw->getLeftCharge()) && raw->getRightCharge()){
            flagadc=1;
            depE=(subCr*cell.getEdepK())/(TMath::Exp((cell.getGainAsym()-axpos)/(cell.getAttLen())));
            leftA=subCr/(slopeAdcR*TMath::Exp(-2.0*axpos/cell.getAttLen()));
            rightA=subCr/slopeAdcR;
          }
        }
        multTof++;
...
void HTofHitF::fillGeometry(HTofHit *hit) {
  ...
  HGeomVector targetPos = fSpecGeometry->getTarget(0)->getTransform().getTransVector();
  ...
  if (targetPos != NULL) r -= targetPos; // correct for target position
