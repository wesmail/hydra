...
Bool_t HShowerParRootFileIo::init(HParSet* pPar,Int_t* set) {
  ...
  if (pFile) {
    ...
    if (strncmp(name,"ShowerHitFinderParams",strlen("ShowerHitFinderParams"))>=0) return read((HShowerHitFPar*)pPar,set);
    if (strncmp(name,"ShowerCalPar",strlen("ShowerCalPar"))>=0) return read((HShowerCalPar*)pPar,set);
...
Bool_t HShowerParRootFileIo::readObject(HShowerParSet *pPar) {
  ...
  TKey *key = (TKey*)gDirectory->GetKey(name,version);
	...
	//  pPar->transToLab();
	  ...
	  printf("Shower HShowerGeometry initialized from ROOT\n"); 
	  return kTRUE;
	}
	...
	Bool_t HShowerParRootFileIo::read(HShowerDigiDetPar* pPar,Int_t* set) {
	  ...
	  printf("Shower HShowerDigiDetPar initialized from ROOT\n"); 
	  return kTRUE;
	}
	...
	Bool_t HShowerParRootFileIo::read(HShowerHitFPar* pPar,Int_t* set) {
	  ...
	  printf("Shower HShowerHitFPar initialized from ROOT\n"); 
	  return kTRUE;
	}
	...
	Bool_t HShowerParRootFileIo::read(HShowerCalPar* pPar,Int_t* set) {
	  ...
	  printf("Shower CalPar initialized from ROOT\n"); 
	  return kTRUE;
	}
	...
	Bool_t HShowerParRootFileIo::read(HShowerUnpackParam* pPar,Int_t* set) {
	  ...
	  printf("Shower Unpack Param initialized from ROOT\n"); 
	  return kTRUE;
	}
