...
Bool_t HShowerParHadAsciiFileIo::init(HParSet* pPar, Int_t* set) {
  ...
  if (pFile) {
    ...
    if (strncmp(name,"ShowerDigiPar",strlen("ShowerDigiPar"))>=0) 
                                return read((HShowerDigiDetPar*)pPar,set);
    if (strncmp(name,"ShowerHitFinderParams",strlen("ShowerHitFinderParams"))>=0) 
                                return read((HShowerHitFPar*)pPar,set);
    if (strncmp(name,"ShowerCalPar",strlen("ShowerCalPar"))>=0) 
