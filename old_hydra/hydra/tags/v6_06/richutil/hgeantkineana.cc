...
// Last update by Thomas Eberl: 02/02/14 14:01:12
//
...
void HGeantKineAna::iniControlHistos()
{
    ...
    med = new TH1D("Medium","Medium",41,0,42);
    pHistArray->Add(med);
    mech = new TH1D("Mechanism","Mechanism",36,0,35);
    pHistArray->Add(mech);
    par = new TH1D("Parent","Parent",56,0,55);
    pHistArray->Add(par);
    par_med = new TH2D("parent_medium","parent_medium",56,0,55,46,0,45);
    pHistArray->Add(par_med);
    par_mech = new TH2D("parent_mechanism","parent_mechanism",56,0,55,36,0,35);
    pHistArray->Add(par_mech);
    mech_mom = new TH2D("mechanism_momentum","mechanism_momentum",36,0,35,100,0,1000);
    pHistArray->Add(mech_mom);
}
...
//  	     aMed == 9) pElectrons->Add(kine);
	...
	if ( aMech==5 && (aID == 2 || aID==3) && (aMed>=8 && aMed<=19)) // direct particle decay
	{
		...
		if(aIDp1==7 && (theta1>15. && theta1<85.))//pion
		{ //in acceptance
			
			    med->Fill(aMed);
			    mech->Fill(aMech);
			    par->Fill(aIDp1);
			    par_med->Fill(aID,aMed);
			    par_mech->Fill(aID,aMech);
			    mech_mom->Fill(aMech,ptot);
			}//pi0
	    }//kine_parent1
	}//Mec==5 and lepton
...
//  	     aMed == 9) pElectrons->Add(kine);
	...
	if ( aMech==6 && (aID == 2 || aID==3) && (aMed>=8 && aMed<=19)) // photon pair production in target or Rich
	{
		...
		if(aIDp1==1)//gamma
		{
			...
			if(aIDp2==7 && (theta2>15. && theta2<85.))
			{//pi0 in acceptance
			    med->Fill(aMed);
			    mech->Fill(aMech);
			    par->Fill(aIDp2);
