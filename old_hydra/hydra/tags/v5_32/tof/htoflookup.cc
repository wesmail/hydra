...
HTofLookupSlot::HTofLookupSlot(Int_t nChan) {
  ...
  for(Int_t i=0;i<nChan;i++) (*array)[i]=new HTofLookupChan();
  nChannels=32;
  maxChannel=nChan-1;
  clear();
}
...
void HTofLookupSlot::fill(Char_t dcType, Int_t channel, 
  ...
  if (channel>maxChannel) {
    ...
    for(Int_t i=maxChannel+1;i<nChannels;i++) (*array)[i]=new HTofLookupChan();
...
HTofLookupCrate::HTofLookupCrate(Int_t numSlots) {
  ...
  for(Int_t i=0;i<numSlots;i++) (*array)[i]=new HTofLookupSlot();
}
...
void HTofLookupCrate::fill(Int_t slot, Char_t modType,
  ...
  if (slot>maxSlot) {
    array->Expand(slot+1);
    for(Int_t i=maxSlot+1;i<=slot;i++) (*array)[i]=new HTofLookupSlot();
    maxSlot=slot;
  }
  (*this)[slot].fill(modType,channel,sector,module,cell,side);
}

HTofLookup::HTofLookup(Int_t nCrates, Int_t nSlots) : HParSet() {
  // The constructor creates an array of pointers of type HTofLookupCrate of
  // size nSlots
  // The container name is set to "TofLookup".
  fName="TofLookup";
  fTitle="Lookup table for Tof unpacker";
  strcpy(detName,"Tof");
  maxCrate=nCrates-1;
  array = new TObjArray(nCrates);
  for (Int_t i=0;i<nCrates;i++) (*array)[i]=new HTofLookupCrate(nSlots);
}
...
void HTofLookup::fill(Int_t crate, Int_t slot, Char_t modType,
  ...
  if (crate>maxCrate) {
    ...
    for(Int_t i=maxCrate+1;i<=crate;i++) (*array)[i]=new HTofLookupCrate();
    maxCrate=crate;
  }
  (*this)[crate].fill(slot,modType,channel,sector,module,cell,side);
}

void HTofLookup::readline(const char *buf, Int_t *set) {
...
Bool_t HTofLookup::writeline(char *buf, Int_t crate, Int_t slot, Int_t chan) {
