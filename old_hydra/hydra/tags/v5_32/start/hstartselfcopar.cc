...
HStartSelfCoParMod::HStartSelfCoParMod(Int_t n) {
  ...
  for(Int_t i=0 ; i<n ; i++)
    (*array)[i] = new HStartSelfCoParChan();
}
...
HStartSelfCoPar::HStartSelfCoPar() : HParSet() {
  // constructor creates an array of pointers of type
  // HStartSelfCoParMod
  // The container name is set to "StartSelfCoPar"
  fName="StartSelfCoPar";
  fTitle="selfcoincidence parameters for Start";
  ...
  for (Int_t i = 0; i < nMod; i++) {
    if (det->getModule(-1,i)>0) (*array)[i] = new HStartSelfCoParMod(nComp);
    else (*array)[i] = new HStartSelfCoParMod(0);
  }
}
