...
void HRichAnalysisPar::clear() {
 ...
 isActiveCentroidCheck = 0;
 isActiveRadiusCheck= 0;
...
Bool_t HRichAnalysisPar::initAscii(HParHadAsciiFileIo* pHadAsciiFile) {
 ...
 try {
  ...
  isActiveCentroidCheck = mydata.ReadInt("Test Ring Centroid Status");
  isActiveRadiusCheck = mydata.ReadInt("Test Ring Radius Status");
 ...
 fThresholdRed = mydata.ReadFloat("Ring threshold reduction at the sector borders");
 fFakeQualRatio = mydata.ReadFloat("Minimal quality ratio for a fake near a bright ring");
 fFakeCentroidCut =  mydata.ReadFloat("Minimal centroid shift of induced fake");
 fFakePadAmplitude = mydata.ReadFloat("Mean pad amplitude for a fake ring");

 fRingRadius = mydata.ReadFloat("Maximal Deviation betweed centroid and ring centre");
 fMeanRadiusError = mydata.ReadFloat("Maximal ring radius error");
...
void HRichAnalysisPar::printParam(){
    cout<<"--------Rich Analysis Parameters-------"<<endl;
    cout<<endl;
    cout<<" Signal cut-off threshold  = "<< iCutOffThresheold<<endl;
    cout<<"---------------------------------------"<<endl;
    cout<<" Superior Algorithm ID = "<< iSuperiorAlgorithmID<<endl;
    cout<<"---------------------------------------"<<endl;
    if(isActiveRingFindFitMatrix){
	cout<<" Pattern Matrix is active "<<endl;
	cout<<" Minimal Ring Quality in Hough Transformation = "<<iMinimalFitMatrixRingQuality<<endl;
    
    cout<<"---------------------------------------"<<endl;
    }
    if(isActiveRingHoughTransf){
	cout<<" Hough Trasformation is active "<<endl;
	cout<<" Minimal Ring Quality in Hough Transformation = "<<iMinimalHoughTransfRingQuality<<endl;
	cout<<"---------------------------------------"<<endl;
    }
    cout<<"---------------------------------------"<<endl;
...
Bool_t HRichAnalysisPar::writeAscii(HParHadAsciiFileIo* pHadAsciiFile) {
 ...
 try {
  ...
  mydata.WriteLine("\n");
  mydata.WriteInt("Test Ring Centroid Status",isActiveCentroidCheck);
  mydata.WriteInt("Test Ring Radius  Status",isActiveRadiusCheck);
  mydata.WriteLine("\n");
...
mydata.WriteFloat("Ring threshold reduction at the sector borders",fThresholdRed);
mydata.WriteFloat("Minimal quality ratio for a fake near a bright ring",fFakeQualRatio);
mydata.WriteFloat("Minimal centroid shift of induced fake",fFakeCentroidCut);
mydata.WriteFloat("Mean pad amplitude for a fake ring",fFakePadAmplitude);
mydata.WriteFloat("Maximal Deviation betweed centroid and ring centre",fRingRadius);
mydata.WriteFloat("Maximal ring radius error",fMeanRadiusError);
