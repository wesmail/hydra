...
protected:
  ...
  enum flags  {WITHOUT_NEIGHBOURS         = 0,
               LEFT_CELL_HIT              = 1,
               RIGHT_CELL_HIT             = 2,
  ...
  Float_t tdifmin; // lower limit for cut in time-above-threshold (time1-time2)
  Float_t tdifmax; // upper limit for cut in time-above-threshold (time1-time2)
  ...
  Bool_t flag[3];  // Boolean array to trigger on flags (WITHOUT_NEIGHBOURS = 0, LEFT_CELL_HIT = 1, RIGHT_CELL_HIT = 2)
 ...
 // Int_t ihit;
  ...
  TH2S *pTime0_diff[3]; // time above threshold (1:t1, 2:t2) vs. drifttime
  ...
  TProfile *pTsum_ev_prof[3][120]; //  selftracking: time sum vs. event#
