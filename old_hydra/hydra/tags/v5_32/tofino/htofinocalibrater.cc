...
Int_t HTofinoCalibrater::execute(void) {
 ...
 if (fStartHitCat) {
    fStartTime = ((HStartHit *)fStartHitCat->getObject(0))->getTime();
  } else {
   fStartTime = 0.0;
  }
  ...
  while ((raw=(HTofinoRaw *)iter->Next())!=0) {
    ...
    // Set and substract time from start. 
    // (fStartTime is zero if no start hit level is filled ) 
    cal->setTime(((HTofinoCalPar*)pTofinoCalPar)->
	         channelToTime(fLoc[0],fLoc[2],raw->getTime()) - fStartTime);
