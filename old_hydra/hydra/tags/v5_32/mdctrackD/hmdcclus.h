...
public:
  ...
  void getLocParent(HLocation &loc) {loc[0]=sec; loc[1]=seg; loc[2]=indexPar;}
//!??  void getLocChilds(HLocation &loc) {loc[0]=sec; loc[1]=seg; loc[2]=indexPar;}
  void print();
  void printPos();
...
public:
  HMdcClusFilter(void){;}
  ~HMdcClusFilter(void){;}
  Bool_t check(HDataObject *clus) {return ((HMdcClus*)clus)->getStatus();}
