...
Bool_t HMdcCalcSizesCells::calcVolCells(Int_t nSec, HMdcSizesCells* fSizesCells) {
  ...
  HGeomTransform secsys; 
   for(Int_t nmo=0; nmo<4; nmo++) {
    ...
    if(!pComVol) {
     printf("Error: GeomCompositeVolume for MDC%i is absenst!!!",nmo);
     return kFALSE;
    }
    for(Int_t nla=0; nla<6; nla++) {
     ...
     if(nCells>0) {
      ...
      if(!pGeomVol) {
       printf("Error: GeomCompositeVolume for Layer %i is absenst!!!",nla);
       return kFALSE;
      }   
      ...
      Double_t dst=pLaGePar.getCatDist()*0.5;
...
//       else volDrGas[nPoint].setZ(dst); 
      ...
      Double_t pitch=pLaGePar.getPitch();         
