...
HMdcLayerGeomPar::HMdcLayerGeomPar(const char* name,const char* title,
  ...
  if (pMdc) {
    ...
    ERROR_msg(DET_MDC,"numWires in container MdcLayerGeomPar not initialized");
    ERROR_msg(DET_MDC,"container MdcGeomStruct not found");
  }
  ntuple=0;
}
  ...
  for(Int_t s=0;s<6;s++) {
    ...
    for(Int_t m=0;m<4;m++) {
	  ...
	  gHades->getMsg()->info(10,DET_MDC,GetName(),
				 "%1i %1i %1i %8.4f %5i %8.4f %8.2f %8.4f\n",
