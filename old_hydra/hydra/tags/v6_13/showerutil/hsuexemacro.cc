...
// -----------------------------------------------------------------------------
// $Id: hsuexemacro.cc,v 1.5 2002-09-05 12:48:11 jaskula Exp $
// -----------------------------------------------------------------------------
...
HSUBranchElement::HSUBranchElement(const char* pBranchName, EBranchType eType)
{
...
const char* HSUBranchElement::getBranchNameSufix(void)
{
static const char* asHSUBranchElementBranchesSufix[HSUBranchElement::kMax]
        = { ".fData", "." };
...
// -n int       - can be get by getOptInt()
// -f float     - can be get by getOptFloat()
...
Int_t HSUExeMacro::loadArgs(int iArg, char* vArg[])
{
...
void HSUExeMacro::printInfo(const char *pProgramName) const
{
...
Bool_t HSUExeMacro::addInputFile(const char* pStr)
{
...
Int_t iRet;
    ...
    printf("Input file: %-50s %8d evts\n", pStr,
            m_pChain->GetTreeOffset()[m_pChain->GetNtrees()]
...
Bool_t HSUExeMacro::addFilesFromList(const char *pStr)
{
...
TClonesArray* HSUExeMacro::getTClonesArray(const char *pBranchName)
{
...
HCategory* HSUExeMacro::getHCategory(const char *pCategoryName)
{
...
TBranch* HSUExeMacro::getBranch(const char *pBranchName)
{
...
TClonesArray* HSUExeMacro::getNewArrayFromBranch(const char *pBranchName)
{
...
HCategory* HSUExeMacro::getNewHCategoryFromBranch(const char *pBranchName)
{
...
TDirectory *pDirSav = gDirectory;
    ...
    if((pCat = (HCategory *)gDirectory->FindObject(pBranchName))
            == NULL)
    {
        pDirSav->cd();
...
HSUBranchElement* HSUExeMacro::findAciveBranch(const char *pBranchName)
{
...
Bool_t HSUExeMacro::openParamFile(const char *pParamFileName)
{
...
TObject* HSUExeMacro::getParamObject(const char *pName)
{
