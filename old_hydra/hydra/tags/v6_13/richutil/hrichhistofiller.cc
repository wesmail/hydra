// $Id: hrichhistofiller.cc,v 1.10 2003-05-22 17:30:18 eberl Exp $
// Last update by Thomas Eberl: 03/05/21 16:00:05
using namespace std;
...
Int_t HRichHistoFiller::fillTrackHistosFromTracklets(TObjArray* trackArray,const Char_t *cNameTrackSample)
{
 ...
 //cout<<"start loop"<<endl;
	...
	if(kNewTrack)
	{
	    ...
	    fillTH1F(HRichHistFac::getHMomtBasename(),samplename.Data(),
		     //l->Pt());
		     hi->getKickMomT());
	    //
	    //
	    //	    TLorentzVector* l = hi->getLVec();
	    fillTH1F(HRichHistFac::getHMomBasename(),samplename.Data(),
		     //     l->Pt());
		     hi->getKickMom());
    ...
    if (nTrackCounter) fillTH1F(HRichHistFac::getHMultBasename(),samplenametracks.Data(),nTrackCounter);
...
// 	    fillTH2F(HRichHistFac::getHMomvsEnergyBasename(),addtohistname.Data(),psum,esum);
    ...
    TString samplenamepair(cNamePairSample);
    samplenamepair.Prepend("Pairs.");
    if (nPairCounter) fillTH1F(HRichHistFac::getHMultBasename(),samplenamepair.Data(),nPairCounter);
    delete [] nTrack2Tuple;
