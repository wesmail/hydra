...
void HIndexTable::setDimensions(Int_t nDim,Int_t *sizes) {
  ...
  for (int i=0;i<prod;i++) fIndexArray.fArray[i]=-1;
#if DEBUG_LEVEL > 2
...
void HIndexTable::Streamer(TBuffer &R__b)
{
   ...
   if (R__b.IsReading()) {
     ...
     if (R__v==1) {
       ...
       for (int i=0;i<fSizes.fN;i++) { prod*=fSizes.fArray[i];}
       fCompactTable.setCapacity(prod);
       fCompactTable.clear();
       for (int i=0;i<fIndexArray.fN;i++) {
	 if (fIndexArray.fArray[i]!=-1) {
       ...
       for (int i=0;i<fCompactTable.getN();i++) {
	 fIndexArray.fArray[fCompactTable.getIndex1(i)]=
	   (fCompactTable.getIndex2(i)==0xFFFF)?-1:fCompactTable.getIndex2(i);
       }
     } else {
...
void HPairListI::setCapacity(Int_t n) {
  if (fCapacity!=n) {
    ...
    delete[] fArray[1];
    if (fCapacity>0) {
      fArray[0]=new UShort_t[n]; 
      fArray[1]=new UShort_t[n];
    }
  }
  clear();
}
