...
HMdcDigitParMod::HMdcDigitParMod() {
  ...
  for (Int_t i = 0; i < 6; i++) {
    array->AddAt( new HMdcDigitParLayer,i);
  }
}
...
HMdcDigitParSec::HMdcDigitParSec() {
  ...
  for (Int_t i = 0; i < 4; i++)
      array->AddAt(new HMdcDigitParMod,i);
}
...
HMdcDigitPar::HMdcDigitPar(const char* name,const char* title,
  ...
  // constructor creates an array of pointers of type HMdcDigitParSec
  strcpy(detName,"Mdc");
  array = new TObjArray(6);
  for (Int_t i = 0; i < 6; i++)
      array->AddAt( new HMdcDigitParSec,i);
  ntuple=0;
}
