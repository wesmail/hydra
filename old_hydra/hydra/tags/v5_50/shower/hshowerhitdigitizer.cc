                            ...
                            Float_t unit) : HShowerDigitizer(name,title)
{
    ...
    m_pRandom = new TRandom(0);

    printf("**** HShowerHitDigitizer::HShowerHitDigitizer\n");
}
...
Int_t             i;
Float_t           fY0, fY1;
const Float_t    *pfParams;
const Float_t    *pfSigmaParams;
    ...
    simhit->getIncidence(fTheta, fPhi);
        ...
        // check efficiency
    pfParams = ((HShowerDigiDetPar*)getDigiParSet())->getEffParams();
    fY0  = CALCULATE_FROM_FIT(pfParams, fBeta);
    if((fY0 < 1.0) && ((fY0 <= 0.0f) || (m_pRandom->Rndm() > fY0)))
        return kFALSE;
    ...
    pfParams      = ((HShowerDigiDetPar *)getDigiParSet())->getMeanParams();
    pfSigmaParams = ((HShowerDigiDetPar *)getDigiParSet())->getSigmaParams();
...
Double_t dSigma;
    ...
    {
        if((fQ = m_pRandom->Landau(dMean, dSigma)) > 0.0)
        {
            if(fQ > 250.0f)
                continue;
