// File: hrichchernovringfitter.cc
//
// Author: Thomas Eberl <Thomas.Eberl@physik.tu-muenchen.de>
// Last update by Thomas Eberl: 01/08/03 10:44:42
//
...
Bool_t HRichChernovRingFitter::fitFoundPhotons()
{
	...
	else mX->Set(nHitCounter,compress(dX,nHitCounter)); 
	
	if (!mY) mY = new TArrayD(nHitCounter,compress(dY,nHitCounter));
	else mY->Set(nHitCounter,compress(dY,nHitCounter));
	
	if (!mCh) mCh = new TArrayD(nHitCounter,compress(dCh,nHitCounter));
	else mCh->Set(nHitCounter,compress(dCh,nHitCounter));
	...
	switchXtoPhi();
	storeFitinHRichHitFit(nHeaderSector,nHitCounter);
...
Bool_t HRichChernovRingFitter::fitFiredPads()
{
	...
	if (!mX) mX = new TArrayD(nPadCounter,compress(dX,nPadCounter));
	else mX->Set(nPadCounter,compress(dX,nPadCounter)); 
	
	if (!mY) mY = new TArrayD(nPadCounter,compress(dY,nPadCounter));
	else mY->Set(nPadCounter,compress(dY,nPadCounter));
	
	if (!mCh) mCh = new TArrayD(nPadCounter,compress(dCh,nPadCounter));
	else mCh->Set(nPadCounter,compress(dCh,nPadCounter));
	...
	storeFitinHRichHitFit(pHit->getSector(),nPadCounter);
	//dumpFitParameters(nHitSector,nHitX,nHitY,pHit->fRingRadius);
...
void HRichChernovRingFitter::storeFitinHRichHitFit(Int_t s, Int_t n)
{
