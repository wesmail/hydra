...
Bool_t HMdcCluster::Test(HMdcCluster& clst, HMdcPrPlane& prPlane,
  ...
  for(Int_t nm=m1; nm<=m2; nm++) {
    for(Int_t nl=nm*6; nl<(nm+1)*6; nl++) {    // for 1 MDC we need in ...!!!!
      Int_t isId=0;
      for(Int_t nh1=0; nh1<numhits[nl]; nh1++){
        for(Int_t nh2=0; nh2<clst.numhits[nl]; nh2++){
          if(hits[nl][nh1]==clst.hits[nl][nh2]) {
            isId++;
            break;
          }
        }
...
//  if( (maxClus<=6 && maxCr >=1) || (maxCr == 2) ) {
  if( maxCr == nMods ) {
    ...
    Double_t tmpX,tmpY;
    Double_t nEvWt=clst.sumBinsWights+sumBinsWights;
    tmpX=(clst.dXmid*clst.dXmid - shX2*shX2 +
          clst.xmid*clst.xmid)*clst.sumBinsWights;
    tmpX+=(dXmid*dXmid + xmid*xmid)*sumBinsWights;
    tmpY=(clst.dYmid*clst.dYmid + clst.ymid*clst.ymid)*clst.sumBinsWights;
    tmpY+=(dYmid*dYmid - shY2*shY2 +
           ymid*ymid)*sumBinsWights;
    clst.xmid=(clst.xmid*clst.sumBinsWights+xmid*sumBinsWights)/nEvWt;
    clst.ymid=(clst.ymid*clst.sumBinsWights+ymid*sumBinsWights)/nEvWt;
    ...
    clst.sumBinsWights=nEvWt;
    for(Int_t m=0; m<4; m++) clst.maxClus[m]=newMax[m];
    clst.nMergedClus += nMergedClus;
    return kTRUE;
  }
  return kFALSE;
}
...
void HMdcCluster::setHitToCluster(Int_t nl, Short_t ncell, Short_t nTm){
 ...
 if(ind==1) {
  ...
  else Warning("setHitToCluster","Very mach hits (>max).");
 }
}
