...
ClassImp(HMdcBitFlipCorPar)
  ...
  HMdcBitFlipCorPar::HMdcBitFlipCorPar(const char* name,const char* title, 
				       const char* context)
	  ...
	  for(Int_t mbo=0; mbo<16; mbo++)
	    {
	      gHades->getMsg()->info(10,HMessageMgr::DET_MDC,GetName(),
				     ...
				     s,m, mbo, bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+0],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+1],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+2],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+3],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+4],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+5],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+6],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+7],bitshiftcorfactor[(s*576)+(m*144)+(mbo*9)+8]);
	    }
	  }
      }
    SEPERATOR_msg("#",60);
}
Bool_t HMdcBitFlipCorPar::init(HParIo* inp,Int_t* set)
{
...
Int_t HMdcBitFlipCorPar::write(HParIo* output)
{
...
void HMdcBitFlipCorPar::putParams(HParamList* l)
{
    ...
    if (!l) return;
    l->addBinary("bitflipcor",     bitshiftcorfactor.GetArray(),3456);
}
Bool_t HMdcBitFlipCorPar::getParams(HParamList* l)
{
  Int_t arraycp[3456];
  if (!l) return kFALSE;
  if(!( l->fillBinary("bitflipcor",     arraycp, 3456)))      return kFALSE;
