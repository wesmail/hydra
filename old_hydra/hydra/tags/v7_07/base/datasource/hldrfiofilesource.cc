...
void HldRFIOFileSource::addFile(Text_t *name,Int_t refId)
{
...
void HldRFIOFileSource::setRobot(const char *robotName)
{
	TString *tmp;

	tmp = new TString(robotName);
	tmp->ToUpper();
	if(tmp->CompareTo("ADSMEXP")==0)
		fCurrentRobot = "adsmexp";
	else
		if(tmp->CompareTo("GSITSMA")==0)
			fCurrentRobot = "gsitsma";
}
