// @(#)$Id: hpidalgringproperties.cc,v 1.3 2003-11-06 12:11:20 christ Exp $
            ...
            Float_t afReturn[], Short_t &nCatIndex)
{
  ...
  if(pTrack->getRingId() < 0)
      ...
      for(Int_t particleCounter=0; particleCounter<pRec->particlesNumber(); particleCounter++)
	{
	  ...
	  if(iPID==2 && fCharge>0) //positron
	    {
	      afReturn[particleCounter]=1.0-pParams->getRichLeptonFraction();
	    }
	  ...
	  if((iPID==-2 || iPID==8 || iPID==14 || iPID==11 || iPID==45 ) && fCharge>0) //fake+, pi+, proton, k+, deuteron
	    {
	      afReturn[particleCounter]=1.0-pParams->getRichHadronFraction();
	    }
  ...
  Int_t fThetaBin = (Int_t)(fKickTheta/10)-1;
  ...
  for(i = 0; i < pRec->particlesNumber(); i++)
      ...
      else
	{
      ...
      afReturn[i] = pParams->getNormalizedPDFValue(iId,fThetaBin+8*CentroidStatus,Eigencoord_One,Eigencoord_Two);
    }
...
{
  Int_t iPID;
  Float_t hadron_yield_correction = pParams->getRichHadronFraction();
  Float_t lepton_yield_correction = pParams->getRichLeptonFraction();
  for(Int_t particleCounter=0; particleCounter<pRec->particlesNumber(); particleCounter++)
    {
      iPID = pRec->getParticleId(particleCounter);
...
// Get CL from data stroed in histograms in HPidAlgRingPropertiesPar
  ...
  for(i = 0; i < pRec->particlesNumber(); i++)
      ...
      else
	{
      ...
      afReturn[i] = pParams->getCLValue(iId,fThetaBin+8*CentroidStatus+16,Eigencoord_One,Eigencoord_Two);
    }
