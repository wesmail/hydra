...
HMdcLookUpTbSec::HMdcLookUpTbSec(Int_t sec, Int_t nSegs,
  ...
  HMdcClustersArrs* pClustersArrs  = HMdcClustersArrs::getObject();
...
void HMdcLookUpTbSec::clearwk(void) {
  ...
  counter[0]  = 0;
  counter[1]  = 0;
  memset(hitsDel,0,12000);   // 12000 = 24*250*sizof(Short_t)
}
...
//      hitsDel[ind1][ind2]=0;
      nHits[ind1]++;
      nHitsTot++;
    } else Error("setCell","%isec. %imod. Too many hits in layer %i.",
        sector+1,mod+1,layer+1);
  }
}
...
Bool_t HMdcLookUpTbSec::calcClusterInSegFloatLevel(Int_t nBin) {
  ...
  stacksArr->init(12);      // init stack and push bin in stack
  while (kTRUE) {
    for(Int_t nTs=0; nTs<8; nTs++) {
      Int_t nb=nBin+neighbBins[nTs];
...
Bool_t HMdcLookUpTbSec::calcClusterInSecFloatLevel(Int_t nBin) {
  Int_t  amp    = HMdcBArray::getNSet(hPlMod[0][nBin])+
                  ...
                  HMdcBArray::getNSet(hPlMod[3][nBin]);
  initCluster(nBin,amp);
  stacksArr->init(24);      // init stack and push bin in stack
  while (kTRUE) {
    for(Int_t nTs=0; nTs<8; nTs++) {
      Int_t nb=nBin+neighbBins[nTs];
      if( !HMdcBArray::testAndUnset(cPlotBAr,nb) ) continue;
...
Int_t HMdcLookUpTbSec::getMaxClus(Int_t m) {
...
//          nSegs=(fMdcDet->getModule(sec,2) || fMdcDet->getModule(sec,3)) ?2:1;
        ...
        if(stacksArr==0) {
          stacksArr = new HMdcClFnStacksArr(5000);
          stack     = new HMdcClFnStack(10000);
        }
        secF->stack     = stack;
  ...
  for(Int_t mod=0; mod<4; mod++) {
    ...
    for(Int_t layer=0; layer<6; layer++) {
      ...
      for(Int_t cell=0; cell<nCells; cell++) {
        ...
        // Lines 0,1,2,3 are lines from points 0-1, 1-2, 2-3, 3-0 respectively
        Int_t yBinMinT = 0;
        Int_t yBinMaxT = 0;
        Int_t xMinP = 0;
        Int_t xMaxP = 0;
	  ...
	  if(cellProj[yBinMaxT].gety() < cellProj[i].gety()) yBinMaxT=i;
          if(cellProj[xMinP].getx() > cellProj[i].getx()) xMinP=i;
	  if(cellProj[xMaxP].getx() < cellProj[i].getx()) xMaxP=i;
        }        
        yBinMinT = fsec.yBinNumInBounds(cellProj[yBinMinT].gety());
        yBinMaxT = fsec.yBinNumInBounds(cellProj[yBinMaxT].gety());
        fLUpTbCell.init(yBinMinT,yBinMaxT);
        
        Int_t yBMinX = fsec.yBinNum(cellProj[xMinP].gety());
        Int_t yBMaxX = fsec.yBinNum(cellProj[xMaxP].gety());
        Int_t nx1t = fsec.nBinX-2;
        Int_t nx2t = 1;
        Double_t x1 = cellProj[xMinP].getx();
        Double_t x2 = cellProj[xMaxP].getx();
        for(Int_t ny=yBinMinT; ny<=yBinMaxT; ny++) {
          if(ny<yBinMaxT) {              
            Double_t y=(ny+1)*fsec.yStep+fsec.yLow;
            calcX(cellProj, y, x1, x2);
          } 
          // For last Y bin take x1 & x2 from previous Y bin or from cell proj. 
          //===== Take into account cell proj. corners:
          if(ny==yBMinX && x1>cellProj[xMinP].getx()) x1=cellProj[xMinP].getx();
          if(ny==yBMaxX && x2<cellProj[xMaxP].getx()) x2=cellProj[xMaxP].getx();
          Int_t nx1i = fsec.xBinNumInBounds(x1);
          Int_t nx2i = fsec.xBinNumInBounds(x2);
...
Bool_t HMdcLookUpTb::calcCellProj(Int_t sec,HMdcTrap& cellSize,
  ...
  Bool_t indb = fabs(pProj[0](0)-pProj[3](0)) > sig;
  for(Int_t i=0; i<2; i++) {
    for(Int_t j=0; j<2; j++) {
  ...
  Bool_t indu = fabs(pProj[1](0)-pProj[2](0)) > sig;
  for(Int_t i=0; i<2; i++) {
    for(Int_t j=0; j<2; j++) {
