...
{  // set up empty task sets for all trigger types
    ...
    fTaskList->AddAt(new HTaskSet("special1"      ,"Main task for SPEC1 events")          , 6);
    fTaskList->AddAt(new HTaskSet("offspill"      ,"Main task for off-spill events")      , 7);
    fTaskList->AddAt(new HTaskSet("special3"      ,"Main task for SPEC3 events")          , 8);
    fTaskList->AddAt(new HTaskSet("MDCcalibration","Main task for MDC calibration events"), 9);
    fTaskList->AddAt(new HTaskSet("special5"      ,"Main task for SPEC5 events")          , 10);
    fTaskList->AddAt(new HTaskSet("illegal1"      ,"Main task for ILLEGAL1 events")       , 11);
    fTaskList->AddAt(new HTaskSet("illegal2"      ,"Main task for ILLEGAL2 events")       , 12);
    fTaskList->AddAt(new HTaskSet("beginrun"      ,"Main task for BEGIN RUN events")      , 13);
    fTaskList->AddAt(new HTaskSet("endrun"        ,"Main task for END RUN events")        , 14);
    fTaskList->AddAt(new HTaskSet("illegal5"      ,"Main task for ILLEGAL5 events")       , 15);
...
HTask* Hades::getTask(Char_t *name)
...
Int_t Hades::eventLoop(Int_t nEvents, Int_t firstEvent, Double_t timeQuota)
{
    ...
    Int_t err                    = 0;
    Int_t errAll                 = 0;
    Int_t errPreviousEvent       = 0;
    Int_t errAllPreviousEvent    = 0;
    EDsState dsCode              = kDsOk;
    EDsState dsCode2             = kDsOk;
    EDsState dsCodePreviousEvent = kDsOk;
    //----------------------------------
	...
	while ((evN < lastEvent) && timerFlag)
	{
		...
		if(evN == 0) { fSecondDataSource->setCursorToPreviousEvent(); }
		...
		// event
		errAllPreviousEvent = 99;
		errPreviousEvent    = 99;
		dsCodePreviousEvent = dsCode;
		continue;
	    }
	    //-------------------------------------------------------
