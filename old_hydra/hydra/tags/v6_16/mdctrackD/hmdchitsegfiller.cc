//*-- Author : A.Ierusalimov & V.Pechenov
...
void HMdcHitSegFiller::setFitParam(HMdcPlane* pl1,HMdcPlane* pl2,Double_t* par, 
    HSymMat4* errMat,Double_t errTOSet) {
  plane1=pl1;
  A1=pl1->A();
  B1=pl1->B();
  x1=par[0];
  ...
  x2=par[2];
  y2=par[3];
  z1=plane1->getZOnPlane(x1,y1);
  z2=plane2->getZOnPlane(x2,y2);
  covMatr=errMat;
  timeOffSet=par[4];
  errTimeOffSet=errTOSet;
}
...
void HMdcHitSegFiller::setClusParam(HMdcClus* clus, Bool_t isCoilOff) {
  ...
  timeOffSet=errTimeOffSet=0.;
}

void HMdcHitSegFiller::calcSegPar(void) {
  Double_t dX=x2-x1;
  Double_t dY=y2-y1;
  Double_t dZ=z2-z1;
...
void HMdcHitSegFiller::fillMdcSeg(HMdcSeg* mdcSeg) {
  calcSegPar();
  calcDerMatrToSeg();
  fillSegPar(mdcSeg);
...
// void HMdcHitSegFiller::fillMdcSegByClus(HMdcClus* clus, HMdcSeg* mdcSeg,
//     Bool_t isCoilOff) {
...
void HMdcHitSegFiller::fillMdcHit(HMdcSizesCellsMod* fSCMod, HMdcHit* mdcHit) {
  ...
  mdcHit->setPar(x,y,xDir,yDir,timeOffSet);
  //---------------------------------------------------------------------  
  ...
  cov.setElement(4,4,errTimeOffSet*errTimeOffSet);
}
