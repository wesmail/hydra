...
Bool_t HRuntimeDb::addParamContext(const char* context) {
  ...
  if (!found) ERROR_msg(DET_ALL,"Unknown context");
...
Bool_t HRuntimeDb::addContainer(HParSet* container) {
  ...
  gHades->getMsg()->warning(10,DET_ALL,this->GetName(),"Container with name %s exists already!",name);
...
HParSet* HRuntimeDb::getContainer(Text_t* name) {
  ...
  if (!c) gHades->getMsg()->error(1,DET_ALL,this->GetName(),"Container %s not created!",name);
...
HRun* HRuntimeDb::addRun(Int_t runId,Int_t refId) {
  ...
  if (!run) {
    ...
    runs->Add(run);
    currentRun=run;
  } else WARNING_msg(10,DET_ALL,"run exists already!");
...
Bool_t HRuntimeDb::writeContainers() {
  ...
  if (!rc) ERROR_msg(DET_ALL,"error while writing container to output");
    ...
    if (isRootFileOutput) {
      if (cont->hasChanged()) {
        ...
        if (cv==0){
          ...
          if (cv>0) {
	      gHades->getMsg()->info(10,DET_ALL,this->GetName(),"%s written to ROOT file, version: %d",c,cv);
      ...
      if (cont->hasChanged()) {
        ...
        if (cv>0) gHades->getMsg()->info(10,DET_ALL,this->GetName(),"%s written to output",c);
...
Bool_t HRuntimeDb::readAll() {
  ...
  if (!(getOutput() && output->check())) {
      SEPERATOR_msg("*",80);
      WARNING_msg(10,DET_ALL,"no output !");
      SEPERATOR_msg("*",80);
      //cout<<"***********************************************************"<<endl;
...
Bool_t HRuntimeDb::initContainers(void) {
  ...
  if (currentFileName.IsNull())
    {
      gHades->getMsg()->info(10,DET_ALL,this->GetName()," initialisation for run id %s",currentRun->GetName());
    }
    //cout<<"     initialisation for run id "<<currentRun->GetName();
  else {
    gHades->getMsg()->info(10,DET_ALL,this->GetName()," initialisation for event file %s",currentFileName.Data());
    gHades->getMsg()->info(10,DET_ALL,this->GetName(),"  run id %s",currentRun->GetName());
    //cout<<"     initialisation for event file "<<currentFileName.Data()<<'\n';
    //cout<<"     run id "<<currentRun->GetName();
  }
  if (len>0) 
    {
      gHades->getMsg()->info(10,DET_ALL,this->GetName(),"%10s %s","-->",refRunName);
      //cout << " --> " << refRunName;
    }
  SEPERATOR_msg("*",60);
  //cout << '\n';
  ...
  if (!rc) 
    ERROR_msg(DET_ALL,"error occured during initialization");
...
Bool_t HRuntimeDb::setInputVersion(Int_t run,Text_t* container,
  ...
  if (r) {
    ...
    else ERROR_msg(DET_ALL,"Container not found");
  }
  else ERROR_msg(DET_ALL,"run not found");
  return kFALSE;
}
...
void HRuntimeDb::print() {
  ...
  SEPERATOR_msg("-",60);
  INFO_msg(10,DET_ALL,"actual containers in runtime database");
  TIter nextCont(containerList);
  HParSet* cont;
  while((cont=(HParSet*)nextCont())) {
    //printf("%s\t\t%s\n",cont->GetName(),cont->GetTitle());
    gHades->getMsg()->info(10,DET_ALL,this->GetName(),"%s\t\t%s",cont->GetName(),cont->GetTitle());
  }
  TIter next(runs);
  HRun* run;
  //cout<<"-----------------  runs, versions  -----------------"<<'\n';
  INFO_msg(10,DET_RUNTIMEDB,"runs, versions");
  //cout<<"run id"<<'\n';
  SEPERATOR_msg("-",80);
  INFO_msg(10,DET_RUNTIMEDB,"run id");
  SEPERATOR_msg("-",40);
  gHades->getMsg()->info(10,DET_RUNTIMEDB,this->GetName(),"%-22s%10s%10s%10s","container","1st-input","2nd-input","output");
  ...
  //cout<<"---------------------  input/output  ----------------------"<<'\n';
  SEPERATOR_msg("-",80);
  INFO_msg(10,DET_RUNTIMEDB,"input/output");
  SEPERATOR_msg("-",40);
  //cout<<"first Input:"<<'\n';
  if (firstInput) 
      {
	  INFO_msg(10,DET_RUNTIMEDB,"first Input:");
	  firstInput->print();
      }
  else 
      INFO_msg(10,DET_RUNTIMEDB,"first Input: none");
      //cout<<" none"<<'\n';
  //cout<<'\n'<<"second Input:"<<'\n';
  SEPERATOR_msg("-",80);
  if (secondInput) 
      {
	  INFO_msg(10,DET_RUNTIMEDB,"second Input:");
	  secondInput->print();
      }
  else INFO_msg(10,DET_RUNTIMEDB,"second Input: none");
      //cout<<" none"<<'\n';
  //cout<<'\n'<<"Output:"<<'\n';
  SEPERATOR_msg("-",80);
  if (output)
      {
	  INFO_msg(10,DET_RUNTIMEDB,"Output:");
	  output->print();
      }
  else INFO_msg(10,DET_RUNTIMEDB,"Output: none");
  //cout<<" none"<<'\n';
  //cout<<"-----------------------------------------------------------"<<'\n';
  SEPERATOR_msg("-",80);
}
...
Bool_t HRuntimeDb::setFirstInput(HParIo* inp1) {
  ...
  else ERROR_msg(DET_RUNTIMEDB, "no connection to input");
...
Bool_t HRuntimeDb::setOutput(HParIo* op) {
  ...
  else ERROR_msg(DET_RUNTIMEDB,"no connection to output");
...
void HRuntimeDb::saveOutput() {
  ...
  writeVersions();
  if (!rc) ERROR_msg(DET_RUNTIMEDB,"error occured during write");
 ...
 HParSet *param=0;
   ...
   if (R__b.IsReading()) {
      ...
      for(Int_t i=0 ; i < nentries ; i++) {
         ...
         R__b >> param;
	 gHades->getMsg()->info(10,DET_RUNTIMEDB,this->GetName(),"Loading container %s",param->GetName());
