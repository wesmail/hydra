// @(#)$Id: hpidtrackfiller.cc,v 1.11 2004-04-30 16:29:02 christ Exp $
            ...
            if(pitRich != NULL)
            {
                ...
                while((pRing = (HRichHit *) pitRich->Next()) != NULL)
                {
					...
					if((pRing->getTheta() - fKickTheta) 
						<(pParams->getLeftKickRichTheta()).fArray[iSector]) continue;

					if((pRing->getTheta() - fKickTheta) 
						>(pParams->getRightKickRichTheta()).fArray[iSector]) continue;
					
					if(((pRing->getPhi() - fKickPhi) * fSinTheta) 
					     <(pParams->getLeftKickRichPhi()).fArray[iSector]) continue;

					if(((pRing->getPhi() - fKickPhi) * fSinTheta) 
					     >(pParams->getRightKickRichPhi()).fArray[iSector]) continue;
