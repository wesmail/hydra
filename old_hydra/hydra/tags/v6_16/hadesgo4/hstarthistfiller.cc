...
void HStartHistFiller::fillRaw(HIterator* iterraw)
{
    ...
    while ((raw=(HStartRaw *)iterraw->Next())!=0) {
		...
		if(mod<3){	
			...
			if(htime_veto_raw  && mod==1) 
				 htime_veto_raw->fill(0,strip,time);
		}
		else{//Hodoscopes
		...
		while ((raw=(HStartRaw *)iterraw->Next())!=0) {
			...
			if(mod==0) {
				...
				while ((raw=(HStartRaw *)iterveto->Next())!=0) {
					...
					if(mod==1)
					hstart_veto_corr_raw->getP()->
						Fill((Axis_t)strip,(Axis_t)vetostrip);
				}
			}
		}
    }
...
void HStartHistFiller::fillCal(HIterator* itercal)
{
    ...
    while ((cal=(HStartCal *)itercal->Next())!=0) {
		...
		nMultHod[nMod-3] = cal->getMult(nMod);
		if((fTimeL>-1 && fTimeR>-1) || fTimeM >-1 || fTime>-1){
			if(hstrips_hodo_cal)hstrips_hodo_cal->fill(0,nMod-3,nStrip);
			if(nMod==3)
				if(htime_hodo_temp)htime_hodo_temp->fill(0,nStrip,fTof);
		}
		if(hstrips_hodo_tof){
...
void HStartHistFiller::fillTrend() {
	...
	for(Int_t s=0;s<16;s++){
		...
		if(htime_trend_hodo)
		 htime_trend_hodo->
			 fill(0,s,htime_hodo_temp->getP(0,s)->GetMean());
		}
	}	
	if(hscaler_tmp){	
		for(int i=0;i<8;i++){
