...
void   HTofHistFiller::putTofParams(HParamList* l,TList* histpool)
{
    ...
    // HParamList (which ist used by the io);
       ...
       l->add("htofmult_raw"           ,mytemp,'s',500);
       htofmulttemp_rawA->getText(mytemp);
       l->add("htofmulttemp_raw"       ,mytemp,'s',500);
       htofmulttrend_rawA->getText(mytemp);
       l->add("htofmulttrend_raw"      ,mytemp,'s',500);
       hhits_rawA->getText(mytemp);
       l->add("hhits_raw"              ,mytemp,'s',500);
       hmulttof_rawA->getText(mytemp);
       l->add("hmulttof_raw"           ,mytemp,'s',500);
       htofpatternL_rawA->getText(mytemp);
       l->add("htofpatternL_raw"       ,mytemp,'s',500);
       htofpatternR_rawA->getText(mytemp);
       l->add("htofpatternR_raw"       ,mytemp,'s',500);
       htoftdcL_rawA->getText(mytemp);
       l->add("htoftdcL_raw"           ,mytemp,'s',500);
       htoftdcR_rawA->getText(mytemp);
       l->add("htoftdcR_raw"           ,mytemp,'s',500);
       htofadcL_rawA->getText(mytemp);
       l->add("htofadcL_raw"           ,mytemp,'s',500);
       htofadcR_rawA->getText(mytemp);
       l->add("htofadcR_raw"           ,mytemp,'s',500);
       htoftdcSum_hitA->getText(mytemp);
       l->add("htoftdcSum_hit"         ,mytemp,'s',500);
       htoftdcDiff_hitA->getText(mytemp);
       l->add("htoftdcDiff_hit"        ,mytemp,'s',500);
       htofadcSum_hitA->getText(mytemp);
       l->add("htofadcSum_hit"         ,mytemp,'s',500);
       htofadcPos_hitA->getText(mytemp);
       l->add("htofadcPos_hit"         ,mytemp,'s',500);
       htofsum_hitA->getText(mytemp);
       l->add("htofsum_hit"            ,mytemp,'s',500);
       htoftheta_hitA->getText(mytemp);
       l->add("htoftheta_hit"          ,mytemp,'s',500);
       htofphi_hitA->getText(mytemp);
       l->add("htofphi_hit"            ,mytemp,'s',500);
       htofyvsx_hitA->getText(mytemp);
       l->add("htofyvsx_hit"           ,mytemp,'s',500);
       hmulttoftofino_rawA->getText(mytemp);
       l->add("htofmulttoftofino_raw"  ,mytemp,'s',500);
}
