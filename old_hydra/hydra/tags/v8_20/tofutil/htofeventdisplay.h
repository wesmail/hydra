...
class HTofEventDisplay : public TGMainFrame {
 ...
 public:
  ...
  void setInput(const Char_t *name) { if(name) sprintf(source,"%s",name); }
  void setOutput(const Char_t *name) { if(name) sprintf(target,"%s",name); }
  void setCalib(const Char_t *name) { if(name) sprintf(calib,"%s",name); }
  const Char_t *getInput() const { return source; }
  const Char_t *getOutput() const { return target; } 
  const Char_t *getCalib() const { return calib; }
