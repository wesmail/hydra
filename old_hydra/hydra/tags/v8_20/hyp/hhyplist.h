...
public:
  ...
  Int_t           getIdxPidTrackCand (Char_t *name, Int_t);            // returns PidTrackCand index for given particle (via name)
  ...
  TLorentzVector  getTLorentzVector (Char_t *, Int_t);            // returns TLorentzVector for given name (p,pi+...)
  ...
  Float_t         getBeta(Char_t *name, Int_t num);
  ...
  Bool_t          setMomentum (Char_t *name, Int_t num, TVector3 vect);   // sets a new (modified) momentum vector

  HPidTrackCand   *getPidTrackCand (Int_t Ipart);
  HPidTrackCand   *getPidTrackCand(Char_t *name, Int_t num);
