...
Bool_t HMdcParRootFileIo::init(HParSet* pPar,Int_t* set) {
  ...
  if (pFile) {
    ...
    if (strncmp(name,"MdcWireStat",strlen("MdcWireStat"))==0) return HDetParRootFileIo::read(pPar);
  ...
  gHades->getMsg()->error(1,HMessageMgr::DET_MDC,"initialization of %s not possible from ROOT file!",name);
  return kFALSE;
}
...
Bool_t HMdcParRootFileIo::read(HMdcModuleGeometry* pGeoPar,Int_t* set) {
  ...
  Text_t* name = (Char_t*)pGeoPar->GetName();
  Int_t version=findInputVersion(name);
   if (version<=0)
  {
...
Bool_t HMdcParRootFileIo::read(HMdcClosePairCutsPar* pClosePairCut,Int_t* set) {
  ...
  Text_t* name=(Char_t*)pClosePairCut->GetName();
  Int_t version=findInputVersion(name);
