...
class HTofRec : public HReconstructor {
 ...
 public:    
  HTofRec(void);
  HTofRec(Text_t name[],Text_t title[],const Char_t *select="", const Char_t *track="");
  ~HTofRec(void);
  Bool_t init(void);
  ...
  void setOutputDir(Char_t *fdir) {sDir = fdir;}
  void setOutputFile(Char_t *ffile) {sFile = ffile;}
