...
// Last update by Angelika Elhardt: 00/10/10 13:55:58
// last modified by T. Eberl: 2000/10/16
...
Bool_t HRichPedestalRaw::checkSectorList(){
	...
	char b = sectorList[i];
	if (!(b>47 && b<55)){
	    cout<<"Attention!!!  "<<sectorList[i]<<" is not a valid input!"<<endl;
	    checkInput = false;
...
void HRichPedestalRaw::bookHistos(){
	...
	sprintf(temp1,"mean_%d",i);
	sprintf(temp2,"mean1d_%d",i);
	sprintf(temp3,"sigma_%d",i);
	sprintf(temp4,"sigma1d_%d",i);
	sprintf(temp5,"stat_%d",i);
...
void HRichPedestalRaw::fill2DHistos(){
	...
	if (mask[i][0]==1){    // wichtig!
		...
		for (int k = 0; k<rowMax; k++){
			...
			cout<<"!2D!numeric prob occured @ event no "
...
void HRichPedestalRaw::createPedestal(){
	...
	if(mask[i][0] == 1){ 
	    ...
	    cout<<"------------------------------------------"<<endl;
	    cout<<"pedestal file for sector "<<i<<" written: "<<endl; 
...
void HRichPedestalRaw::createCalibration(){
	...
	for (Int_t i = 0; i<colMax; i++){
		...
		}else     calOut<<j<<"	"<<i<<"	1"<<"	"<<0<<endl; // zero calibration parameters for nonactive sectors !
	    }
	}	
	cout<<endl;
    }
