...
//timer.Start();
 for(Int_t mod=0; mod<4; mod++) maxClust[mod]=imax[mod];
 doPlot();
...
//  if(hitplot[nc]>0 && testBiN[hitplot[nc]]>=maxCl) {
   if(hitplot[nc]>0 && tablBiN[hitplot[nc]]>=maxCl) {
//  maxLocal=imax-tablBiN[bMboard[nc]];
//  if(maxLocal<4) maxLocal=4;
//  maxCl=maxLocal;
//  if(hitplot[nc]>0 && tablBiN[hitplot[nc]]>=maxCl) { // For beamtime 9.99 only
  //---New cluster---------------------------------------
...
// Determining of min.Xbin & max.Xbin for each line (y) in cluster
 ...
 //   Double_t wt=(Double_t)(testBiN[-hitplot[nci]]-maxCl+1);
    Double_t wt=(Double_t)(tablBiN[-hitplot[nci]]-maxCl+1);
//    Double_t wt=(Double_t)(tablBiN[-hitplot[nci]]-maxCl+1); // For beamtime 9.99
    binsCl+=wt;
    ...
    for(Int_t nTs=0; nTs<8; nTs++) {
     ...
     if(hitplot[nBtmp]>0) {
//      if(testBiN[hitplot[nBtmp]]>=maxCl) {
       if(tablBiN[hitplot[nBtmp]]>=maxCl) {
//      if(tablBiN[hitplot[nBtmp]]>=maxCl) {  // For beamtime 9.99 only
       stack[stackB++]=nBtmp;
       hitplot[nBtmp]*=-1;
      }
     }
...
// nClusters=clusters.Test(Ap,Bp,Dp);
  ...
  for(Int_t nClS=nClF+1; nClS<nClusters; nClS++) {
   ...
   if(fabs(pClF.getX()-pClS.getX()) >= 30.) continue;  // 30. mm !???
...
//!!!!!!!!!!!!!!!!
     ...
     if(fabs(pClT.getX()-pClF.getX()) >= 30.) continue;  // 30. mm !???
...
// clusters.Sort(); // Onli after calcxy!!!
 ...
 for(Int_t nCl=nClFirst; nCl<nClusters; nCl++) {
  Int_t nTotCell=0;
  Int_t nCellDb=0;
  HMdcCluster& pCl=clusters[nCl];
...
//---------------------------------------------------------
HMdcLookUpTb::HMdcLookUpTb(Int_t inBinX=630,Bool_t isCoilOff=kFALSE) :
                                                            HParSet() {
...
//----------------------------------------------------------
HMdcCalcLookUpTb::HMdcCalcLookUpTb(Bool_t isCoilOff=kFALSE) {
