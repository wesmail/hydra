...
Int_t HMdc12Clusters::addCluster(Short_t maxClus) {
 clusters->AddLast(new HMdcCluster(maxClus));
...
Bool_t HMdcCluster::Test(HMdcCluster& clst, Double_t Ap, Double_t Bp, 
  ...
  Int_t imax=maxClus;
  if(clst.maxClus < imax) imax=clst.maxClus;
  Short_t newMax=maxClus;
  if(clst.maxClus > maxClus) newMax=clst.maxClus;
  Int_t maxCr=0;
  for(Int_t nl=0; nl<24; nl++) {    // for 1 MDC we need in ...!!!!
    ...
    Int_t m=nl/6;
    if(isId>0) nIdent[m]++;
    if(nIdent[m] > 3 || nIdent[m] >= imax) maxCr++;
  }
