...
Int_t HTofHitF::execute(void) {
  ...
  while ( (raw=(HTofRaw *)iter->Next())!=NULL) {
	...
	if(gTofPos) {
	  ...
	  x = (Float_t) gTofPos->getTofLabX(fLoc[0],fLoc[1],fLoc[2],xtof);
	  y = (Float_t) gTofPos->getTofLabY(fLoc[0],fLoc[1],fLoc[2],xtof);
	  z = (Float_t) gTofPos->getTofLabZ(fLoc[0],fLoc[1],fLoc[2],xtof);
	  dist = (Float_t) TMath::Sqrt(x*x + y*y + z*z);

	  if(dist>0.0) theta = rad2deg * TMath::ACos(z/dist);
	  phi = rad2deg * TMath::ATan2(y,x);
	  if(phi < 0.0) phi += 360.;
