...
HKickTrackFPar::HKickTrackFPar(void) {
  fName="KickTrackFPar";
  fTitle="Parameter container for the track finder of the kick plane";
  strcpy(detName,"Mdc");
  clear();
}
...
Int_t HKickTrackFPar::writeFile(fstream *pFile) {
    ...
    Text_t com[]="###############################################################################\n"
      ...
      "# Format:\n"
      "#  pMin1 pMin2 fMaxXPull1 fMaxXPull2 fXPullStep\n"
      "###############################################################################\n";
...
void HKickTrackFPar::printTo(ostream &out) {
  out << fPMin << fPMin2 << fMaxXPull1 << fMaxXPull2 << fXPullStep << endl;
}
