...
Char_t buffer2[1024];
Int_t breakline[20];
Int_t filler, underliner, precounter, counter, pos, stop, i,j,k,l,m;

for (i=0; i<20; i++) breakline[i] = 0;
buf[0] = '\0';
buffer[0] = '\0';
buffer2[0] = '\0';
...
while (counter < j) {
 ...
 if (pos < 74) {
  if (counter + pos <= precounter + 74) {
   counter += pos;
   if (counter == j) breakline[i] = precounter = counter;
  } else {
 ...
 if (NodeNumber()) {
  ...
  if (strlength(name)==0 || strempty(name)) {
   ErrorMsg(1,"HAsciiSectionList::ToNodeSection",
    1,"Section name in function not specified. Pointer to the first section 
...
c - Char_t, s - string).");
 ...
 if ((typ == 'i' && (fDataBank != 1 && fDataBank != 11)) ||
  ...
  Char_t tempbuf[10];
  tempbuf[0] = '\0';
  if (fDataBank==1 || fDataBank==11) strcopy(tempbuf,"Int_t");
   else if (fDataBank==2 || fDataBank==22) strcopy(tempbuf,"Float_t");
    else if (fDataBank==3 || fDataBank==33) strcopy(tempbuf,"Char_t");
     else if (fDataBank==4 || fDataBank==44) strcopy(tempbuf,"string");
  ErrorMsg(2,"HAscii::CheckDataType",
...
// Reading data of given keyword key as string.
 ...
 if (strcompare(cKey,fKeyLine) != 0) {
  ...
  if (!ConvertToString()) {
   ErrorMsg(2,"HAscii::ReadString",
    1,"Reading of \'string\' data aborted.");
   throw (Bool_t) kFALSE;
  }
 }
return ReadString();
 ...
 if (strcompare(cKey,fKeyLine) != 0) {
  ...
  if (!ConvertToInt()) {
   ErrorMsg(2,"HAsciiKey::ReadInt",
    1,"Reading of \'Int_t\' data aborted.");
   throw (Bool_t) kFALSE;
  }
 }
return ReadInt(pos); 
 ...
 if (strcompare(cKey,fKeyLine) != 0) {
  ...
  if (!ConvertToFloat()) {
   ErrorMsg(2,"HAsciiKey::ReadFloat",
    1,"Reading of \'Float_t\' data aborted.");
   throw (Bool_t) kFALSE;
  }
 }
 ...
 if (strcompare(cKey,fKeyLine) != 0) {
  ...
  switch (typ) {
    ...
    case 'c': if (!ConvertToChar()) {
               ErrorMsg(2,"HAsciiKey::Read",
                1,"Reading of \'Char_t\' data aborted.");
               throw (Bool_t) kFALSE;
              }
              break;
...
// Reading one Int_t digit from column col in position row.
 ...
 if (fpActiveSection->fpRawData->fpKeyTypeId[col] < 0) {
  ...
  if (!ConvertToInt()) {
   ...
   sprintf(buf,"%d",col);
   ErrorMsg(2,"HAsciiRaw::ReadInt",
    3,"Conversion of raw data column ",buf," to \'Int_t\' aborted.");
   throw (Bool_t) kFALSE;
  }
...
// Reading one Float_t digit from column col in position row.
 ...
 if (fpActiveSection->fpRawData->fpKeyTypeId[col] < 0) {
  ...
  if (!ConvertToFloat()) {
   ...
   sprintf(buf,"%d",col);
   ErrorMsg(2,"HAsciiRaw::ReadFloat",
    3,"Conversion of raw data column ",buf," to \'Float_t\' aborted.");
   throw (Bool_t) kFALSE;
  }
...
// Reading one Char_t character from column col in position row.
 ...
 if (fpActiveSection->fpRawData->fpKeyTypeId[col] < 0) {
  ...
  if (!ConvertToChar()) {
   ...
   sprintf(buf,"%d",col);
   ErrorMsg(2,"HAsciiRaw::ReadChar",
    3,"Conversion of raw data column ",buf," to \'Char_t\' aborted.");
   throw (Bool_t) kFALSE;
  }
