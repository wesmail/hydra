...
//*-- Author : R. Holzmann
//*-- Modified: 09/06/2000 by R. Holzmann
...
//
  ...
  if (!fGeantRichPhotCat || !fGeantRichDirCat || !fGeantRichMirrorCat) {
     ...
     if (fInputFile->cd("dirSimul")) { // change dir, read category definition
        ...
        Int_t ini[2] = {6,200};      // use old default definition 
        fGeantRichPhotCat = new HMatrixCategory("HGeantRichPhoton",2,ini,1.0);
        fGeantRichDirCat = new HMatrixCategory("HGeantRichDirect",2,ini,1.0);
        Int_t ini1[2] = {6,50};
        fGeantRichMirrorCat = new HMatrixCategory("HGeantRichMirror",2,ini1,1.0);
     }
