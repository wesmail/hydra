...
protected:
  Float_t  tNhits; // total num. of hits
...
public:
  ...
  Double_t getSingle(void) {return single;}
  Double_t getErrSingle(void) {return errSingle;}
...
protected:
  ...
  Short_t nCells;
  Int_t *nCellStat; // Total num. times a cell was fired
  ...
  HMdcEff effXY;
  TCanvas* canvXY[10];
  Text_t nameCanvXY[10][20];
  Text_t titleCanvXY[10][60];
...
public:
  HMdcEfficiencyLayer(Short_t sec, Short_t mod, Short_t lay, Int_t nBiXY);
  ~HMdcEfficiencyLayer();
  void clear();
  ...
  Int_t getNumXYbins(){return nBinsXY;}  //nBinsX=nBinsY=nBinsXY
  ...
  void drawCanvTotNhits() {drawCanvXY(0);}
  void drawCanvNTracks() {drawCanvXY(1);}
  void drawCanv0Hits() {drawCanvXY(2);}
  void drawCanv1Hits() {drawCanvXY(3);}
  void drawCanv2Hits() {drawCanvXY(4);}
  void drawCanvEfficiency() {drawCanvXY(5);}
  void drawCanvSingle() {drawCanvXY(6);}
  void drawCanvP0() {drawCanvXY(7);}
  void drawCanvP1() {drawCanvXY(8);}
  void drawCanvP2() {drawCanvXY(9);}
...
protected:
  ...
  Int_t nBinsXY;  // Num. of bins in plot of efficiency for X and Y exes
  ...
  Float_t yUp;    //
  Float_t dY;     //
  Int_t *n0xy;    // arrays for keeping of inform. about eff. of MDC regions
  Int_t *n1xy;    //
  Int_t *n2xy;    //
...
public:
  ...
  void clear();
  void add0Hit(Int_t nB) { n0++; if(nBinsXY) n0xy[nB]++;}
  void add1Hit(Int_t nB) { n1++; if(nBinsXY) n1xy[nB]++;}
  void add2Hit(Int_t nB) { n2++; if(nBinsXY) n2xy[nB]++;}
