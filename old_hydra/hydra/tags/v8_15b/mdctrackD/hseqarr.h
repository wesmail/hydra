...
class HSeqArr : public TObject {
  ...
  protected:
    ...
    Bool_t tpOk(ESeqArrType tp) {if(type==tp) return kTRUE; return err(tp);}
  ...
  public:
    ...
    Bool_t isBoundOk(UInt_t i) const {return i < size;}
...
class HSeqArrIter : public TObject {
  ...
  public:
    ...
    Bool_t setCursor(Int_t indArr=-1);
    UInt_t getCursor(void) const {return index/array->wordLength;}
