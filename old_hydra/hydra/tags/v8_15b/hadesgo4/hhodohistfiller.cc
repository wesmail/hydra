...
void HHodoHistFiller::fillTrend()
{
  ...
  if(hEfficiency){
    if( effi_count_m0_all>=100){
      hEfficiency->fill(0,0,100.0*effi_count_m0/effi_count_m0_all);
      effi_count_m0=0;
      effi_count_m0_all=0;
    }
    if( effi_count_m2_all>=100){
      hEfficiency->fill(0,1,100.0*effi_count_m2/effi_count_m2_all);
      effi_count_m2=0;
      effi_count_m2_all=0;
    }
    if( effi_count_m3_all>=100){
      hEfficiency->fill(0,2,100.0*effi_count_m3/effi_count_m3_all);
      effi_count_m3=0;
      effi_count_m3_all=0;
    }
  }
...
void   HHodoHistFiller::putHodoParams(HParamList* l,TList* histpool)
{
	...
	l->add("hHodoProf1"                  ,mytemp,'s',500);
	hProf2A->getText(mytemp);
	l->add("hHodoProf2"                  ,mytemp,'s',500);
	hProf3A->getText(mytemp);
	l->add("hHodoProf3"                  ,mytemp,'s',500);
	hProf4A->getText(mytemp);
	l->add("hHodoProf4"                  ,mytemp,'s',500);
	
	hRawTime1A->getText(mytemp);
	l->add("hHodoRawTime1"               ,mytemp,'s',500);
	hRawTime2A->getText(mytemp);
	l->add("hHodoRawTime2"               ,mytemp,'s',500);
	hRawTime3A->getText(mytemp);
	l->add("hHodoRawTime3"               ,mytemp,'s',500);
	hRawTime4A->getText(mytemp);
	l->add("hHodoRawTime4"               ,mytemp,'s',500);
	
	hRawWidth1A->getText(mytemp);
	l->add("hHodoRawWidth1"              ,mytemp,'s',500);
	hRawWidth2A->getText(mytemp);
	l->add("hHodoRawWidth2"              ,mytemp,'s',500);
	hRawWidth3A->getText(mytemp);
	l->add("hHodoRawWidth3"              ,mytemp,'s',500);
	hRawWidth4A->getText(mytemp);
	l->add("hHodoRawWidth4"              ,mytemp,'s',500);
	
	hCrT1A->getText(mytemp);
	l->add("hHodoCrT1"                   ,mytemp,'s',500);
	hCrT2A->getText(mytemp);
	l->add("hHodoCrT2"                   ,mytemp,'s',500);
	hCrT3A->getText(mytemp);
	l->add("hHodoCrT3"                   ,mytemp,'s',500);
	hCrT4A->getText(mytemp);
	l->add("hHodoCrT4"                   ,mytemp,'s',500);	

	hFib1MultA->getText(mytemp);
	l->add("hHodoFib1Mult"               ,mytemp,'s',500);
	hFib2MultA->getText(mytemp);
	l->add("hHodoFib2Mult"               ,mytemp,'s',500);
	hFib3MultA->getText(mytemp);
	l->add("hHodoFib3Mult"               ,mytemp,'s',500);
	hFib4MultA->getText(mytemp);
	l->add("hHodoFib4Mult"               ,mytemp,'s',500);
...
/*	if(!(l->fill("hHodohits_raw",mytemp,500))) {return kFALSE;}
	...
	else
	{
		...
		if(hEfficiencyA->getActive()==1)
		{
         ...
         for(Int_t s=0; s<3; s++){
			 hEfficiency->getP(0,s)->SetLineColor(colorsSector[s]);
			 hEfficiency->getP(0,s)->SetMarkerColor(colorsSector[s]);
         }
		}
	}
