...
HPidPDFBuilderExp::HPidPDFBuilderExp(const Char_t* Extension,const Char_t* OutputFileBase)
{
  N_Cand_Total = 0;

  evcount=0;
  
  OutFileName = new TString(OutputFileBase);
  ...
  //CandidateProperties->SetDirectory(kFALSE);
...
Bool_t HPidPDFBuilderExp::init(void) 
{
  ...
  /*
  pPartCategory = gHades->getCurrentEvent()->getCategory(catPidPart);
    if(!pPartCategory)
    {
  ...
  if((pIteratorPart = (HIterator *)pPartCategory->MakeIterator()) == NULL)
    {
      Error("init", "Cannot make an iterator for HPidPart Category");
      return kFALSE;
    }
  */
