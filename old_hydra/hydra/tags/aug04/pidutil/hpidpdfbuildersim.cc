 ...
 if(!strcmp(mode,"withring"))
  {
    ...
    Int_t nMatchesFull, nMatchesMdcMeta, dummy;
  
    
    HPidFL::getGeantMatches(theCandidate, nCommonLeptonsRichMdc,nMatchesMdcMeta,nMatchesFull,nCommonLeptonsFull, 
			    nCommonLeptonsMdcMeta,dummy);
    
    //Int_t wait;
    //cout << "OLD task: ncommonleptonsmdcmeta: " << nCommonLeptonsMdcMeta << endl;
    //cin >> wait;
    if(!nMatchesMdcMeta)
      {
 ...
 if(!strcmp(mode,"noring"))
   {
     ...
     Int_t dummy;
     HPidFL::getGeantMatches(theCandidate,nMatchesMdcMeta,nMatchesFull, nCommonLeptonsMdcMeta, dummy);
  ...
  while((currentCandidate = (HPidTrackCand*) pIteratorInput->Next())!=NULL)
      ...
      if(currentCandidate->getRingId() == ringcounter && currentCandidate->getKickTrackId()>=0)
	{
	  HPidFL::getGeantMatches(currentCandidate, nCommonLeptonsRichMdc,nMatchesMdcMeta,nMatchesFull,
				  nCommonLeptonsFull, nCommonLeptonsMdcMeta, dummy);
