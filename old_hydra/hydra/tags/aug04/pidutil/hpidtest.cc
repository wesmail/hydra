...
Bool_t HPidTest::init(void) 
{
  ...
  recognizedParticles = new TNtuple("recognizedParticles","Recognized Particles","rec_ID:true_ID:exp_mom:true_mom:theta:seenInRICH:process:chi2:kick_charge:system:sum0:sum1:sum2:tof:sector");
...
Int_t HPidTest::execute(void)
{
  ...
  while((theParticle = (HPidParticleSimExtHit*) pIteratorParticle->Next())!=NULL)
    { 
      ...
      itsSector = theParticle->getKickTrack()->getSector();
      ...
      if(itsTrueId>0) //no fakes!
	{
	  ...
	  recognizedParticles->Fill(itsReconstructedId,
				    ...
				    itsSum0,itsSum1,itsSum2, itsTof, itsSector);
	}
      else //fakes here
	{
	  if(!(kickCharge==1.0 || kickCharge==-1.0)) cout <<"MegaOUCH!"<< endl;
