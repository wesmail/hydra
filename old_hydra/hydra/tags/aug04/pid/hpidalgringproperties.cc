// @(#)$Id: hpidalgringproperties.cc,v 1.5 2004-07-08 13:45:51 christ Exp $
            ...
            Float_t afReturn[], Short_t &nCatIndex)
{
  ...
  Bool_t isSpecialSector=kFALSE;
  Int_t sec = pKickTrack->getSector();
  if(sec==0) //we need the special normalization!
    {isSpecialSector=kTRUE;}
  ...
  if(pTrack->getRingId() < 0)
      ...
      for(Int_t particleCounter=0; particleCounter<pRec->particlesNumber(); particleCounter++)
	{
	    ...
	    if(iPID==2 && fCharge>0) //positron
	    {
	      ...
	      afReturn[particleCounter]=1.0-pParams->getRichLeptonFraction(TrackMomentum,fKickTheta,isSpecialSector);
	    }
	    ...
	    if(iPID==8 && fCharge>0) //pi+, proton, k+, deuteron
	    {
	      ...
	      afReturn[particleCounter]=1.0-pParams->getRichPiPlusFraction(TrackMomentum,fKickTheta,isSpecialSector);
	    }

	    if((iPID>9) && fCharge>0) //pi+, proton, k+, deuteron
	    {
	      //afReturn[particleCounter]=1.0-pParams->getRichHadronFraction(TrackMomentumBin);
	      afReturn[particleCounter]=1.0-pParams->getRichProtonFraction(TrackMomentum,fKickTheta,isSpecialSector);
	    }
	    
	    if(iPID==9 && fCharge<0) //pi-
	    {
	      //afReturn[particleCounter]=1.0-pParams->getRichHadronFraction(TrackMomentumBin);
	      afReturn[particleCounter]=1.0-pParams->getRichPiMinusFraction(TrackMomentum,fKickTheta,isSpecialSector);
	    }

	    if((iPID==-1 && fCharge<0) || (iPID==-2 &&fCharge>0)) //fake=, fake-
	    {
	      afReturn[particleCounter]=1.0-pParams->getRichFakeFraction(TrackMomentum,fKickTheta,isSpecialSector);
	    }
  ...
  if(fCentroid>=0.0)
  ...
  else
    {
      ...
      //RingPattern4D[0]=fPadNr;
      //RingPattern4D[1]=fPatMat;
  ...
  for(i = 0; i < pRec->particlesNumber(); i++)
	...
	{
	  /*
	  cout << "Evaluationg particle with ring! Zero PDF value returned" << endl;
	  ...
	  if(fCentroid>-1)
	    ...
	    {
	      cout << "Ring has no defined centroid! " << endl;
	      cout << "PMQ:" << RingPattern4D[0] << endl;
	      cout << "PadNr:" << RingPattern4D[1] << endl;
	      cout << "locmax:" << RingPattern4D[3] << endl;
	      cout << "avcharge:" << RingPattern4D[4] << endl; 
	    }
	    //cin >> wait;*/
	}
    }
  ...
  if(allZeros) cout << "Error for track with ring! ZERO PDF value returned for all species! " << fCentroid <<endl;

  correctRelativeYields(afReturn,pTrack->getKickTrack()->getP(),fKickTheta,isSpecialSector);
  return kTRUE;
}
...
Bool_t HPidAlgRingProperties::correctRelativeYields(Float_t* pdfValues,Float_t TrackMomentum, 
						    Float_t fKickTheta, Bool_t isSpecialSector)
{
  Int_t iPID;

  Float_t proton_yield_correction = pParams->getRichProtonFraction(TrackMomentum,fKickTheta,isSpecialSector);
  Float_t piplus_yield_correction = pParams->getRichPiPlusFraction(TrackMomentum,fKickTheta,isSpecialSector);
  Float_t piminus_yield_correction = pParams->getRichPiMinusFraction(TrackMomentum,fKickTheta,isSpecialSector);
  Float_t lepton_yield_correction = pParams->getRichLeptonFraction(TrackMomentum,fKickTheta,isSpecialSector);
  Float_t fake_yield_correction = pParams->getRichFakeFraction(TrackMomentum,fKickTheta,isSpecialSector);
