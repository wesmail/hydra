...
void HMdcTrackDContFact::setAllContainers() {
  ...
  HContainer* dedx= new HContainer("MdcDeDx",
                                   "parameters for Mdc dedx calculation",
                                   "MdcDeDxProduction");
  containers->Add(dedx);
...
HParSet* HMdcTrackDContFact::createContainer(HContainer* c) {
  ...
  if (strcmp(name,"MdcDeDx")==0)
    return new HMdcDeDx(c->getConcatName().Data(),c->GetTitle(),c->getContext());
  return 0;
}
