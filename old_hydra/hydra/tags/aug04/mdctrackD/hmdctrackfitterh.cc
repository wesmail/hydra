...
void HMdcTrackFitterH::setParam(void) {
  ...
  if(mdc>=0) {
    ...
    if(segment==0) {
      ...
      fitInOut->fitPar->getWeightsSeg1(sector,weightPar);
      stepNorm =  fitInOut->fitPar->getStepNormSeg1(sector);
      ...
      fitInOut->fitPar->getWeightsSeg2(sector,weightPar);
      stepNorm =  fitInOut->fitPar->getStepNormSeg2(sector);
...
void HMdcTrackFitterH::fillOutput(void) {
  ...
  chi2perDF = funMin/(numOfGoodWires - numOfParam +1);
