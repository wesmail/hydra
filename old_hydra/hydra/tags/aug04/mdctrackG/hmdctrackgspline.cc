...
void HMdcTrackGSpline::getMomentum(Bool_t cond,Int_t nchambers)
{
   ...
   for(Int_t i=1; i<N2; i++)
      {
	 qSpline+=(det3(deltaConst1)/det3(delta)+det3(deltaConst2)/det3(delta)*(equationz[i]-equationz[0])+1/precon*Yequation[i]-equationy[i])*
	    (det3(deltaConst1)/det3(delta)+det3(deltaConst2)/det3(delta)*(equationz[i]-equationz[0])+1/precon*Yequation[i]-equationy[i]);
      }
   qSpline/=45;
...
Double_t HMdcTrackGSpline::calcMomentum(HGeomVector  *pp,Bool_t cond,Double_t targ,Int_t nchambers)
{
   ...
   for(Int_t i=0;i<48;i++)
	 ...
	 b->calcField(xVector,fieldVector,0.92);
	 Bx[i]=fieldVector[0];
	 By[i]=fieldVector[1];
	 Bz[i]=fieldVector[2];
...
void  HMdcTrackGSpline::transSpline(Float_t phi,Float_t teta, HGeomVector &in,HGeomVector & out)
{
   ...
   if(teta==0.)
	 ...
	 Float_t inXtemp=in.getX();
	 Float_t inYtemp=in.getY();
	 Float_t inZtemp=in.getZ();
