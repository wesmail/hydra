...
Int_t HTofRec::execute(void){
  ...
  Float_t rich_theta[20], rich_phi[20];
  Int_t rich_sec[20];
  Int_t rich_count = 0;
  if (fRichMode)  // begin of RICH loop
    {
      iterRich->Reset();
      while (((rich=(HRichHit *)iterRich->Next()) != 0)&&(rich_count<20))
	{
  ...
  if (fSpline)  
    {
      Float_t sp_tof[50], sp_p[50], sp_len[50];
      Float_t start_tof = 0., rec_mass = 0.;
      Float_t chi2;
      Int_t sp_pol[50], sp_sys[50];
      Int_t sp_count  = 0;
      Int_t rec_ind   = -1;
      Int_t isMinus   = 0;
      Int_t isRich    = 0;
      iterSpline->Reset();
      while (((spline=(HSplineTrack *)iterSpline->Next()) != 0)&&(sp_count<50)) // begin of SPLINE loop
	{
	  if (spline->getSystem()!=-1&&spline->getP()>0.&&spline->getSector()!=2&&spline->getSector()!=5)  // if track is correlated to META
	    {  
  ...
  if (fKickLowres)
    {
      Float_t kl_tof[50], kl_p[50], kl_len[50];
      Float_t start_tof = 0, rec_mass = 0;
      Float_t chi2;
      Int_t kl_pol[50], kl_sys[50];
      Int_t kl_count  = 0;
      Int_t rec_ind   = -1;
      Int_t isMinus   = 0;
      Int_t isRich    = 0;
      ...
      while (((kickL=(HKickTrack *)iterKickLowres->Next()) != 0)&&(kl_count<50)) // begin of lowres KICKloop
	{
  ...
  if (fKickLowresB)
    {
      Float_t kBl_tof[50], kBl_p[50], kBl_len[50];
      Float_t start_tof = 0, rec_mass = 0;
      Float_t chi2;
      Int_t kBl_pol[50], kBl_sys[50];
      Int_t kBl_count = 0;
      Int_t rec_ind   = -1;
      Int_t isMinus   = 0;
      Int_t isRich    = 0;
      ...
      while (((kickBL=(HKickTrackB *)iterKickLowresB->Next()) != 0)&&(kBl_count<50))
	{
  ...
  if (fKickHires)
    {
      Float_t kh_tof[50], kh_p[50], kh_len[50];
      Float_t start_tof = 0, rec_mass = 0;
      Float_t chi2;
      Int_t kh_pol[50], kh_sys[50];
      Int_t kh_count  = 0;
      Int_t rec_ind   = -1;
      Int_t isMinus   = 0;
      Int_t isRich    = 0;
      ...
      iterKickHires->Reset();
      while (((kickH=(HKickTrack123B *)iterKickHires->Next()) != 0)&&(kh_count)) // begin of hires loop
	{
...
void HTofRec::track_recalc(Int_t tracking, Float_t start, Int_t flag)
{
  ...
  if (tracking==0)
	...
	{
	  if (spline_ch->getSystem()!=-1&&spline_ch->getSector()!=2&&spline_ch->getSector()!=5)
	    {
	      if(fillNTuple)
		{
