...
extern "C" {
   ...
   FILE *rfio_fopen(char *pcFile, char *pcOptions);
...
Int_t HTRFIOFile::SysAccess(const char *filepath, int mode)
{
	srawAPIFile *fRemote = NULL;
	int res=0;


	fRemote = (srawAPIFile *)rfio_fopen((char*)filepath, "r");
	if (fRemote)
    {
...
//______________________________________________________________________________
Int_t HTRFIOSystem::MakeDirectory(const char *dir)
{
...
//______________________________________________________________________________
void *HTRFIOSystem::OpenDirectory(const char *dir)
{
...
//______________________________________________________________________________
const char *HTRFIOSystem::GetDirEntry(void *dirp)
{
...
//______________________________________________________________________________
Int_t HTRFIOSystem::GetPathInfo(const char *path, FileStat_t &buf)
{
...
//______________________________________________________________________________
Bool_t HTRFIOSystem::AccessPathName(const char *path, EAccessMode mode)
{
   ...
   TUrl url(path);
	...
	fRemote = (srawAPIFile *)rfio_fopen((char*)url.GetFile(), "r");
	if (fRemote)
    {
      res = rfio_fclose(fRemote);
      if(res!=0)
