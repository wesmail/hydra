...
//
// $Id: hpidevtinfo.h,v 1.2 2007-06-22 17:59:26 slang Exp $
//
//-----------------------------------------------------------------------------
...
class HPidEvtInfo : public TObject
{
   ...
   public:
	 ...
	 return
	    s < MAX_SECTOR  ?  aRichMult[s]  :  0;
      }
      Int_t getTofMult(Int_t s) const {
	 return
	    s < MAX_SECTOR  ?  aTofMult[s]  :  0;
      }
      Int_t getShowerMult(Int_t s) const {
	 return 
	    s < MAX_SECTOR  ?  aShowerMult[s]  :  0;
      }
      Int_t getPidTrackCandMult(Int_t s) const {
	 return
	    s < MAX_SECTOR  ?  aPidTrackCandMult[s]  :  0;
      }
      Int_t getMdcClusMult(Int_t s, Int_t io) const { 
	 return
	    (s < MAX_SECTOR  &&  io < MAX_IOSEG)  ?  aMdcClusMult[s][io]  :  0;
      }
   ...
   ClassDef(HPidEvtInfo, 1) // Event characteristic info 
};
