...
public:
  HRpcLookup(const char* name="RpcLookup",
             const char* title="Lookup table for Rpc unpacker",
             const char* context="Nov05 Test run",
             Int_t numChannels=1);
  ~HRpcLookup();
  HRpcLookupChan& operator[](Int_t i) {
    return *static_cast<HRpcLookupChan*>((*array)[i]);
  ...
  Bool_t writeline(char*, Int_t);   // buffer, channel
