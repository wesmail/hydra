 ...
 * 
 * $Id: hdstembedding.cc,v 1.1 2007-06-25 19:00:39 slang Exp $
 *
 */
...
void HDstEmbedding::setupInput()
{
   ...
   if (dstCfg.richIsUsed)
   {
      for(UInt_t i = 0; i < sizeof(richUnpackers) / sizeof(Int_t); i++)
      {
	 hld_source->addUnpacker( new HRichUnpackerCal99( richUnpackers[i] ) );
	 hld_source->addUnpacker( new HRichUnpackerRaw99( richUnpackers[i] ) );
      }
   }
