makeDefaultPidTrkFillerPar()
{
  ...
  out->open("DefaultPidTrkFillerPar_RICH_MDC_META.root", "RECREATE");
  rtdb->setOutput(out);
  ...
  const Int_t nTargetBins=2; //There are two default bins with range +/- 10 cm
  Float_t targetBins[nTargetBins+1]={-100.0, 0.0, 100.0};
  ...
  //By default we do not apply any shift to the ring!
  Float_t value=0.0;
