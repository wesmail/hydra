...
void HMdcCalibrater2::setParContainers() {
  ...
  // database
    ...
    if (!cal2par) {
        cal2par=new HMdcCal2Par;
        gHades->getRuntimeDb()->addContainer(cal2par);
    }
}
...
void HMdcCalibrater2::calibrate(HMdcCal1* cal1,HMdcCal2* cal2) {
    ...
    if(loc[1]==0||loc[1]==1) dummy=85;
    if(loc[1]==2||loc[1]==3) dummy=85;
    cal2->setNHits(nHits);
    cal2->setDist1(cal2par->calcDistance(loc[0],loc[1],dummy,cal1->getTime1()),0.F);

    if (nHits==-2 || nHits==+2)
    {
        cal2->setDist2(cal2par->calcDistance(loc[0],loc[1],dummy,cal1->getTime2()),0.F);
