...
void HRichRingFindIPU::RingSearch(){
  ...
  Int_t  iRegionThresholds[iNumberOfRegions];
...
///// NEW PATTERNCOORD
    ...
    iRegionThresholds[0] = 8;
    iRegionThresholds[1] = -4;
...
// initialisation of centers to 0 
  ...
  for(Int_t lr=0; lr<96; lr++)  {
	...
	for (Int_t j=0; j< iNumberOfRegions; j++){
	  if (  (iRegionThresholds[j] >= 0 && iGroupsInRegion[j] > iRegionThresholds[j]) || (iRegionThresholds[j] < 0 && iGroupsInRegion[j] < -iRegionThresholds[j]) ) {
