...
//  raw data category
//
//  Need to implement overflow handling, quenching ?
//
/////////////////////////////////////////////////////////////////////
...
void HTofDigitizer::initParContainer() {
 ...
 fDigiPar=(HTofDigiPar *)gHades->getRuntimeDb()->getContainer("TofDigiPar");
  if (!fDigiPar) {
    fDigiPar=new HTofDigiPar;
    gHades->getRuntimeDb()->addContainer(fDigiPar);
  }
}
...
Int_t HTofDigitizer::execute(void) {
  ...
  const Float_t timeResZero = 0.1;     // time resolution in the centre of the strip [ns].
  const Float_t relAmplResol = 0.09;   // sigma of Landau distribution.
