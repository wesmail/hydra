...
HKickTrackBaseF::HKickTrackBaseF(Text_t *name, Text_t *title) : HReconstructor(name,title)
{
   C=299792458;
   pi=acos(-1.);
   //fCatSplineTrack=0;
   ...
   while((pMetaMatch=(HMetaMatch*)(fMetaMatchIter->Next()))!=0)
	 ...
	 if(metaInd>=0){ 
		...
		if(fTofGeometry){
  	    ...
  	    if(tRans[sector]==0){ //- if no transformation is defined for the sector, then define it
  	       tRans[sector]=new HGeomTransform();
  	       if(!fGetCont->getLabTransSec(*(tRans[sector]),sector)) continue; //if not possible - skip this sector
  	    }

  	    pointMeta=tRans[sector]->transTo(pointMeta); //we transform META-hit into MDC-sector system
  	    xTof=pointMeta.getX();
  	    yTof=pointMeta.getY();
  	    zTof=pointMeta.getZ();
         }
