...
// used. For converting ROOT graphis ps2pdf has to be available.
...
HReport::HReport(const char* name,const char* title)
    : TNamed(name,title)
{
...
void HReport::openTex(TString tdir,TString tfile, TString titlename, TString authorname)
{
    ...
    if(!gSystem->AccessPathName(texdir))
    {
	Char_t* suffix[8]={".lot",".toc",".aux",".lof",".out",".log",".lol",".tex"};
	for(Int_t i=0;i<8;i++)
	{
...
void HReport::writeTex(TString text,Bool_t skip_curly)
{
	...
	for(Int_t i=0;i<size;i++)
	{
	    ...
	    {   // add line break if line is to long and no braces/math mode is open
