...
void HMdcClusterToHit::fillContainers(void) {
  ...
  if(fSegCat && tFillSeg[seg]!=-1 && (typeClFn==0 || nMods==1 || 
    ...
    if(!fSeg) {
      Warning("execute","S.%i No HMdcSeg slot available",sec+1);
      return;
    }
  ...
  for(Int_t imod=0; imod<2; imod++) {
    ...
    if(!fHit) {
      Warning("execute","S.%i No HMdcHit slot available",sec+1);
      continue;
    }
...
void HMdcClusterToHit::fillTrkCandISeg(Short_t sec,Int_t indexSeg) {
  ...
  else Warning("fillTrkCandISeg"," No slot available in catMdcTrkCand");
}
...
void HMdcClusterToHit::fillTrkCandOSeg(Short_t sec,Int_t indexSeg) {
  ...
  } else Warning("fillTrkCandOSeg"," No slot available in catMdcTrkCand");
}
