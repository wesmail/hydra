...
class HMdcTrackFitter : public TObject {
  ...
  protected:
    ...
    Int_t sector;                       // sector
    HMdcList24GroupCells listCells;     // list cells in cluster
    HMdcList24GroupCells listCellsOrg;  // list no fitted cells in cluster
...
//     Double_t zRight[ARRSIZE];     // Array with z-coord. of right edge of wire
    ...
    Int_t nTimesInFit[4];               // Num. of hits in MDC's for fit
    Int_t indFWireInLay[24];            // first wire in layer index =-1 - no w.in lay.
    Int_t indLWireInLay[24];            // last wire in layer index
...
//    FillerFlag filler;
  ...
  public:
    ...
    Bool_t fillListHits(HMdcList12GroupCells* lCells1,
			HMdcList12GroupCells* lCells2=0);
...
//     Double_t* getZRightArr(void) {return zRight;}
    ...
    void setFittingTimesList(Int_t mod=-1);
