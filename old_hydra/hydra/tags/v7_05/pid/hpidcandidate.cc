// @(#)$Id: hpidcandidate.cc,v 1.8 2005-01-11 11:06:41 halo Exp $
...
{
// Returns HPidTrackCand object corresponding to iTrackCandIndex (if exists)
...
// The input array must have getNAlgorithms() members

  if(sizeof(aeAlgs)!=NUM_ALGORITHMS)
    {
...
{
// Set whole vector of particle Ids.
// The input array must have getNParticles() members



///**//**/fprintf( stderr, "In HPidCandidate::setParticleIds :\n" );
///**//**/fprintf( stderr, "anPart size = %i, NUM_PARTICLES = %i, IDs : ", nrOfParticles, NUM_PARTICLES );
///**//**/for( Int_t loop = 0; loop < 2*nrOfParticles; loop++ ) fprintf( stderr, "%i   ", anPart[loop] );
///**//**/fprintf(stderr, "\n" );
...
// Static function for making the category for the specified class name.
  ...
  HCategory  *pCat;
  static char* sCatName="HPidCandidate"; 
  HEvent     *pEvent;
    ...
    //freeDynamicMemory() function of the category is called automatically.
...
Float_t fSum = 0.0f;
    ...
    for(i = 0; i < iMax; i++)
    {
        ...
        if(getValueByIndex(iRelInt,i) < 0.0)
	  {
	    ...
	    cout << "resetting negative relint value!" << endl;
	    //fRelInt[i] = 0.0;
#warning This might need intervention in future
	  }
        //fOut[i] *= fRelInt[i];
 ...
 Float_t pdf_factor=0.0;
    ...
    if(iAlgs <= 0)
    {
	    ...
	    */
		...
		if(pdf_factor>0)
		  //cout << "Getting value " << pdf_factor <<" from algorithm: " << a << " for particle: " << i<< endl;
		if(pdf_factor >= 0.0f)
		  {
		    if(fOut[i] < 0.0f)
		      fOut[i] = pdf_factor;
...
Int_t iIdx;
    ...
    if((iIdx = getParticleIndexById(nPID)) < 0)
    {
        Error("getMergedVect", "Particle %d not in the container", nPID);
        return -1.0f;
    }
