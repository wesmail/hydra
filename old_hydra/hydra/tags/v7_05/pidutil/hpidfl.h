...
public:
  ...
  static Float_t          getKineMomentum(HPidTrackCandSim* pTrackCandSim, UInt_t detectors=kTrackFullHadron);
  ...
  static void transformSectorToLab(HGeomVector &base,HGeomVector &dir, Int_t sector);
  static void transformSegToStraight(Double_t z, Double_t rho, Double_t phi, Double_t theta, HGeomVector &base, HGeomVector &dir);
    static void transformToStraight(HMdcSeg *pSeg, HGeomVector &base,HGeomVector &dir);
  static void transformSegmentToLabStraight(HMdcSeg *pSeg, HGeomVector &base, HGeomVector &dir);

  static Float_t computeDistanceToVertex(HMdcSeg* pSeg);

  static Bool_t getParameterContainers();
  //HParSet* getParameterContainer(TString* name());
  static Bool_t initSizesCells();
