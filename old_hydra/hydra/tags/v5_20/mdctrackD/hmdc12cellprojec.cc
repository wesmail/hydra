...
HMdc12CellProjec::HMdc12CellProjec() {
  ...
  for (Int_t nSec = 0; nSec < 6; nSec++) {
    if( !fmdc->getModule(nSec,0) && !fmdc->getModule(nSec,1) ) continue;
...
if(y<0. || y>1000.) Printf("s%i m%i l%i c%i p0,3  x===%f y===%f",nSec,nMod,nLayer,nCell,x,y);
        cellproj[0].set(x,y);
        cellproj[3].set(x,y); 
       }
       if(!indu) {
...
if(y<0. || y>1000.) Printf("s%i m%i l%i c%i p1,2  x===%f y===%f",nSec,nMod,nLayer,nCell,x,y);
        cellproj[1].set(x,y);
        cellproj[2].set(x,y); 
       }      
      }
...
//  Double_t par=1.0;   // <<<===================== use MDC1
  ...
  Double_t tmp;
  Double_t transNew[3];
  HGeomCompositeVolume* pComVol0=fGeomPar->getRefVolume(0); //MDC1
  HGeomCompositeVolume* pComVol1=fGeomPar->getRefVolume(1); //MDC2
  Int_t nmod=(pComVol0) ? 1 : 0;
  nmod+=(pComVol1) ? 2 : 0;
  if( nmod == 0 ) return kFALSE;
  
  HGeomVolume* pGeomVol;
  if( nmod == 3 ) {
    pGeomVol=pComVol0->getComponent(5);          // Layer 6
    if(!pGeomVol) {
      printf("Error: GeomCompositeVolume for Layer 6 is absent!\n");
      return kFALSE;
    }
    coordSys->setTransform(pGeomVol->getTransform());   //From layer to module
    coordSys->transFrom(pComVol0->getTransform());       //From layer to sector
    tmp=(coordSys->getRotMatrix())(8);
    tmp=1./(tmp*tmp);
    tanTheta1=sqrt(tmp-1.);
    zMdc1=(coordSys->getTransVector()).getZ()*tmp; 

    pGeomVol=pComVol1->getComponent(0); // Layer 1
    if(!pGeomVol) {
      printf("Error: GeomCompositeVolume for Layer 1 is absent!\n");
      return kFALSE;
    }
    coordSys->setTransform(pGeomVol->getTransform());   //From layer to module
    coordSys->transFrom(pComVol1->getTransform());       //From layer to sector
    ...
    if(par==0.0) printf(
      "\n===> Sec.%i: Using MDC1, plane 6 as projection plane\n",sec);
    else if(par==1.0) printf(
      "\n===> Sec.%i: Using MDC2, plane 1 as projection plane\n",sec);
    else printf(
    "\n===> Sec.%i: Using plane between MDC1 and MDC2 (p=%f) as projection plane\n",sec,par);
    ...
    HGeomCompositeVolume* pComVol=(pComVol0) ? pComVol0 : pComVol1;
    coordSys->setTransform(pComVol->getTransform());
