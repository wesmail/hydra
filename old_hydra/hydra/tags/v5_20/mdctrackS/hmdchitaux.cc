...
HMdcHitAux :: HMdcHitAux (HMdcHitAux &copy){
  ...
  gDebuger->enterFunc("HMdcHitAux::HMdcHitAux(copy)");
    ...
    myErrorPoint[0] = copy.getXError();
    myErrorPoint[1] = copy.getYError();
    myErrorPoint[2] = copy.getZError();
    mySlope[0] = copy.getSlopeX();
    mySlope[1] = copy.getSlopeY();
    myErrorSlope[0] = copy.getErrorSlope1();
    myErrorSlope[1] = copy.getErrorSlope2();
    myChi = copy.getChi();
...
void HMdcHitAux ::  clear(void){
    ...
    for(i=0;i<=2;i++){
      myPoint[i] = 20000.;
      myErrorPoint[i] = 20000.;
    }
    for(i=0;i<2;i++){
      mySlope[i]= 20000.;
      myErrorSlope[i]=20000.;
    }
    myChi= 20000.;
    myNumberOfHits=0;
...
void HMdcHitAux :: setError(Float_t xError, Float_t yError, Float_t zError){
  myErrorPoint[0] = xError;
  myErrorPoint[1] = yError;
  myErrorPoint[2] = zError;  
}

void HMdcHitAux :: setErrorSlope(Float_t xerror, Float_t yerror){
  myErrorSlope[0] = xerror;
  myErrorSlope[1] = yerror;
}
...
void HMdcHitAux :: addCal(HMdcCal3* data, Int_t pos){  
  ...
  file << "X"       << '\t' << myPoint[0] << " +- " << myErrorPoint[0] << endl;
  file << "X Slope" << '\t' << mySlope[0] << " +- " << myErrorSlope[0] << endl;
  file << "Y"       << '\t' << myPoint[1] << " +- " << myErrorPoint[1] << endl;
  file << "Y Slope" << '\t' << mySlope[1] << " +- " << myErrorSlope[1] << 
           '\n'<< '\n';
