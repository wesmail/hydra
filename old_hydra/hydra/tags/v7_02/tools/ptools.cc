  ...
  for (Int_t binx=first; binx<=last;binx++)
    {
      ...
      Double_t xpos = hist->GetBinLowEdge(binx)+hist->GetBinWidth(binx)*0.9;
      ...
      TLatex *tex = new TLatex(xpos,ypos,val.Data());
...
TPaveStats* PTools::getHistStats(TH1 *hist)
{
  ...
  if (hist)
	...
	{
	  ::Error("getHistStats","stats do not exists, returning NULL pointer");
	  return stats;
	}
    }
  else
...
TPaveStats* PTools::getHistStats(TPad* pad, TString name, Bool_t noUpdate)
{
  ...
  if (pad)
    {
      ...
      return getHistStats(((TH1*) pad->FindObject(name.Data())));
    }  
  else
    {
      ::Error("getHistStats()","pad does not exist, returning NULL pointer");
      return NULL;
    }
}
