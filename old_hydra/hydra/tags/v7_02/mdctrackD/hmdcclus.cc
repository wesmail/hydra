...
void HMdcClus::printPos(void) const {
  printf("Cluster: Sec.%i Seg.%i Mod.%iClust.type=%i Index=%i.",
      sec+1,seg+1,mod+1,typeClFinder,index);
  if(indexPar>=0) printf("  Index of parent=%i\n",indexPar);
  else if(indCh1>=0 && indCh2>=0)
    printf("  Indexes of childs = %i - %i\n",indCh1,indCh2);
  else printf("\n");
  if(seg==0 || typeClFinder!=2) printf("  Level %i:%i,",minCl1,minCl2);
  else printf("  Level %i,",minCl1+minCl2);
