...
void HMdcCluster::fillClus(HMdcClus* fClus, Int_t nLst) {
  ...
  if(clusMod1==0) {
    ...
    if(nDrTm>0 && fClus->getMinCl(0)>0) {
      ...
      if(fClus->isGeant()) ((HMdcClusSim*)fClus)->calcTrListMod(list,0);
  ...
  if(clusMod2==0) {
    ...
    if(nDrTm>0 && fClus->getMinCl(1)>0) {
      ...
      if(fClus->isGeant()) ((HMdcClusSim*)fClus)->calcTrListMod(list,1);
...
Bool_t HMdcLookUpTb::calcPrPlane(Int_t sec){
  ...
  ((*this)[sec]).prPlane.setPlanePar(coordSys);
  return kTRUE;
}
...
Bool_t HMdcLookUpTb::calcTarget(Int_t sec){
  ...
  HGeomVolume* tr1=fSpecGeomPar->getTarget( 0);
  HGeomVolume* tr2=fSpecGeomPar->getTarget(nT);
  if(!tr1 || !tr2) {
    Error("calcTarget","GeomVolume for target absent!");
    return kFALSE;
  }
  if( tr1->getTransform().getTransVector().getZ() >
      tr2->getTransform().getTransVector().getZ() ) {
    HGeomVolume* tr=tr1;
    tr1=tr2;
    tr2=tr;
  }
  
  fsec.targVc[0]=tr1->getTransform().getTransVector();
  Double_t dz1=tr1->getPoint(0)->getZ();
  if(dz1 > tr1->getPoint(2)->getZ()) dz1=tr1->getPoint(2)->getZ();
  fsec.targVc[0].setZ( fsec.targVc[0].getZ() + dz1 - targLenInc[0]);
  
  fsec.targVc[1]=tr2->getTransform().getTransVector();
  Double_t dz2=tr2->getPoint(2)->getZ();
  if(dz2 < tr2->getPoint(0)->getZ()) dz2=tr2->getPoint(0)->getZ();
  fsec.targVc[1].setZ( fsec.targVc[1].getZ() + dz2 + targLenInc[1]);
