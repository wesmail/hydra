// @(#)$Id: hpidtrackfiller.cc,v 1.34 2006-08-16 16:13:35 halo Exp $
...
// -----------------------------------------------------------------------------
  int HPidGeantTrackSet::nullparent=0;     
...
Int_t HPidTrackFiller::execute(void)
{
  ...
  Bool_t hasCorrelatedRing[nMomAlgs]; 
  ...
  Float_t distToClosestLeptonCandidate = 180.0; 
  Float_t distToClosestHadronCandidate = 180.0; 
  Bool_t closestLeptonCandidateIsFitted = kFALSE;
  Bool_t closestHadronCandidateIsFitted = kFALSE;

  //loop over all pidtrackcandidates
  while((currentCandidate = (HPidTrackCand*)pitCand->Next()) != NULL)
    {
      tmpOpangle = 180.0;
      distToClosestLeptonCandidate = 180.0; 
      distToClosestHadronCandidate = 180.0; 
      ...
      while( (pMatch = (HMetaMatch *)pitMatch->Next()) != NULL)
	{
	  ...
	  //check whether this is a possible lepton candidate closer than the closest we already found
	  if(tmpOpangle < distToClosestLeptonCandidate && pMatch->getARichInd(0)>-1)
	    { 
	      distToClosestLeptonCandidate = tmpOpangle;
	      if(pMMSeg->getChi2()>-1)
		{
		  closestLeptonCandidateIsFitted=kTRUE;
		}
	      else
		{
		  closestLeptonCandidateIsFitted=kFALSE;
	  ...
	  //check whether this is a possible hadron candidate closer than the closest we already found
	  if(tmpOpangle < distToClosestHadronCandidate && pMatch->getARichInd(0)<0)
	    {
	      distToClosestHadronCandidate = tmpOpangle;
	      if(pMMSeg->getChi2()>-1)
		{
		  closestHadronCandidateIsFitted=kTRUE;
		}
	      else
		{
		  closestHadronCandidateIsFitted=kFALSE;
	  ...
	  currentCandidate->getTrackData()->fAngleWithClosestLeptonCandidate=distToClosestLeptonCandidate;
	  currentCandidate->getTrackData()->fAngleWithClosestHadronCandidate=distToClosestHadronCandidate;
	  currentCandidate->getTrackData()->closestHadronCandidateIsFitted=closestHadronCandidateIsFitted;
	  currentCandidate->getTrackData()->closestLeptonCandidateIsFitted=closestLeptonCandidateIsFitted;
	}
    }
