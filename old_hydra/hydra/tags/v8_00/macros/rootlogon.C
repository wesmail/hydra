...
void rootlogon()
{
  ...
  printf("Adding Hydra include paths ...\n");
  gSystem->AddIncludePath(gSystem->Getenv("HYDRA_INCLUDES"));

  gROOT->GetPluginManager()-> AddHandler("TFile", "^rfio:", "HTRFIOFile","RFIOtsm","HTRFIOFile(const char*, Option_t *, const char *, Int_t)");

  printf("\nWelcome to the Hydra Root environment of HADES\n\n");
