...
Bool_t HRichUnpackerCurrent::writePdfFile(void) {
  ...
  if (pdfFile.EndsWith(".ps")) 
    pdfFile.ReplaceAll(".ps","");
  pdfFile.Append(".pdf");

  command =  "ps2pdf " + psFileName + " " + pdfFile;

  setStyle();

  if (!(fPS = new TPostScript(psFileName, 111))) {
    cerr << "ERROR: Can not open output PS file" << endl;
    return kFALSE;
  }
