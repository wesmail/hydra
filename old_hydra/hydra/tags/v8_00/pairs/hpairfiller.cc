// $Id: hpairfiller.cc,v 1.12 2006-08-16 15:42:18 halo Exp $
// Author: Thomas.Eberl@ph.tum.de, last modified : 2006-06-30 16:38:51
//
// ---------------------------------------------------------------------------
...
Bool_t HPairFiller::fillPairCategory(TObjArray* pPairs)
{
	...
	for (Int_t iPairIdx=0; iPairIdx<pPairs->GetLast()+1; iPairIdx++)
	{
		...
		if (kExt)
		{
		    if (!fillPair((HPairSimExt*)(*pPairs)[iPairIdx])) 
			return kFALSE;
		}
		else
		{
		    if (!fillPair((HPairSim*)(*pPairs)[iPairIdx])) 
			return kFALSE;
		}
	    }
	}
...
// ---------------------------------------------------------------------------
Bool_t HPairFiller::fillPair(HPairSimExt* pPairSim)
{
    HLocation dummyLoc;
    dummyLoc.set(1,0);
    if (pPairSim)
    {
	HPairSimExt* pPairPlacement = (HPairSimExt*)((HLinearCategory*)
					 getPairCat())->getNewSlot(dummyLoc);

	if (pPairPlacement) 
	{
	    HPairSimExt* pPairCp = new (pPairPlacement) HPairSimExt();
	    *pPairCp = *pPairSim;
...
HPairExt *pPairE = NULL;
HPairSimExt *pPairSimE = NULL;
 Bool_t kExt = kTRUE;
