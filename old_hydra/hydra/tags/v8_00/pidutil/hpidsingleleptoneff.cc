                                     ...
                                     "Filler of HPidParticle category")
{
    ...
    effEle = new TNtuple("effEle","effEle","pRecon:pGeant:thetaRecon:thetaGeant:phiRecon:phiGeant:ptGeant:isCut:isCP:commonTrack");
    effPosi = new TNtuple("effPosi","effPosi","pRecon:pGeant:thetaRecon:thetaGeant:phiRecon:phiGeant:ptGeant:isCut:isCP:commonTrack");
    fakeEle = new TNtuple("fakeEle","fakeEle","pRecon:pGeant:thetaRecon:thetaGeant:phiRecon:phiGeant:ptGeant:isCut:isCP:commonTrack");
    fakePosi = new TNtuple("fakePosi","fakePosi","pRecon:pGeant:thetaRecon:thetaGeant:phiRecon:phiGeant:ptGeant:isCut:isCP:commonTrack");
    ...
    // cout<<" mech  "<<mech<<"  medium "<<medium<<endl;
	 ...
	 if(comDet>0&&checkLeptonId(pair)){//only lepton-lepton pairs are checked
	   ...
	   if (pair->getIsCutFlag()==0){
	     ...
	     if(comDet>=76 && isGoodPair==0 ){
	       if(id==3) effEle->Fill(momExp,mom1,thetaExp,pD1.Theta(),phiExp,pD1.Phi(),pD1.Pt(),0,cpFlag,comDet);
	       if(id==2) effPosi->Fill(momExp,mom1,thetaExp,pD1.Theta(),phiExp,pD1.Phi(),pD1.Pt(),0,cpFlag,comDet);
	       isGoodPair=1;
	     }
	     if(comDet<76){
	       if(id==3) fakeEle->Fill(momExp,mom1,thetaExp,pD1.Theta(),phiExp,pD1.Phi(),pD1.Pt(),0,cpFlag,comDet);
	       if(id==2) fakePosi->Fill(momExp,mom1,thetaExp,pD1.Theta(),phiExp,pD1.Phi(),pD1.Pt(),0,cpFlag,comDet);
       ...
       while(( part= ( HPidParticleSim *)pidPartInput->Next()) != NULL)
	 {
	   ...
	   if(trackNb==part->getGeantTrackSet()->getGeantTrackID()&&
	     ...
	     if(id==3) effEle->Fill(part->P(),mom1,part->Theta(),pD1.Theta(),part->Phi(),pD1.Phi(),pD1.Pt(),-1,-1,part->getGeantTrackSet()->getMostCommonCorrelation());
	     if(id==2) effPosi->Fill(part->P(),mom1,part->Theta(),pD1.Theta(),part->Phi(),pD1.Phi(),pD1.Pt(),-1,-1,part->getGeantTrackSet()->getMostCommonCorrelation());
	     break;  
	     // cout<<"  ****************************************   "<<endl;
