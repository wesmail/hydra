...
//    void setDistanceParams(Float_t dpar1, Float_t dpar2, Float_t dpar3);
    ...
    ClassDef(HTofinoCalParCell,4) //Tofino calibration parameters per cell
...
private:
    ...
    Float_t m_fADCThreshold; // ADC Threshold (chan.) for the cell
    Float_t m_fADCPedestal; // ADC Pedestal (chan.) for the cell
    Float_t m_fElossParam[4]; // K and A0-A2 parameters for (Eloss = K*(ADC-ADCPedestal) + A0+A1*Row+A2*Row*Row) calculation 
