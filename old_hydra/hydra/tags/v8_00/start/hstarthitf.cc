...
Int_t HStartHitF::execute(void) {
  ...
  if(eventid!=1){ return 0; } // do nothing for events != "real" 
...
Bool_t HStartHitF::finalize(void) {
    ...
    TFile* hadesOutput=gHades->getOutputFile();
    if(fControlHistograms==kTRUE&&hadesOutput){
	hadesOutput->cd();
	fHistograms->Write();
    }
  return kTRUE;
