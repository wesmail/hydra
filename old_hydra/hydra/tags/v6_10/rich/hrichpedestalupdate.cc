...
void HRichPedestalUpdate::calculateShifts(){
  ...
  Float_t multiply_sigma_orig = ( (HRichThresholdPar*)fThresholdPar )->getMultiply_sigma(); //gamma
  Float_t multiply_mean_orig = ( (HRichThresholdPar*)fThresholdPar )->getMultiply_mean(); //beta
  Float_t additional_offset_orig = ( (HRichThresholdPar*)fThresholdPar )->getOffset(); //alpha
	...
	if(mask[k][0] == 1){//valid sectors
	  ...
	  if (pCell) {
	    ...
	    if(pCell->getSigma()>0.0){
	      ...
	      if(fraction>=0.9987)
		{
		  cout << "Fraction of detected noise events is " <<fraction <<", assuming position -3.0 sigma" << endl;
		...
		alphaprime = getalpha(fraction);
	      ...
	      pShift[sIx][i][j]= (multiply_sigma_orig-alphaprime) * pCell->getSigma() + pCell->getOffset() * (multiply_mean_orig - 1. ) + additional_offset_orig;
	      //driftHisto[k]->Fill(i,j, multiply_sigma_orig - alphaprime);
	    } // if getsigma > 0
	  } //if pCell
	}// if mask
      }// row
