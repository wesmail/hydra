...
//  	}  	
   ...
   if (fDebugMode)  printf("MU-Data decoding %i Dileptons \n", 
	...
	//	muDiLept->setMass( (Float_t) (dataw));    Wrong!!!!!!!!!!!!!!!!!!!!!!!!!1
	//if (muDiLept->getMass() <0)
	  cout << "inv mass " << muDiLept->getMass() << "    Dataw " << *pMass << endl;
      }
    } // ! dileptons 
  ...
  if (length>maxLength) {
	...
	while ( bc < 8 ) {
	  ...
	  if ( (tempPat & 0x1) != 0 )  { 
	    if ( bc < 8 ) {
	      col = 95 - fifo * 8 - bc;
	      theta = fMatchUParam->getTheta(row-23, col);
	      phi = 30+fMatchUParam->getPhi(row-23, col);
	      if  (segId < 5)
		phi = phi + (segId+1)*60;
...
//---------------------------------------------------------------------------
  ...
  Int_t col_sh, row_sh;
  Float_t theta_sh, phi_sh;
  Int_t sec_sh;

  iterShower->Reset();
  while (( pMUShower = (HMatchUShower *)iterShower->Next()) != 0) {
    col_sh = pMUShower->getColumn();
    row_sh = pMUShower->getRow();
    sec_sh = pMUShower->getSector();
    theta_sh = fMatchUParam->getThetaSh(col_sh, row_sh);
    phi_sh = fMatchUParam->getPhiSh(col_sh,row_sh);
    if (sec_sh < 5)
      phi_sh = phi_sh + (sec_sh+1)*60;
    pMUShower->setTheta(theta_sh);
    pMUShower->setPhi(phi_sh); 
  }
