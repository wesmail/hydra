...
void HMdcCal2ParSim::transformToDistance(Int_t s,Int_t m,Int_t a,Float_t binsizetime,HMdcCal2Par* cal2par)
{
 // fills arrays of MdcCal2Par (time) from arrays of MdcCal2ParSim (distance)
	...
	if( (time1_2-time1_1)!=0)
	{
		...
		cout<<"dsearch==dsearchold "<<m<<" "<<a<<" "<<b
		    <<" vd (um/ns)"<<vdrift*1000<<" ds "<<dsearch<<" es (ns)"<<dsearchErr
		    <<" e1 "<<time1_1Err<<" e2 "<<time1_2Err<<" res (um)"<<dsearchErr*vdrift*1000<<endl;
	    }

	    if( (time1_2Err-time1_1Err)!=0)
	    {
		...
		if(dsearch==dsearchold)
		{
		    cout<<"take the error from previous point "<<m<<" "<<a<<" "<<b
			<<" vd (um/ns)"<<vdrift*1000<<" ds "<<dsearch<<" es (ns)"<<dsearchErr
			<<" e1 "<<time1_1Err<<" e2 "<<time1_2Err<<" res (um)"<<dsearchErr*vdrift*1000<<endl;
		}
	    ...
	    if(dsearch==0)
	    {
		cout<<"dsearch==0 "<<m<<" "<<a<<" "<<b
		    <<" e1 "<<time1_1Err<<" e2 "<<time1_2Err
		    <<" t1 "<<time1_1<<" t2 "<<time1_2<<endl;
	    }
	}
	else
	{
