...
//  Int_t loc2FirstCl=counter;
  ...
  for(Int_t cl=0; cl<nClsArr; cl++) {
    ...
    clus->setShape(sigma1,sigma2,alpha);
    clus->setIndexParent(indexPar);
    if(clus->isGeant()) ((HMdcClusSim*)clus)->getNTracks();
  }  
  return nClusters;
}
...
//  Float_t binsInCl=0.;
  ...
  while(stackT != stackB) {
    ...
    Double_t wt=
        (Double_t)(HMdcBArray::getNSet2B(prPlotSeg2->plot1b+nBin*2)-minBinS);
    //region of cluster ???
