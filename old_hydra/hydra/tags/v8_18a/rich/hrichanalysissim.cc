...
//allocate input/output categories
	...
	// ????????????????????????????????????????????????????????????????????
	m_pHitCat=pRichDet->buildCategory(catRichHitSim); //et why Matrix ?!
	// ????????????????????????????????????????????????????????????????????
...
//cout << "In execute after reset" << endl;
  ...
  for(secCount = 0; secCount < 6; secCount++) 
      if (((HRichGeometryPar*)fpGeomPar)->getSectorActive(secCount) > 0) {
  ...
  while((pCal = (HRichCal *)fIter->Next()))
  {
     sec = pCal->getSector();
     padnr = pCal->getCol() + pCal->getRow()*maxCols;
     pCal->setIsCleanedSingle(pPads[sec][padnr].getIsCleanedSingle());
     pCal->setIsCleanedHigh(pPads[sec][padnr].getIsCleanedHigh());
  }
...
void HRichAnalysisSim::sortTracks(HRichHitSim *hit) {
