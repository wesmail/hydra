...
HRichUnpackerCal::HRichUnpackerCal(Int_t rId) {
    richId = rId;
    pCalCat=NULL;
    fCalPar = NULL;
    fEventNr = 0;
...
Int_t HRichUnpackerCal::getSubEvtId(void) const {
    return richId; 
}

Int_t HRichUnpackerCal::execute() {
    HLocation loc;
    loc.set(3,0,0,0);
    fEventNr++;
    if (pSubEvt) {
...
Bool_t HRichUnpackerCal::diagnose(Int_t h_charge,Int_t h_col,Int_t h_row,Int_t h_sec, Int_t h_fEventNr) {
