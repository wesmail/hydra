...
void HMdcEfficiencyMod::setNameCanv(void){
  ...
  sprintf(buf,"%s S%i M%i Pattern",fileName.Data(),
      sector+1,module+1);
  titleCanv[0]=buf;
  sprintf(buf,"%s S%i M%i Num. of tracks and zero, single and Double_t hits",
  ...
  titleCanv[3]=buf;
  sprintf(buf,
      "%s S%i M%i Probability to have zero, single and Double_t hit",
      fileName.Data(),sector+1,module+1);
  titleCanv[4]=buf;
  nPads=0;
...
//  if(gROOT->GetListOfCanvases()->FindObject(nameCanvEventCurr)) return;
  ...
  if(canvEventProj) {
    ...
    canvEventProj=new TCanvas(nameCanvEventCurr.Data(),titleCanvEvent.Data(),
        20,1,800,600);
...
//      fmod.updateCanv();
  ...
  fRootOut->cd();
  Char_t psFile[500];
  sprintf(psFile,"%s_%s.pdf",fileName.Data(),"Summary");
  canvasSummary->SaveAs(psFile);
...
void HMdcEfficiency::saveAs(Char_t* fileName, Char_t* type) {
  Char_t psFileOp[500];
...
void HMdcEfficiency::savePDF(Char_t* fileName, Char_t* type) {
...
void HMdcEfficiency::makePDF(Char_t* psFile) {
...
void HMdcEfficiency::drawSummary(Bool_t flag) {
  ...
  if(histSummary==0) {
    ...
    Char_t* xLab[6][4]={{"I1","II1","III1","IV1"},
                        {"I2","II2","III2","IV2"},
                        {"I3","II3","III3","IV3"},
                        {"I4","II4","III4","IV4"},
                        {"I5","II5","III5","IV5"},
    ...
    for(Int_t m=0;m<4;m++) for(Int_t s=0;s<6;s++)
        ax->SetBinLabel(s*6+m*36+3,xLab[s][m]);
  ...
  for(Int_t mod=0;mod<4;mod++) for(Int_t sec=0;sec<6;sec++) {
    ...
    for(Int_t lay=0;lay<6;lay++) {
      HMdcEfficiencyLayer& flay=fmod[lay];
      if(!&flay) continue;
      TH1F* hsEff=flay.hists[6];
      TH1F* hsPat=flay.hists[2];
      if(hsEff==0 || hsPat==0) continue;
      Double_t effSum  = 0.;
      Double_t effSum2 = 0.;
      Int_t    sen     = 0;
      Int_t nbins = flay.nCells;
...
if(vl<0.8) continue; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if(sen>0) {
        ...
        Double_t err=sqrt( (effSum2/sen-effL*effL)/sen );
