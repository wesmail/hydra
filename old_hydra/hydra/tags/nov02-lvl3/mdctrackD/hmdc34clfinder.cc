...
HMdc34ClFinderSec::~HMdc34ClFinderSec() {
  ...
  stack=0;
  for(Int_t mod=2; mod<4; mod++) if(xCMin[mod]) {
    delete [] xCMin[mod];
    xCMin[mod]=0;
  }
}
...
//  prPlotSeg2->clearArr();  // cleanig
  if(minAmp[2]>0 && minAmp[3]>0) makeSeg2Plot();
...
void HMdc34ClFinderSec::makeSeg2Plot() {
  Int_t mBm3=minAmp[2];
  Int_t maxB4Sc=0;
  Int_t minB4Sc=prPlotSeg2->size;
  ...
  for(Int_t mod=2; mod<4; mod++) {
    ...
    for(Int_t lay=0; lay<6; lay++) {
      ...
      if(maxAmp[mod]-nFiredLay+1>=minAm) {
        ...
        else if(nFiredLay<minAmp[mod]) makeLayProjV1b(lay);
        else {
          while(test) {
            if(setNextCell()) {
              setYLimits();
        ...
        if(mod==2 || nFiredLay<minAmp[mod]) makeLayProjV2(lay);
        else {
          while(test) {
            if(setNextCell()) {
              setYLimits();
  ...
  while(nClus>1) {
    ...
    for(Int_t cl1=0; cl1<nClsArr-1; cl1++) {
      ...
      for(Int_t cl2=cl1+1; cl2<nClsArr; cl2++) {
        ...
        if(minAmp[2]>0 && cLCSeg2.compare(cLCSeg2s,0, 5)<4) continue;
        if(minAmp[3]>0 && cLCSeg2.compare(cLCSeg2s,6,11)<4) continue;
