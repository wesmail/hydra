...
//################################## DIGISETUP #############################
HMdcDigitizerSetup::HMdcDigitizerSetup(const char* name,const char* title,HMdcSetup* set)
    : TNamed(name,title)
{
...
void HMdcDigitizerSetup::printParam(void)
{
    ...
    SEPERATOR_msg("-",60);
    INFO_msg(10,DET_MDC,"HMdcDigitizerSetup:");
...
//################################## CALIBRATER1SETUP #############################
HMdcCalibrater1Setup::HMdcCalibrater1Setup(const char* name,const char* title,HMdcSetup* set)
    : TNamed(name,title)
{
...
void HMdcCalibrater1Setup::printParam(void)
{
    // prints the parameters to the screen
    SEPERATOR_msg("-",60);
    INFO_msg(10,DET_MDC,"HMdcCalibrater1Setup:");
    gHades->getMsg()->info(10,DET_MDC,GetName()
...
//################################## TRACKFINDERSETUP #############################
HMdcTrackFinderSetup::HMdcTrackFinderSetup(const char* name,const char* title,HMdcSetup* set)
    : TNamed(name,title)
{
...
void HMdcTrackFinderSetup::printParam(void)
{
    // prints the parameters to the screen
    SEPERATOR_msg("-",60);
    INFO_msg(10,DET_MDC,"HMdcTrackFinderSetup:");
    gHades->getMsg()->info(10,DET_MDC,GetName()
			   ,"MagnetStatus    =  %i :  0 = MagnetOn,  1 = MagnetOff"
			   ,isCoilOff);
    gHades->getMsg()->info(10,DET_MDC,GetName()
			   ,"ClustFinderType =  %i :  0 = combined chamber clusters, 1 = single chamber clusters"
			   ,typeClustFinder);
    gHades->getMsg()->info(10,DET_MDC,GetName()
...
//################################## CLUSTERTOSEGSETUP #############################
HMdcClusterToHitSetup::HMdcClusterToHitSetup(const char* name,const char* title,HMdcSetup* set)
    : TNamed(name,title)
{
...
void HMdcClusterToHitSetup::printParam(void)
{
    // prints the parameters to the screen
  SEPERATOR_msg("-",60);
  INFO_msg(10,DET_MDC,"HMdcClusterToHitSetup:");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"ModForSeg1      =  %i : -1 = don't fill HMdcSeg for segment 1,"
			 ,mdcForSeg1);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"                        0 = fill HMdcSeg by cluster in MDC1 if typeClFinder=1");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"                            and sector has mdc1&2, else by cluster in existing MDC,");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"                        1 =  -/- in mdc2,");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"ModForSeg2      =  %i : -1 = don't fill HMdcSeg for segment 2,"
			 ,mdcForSeg2);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"                        2 = fill HMdcSeg by cluster in MDC3 if typeClFinder=1");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"                           and sector has mdc3&4 and MagnetOff,");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"                            else by cluster in existing MDC,");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"                        3 =  -/- in MDC4\n");
}
...
//################################## FITTERSETUP #############################
HMdc12FitSetup::HMdc12FitSetup(const char* name,const char* title,HMdcSetup* set)
    : TNamed(name,title)
{
...
void HMdc12FitSetup::printParam(void)
{
    // prints the parameters to the screen
  SEPERATOR_msg("-",60);
  INFO_msg(10,DET_MDC,"HMdc12FitSetup:");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"FitAuthor       =  %i :  0 = Hejdar     , 1 = Alexander"
			 ,fitauthor);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"DrTimeCalcVers  =  %i :  0 = polynom    , 1 = HMdCal2ParSim::calcTime"
			 ,fitVersion);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"FitType         =  %i :  0 = segment fit, 1 = independent mdc fit"
			 ,fitIndep);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"DebugCategorie  =  %i :  1 = create and fill catMdcClusFit&catMdcWireFit, 0 = don't cr."
			 ,fitNTuple);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"DebugPrint      =  %i :  1 = print debug information, 0 = don't print"
			 ,fitPrint);
}
...
//################################## COMMONSETUP #############################
HMdcCommonSetup::HMdcCommonSetup(const char* name,const char* title,HMdcSetup* set)
    : TNamed(name,title)
{
...
void HMdcCommonSetup::printParam(void)
{
    // prints the parameters to the screen
  SEPERATOR_msg("-",60);
  INFO_msg(10,DET_MDC,"HMdcCommonSetup:");
  gHades->getMsg()->info(10,DET_MDC,GetName(),"isSimulation    =  %i :  0 = real data, 1 = simulation"
			 ,isSimulation);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"analysisLevel   =  %i :  0 = raw, 1 = cal1, 2 = cal2, 3 = hit, 4 = fit"
			 ,analysisLevel);
  gHades->getMsg()->info(10,DET_MDC,GetName(),"tracker         =  %i :  1 = dubna, 2 = santiago"
			 ,tracker);
}
