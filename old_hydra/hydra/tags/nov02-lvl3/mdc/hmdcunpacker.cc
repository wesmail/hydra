...
// (Co-Author meanwhile Author) Modified: February 26, 03 by Peter Zumbruch <P.Zumbruch@gsi.de>
...
void HMdcUnpacker::determineDecodeVersion(void)
{
  ...
  HDataSource* source = 0;
  source = gHades->getDataSource();
  if (source != 0)
    {
      ...
      if (!versionSetManual)
	{
	  ...
	  Error("determineDecodeVersion()",
		" SubEvtId %i - obsolete function call cannot determine decodeVersion, see documentation ... exiting",
		subEvtId);
	  exit(EXIT_FAILURE);
	}
    ...
    {
      Error("determineDecodeVersion()",
	    " SubEvtId %i - no data source defined in gHades, add source to gHades before calling the ctor of hmdcunpacker ... exiting"
	    ,subEvtId);
      exit(EXIT_FAILURE);
    }
}
...
Bool_t HMdcUnpacker::checkMboRawStructBounds(Int_t sector,Int_t module, Int_t mbo, Int_t tdc , UInt_t dataword)
{
