...
void HGeomTrd1::calcVoluParam(HGeomVolume* volu) {
  Double_t fac=20.;
...
TShape* HGeomTrd1::createRootVolume(HGeomVolume* volu) {
  calcVoluParam(volu);
  Text_t* name=(char*)volu->GetName();
  TArrayD& p=*param;
  TShape* s=new TTRD1(name,name,"",(Float_t)(p[0]),(Float_t)(p[1]),
         (Float_t)(p[2]),(Float_t)(p[3]));
  return s;
}
