...
void HRuntimeDb::setCurrentEventFile(Int_t n) {
...
HEventFile* HRuntimeDb::getEventFile(Int_t n) {
...
void HRuntimeDb::removeEventFile(Int_t n) {
...
void HRuntimeDb::writeVersions() {
  ...
  if (getOutput() && output->check()) {
    if (versionsChanged &&
          (strcmp(output->IsA()->GetName(),"HParRootFileIo")==0)) {
      output->cd();
...
Bool_t HRuntimeDb::initContainers() {
  ...
  Text_t* refFileName=(char*)file->getRefFileName();
