...
//  For details: see User's Guide and Developer's Guide in hadascii.doc 
//  file or on webpage http://www.gsi.de/~przygoda
//  ---------------------------------------------------------------------------
...
// --------------------- string and char operations ---------------------
 ...
 Int_t GetExit();
   // error program termination function
 void SetExit(Int_t stat);
   // setting error program termination function
...
// ***************************************************************************
  ...
  public:  
   ...
   friend ostream& operator<< (ostream& stream, HAsciiSectionRaw& section) {
    stream << "Number of keywords (columns): " << section.fKeys << endl;
    if (section.fKeys > 0) {
     ...
     stream << "Size (nr of elements in column): " << section.fElements << endl;
    }
    return stream;
   }
...
// ***************************************************************************
 ...
 public:
  ...
  friend ostream& operator<< (ostream& stream, HAsciiSection& section) {
   ...
   " End: " << section.GetSectionEnd() << endl;
   stream << "Raw data: ";
   if (section.GetSectionRaw()) {
    stream << " present.\n";
    stream << section.GetSectionRaw() << endl;
   }
...
// ***************************************************************************
 ...
 public:
  ...
  friend ostream& operator<< (ostream& stream, HAsciiSectionList& list) {
...
// ***************************************************************************
 ...
 public:
  ...
  fstream fStreamFileRead; // - stream object handling file reading
  ...
  friend ostream& operator<< (ostream& stream, HAsciiFile& file) {
   stream << "File: " << file.GetFileName() << 
             " Size: " << file.GetFileSize() << endl;
   return stream;
  }
...
// ***************************************************************************
 ...
 public:
  ...
  friend ostream& operator<< (ostream& stream, HAsciiFileList& list) {
...
// ***************************************************************************
 ...
 protected:
  ...
  Int_t fDataInt[1024]; // - container for data of Int_t type
  Int_t *fpDataInt; 
    // - dynamically created container for data of Int_t type
    // if number of units is larger than 1024
  Float_t fDataFloat[1024]; // - container for data of Float_t type
  Float_t *fpDataFloat;
    // - dynamically created container for data of Float_t type
    // if number of units is larger than 1024
  Char_t fDataChar[1024]; // - container for data of Char_t type 
  Char_t *fpDataChar;
    // - dynamically created container for data of Char_t type
    // if number of units is larger than 1024
  Char_t fDataString[1024]; // - container for string (set of characters terminated by '\0')
  Char_t *fpDataString;
    // - dynamically created container for string 
    // if string length is larger than 1023
