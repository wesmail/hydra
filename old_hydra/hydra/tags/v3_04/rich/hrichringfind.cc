...
Bool_t HRichRingFind::init(HRichAnalysis* showMe) {
 ...
 iPadPlane.Set(((HRichGeometry*)showMe->getGeomPar())->getPadsNr());
 iRingNr = 0;
 iAlgorithm1RingNr = 0;
 iAlgorithm2RingNr = 0;
...
for (m = 0; m < showMe->GetLabelNr(); m++) {
 ...
 HRichPad *pPadsFired = NULL;
 if (nrFired) pPadsFired = new HRichPad[nrFired];
 ...
 for (j = showMe->GetLabel(m)->iLowerY; j <= showMe->GetLabel(m)->iUpperY; j++)
  for (i = showMe->GetLabel(m)->iLeftX; i <= showMe->GetLabel(m)->iRightX; i++)
   if (showMe->GetPad(i,j)->getAmplitude() > 0  && 
    ...
    pPadsFired[k].setPadX(i);
    pPadsFired[k].setPadY(j);
    pPadsFired[k].setPadAmplitude(showMe->GetPad(i,j)->getAmplitude());
    k++;
    if (k >  nrFired) {
     exit(1);
    }
 ...
 for (i = 0; i < nrFired; i++)
  for (j = i+1; j < nrFired; j++)
   if ((pPadsFired[i].getPadX()-pPadsFired[j].getPadX()) && 
       (pPadsFired[i].getPadY()-pPadsFired[j].getPadY())) {
    fDistance = sqrt((pPadsFired[i].getPadX()-pPadsFired[j].getPadX())*
                (pPadsFired[i].getPadX()-pPadsFired[j].getPadX()) +
                (pPadsFired[i].getPadY()-pPadsFired[j].getPadY())*
                (pPadsFired[i].getPadY()-pPadsFired[j].getPadY()));
    if (fDistance > pAnalysisParams->iRingRadius/2 && fDistance < pAnalysisParams->iRingMatrixSize)
     for (k = j+1; k < nrFired; k++)
      if ((pPadsFired[j].getPadX()-pPadsFired[k].getPadX()) && 
          (pPadsFired[j].getPadY()-pPadsFired[k].getPadY())) {
       fDistance = sqrt((pPadsFired[j].getPadX()-pPadsFired[k].getPadX())*
                   (pPadsFired[j].getPadX()-pPadsFired[k].getPadX()) +
                   (pPadsFired[j].getPadY()-pPadsFired[k].getPadY())*
                   (pPadsFired[j].getPadY()-pPadsFired[k].getPadY()));
       if (fDistance > pAnalysisParams->iRingRadius/2 && fDistance < pAnalysisParams->iRingMatrixSize) {
 ...
 // ................. create  map .........
         ...
         fDivX = (pPadsFired[j].getPadX()-pPadsFired[k].getPadX())*
                (pPadsFired[i].getPadY()-pPadsFired[j].getPadY())-
                (pPadsFired[i].getPadX()-pPadsFired[j].getPadX())*
                (pPadsFired[j].getPadY()-pPadsFired[k].getPadY());
         fDivY = (pPadsFired[j].getPadX()-pPadsFired[k].getPadX())*
                (pPadsFired[i].getPadY()-pPadsFired[j].getPadY())-
                (pPadsFired[i].getPadX()-pPadsFired[j].getPadX())*
                (pPadsFired[j].getPadY()-pPadsFired[k].getPadY());
         ...
         if (fDivX == 0.0 || fDivY == 0.0) { }
          ...
          fRingX = 0.5*((pPadsFired[j].getPadX()*pPadsFired[j].getPadX()-
                        pPadsFired[k].getPadX()*pPadsFired[k].getPadX()+
                        pPadsFired[j].getPadY()*pPadsFired[j].getPadY()-
                        pPadsFired[k].getPadY()*pPadsFired[k].getPadY())*
                       (pPadsFired[i].getPadY()-pPadsFired[j].getPadY()) - 
                       (pPadsFired[i].getPadX()*pPadsFired[i].getPadX()-
                        pPadsFired[j].getPadX()*pPadsFired[j].getPadX()+
                        pPadsFired[i].getPadY()*pPadsFired[i].getPadY()-
                        pPadsFired[j].getPadY()*pPadsFired[j].getPadY())*
                       (pPadsFired[j].getPadY()-pPadsFired[k].getPadY())) / fDivX;

          fRingY = 0.5*((pPadsFired[i].getPadX()*pPadsFired[i].getPadX()-
                        pPadsFired[j].getPadX()*pPadsFired[j].getPadX()+
                        pPadsFired[i].getPadY()*pPadsFired[i].getPadY()-
                        pPadsFired[j].getPadY()*pPadsFired[j].getPadY())*
                       (pPadsFired[j].getPadX()-pPadsFired[k].getPadX()) - 
                       (pPadsFired[j].getPadX()*pPadsFired[j].getPadX()-
                        ...
                        pPadsFired[j].getPadY()*pPadsFired[j].getPadY()-
                        pPadsFired[k].getPadY()*pPadsFired[k].getPadY())*
                       (pPadsFired[i].getPadX()-pPadsFired[j].getPadX())) / fDivY;

          fRingR = sqrt((pPadsFired[i].getPadX()-fRingX)*
                       (pPadsFired[i].getPadX()-fRingX)+
                       (pPadsFired[i].getPadY()-fRingY)*
                       (pPadsFired[i].getPadY()-fRingY));
...
void HRichRingFind::CalcRingParameters(HRichAnalysis *showMe) {
 ...
 Int_t i,j,k,l,m,n,
       iCount, iIsPhot4, iIsPhot8, iPhot4Nr, iPhot8Nr;
