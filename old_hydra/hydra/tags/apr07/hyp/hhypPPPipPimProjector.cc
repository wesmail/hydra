...
//              cerr << algoName << " got no TLorentzVector " << endl;
    ...
    if (simuflag == 1) {
      ...
      if ((my_pip != NULL) && (my_pim != NULL)
        ...
        HPidGeantTrackSet *pipGeantSet =
          (HPidGeantTrackSet *) my_pip->getGeantTrackSet();
        HPidGeantTrackSet *pimGeantSet =
          (HPidGeantTrackSet *) my_pim->getGeantTrackSet();
        HPidGeantTrackSet *p1GeantSet =
          (HPidGeantTrackSet *) my_p1->getGeantTrackSet();
        HPidGeantTrackSet *p2GeantSet =
          (HPidGeantTrackSet *) my_p2->getGeantTrackSet();
        ...
        if (pip_geant_track >= 0) {
...
#if DEBUG==1
          // DEBUG 
          HLinearCategory *myCatGeantKine =
            (HLinearCategory *) gHades->getCurrentEvent()->
          ...
          if (pim_geant_track > tracks)
            tracks = pim_geant_track;
          if (pip_geant_track > tracks)
            tracks = pip_geant_track;
          if (p1_geant_track > tracks)
            tracks = p1_geant_track;
          if (p2_geant_track > tracks)
            tracks = p2_geant_track;
          ...
          if (pip_id == 8 && pim_id == 9 && p1_id == 14 && p2_id == 14) {
            ...
            cout << "ID         \t" << pip_id << "\t\t" << pim_id << "\t\t" <<
              p1_id << "\t\t" << p2_id << endl;
            cout << "GeantTrack \t" << pip_geant_track << "\t\t" <<
              pim_geant_track << "\t\t" << p1_geant_track << "\t\t" <<
              p2_geant_track << endl;
            cout << "ParentTrack\t" << pip_parentTrack << "\t\t" <<
              pim_parentTrack << "\t\t" << p1_parentTrack << "\t\t" <<
              p2_parentTrack << endl;
            ...
            if (pip_parentTrack > 0) {
              HGeantKine     *parent1 =
                (HGeantKine *) myCatGeantKine->getObject(pip_parentTrack - 1);
            ...
            if (pim_parentTrack > 0) {
              HGeantKine     *parent1 =
                (HGeantKine *) myCatGeantKine->getObject(pim_parentTrack - 1);
              pip_parentID = parent1->getID();
              pip_parentParent = parent1->getParentTrack();
              cout << "Parent of pi- ID = " << pip_parentID
                << "\nParentParent     = " << pip_parentParent << endl;
            ...
            if (p1_parentTrack > 0) {
              HGeantKine     *parent1 =
                (HGeantKine *) myCatGeantKine->getObject(p1_parentTrack - 1);
              pip_parentID = parent1->getID();
              pip_parentParent = parent1->getParentTrack();
            ...
            if (p2_parentTrack > 0) {
              HGeantKine     *parent1 =
                (HGeantKine *) myCatGeantKine->getObject(p2_parentTrack - 1);
              pip_parentID = parent1->getID();
              pip_parentParent = parent1->getParentTrack();
            ...
            for (Int_t i = 0; i < tracks; i++)
              cout << "Particle(" << i +
                1 << ") = " << ((HGeantKine *) myCatGeantKine->getObject(i))->
                getID() << endl;
          }                     // end if(pi+ &&pi- &&p1&& p2)
#endif
        }
    ...
    if (!mylist->getUserValue(FILLER_MISSING_PID, miss_pid))
      miss_pid = -999;
...
Bool_t HHypPPPipPimProjector::init()
{
  ...
  TString
    st_base
    ("pp_miss:miss4:pippim_invmass:ppmiss_theta:pp_theta:fProbAlg:miss_pid");
  TString st_trigger(":downscalingFlag:triggerDecision");
  TString st_dtof_kine(":dtof:pidtr:kine_chi2:kine_chi24:fakes");
  TString st_full_lorentz(":p1_mx:p1_my:p1_mz"
                          ":p2_mx:p2_my:p2_mz"
                          ":pip_mx:pip_my:pip_mz" ":pim_mx:pim_my:pim_mz");

  // Create String
  if (nt_trigger)
    st_base += st_trigger;
  if (nt_dtof_refit)
    st_base += st_dtof_kine;
  if (nt_full_lorentz)
    st_base += st_full_lorentz;

  if (simuflag != 0) {
    // Add String for sims
    TString st_base_geant(":pip_id:pim_id:p1_id:p2_id"
                          ":pip_geninfo:pim_geninfo:p1_geninfo:p2_geninfo");
    TString st_full_geant(":g_p1_mx:g_p1_my:g_p1_mz"
