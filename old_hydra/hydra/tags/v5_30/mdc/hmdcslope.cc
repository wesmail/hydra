...
//*-- Last Modification: 12-10-2000 P.Zumbruch 
				     ...
				     Int_t sector, Int_t module, Int_t motherboard, Int_t tdc)
{
  ...
  Float_t xmaxfitbin=(proj->GetMean())+10*0.0001; // before 10*0.0005 ttoooo big ?  
  TF1 *f = new TF1("f1","gaus", xminfitbin, xmaxfitbin); 
  proj->Fit("f1","RNQ");
  ...
  if (trendCalculationActive)
    {
      ...
      //     slopeTrend = new Int_t[6*4*16*96][5000];
      //  slopeTrend = new MboSlopeTrendIntField;
      /* for(Int_t sectorIter=0; sectorIter < 6; sectorIter++)
	 {
	 for(Int_t moduleIter=0; moduleIter < 4; moduleIter++)
	 {
	 slopeTrend[sectorIter][moduleIter] = new MboSlopeTrendIntField[1];
	 // slopeTrend[sectorIter][moduleIter] = new MboSlopeTrendIntField[1];
	 // errorTrend[sectorIter][moduleIter] = new MboErrorTrendIntField[1];
	 }
	 }
      */
    }
			       ...
			       Float_t delta, Float_t *myslope, Float_t *myerror)
{
  ...
  /* ***
     TH1F *hFitarea = new TH1F("fit","fit",11000,0,1100);
     hFitarea->SetBinContent((Int_t)(delta*10+1),t0);
     hFitarea->SetBinContent((Int_t)(delta*20+1),t1);
     hFitarea->SetBinContent((Int_t)(delta*30+1),t2);
     hFitarea->SetBinContent((Int_t)(delta*40+1),t3);
     hFitarea->SetBinContent((Int_t)(delta*50+1),t4);
     *** */
  ...
  for (Int_t myiter=1;myiter <=5; myiter++)
    {
      //***      hFitarea->SetBinError((Int_t)(delta*myiter*10+1),binError);
      hFitarea->SetBinError((Int_t) myiter, binError);
    }
  
  // calculation of slope & chisquare
  
  //***  TF1 *f = new TF1("f1","pol1", 0,1050); // call fit function poynom 1.order in x-range
  ...
  /* ***
   *slope = (Float_t) ((-1)/f->GetParameter(1));
   *error = (Float_t) (f->GetChisquare()/f->GetNDF());
  *** */
...
inline Int_t HMdcSlope::execute(void)
{
  ...
  if (trendCalculationActive)
      ...
      while ((raw=(HMdcRaw*)iter->Next()))
	{
	  ...
	  //Modification on 12/10/00 for calibration tests
// 	  raw->setSlope(slope);
// 	  raw->setSlopeError(error);
	  //END of Modification
      ...
      while ((raw=(HMdcRaw*)iter->Next()))
	{
	  ...
	  if (!selectorActive || 
	      ...
	      for (Int_t methodIter=0; methodIter <= HMDC_SLOPE_NALGORITHM; methodIter++)
		{
	      ...
	      //Modification on 12/10/00 for calibration tests
// 	      raw->setSlope(slope[0]);
// 	      raw->setSlopeError(error[0]);
	      //END of Modification
