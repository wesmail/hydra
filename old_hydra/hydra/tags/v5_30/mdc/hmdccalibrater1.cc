...
// HMdcCalibrater1(name,title,option)
//        option =  1 No Start and Cal (default)
//                  2 Start and Cal
...
// If no Cal (3 or 4 )is selected no input from HMdcCalParRaw is needed
...
HMdcCalibrater1::HMdcCalibrater1(Text_t* name,Text_t* title,Int_t vers)
...
Bool_t HMdcCalibrater1::init(void) {
  ...
  if(StartandNoCal||StartandCal){
      startcalCat=gHades->getCurrentEvent()->getCategory(catStartCal);
      if (!startcalCat) {
	  startcalCat=gHades->getSetup()->getDetector("Start")->buildCategory(catStartCal);
	  if (!startcalCat) return kFALSE;
	  else {gHades->getCurrentEvent()->addCategory(catStartCal,startcalCat,"Start");
  ...
  if(StartandNoCal||StartandCal){
      iterstart=(HIterator *)startcalCat->MakeIterator("native");
  }

   cout<<"***********************HMdcCalibrater1**************************"<<endl;
   cout<<"*  options  1 (default) no Start and calibration parameters    *"<<endl;
   cout<<"*           2           Start and calibration parameters       *"<<endl;
   cout<<"*           3           no Start and no calibration parameters *"<<endl;
   cout<<"*           4           Start and no calibration parameters    *"<<endl;
   cout<<"*                                                              *"<<endl;
   if(NoStartandCal)
...
Float_t HMdcCalibrater1::getstarttime(){
  ...
  iterstart->Reset();
  Float_t starttime=0;
  while ((startcal=(HStartCal *)iterstart->Next())!=0) {
	...
	while ((raw=(HMdcRaw *)iter->Next())!=0) {
		...
		if (cal) {
			...
			if (abs(nHits)>=2){
			    ...
			    break;
			    default:   cal->setTime2(tdc.getOffset() -
