...
//HOrdCategory. 
//
// Category  creating sets of HDataObjects in a linear 
// TClonesArray.
// To create such a category the caller needs to define
// the sets.  
// For example, let's suppose that we want to create sets of HDataObjects
...
//
// Inside a set of HDataObjects, no lexicographic order is assumed !
//
//
// B) Accessing HDataObjects in HOrdCategory
//  
...
//           // select (sec=5,mod=2) HDataObjects
//            ... so something 
//           iter0->Reset();  
//           while (  (aHit = (Hit*) iter0->Next()) != 0 ){
//           // select (sec=5,mod=0) HDataObjects          
//            ... do something  
//         } 
//   }
// }
...
void HOrdCategory::makeBranch(TBranch *parent) {
  ...
  if (parent==NULL) Error("makeBranch","parent is NULL");
    ...
    if(fData) {
    TBranch *b=NULL;
      sprintf(name,"%s.fOrdData",getClassName());
      b=gHades->getTree()->makeBranch(name,
				      fData->IsA()->GetName(),
				     fData,2000,1);
         if (b) parent->GetListOfBranches()->Add(b);
...
HDataObject *HOrdCategory::getObject(HLocation &aLoc) {
// no bound check and use only aLoc[0]
return (HDataObject *)fData->UncheckedAt(aLoc[0]);
}

HDataObject *HOrdCategory::getObject(Int_t index) {
// no bound check
return (HDataObject *)fData->UncheckedAt(index); 
} 


HDataObject *HOrdCategory::getObject(HLocation &aLoc, Int_t index) {
// no bound check and use only aLoc[0]
...
if(setup){
 ...
 TObjArray* obj = fData->getSubArray(currentlinearAddr); 
   if(obj){
       return (HDataObject *) obj->UncheckedAt(index);
   }

  if ( index >  getCurrentLinearLenght() ) return NULL; 
return (HDataObject *)fData->UncheckedAt(getCurrentLinearOffset()+index);
}
return NULL; 
}
