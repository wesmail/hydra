...
HStartLookupTdc::HStartLookupTdc(Int_t n) {
  ...
  for(Int_t i=0;i<n;i++) (*array)[i] = new HStartLookupChan();
}
...
HStartLookup::HStartLookup(Int_t numTdc, Int_t numChan) : HParSet() {
  ...
  for (Int_t i = 0; i < nTdc; i++) (*array)[i] = new HStartLookupTdc(numChan);
}
