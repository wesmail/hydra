...
Float_t HRichDigitizer::calcCharge(Float_t sourceCharge, Float_t distance, 
  ...
  Float_t distanceSquared=distance*distance;

  return ((((HRichDigitisationPar*)fDigitisationPar)->getCouplingFactor() * 
           sourceCharge / twoPI) *
    (atan(Xd*Yd/(distance*sqrt(double(distanceSquared+Xd*Xd+Yd*Yd))))
   - atan(Xd*Yu/(distance*sqrt(double(distanceSquared+Xd*Xd+Yu*Yu))))
   + atan(Xu*Yu/(distance*sqrt(double(distanceSquared+Xu*Xu+Yu*Yu))))
   - atan(Xu*Yd/(distance*sqrt(double(distanceSquared+Xu*Xu+Yd*Yd))))));
}
//============================================================================
...
void HRichDigitizer::digitiseCherenkovHits(HGeantRichPhoton *pCerHit) {
 ...
 HRichFrame *pFrame = ((HRichGeometryPar*)fGeometryPar)->getFramePar();
     ...
     Float_t yHitcm = (pCerHit->getY()/10.0F);
...
void HRichDigitizer::digitiseDirectHits(HGeantRichDirect *pDirHit) {
 ...
 HRichFrame *pFrame = ((HRichGeometryPar*)fGeometryPar)->getFramePar();
     ...
     Float_t yHitcm = pDirHit->getY()/10.0F;
