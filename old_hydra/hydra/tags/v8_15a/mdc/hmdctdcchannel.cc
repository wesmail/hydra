...
HMdcTdcChannelMod::HMdcTdcChannelMod(Int_t sec, Int_t mod, Int_t mbo)
{
  ...
  if (pMdc)
	...
	array->AddAt(new HMdcTdcChannelMbo((Int_t)(rMbo.getNTdcs()/8),(char*)rMbo.GetName()),i);
      }
    }
  else
    {
...
Bool_t  HMdcTdcChannel::writeChannelMasks(char *ofilename, Int_t  plane, Int_t sector)
{ 
  FILE *Ofile;
...
Bool_t HMdcTdcChannel::readRocFile(const char *filename)
{
   ...
   *((char*)gSystem->BaseName( tmpfilename ) - 1) = '\0';    // override "/" 
   ...
   strcat( (char *)filename, ".threshold" );
...
Int_t HMdcTdcChannel::readRocFileVersion(const char *filename)
{
