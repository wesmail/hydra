...
Bool_t HRuntimeDb::initContainers(void) {
  ...
  if (!rc) 
    ERROR_msg(HMessageMgr::DET_ALL,"error occured during initialization");
...
Bool_t HRuntimeDb::makeParamFile(const char* fName,const char* experiment,
...
Bool_t HRuntimeDb::fillParamFile() {
  ...
  HParIo* pio=0;
  if (firstInput&&firstInput->InheritsFrom("HParOraIo")) pio=firstInput;
  else if (secondInput&&secondInput->InheritsFrom("HParOraIo")) pio=secondInput;
  if (!pio) {
  ...
  while ((pRunInfo=(HRunInfo*)iterInfo())!=0) {
    ...
    if (!run) {
      ...
      gHades->getMsg()->info(10,HMessageMgr::DET_ALL,this->GetName(),
                             " initialisation for event file %s",currentFileName.Data());
      gHades->getMsg()->info(10,HMessageMgr::DET_ALL,this->GetName(),
                             "  run id %s",currentRun->GetName());
      SEPERATOR_msg("*",60);
      while ((cont=(HParSet*)iterCont())) {
        if (!cont->isStatic()) rc=cont->init();
        if (!rc) {
      ...
      while ((cont=(HParSet*)iterCont())) {
        HParVersion* v=run->getParVersion((char*)(cont->GetName()));
  ...
  fout<<"Number runs of not fully initialized: "<<numErrors<<'\n';
  fout<<"//------------------------------------------------------------------------------\n";
  TIter iterRuns(runs);
  HRun* run;
  fout<<"-----------------  runs, versions  -----------------\n";
