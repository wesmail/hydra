...
Float_t HRtEfficiencyMatrix::getP(UInt_t binP) const { 
  return fMinP + binP*fBinSizeP + fBinSizeP/2.;
}

Float_t HRtEfficiencyMatrix::getTheta(UInt_t binTheta) const { 
  return fMinTheta + binTheta*fBinSizeTheta + fBinSizeTheta/2.;
}

Float_t HRtEfficiencyMatrix::getPhi(UInt_t binPhi) const { 
  return fMinPhi + binPhi*fBinSizePhi + fBinSizePhi/2.;
}

Int_t HRtEfficiencyMatrix::getBinP(Float_t p) const { 
  return int(floor((p - fMinP) /  fBinSizeP)); 
}

Int_t HRtEfficiencyMatrix::getBinTheta(Float_t theta) const { 
  return int(floor((theta - fMinTheta)) / fBinSizeTheta); 
}

Int_t HRtEfficiencyMatrix::getBinPhi(Float_t phi) const { 
