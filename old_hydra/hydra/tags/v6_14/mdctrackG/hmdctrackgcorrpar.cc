...
void HMdcTrackGCorrPar:: initCorrections(TString inputE,TString inputP)
{
    corr->init(inputE,inputP);
}
...
Double_t HMdcTrackGCorrPar:: calcMomentum(HMdcSegSim *seg[2] ,Bool_t cond,Double_t target)
{
  return spline->calcMomentum(seg,cond,target);
}
...
Double_t HMdcTrackGCorrPar:: calcMomentum123(HMdcSegSim *seg[2] ,Bool_t cond,Double_t target)
{
  return spline->calcMomentum123(seg,cond,target);
}
...
//void HMdcTrackGCorrPar::calcField(Double_t *a,Double_t *b,Int_t i){ field->calcField(a,b,i);}
