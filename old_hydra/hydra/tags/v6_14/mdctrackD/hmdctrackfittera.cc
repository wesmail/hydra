...
void HMdcTrackFitterA::derivativesOfFunctional( Double_t fun0, Double_t* par,
  ...
  agrad = 0.0;
  for(Int_t k = 0; k < numOfParam; k++) {
    Double_t funk = valueOfFunctional(par,k,stepD);
    Double_t funk1 = valueOfFunctional(par,k,-stepD);
    ...
    for(Int_t l = k+1; l < numOfParam; l++) grad2[k][l] = grad2[l][k] =
      ( fun0 - funk + valueOfFunctional(par,k,stepD,l,stepD) -
                      valueOfFunctional(par,l,stepD) ) * oneDvStepD2;  
  }
  agrad = sqrt(agrad);
...
void HMdcTrackFitterA::secondDerivOfFunct(void) {
  ...
  for(Int_t k = 0; k < numOfParam; k++) {
    Double_t funk = valueOfFunctional(parameter,k,stepD);
    for(int time=firstTime; time<lastTime; time++) dTdA[k][time]=drTime[time];
    Double_t funk1 = valueOfFunctional(parameter,k,-stepD);
    for(int time=firstTime; time<lastTime; time++) {
    ...
    for(Int_t l=k+1; l<numOfParam; l++) grad2[k][l] = grad2[l][k] =
      (funMin - funk + valueOfFunctional(parameter,k,stepD,l,stepD) -
                       valueOfFunctional(parameter,l,stepD) ) * oneDvStepD2;
  }
  return;
}
