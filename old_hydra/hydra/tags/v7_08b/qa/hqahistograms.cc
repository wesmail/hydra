...
char title[256];
  ...
  stLatchHist = new TH1F("stLatchHist"," Trigger Pattern (from 1)",10,0,10);
  stLatchHist->SetXTitle("Trigger Input");
  histList->Add(stLatchHist,"start--gridx");
...
Int_t   SectColor[6]={   4 ,  2   ,  1  ,  8   , 6  ,  5 };
  ...
  sprintf(title,"RichHit-KickTrack: tofTOF_vs_startstrip (|#Delta #Theta|<5 deg,|#Delta #Phi|<5 deg)");
...
{
  hNTrackCandPerSec = new TH1F("hNTrackCandPerSec","TrackCand/sec",6,0,6);
  hNTrackCandLepPerSec = new TH1F("hNTrackCandLepPerSec","TrackCand/sec with ring",6,0,6);
}
void HQAHistograms::bookHistMatching(){
  ...
  hseckick   ->SetXTitle("sector");   hseckick   ->SetYTitle("N"); histList->Add(hseckick   ,"metamatching--");
  hsecspline ->SetXTitle("sector");   hsecspline ->SetYTitle("N"); histList->Add(hsecspline ,"metamatching--");
  hseckick0  ->SetXTitle("sector");   hseckick0  ->SetYTitle("N"); histList->Add(hseckick0  ,"metamatching--");
  hsecspline0->SetXTitle("sector");   hsecspline0->SetYTitle("N"); histList->Add(hsecspline0,"metamatching--");
  hseckick1  ->SetXTitle("sector");   hseckick1  ->SetYTitle("N"); histList->Add(hseckick1  ,"metamatching--");
  hsecspline1->SetXTitle("sector");   hsecspline1->SetYTitle("N"); histList->Add(hsecspline1,"metamatching--");
  ...
  hXdiffvsshowersector->SetXTitle("sector");   hXdiffvsshowersector->SetYTitle("Xdiff");   histList->Add(hXdiffvsshowersector,"metamatching--");
  hXdiffvstofstrip    ->SetXTitle("tofstrip"); hXdiffvstofstrip    ->SetYTitle("Xdiff");   histList->Add(hXdiffvstofstrip    ,"metamatching--");
  hYdiffvsshowersector->SetXTitle("sector");   hYdiffvsshowersector->SetYTitle("Ydiff");   histList->Add(hYdiffvsshowersector,"metamatching--");
  hYdiffvstofstrip    ->SetXTitle("tofstrip"); hYdiffvstofstrip    ->SetYTitle("Ydiff");   histList->Add(hYdiffvstofstrip    ,"metamatching--");
  hshower_quality     ->SetXTitle("sector");   hshower_quality     ->SetYTitle("Quality"); histList->Add(hshower_quality     ,"metamatching--");
  htof_quality        ->SetXTitle("sector");   htof_quality        ->SetYTitle("Quality"); histList->Add(htof_quality        ,"metamatching--");
