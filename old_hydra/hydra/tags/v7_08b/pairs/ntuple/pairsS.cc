...
using namespace std;
pairsS::pairsS(TTree *tree, TString eventGenerator)
{
...
// used to generate this class and read the Tree.
    ...
    bPluto=bUrQMD=bExp=kFALSE;

    if(eventGenerator.CompareTo("pluto")==0)
    {
	cout<<"##################################"<<endl;
        cout<<"generator pluto"<<endl;
	cout<<"##################################"<<endl;
        bPluto=kTRUE;
    }
    else if(eventGenerator.CompareTo("urqmd")==0)
    {
	cout<<"##################################"<<endl;
	cout<<"generator urqmd"<<endl;
	cout<<"##################################"<<endl;
  	bUrQMD=kTRUE;
    }
    else if(eventGenerator.CompareTo("exp")==0)
    {
	cout<<"##################################"<<endl;
	cout<<"generator exp"<<endl;
	cout<<"##################################"<<endl;
  	bExp=kTRUE;
    }
    else
    {
        cout<<"##################################"<<endl;
        cout <<"argument not know , use pluto !!!"<<endl;
        cout<<"##################################"<<endl;
	bPluto=kTRUE;
    }
...
void pairsS::Init(TTree *tree)
{
   ...
   for(Int_t icut=0; icut<MAXCUT; icut++)
   {
       ...
       sprintf(name,"hpolar_cut%d_truecb",icut);
       hpolar_cut_truecb[icut] = new TH1F(name,name,45,0,180);
       hpolar_cut_truecb[icut]->Sumw2();
       harr->Add(hpolar_cut_truecb[icut]);
	   ...
	   harr->Add(hpt_cut_pol[icut][ipol]);
           ...
           sprintf(name,"hpolar_cut%d_pol%d",icut,ipol);
	   hpolar_cut_pol[icut][ipol] = new TH1F(name,name,45,0,180);
           hpolar_cut_pol[icut][ipol]->Sumw2();
           harr->Add(hpolar_cut_pol[icut][ipol]);
	   ...
	   harr->Add(hpt_cut_true[icut][ipair]);
           ...
           sprintf(name,"hpolar_cut%d_true%d",icut,ipair);
	   hpolar_cut_true[icut][ipair] = new TH1F(name,name,45,0,180);
           hpolar_cut_true[icut][ipair]->Sumw2();
           harr->Add(hpolar_cut_true[icut][ipair]);
       }

   }
   // Init efficiency matrices

   pLowRangeEffFile = new TFile("/d/hades06/data/sudol/dilep/kick/new/laura/smoothedMatrixEle.root","READ");
   if (pLowRangeEffFile) 
   {
       pLowRangeEffFile->cd();
       p3DeffEle = (TH3F*) pLowRangeEffFile->Get("effi3DEleAllCut"); 
   }
   else Error("Init","pointer to eff matrix file is NULL");

   pHighRangeEffFile = new TFile("/d/hades06/data/sudol/dilep/kick/new/laura/smoothedMatrixPosi.root","READ");
     
   if (pHighRangeEffFile) 
   {
       pHighRangeEffFile->cd();
       p3DeffPosi = (TH3F*) pHighRangeEffFile->Get("effi3DPosiAllCut"); 
...
void pairsS::Loop()
{
    ...
    const Double_t r2d = 57.29577951;
    ...
    if(bUrQMD)
    { // number of events in UrQMD simulation
	evt =110582360.;
    }
    else if(bPluto)
    { // number of events in PLUTO simulation
	evt = 27800000.*5.3 * 0.98;
    }
	...
	harr->Add(hpt_sig0_cut_norm[icut]);
        //
	hpolar_back0_cut[icut] = getBackg(hpolar_cut_pol[icut][0],
					 hpolar_cut_pol[icut][2],0);
	hpolar_sig0_cut[icut]  = getSignal(hpolar_cut_pol[icut][1],
					  hpolar_back0_cut[icut]);
	
	hpolar_back0_cut_norm[icut] = getNorm(hpolar_back0_cut[icut],evt);
	hpolar_sig0_cut_norm[icut]  = getNorm(hpolar_sig0_cut[icut],evt);
	
	harr->Add(hpolar_back0_cut[icut]);
	harr->Add(hpolar_sig0_cut[icut]);
	harr->Add(hpolar_back0_cut_norm[icut]);
	harr->Add(hpolar_sig0_cut_norm[icut]);
	...
	hpolar_back1_cut[icut] = getBackg(hpolar_cut_pol[icut][0],
					 hpolar_cut_pol[icut][2],1);
	hpolar_sig1_cut[icut]  = getSignal(hpolar_cut_pol[icut][1],
					  hpolar_back1_cut[icut]);
	
	hpolar_back1_cut_norm[icut] = getNorm(hpolar_back1_cut[icut],evt);
	hpolar_sig1_cut_norm[icut]  = getNorm(hpolar_sig1_cut[icut],evt);
	
	harr->Add(hpolar_back1_cut[icut]);
	harr->Add(hpolar_sig1_cut[icut]);
	harr->Add(hpolar_back1_cut_norm[icut]);
	harr->Add(hpolar_sig1_cut_norm[icut]);
	///////////////////////////////////////////////////////////////	
	...
	for(Int_t ipol=0; ipol<MAXPOL; ipol++)
	{
	    ...
	    hpolar_cut_pol_norm[icut][ipol] = 
		getNorm(hpolar_cut_pol[icut][ipol],evt);
	    ...
	    harr->Add(hpolar_cut_pol_norm[icut][ipol]);
	}
	...
	hpolar_cut_truecb_norm[icut] =
	    getNorm(hpolar_cut_truecb[icut],evt);
	...
	harr->Add(hpolar_cut_truecb_norm[icut]);
	...
	for(Int_t ipair=0; ipair<MAXPAIR; ipair++)
	{
	    ...
	    hpolar_cut_true_norm[icut][ipair] = 
		getNorm(hpolar_cut_true[icut][ipair],evt);
	    ...
	    harr->Add(hpolar_cut_true_norm[icut][ipair]);
	}
    }
...
void pairsS::fillHistograms(Int_t cut, Float_t weight) 
{
    ...
    hpolar_cut_pol[cut][pol]->Fill(polar,weight*polarweight);
    ...
    if(bPluto)
    {
		...
		hpolar_cut_truecb[cut]->Fill(polar,weight*polarweight);
	    }
	     ...
	     GCommonDet2 >= 76) 
	{
	    ...
	    hpolar_cut_true[cut][0]->Fill(polar,weight*polarweight);
		...
		hpolar_cut_true[cut][1]->Fill(polar,weight*polarweight);
		break;
		...
		hpolar_cut_true[cut][2]->Fill(polar,weight*polarweight);
		break;
		...
		hpolar_cut_true[cut][3]->Fill(polar,weight);
		break;
		...
		hpolar_cut_true[cut][4]->Fill(polar,weight*polarweight);
		break;
		...
		hpolar_cut_true[cut][5]->Fill(polar,weight);
		break;
		...
		hpolar_cut_true[cut][6]->Fill(polar,weight*polarweight);
		break;
		...
		hpolar_cut_true[cut][7]->Fill(polar,weight*polarweight);
		break;
	...
	else if(bUrQMD)  // U R Q M D
	{
		...
		case 2: // eta dalitz 
		    ...
		    hrap_cut_true[cut][3]->Fill(rap,weight);
		    hpt_cut_true[cut][3]->Fill(pt,weight);
		    hpolar_cut_true[cut][3]->Fill(polar,weight*polarweight);
		    break;
...
Float_t pairsS::getEfficiencyFactor()
{
    ...
    if(	phi1>180.)   phi1_tmp = 180. - phi1;
    if(	phi2>180.)   phi2_tmp = 180. - phi2;
    
    if (chrg1==1) // positron
    {
	fEff1 = p3DeffPosi->
	    GetBinContent(p3DeffPosi->FindBin(d2r*phi1_tmp,d2r*theta1,mom1));
    }
    else if (chrg1==-1) // electron
    {
	fEff1 = p3DeffEle->
	    GetBinContent(p3DeffEle->FindBin(d2r*phi1_tmp,d2r*theta1,mom1));
    }
    ...
    {
	fEff2 = p3DeffPosi->
	    GetBinContent(p3DeffPosi->FindBin(d2r*phi2_tmp,d2r*theta2,mom2));
    }
    else if (chrg2==-1) // electron
      {
	  fEff2 = p3DeffEle->GetBinContent(p3DeffEle->FindBin(d2r*phi2_tmp,d2r*theta2,mom2));
      }
