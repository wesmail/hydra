...
Bool_t HMdcCalibrater1::init(void) {
  ...
  if (!calCat) {
      if(embedding==0)calCat=gHades->getSetup()->getDetector("Mdc")->buildCategory(catMdcCal1);
      if(embedding==1)
      {
          HMdcDetector* mymdc=(HMdcDetector*)(gHades->getSetup()->getDetector("Mdc"));
	  calCat=mymdc->buildMatrixCategory("HMdcCal1Sim",0.5F);
      }
...
void HMdcCalibrater1::printStatus()
{
    ...
    printf ("Embedding       =  %i :  0 = noEmbedding  , 1 = Embedding\n",                       embedding);
    printf ("--------------------------------------------------------------------------------------------\n");
    hasPrinted=kTRUE;
}
