...
Bool_t HMdcParRootFileIo::read(HMdcCellEff* pCellEff,Int_t* set) {
  ...
  Char_t message[200];
  sprintf(message,"%s%s",name,": module types initialized from Root file: 0 1 2 3");
  cout<<message<<endl;
...
Bool_t HMdcParRootFileIo::read(HMdcClosePairCutsPar* pClosePairCut,Int_t* set) {
  ...
  if(r){
    ...
    for(Int_t th=0;th<pMdc.getSize();th++) {
      
      HMdcClosePairCutsParPhi& r_phi=rMdc[th];
      HMdcClosePairCutsParPhi& p_phi=pMdc[th];
