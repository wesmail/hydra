...
HMdcCalParRawMod::HMdcCalParRawMod(Int_t sec, Int_t mod, Int_t mbo) {
  ...
  if (pMdc) {
    ...
    for (Int_t i=0; i<rMod.getSize(); i++) {
      ...
      array->AddAt(new HMdcCalParMbo(rMbo.getNTdcs(),(char*)rMbo.GetName()),i);
    }
  }
...
HMdcCalParRaw::HMdcCalParRaw(const char* name,const char* title,
  ...
  // prints the container
	...
	for(Int_t m=0;m<sec.getSize();m++) {
		...
		for(Int_t c=0;c<mbo.getSize();c++){
		    printf("%1i %1i %2i %s %3i % 10.5f % 10.2f % 12.5f % 12.2f %1i %1i\n",
			   s, m, l, mbo.GetName(),c,
