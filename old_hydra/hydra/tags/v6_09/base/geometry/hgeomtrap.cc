...
void HGeomTrap::calcVoluParam(HGeomVolume* volu) {
...
TShape* HGeomTrap::createRootVolume(HGeomVolume* volu) {
  calcVoluParam(volu);
  Text_t* name=(char*)volu->GetName();
  TArrayD& p=*param;
  TShape* s=new TTRAP(name,name,"",(Float_t)(p[0]),(Float_t)(p[1]),
         (Float_t)(p[2]),(Float_t)(p[3]),(Float_t)(p[4]),
         (Float_t)(p[5]),(Float_t)(p[6]),(Float_t)(p[7]),
         (Float_t)(p[8]),(Float_t)(p[9]),(Float_t)(p[10]));
  return s;
}
...
void HGeomTrap::calcVoluPosition(HGeomVolume* volu,
  ...
  if (!intrinsicRot) {
    intrinsicRot=new HGeomRotation();
