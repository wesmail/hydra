...
void HRun::print() {
  ...
  //cout<<"run: "<<fName<<'\n';
  gHades->getMsg()->info(10,DET_RUNTIMEDB,this->GetName(),"run: %d",getRunId());
  TIter next(parVersions);
  HParVersion* v;
  while ((v=(HParVersion*)next())) {
    gHades->getMsg()->info(10,DET_RUNTIMEDB,this->GetName(),"%-22s%10d%10d%10d", v->GetName(),v->getInputVersion(1),v->getInputVersion(2), v->getRootVersion());
