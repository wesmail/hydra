...
//HShowerHitFinderIPU
//
// HShowerHitFinderIPU reconstructor searches local maxima in CAL level
//
//////////////////////////////////////////////////////////////////////
...
HShowerHitFinderIPU::HShowerHitFinderIPU()
{
   fIter = NULL; 
   m_Loc.set(4, 0, 0, 0, 0);
   setFillPID();
...
Bool_t HShowerHitFinderIPU::init() {
...
Bool_t HShowerHitFinderIPU::initParameters() {
...
void HShowerHitFinderIPU::setCriterium(HShowerCriterium* pCrit) {
   if (m_pCriterium) delete m_pCriterium; 
   m_pCriterium = pCrit;
}

Bool_t HShowerHitFinderIPU::finalize() {
    return kTRUE;
}

HShowerHitFinderIPU& HShowerHitFinderIPU::operator=(HShowerHitFinderIPU &c) {

  return c;
}

Int_t HShowerHitFinderIPU::execute()
{
  HShowerCal *pCal;
  m_pCellArr->update();
...
Bool_t HShowerHitFinderIPU::lookForHit(HShowerCal* cal, HLocation& fLoc) {
#if DEBUG_LEVEL>2
  gDebuger->enterFunc("HShowerHitFinderIPU::execute");
...
#if DEBUG_LEVEL>2
  gDebuger->leaveFunc("HShowerHitFinderIPU::execute");
#endif
  updateFiredCells(fLoc);
  return isHit;
}

void HShowerHitFinderIPU::fillPID(HShowerHit* hit, HShowerPID* pid) {
   Float_t fX, fY, fZ;
...
void HShowerHitFinderIPU::fillSums(HShowerHit* hit, HLocation &fLoc) {
  HLocation fTmpLoc;
...
void HShowerHitFinderIPU::calcCoord(HShowerHit* hit, HLocation &fLoc) {
  ...
  ((HShowerGeometry*)getGeometry())->getSphereCoord(fLoc, vLabCoord);
...
Bool_t HShowerHitFinderIPU::isLocalMax(HLocation &fLoc) {
  HShowerCal* pCell;
...
pCell =  (HShowerCal*)m_pCellArr->getObject(fTmpLoc);
   ...
   charge1_int =  (int)pCell->getCharge();
   if(charge_int <= (charge1_int )) return 0;

// lower row middle column
fTmpLoc[2] = row+1;
fTmpLoc[3] = col;
pCell =  (HShowerCal*)m_pCellArr->getObject(fTmpLoc);
...
for(fTmpLoc[3] = col - 1; fTmpLoc[3] <= col + 1; fTmpLoc[3]++)
    {
       ...
       //charge1 =  pCell->getCharge();
       charge1_int = (int) pCell->getCharge();
   if((charge_int <= (charge1_int  )) && fTmpLoc[3]!=col) return 0;
//   if(charge_int < charge1_int ) return 0;
...
Float_t HShowerHitFinderIPU::calculateVar(HLocation &fLoc, int nRange, float avg)
{
  int rowL, rowU, colL, colR;
...
Float_t HShowerHitFinderIPU::calculateSum(HLocation &fLoc, int nRange, int* pncs)
{
...
HShowerHitHeader* HShowerHitFinderIPU::getHitHeader(HLocation &fLoc) {
  HLocation fTmpLoc;
...
void HShowerHitFinderIPU::updateClusters(HLocation &fLoc) {
   HShowerHitHeader *pHitHdr = getHitHeader(fLoc);
   if (pHitHdr)
      pHitHdr->incClusters();
}

void HShowerHitFinderIPU::updateLocalMax(HLocation &fLoc) {
   HShowerHitHeader *pHitHdr = getHitHeader(fLoc);
   if (pHitHdr)
      pHitHdr->incLocalMax();
}

void HShowerHitFinderIPU::updateFiredCells(HLocation &fLoc) {
   HShowerHitHeader *pHitHdr = getHitHeader(fLoc);
   if (pHitHdr)
