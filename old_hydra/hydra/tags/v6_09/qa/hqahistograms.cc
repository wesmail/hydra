...
void HQAHistograms::bookHistMdc() {
  ...
  Float_t t1tatx[2] = {-100.,800.};
  Float_t t1taty[2] = {   0.,600.};
  Int_t   t1tatn[2] = { 450 ,600 };
  //
  for(Int_t im=0;im<4; im++){
    ...
    mdcCal1_t2mt1_vs_t1_plane[im]->SetXTitle("t1 (ns)");
    mdcCal1_t2mt1_vs_t1_plane[im]->SetYTitle("t2-t1 (ns)");
  ...
  for(Int_t im=0;im<4; im++){
    for(Int_t is=0;is<6; is++){
       ...
       mdcCal1_t2mt1_vs_t1[is][im]->SetXTitle("t1 (ns)");
       mdcCal1_t2mt1_vs_t1[is][im]->SetYTitle("t2-t1 (ns)");
  ...
  for(Int_t i=0; i<7; i++){
      ...
      mdcCutCounts[i]->SetXTitle("p1{s1,s2,...,s6}, p2{s2,...,s6},...,p4{s1..}");
