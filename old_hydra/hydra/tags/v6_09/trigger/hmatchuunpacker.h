...
protected:
  ...
  TIterator *iterRich;
  TIterator *iterShower;

  HShowerUnpackParam* fShowerParam; // shower parameter container
  HMatchUUnpackParam* fMatchUParam; // MU parameter container (no proper param handling!)
 ...
 // tof decoding
  ...
  //Scalers decoding 
  Int_t getScaler(Int_t d) {  return (d & 0xffffffff);}
...
public:
  HMatchUUnpacker(Int_t unites=0,Bool_t compression=kFALSE, Int_t flog=0);
