...
void HMdcTrackFinder::fillCatMdcClus(Int_t sec) {
  ...
  for(Int_t ncl=0; ncl<nClus; ncl++) {
    ...
    for(Int_t seg=seg1; seg<=seg2; seg++) {
      ...
      clusNew->setNMergClust(clus.getNMergClust());
      if(isGeant) clusSimNew->getNTracks();
    }
  }
}
