...
const char* g_cpstr_HSHC_Methods[HSHC_MAX] =
...
# warning "calElect(): passing float through int argument!! fix me."
    if(((fMean = getMean(0, m_nDataSize, fMeanThreshold)) < 0)
        || ((fMean1 = getMean(0, fMean, fMeanThreshold)) < 0)
        || ((fMean2 = getMean(fMean, m_nDataSize, fMeanThreshold)) < 0)
...
# warning "runSigma(): passing float through int argument!! fix me."
    if((fMean = getMean(0, m_nDataSize, fMeanThreshold)) < 0)
        return;

# warning "runSigma(): passing float through int argument!! fix me."
    fSigma = getSigma(fMean, 0, m_nDataSize, fMeanThreshold);
...
# warning "runThreshold(): passing double through int argument!! fix me."
    if(((fMean = pCell->getOffset()) >= 0.0f)
        && ((fMean = -getMean(0, m_nDataSize, 0.0)) > 0))
    {
        return;
    }
...
# warning "runIntegrate(): passing double through int argument!! fix me."
    if(((fMean = pCell->getOffset()) >= 0.0f)
        && ((fMean = -getMean(0, m_nDataSize, 0.0)) > 0))
    {
        return;
