...
//*-- Modified : A.Sadovsky (10.12.2003) for HBaseTrack style output
//*-- Copyright : GENP
...
void HKickTrackFB::checkMETAoverlap(Int_t i,Int_t j){
  ...
  for(Int_t k=0; k<fNTofHits; k++){
    ...
    if((canddir.scalarProduct(candtofdir)) > 0.999) {
      fCandidates(i,j).markMETAoverlap();
      fCandidates(i,k).setShowerHit(fCandidates(i,j).getOuterHit());
    }
  }  
}
...
HCategory *HKickTrackFB::makeOutputCategory(HEvent *event) {
  ...
  if (!fCandidateOut) {
    Int_t ssize[2]={6,100};
    fCandidateOut = new HMatrixCategory("HKickTrackB",2,ssize,0.5);                //fCandidateOut = new HMatrixCategory("HKickTrack",2,ssize,0.5);
    if (fCandidateOut) event->addCategory(catKickTrackB, fCandidateOut, "Tracks"); //if (fCandidateOut) event->addCategory(catKickTrack, fCandidateOut, "Tracks");
    else {
  ...
  if (c) {
    ...
    if (candidate.getSystem() == 0) { //Specific stuff for Shower
      ...
      if (!oHit) {
        ...
        Short_t showerHitInd=fShowerHits->getIndex(oHit);
      ...
      Short_t showerHitInd=(shower) ? fShowerHits->getIndex(shower):-1;
      if(fTofClusterMode){
        HTofCluster *oCluster = 0;
...
# warning "Temporary solution for nov02 DST 01.2004: Index HTofHit is used for matching with HMetaMatch ONLY. !!!!!!"
          // temporary solution for nov02 DST 01.2004 (index HTofHit only for MetaMatch matching) !!!!!!
          hLoc.set(3,oCluster->getSector(),oCluster->getModule(),oCluster->getCell());
          Short_t tofHitInd=fTofClusters->getIndex(hLoc);
          // --------------------------------------------
        ...
        if (!oHit) {
          ...
          Short_t tofHitInd=fTofHits->getIndex(oHit);
          c->setMetaHitInd(showerHitInd, tofHitInd);
          c->setOuterHitId(oHit->getSector()*1000 + oHit->getModule()*100 + oHit->getCell());
        }
      }
...
void HKickTrackFB::readHits(HLocation &loc) {
  ...
  while ( (hit=(HMdcSeg *)fIter->Next()) != 0 ) {
    ...
    imp->setOriginalSegmentId(fInput->getIndex(hit));
