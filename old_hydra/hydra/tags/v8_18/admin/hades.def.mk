...
HYDRA_LIBS += -lMdc -lRichUtil -lRich -lRpc -lHodo
HYDRA_LIBS += -lTriggerUtil -lTrigger -lStart
HYDRA_LIBS += -lOra -lQA -lTools -lRFIOtsm -lHydra
...
CERNLIB_LIBS      += -lkernlib -llapack3 -lblas -lg2c -lm -lnsl -ldl

ROOTCINT       := $(ROOTSYS)/bin/rootcint
ROOTCONFIG     := $(ROOTSYS)/bin/root-config
ROOT_INC_DIRS  := $(shell $(ROOTCONFIG) --incdir)
ROOT_LIB_FLAGS := $(shell $(ROOTCONFIG) --glibs)
ROOT_LIB_DIRS  := $(subst -L,,$(filter -L%,$(ROOT_LIB_FLAGS)))
ROOT_LIBS      := $(filter-out -L%,$(ROOT_LIB_FLAGS))
ROOT_LIBS      += -lTreePlayer -lMinuit -lThread -lProof
...
# C compiler flags
CC         := gcc
SO_C_FLAGS  = -O2 -Wall -pipe -pthread -fPIC -g $(CPP_FLAGS)
APP_C_FLAGS = -O2 -Wall -pipe -pthread -g $(CPP_FLAGS)

# C++ compiler flags
CXX          := g++
SO_CXX_FLAGS  = -O2 -Wall -pipe -pthread -fPIC -g $(CPP_FLAGS)
APP_CXX_FLAGS = -O2 -Wall -pipe -pthread -g $(CPP_FLAGS)
...
F77          := g77
SO_F77_FLAGS  = -g $(FPP_FLAGS)
APP_F77_FLAGS = -g $(FPP_FLAGS)
...
LD           := g++
SO_LD_FLAGS   = -g -shared -Wl,-soname,lib$(LIB_NAME).$(SO_EXT)
