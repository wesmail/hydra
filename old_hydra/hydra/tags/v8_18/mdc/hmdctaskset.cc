...
HMdcDeDxMaker*         HMdcTaskSet::dedxmaker           =0;
...
HTask* HMdcTaskSet::make(const Char_t *select, Option_t *option) {
  ...
  {   // dubna tracker / hit and higher
      ...
      {   // trackfinder and higher
	  if(versionDeDx==1){
	  dedxmaker=new HMdcDeDxMaker("mdcdedx","mdcdedx");
	  tasks->add(dedxmaker);
	  }
	  if(versionDeDx==2){
  ...
  if(commonsetlocal->getAnalysisLevel()==4 )
  {   // trackfinder and higher
      tasks->add( new HMdcVertexFind("vertexfinder","vertexfinder",HMdcVertexFind::kSegments,kTRUE));
  }
...
HMdcDeDxMaker* HMdcTaskSet::getDeDxMaker()
{
    ...
    {
     	Error("HMdcTaskSet:getDeDxMaker():"
	      ,"\n POINTER TO MDCDEDXMAKER IS ZERO! \n YOU NEED TO RUN IN FIT MODE!");
        exit(1);
    }
}
...
void HMdcTaskSet::printStatus()
{
    ...
    if(dedxmaker)  dedxmaker  ->printStatus();
    SEPERATOR_msg("#",60);
}
