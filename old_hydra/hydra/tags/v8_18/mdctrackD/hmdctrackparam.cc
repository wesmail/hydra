...
void HMdcTrackParam::calcTimeOffsets(Int_t tofFlag) {
  ...
  if(timeOffsetFlag==1) for(Int_t m=0;m<nMods;m++) timeOffset[m]=(sWht[m]>0.) ?
      -sDev[m]/sWht[m]:0.;
...
Double_t HMdcTrackParam::getSumWtNorm(Int_t m) const {
  ...
  if(isTmoffFixed<0 || m<0 || m>=nMods) return 0.;
  ...
  return sWht[m];                                   // One Toff per module
}
...
void HMdcTrackParam::calcTimeOffsetsErr(void) {
  if(isTmoffFixed<0) return;
  if(timeOffsetFlag==1) for(Int_t m=0;m<nMods;m++)
...
void HMdcTrackParam::setFixedTimeOffset(Double_t o1,Double_t o2,
  ...
  timeOffset[3] = o4;
  isTmoffFixed  = -1;
  for(Int_t m=4;m<8;m++) timeOffset[m] = 0;
}

void HMdcTrackParam::setTimeOffsets(Double_t* tos,Int_t size) {
  if(size<0) size = 0;
  if(size>0 && size<8) for(Int_t m=0;m<size;m++) timeOffset[m] = tos[m];
  for(Int_t m=size;m<8;m++) timeOffset[m] = 0;
}
