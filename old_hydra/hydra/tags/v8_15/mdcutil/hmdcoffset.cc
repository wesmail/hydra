		...
		{  // loop over cells in layer
			...
			if(useTimeCuts)  // if Time Cuts and Start Time
			{
				...
				// we have to use Float_ts here
                                ...
                                Int_t bin =  (Int_t)(2048 - ((Float_t)(raw->getTime(1)) * (*calparraw)[s][mo][mb][t].getSlope()) + starttime - wireoff - tof);
				if(!perMBO) {
				    (*hreverse)[s][mo][mb][t][bin] ++;
				}
				else {
				    for(Int_t t1 = 0; t1 < 96; t1 ++){
					(*hreverse)[s][mo][mb][t1][bin] ++;
				    }
				}
			    }
			}
...
Int_t HMdcOffset::executeRaw()
{
	...
	while ((raw = (HMdcRaw*)iter->Next()))
	{
		...
		{
		    // we have to use Float_ts here
                    Int_t bin = (Int_t)(2048 - ((Float_t)(raw->getTime(1)) * (*calparraw)[s][mo][mb][t].getSlope()) + starttime - tof);
		    if(!perMBO) {
			(*hreverse)[s][mo][mb][t][bin] ++;
		    }
		    else {
