...
Bool_t HMdcParAsciiFileIo::init(HParSet* pPar,Int_t* set) {
  ...
  if (pFile) {
    ...
    gHades->getMsg()->error(1,HMessageMgr::DET_MDC,GetName(),"initialization of %s not possible from file!",name);
   
    return kFALSE;
  }
  ERROR_msg(HMessageMgr::DET_MDC,"no input file open");
  return kFALSE;
}
...
Int_t HMdcParAsciiFileIo::write(HParSet* pPar) {
  ...
  if (pFile) {
    ...
    gHades->getMsg()->error(1,HMessageMgr::DET_MDC,GetName(),"%s could not be written to Ascii file",name);
    return -1;
  }
  ERROR_msg(HMessageMgr::DET_MDC,"no output file open");
  return -1;
}
