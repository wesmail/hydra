// $Id: hpidsingleleptoneff.cc,v 1.10 2008-01-04 13:06:27 kempter Exp $
...
void HPidSingleLeptonEff::bookNTuple(){
    ...
    TString sVarList("pRecon:pGeant:thetaRecon:thetaGeant:phiRecon:phiGeant:ptGeant:isCut:isCP:commonTrack:isDirect:cutNb:nHadron:nLepton:angleHadronFitted:angleHadronNonFitted:angleLeptonFitted:angleLeptonNonFitted:isRing:innerChi2:rkChi2:metaMatchQality:metaMatchRK:vertX:vertY:vertZ");
...
Int_t HPidSingleLeptonEff::execute(void){
    ...
    Int_t * partIndex = new Int_t[6];
    for(int i = 0;i<6;i++) partIndex[i] = 0;
    Int_t countLep = 0;
     ...
     while(( pair1= (HPairSim  *)pitInput1->Next()) != NULL)
     {
        ...
        if(comDet>=76){
            ...
            if(pair1->getPid1()>0 || pair1->getPid2()>0)
                if(pair1->getIsCutFlag()!=0) bDirectCut=1;
        }
     }
...
// ----------- track & particle data retrieved here
       ...
       Float_t isRing=0., hadrFitted=-1., hadrNonFitted=-1., leptFitted=-1., leptNonFitted=-1., innerChi=-1., rkChi=-1., metaMatchQuality=-1., mmRK=-1., xVert=0., yVert=0., zVert=0.;
       ...
       while(( part = (HPidParticleSim*)pidPartInput->Next()) != NULL) {
           if(trackNb==part->getGeantTrackSet()->getGeantTrackID()&&
              ...
              (part->getPid()==2 || part->getPid()==3)){
                ...
                isRing = static_cast<Float_t>( pHitData->getRingCorrelation(4) );
                hadrFitted = pTrackData->getAngleWithClosestCandidate(-1, 1);
...
// ---------------------------------------------------
               ...
               Float_t aVar[]={momExp,mom1,thetaExp,pD1.Theta(),phiExp,pD1.Phi(),pD1.Pt(),0,cpFlag,comDet,bDirectCut,0,nHadCount,nLepCount,hadrFitted,hadrNonFitted,leptFitted,leptNonFitted,isRing,innerChi,rkChi,metaMatchQuality,mmRK,xVert,yVert,zVert};
		     
	       if(id==3) effEle->Fill(aVar);
	       if(id==2) effPosi->Fill(aVar);
               if(pair->getIsCutFlag()==0) isGoodPair=1;
	     }
	   }
       ...
       while(( part= ( HPidParticleSim *)pidPartInput->Next()) != NULL)
	 {
	   if(trackNb==part->getGeantTrackSet()->getGeantTrackID()&&
	      ...
	      (part->getPid()==2 || part->getPid()==3)){
           ...
           Float_t aVar[] = {part->P(),mom1,part->Theta(),pD1.Theta(),part->Phi(),pD1.Phi(),pD1.Pt(),-1,-1,part->getGeantTrackSet()->getMostCommonCorrelation(),-1,0,-1,-1,hadrFitted,hadrNonFitted,leptFitted,leptNonFitted,isRing,innerChi,rkChi,metaMatchQuality,mmRK,xVert,yVert,zVert};
...
Int_t  HPidSingleLeptonEff::checkTrackId(HPairSim *p,Int_t lepTrackNb){  
	...
	pg.getTrackNumber1()==lepTrackNb)
	 ...
	 {
	     if((pg.getParentTrackNumber1()==0 &&(p->getPid1()!=2 && p->getPid1()!=3)) ||
