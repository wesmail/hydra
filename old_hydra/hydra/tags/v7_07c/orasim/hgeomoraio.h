...
private:
  ...
  class HOraDetVers : public TNamed { // Container class for detector version 
    public:
      ...
      HOraDetVers(const char* pName=0) {
        SetName(pName);
        detectorId=-1;
        geomVersion=-1;
      }
  ...
  class HGeomOraCopyNode : public TNamed {
    public:
      HGeomNode* pNode;
      HGeomOraCopyNode(const char* name,HGeomNode* node) {
        SetName(name);
        pNode=node;
      }
      ~HGeomOraCopyNode() {}
...
private:
  ...
  Int_t createVersion(const char*,TString&,TString&);
  Int_t createHitVersion(const char* part,const char* hitSet);
  ...
  const char* calcRefTransform(HGeomNode*,HGeomTransform&);
