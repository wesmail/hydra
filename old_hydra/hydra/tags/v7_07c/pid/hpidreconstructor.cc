// @(#)$Id: hpidreconstructor.cc,v 1.11 2005-01-11 11:06:47 halo Exp $
...
Float_t       *pCL;
    ...
    while((pTrack = (HPidTrackCand *)pitInput->Next()) != NULL)
    {
      ...
      if(pTrack->getTrackData()->nPolarity[iSelectedMomAlg]==0) continue;
      //Skip all particle candidates without hit in META
      if(pTrack->getHitData()->iSystem==-1) continue;

      //Temporary - the alorithm cannot handle hits in shower
      //if(pTrack->getHitData()->iIndTOF==-1) continue;
	...
	pCand->setTrackCandIndex(pInputCat->getIndex(pTrack));
        ...
        while((pAlg = (HPidAlgorithm *)pitList->Next()) != NULL)
        {
	      ...
	      iAlgs++;
            ...
            if(bCalcCL)
            {
                ...
                pCand->setAlgorithmIdByIndex(iAlgs,
	    ...
	    for(Int_t i =0;i<pCand->getNParticles() ; i++)
	      {
		//cout << "PDF-value after computation: " << pPDF[i]  <<endl;
	      }
...
Int_t          iSys, iSect; /*ADDON*/
    ...
    iSys   = pKT->getSystem();
    */
    //cout << "Calculating relative intensities" << endl;
    ...
    if(fMom<0) return ; //the desired algorithm gave no results!

    fTheta = pHitData->fMdcTheta;
    iSys = pHitData->iSystem;
    iSect = pHitData->nSector; /*ADDON*/
            ...
            iPart++)
    {
      ...
      //if(fRelInt[iPart]>0)
      //cout << "RelInt for species " << iPart << " is " << fRelInt[iPart] << endl;
    }
}
