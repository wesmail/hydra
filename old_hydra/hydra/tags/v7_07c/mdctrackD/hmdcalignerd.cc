...
HMdcAlignerD::HMdcAlignerD(Text_t *name, Text_t *title, Bool_t isCOff, Int_t typeClFn)
...
HMdcAlignerD::HMdcAlignerD(Text_t *name, Text_t *title) : 
...
Bool_t HMdcAlignerD::init(void) {
  event=new HMdcEvntListCellsAndTimes;
...
//     nWires++;
    ...
    if(nHits==1) event->addTime(s,m,l,c,cal->getTime1());
    else if(nHits==2) event->addTime(s,m,l,c,cal->getTime2());
    else continue; // to keep 2 dr.times in EvntListCellsAndTimes is not possible yet
  }
...
//       }
	 ...
	 while(storeClusters->getNextCluster(*event)) {     //loop over clusters
	    ...
	    if(!fitter->fitCluster(300000)) {
	       storeClusters->setNewClustPar(-999, -999, -999, -999);
	       continue;
	    }
...
// 	      << " " << fitter->getFinalParam()->z2() << " " << fitter->getChi2() << endl;
	    ...
	    storeClusters->setNewClustPar(xFirstMod, yFirstMod, xLastMod, yLastMod);
...
Double_t clusterFunctional(TObject * obj,Int_t& nPar, Double_t* par) {
   ...
   HMdcEvntListCellsAndTimes * event = aligner->getEvent();
...
//    }
   ...
   Double_t nClus = 0;
   Double_t nCells = 0;   
   HMdcEvntListCellsAndTimes  cluster;
