...
// Modified: Apr 07, 2004 by Peter Zumbruch <P.Zumbruch@gsi.de>
...
TString HMdcUnpacker::myexec(char * in)
{
...
Bool_t HMdcUnpacker::init() 
{
  ...
  if (excludeBlackList)
  {
	  ...
	  for (int arrayIndex=0; arrayIndex<excludeBlackListSize; arrayIndex++)
	  {
      ...
      while( data < end ) 
	{
	  ...
	  for(UInt_t* deb=data+rocEvtHeaderSize; deb<enddata; deb++)
		...
		{ 
		  ...
		  for (Int_t arrayIndex=0; arrayIndex< excludeBlackListSize; arrayIndex++)
		  {
		      if ((*deb & excludeBlackListMask[arrayIndex])==excludeBlackListAdress[arrayIndex])
		      {
			  excludedBlackListCounters[arrayIndex]++;
			  if (unpackerDebug && !noComment) WarningExcludeDataWords(subEvtId, *deb,
									   excludeBlackListMask[arrayIndex],
									   excludeBlackListAdress[arrayIndex]);
                          skipDataWord=kTRUE;
			  break;
...
// --------------------------------------------------------------------------------
void HMdcUnpacker::WarningExcludeDataWords(Int_t subEvtId, UInt_t data, UInt_t mask, UInt_t adress)
{
