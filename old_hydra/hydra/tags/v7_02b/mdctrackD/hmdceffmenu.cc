...
HMdcEffMenu::HMdcEffMenu(HMdcEfficiency *feff) : 
        TDialogCanvas("EffMdc","MDC Efficiency",300,500) {
  mdcEff=feff;
...
void HMdcEffMenu::menu(){
  ...
  fileName+=" MdcEfficiency";
  SetTitle(fileName.Data());
  for(Int_t s=0; s<6; s++){
    indSec[s]=0;
    for(Int_t m=0; m<4; m++){
      indMod[s][m]=0;
  ...
  text->DrawText(0.03,oY+dY*14+0.01,"Hist.grouping:");
  ...
  bHist[0]=new TButton("Pattern","effMenu->selectingHist(0)",oX+dX*2,oY+dY*13,oX+dX*2+wX,oY+dY*13+hY);
  ...
  bHist[3]=new TButton("Single","effMenu->selectingHist(3)",oX+dX*2,oY+dY*12,oX+dX*2+wX,oY+dY*12+hY);
  drawButton(bHist[3],42,indHist[3]*5+1);
  bHist[4]=new TButton("Probability","effMenu->selectingHist(4)",oX+dX*3,oY+dY*12,oX+dX*3+wX,oY+dY*12+hY);
  ...
  button[17]=new TButton("Refresh","effMenu->calcEff()",0.80,oY+dY*10.5,0.99,oY+dY*10.5+hY);
  ...
  drawButton(bPlot[2],42,indPlot[2]*5+1);
  ...
  drawButton(button[16]);
   ...
   button[20]=new TButton("PDF","effMenu->saveHist()",0.80,oY+dY*2,0.99,oY+dY*2+hY);
   drawButton(button[20]);

//  button[18]=new TButton("Quit","HMdcEffMenu::deleteMenu(); delete gHades;",0.75,oY+dY*1,0.99,oY+dY*1+hY);
  button[18]=new TButton("Quit"," delete gHades; HMdcEffMenu::closeDial();",0.75,oY+dY*1,0.99,oY+dY*1+hY);
...
void HMdcEffMenu::drawButton(TButton* bt, Int_t nFnt, Int_t nTCl){
  bt->SetTextFont(nFnt);
...
void HMdcEffMenu::doEvents(Int_t nEv) {
  ...
  if(evN>0 && evN>=nEvDo) {
    ...
    while(kTRUE) {
      ...
      if(evId==0 || evId==1) break;   //!!!!!! Event Id !!!!!!!!!!!!!
      Int_t evN=gHades->eventLoop(1);
      if(evN<=0) break;
      nTotEv+=evN;
      isSkip=kTRUE;
    }
    if(isSkip) mdcEff->calcEfficiency(calcEffFlag);
  }
