...
protected:
  ...
  Int_t        segNumber;       // segment sequential number in track
  Char_t       nHitsMdc[2];     // num. of hits in each mdc (layers only!)
...
public:
  ...
  void    analyseClust(HMdcClusSim* pClus,
                Int_t trInd,Int_t modICl);
  ...
  Bool_t  getLayerHitPosSec(Int_t m,Int_t l,HGeomVector& hit,
                            Bool_t extrFlag=kTRUE);
  Bool_t  getLayerHitPosLab(Int_t m,Int_t l, HGeomVector& hit,
                            Bool_t extrFlag=kTRUE);
  ...
  Bool_t  isSegNotRec(void)             {return !testBit(16);}
  Bool_t  is1or2HitsNotRec(void)        {return !testBit(8);}
  void    clearSegClus(void)            {pClusBest=pClusBestCh[0]=pClusBestCh[1]=0;}

protected:
  void    addFirstHit(HGeantMdc* pGeantMdc,Bool_t* mdcSecSetup,Int_t ns);
  ...
  Bool_t  testBit(UChar_t bit)          {return (segmentStatus&bit)==bit;}
  Bool_t  isSegClusBetter(HMdcClusSim* pClus,Int_t trInd);
  Bool_t  isModClusBetter(HMdcClusSim* pClus,Int_t trInd,Int_t mInd);
  Int_t   setModClustPos(HMdcClusSim* pMdcClusSim,
                        Int_t indtr,Int_t mInd);
...
//  void   setMdcEvntListCells(HMdcEvntListCells* pt=0);
  ...
  Int_t         testGeantTrack(HGeantKine* pGK);
  Int_t         testGeantTrack(Int_t trNum);
  ...
  Char_t        getNGeantMdcSectors(void) const {return (mdcSector<0) ?
                                                        -mdcSector-1:1;}
  Char_t        getNDigiMdcSectors(void) const  {return (mdcWSector<0) ?
                                                         1-mdcWSector-1:1;}
  ...
  Bool_t        getLayerHitPos(Int_t ns, Int_t m, Int_t l, HGeomVector& hit,
                               Bool_t extrFlag=kTRUE);
  Bool_t        getLayerHitPosSec(Int_t ns, Int_t m, Int_t l, HGeomVector& hit,
                                  Bool_t extrFlag=kTRUE);
  Bool_t        getLayerHitPosLab(Int_t ns, Int_t m, Int_t l, HGeomVector& hit,
                            Bool_t extrFlag=kTRUE);
  ...
  static void   printDebug(HGeantMdc* pGeantMdc,Int_t i,Char_t* st=0);

private:
  Bool_t        nSegOk(Int_t ns)                {return ns>=0 && ns<nSegments;}
  Bool_t        nSecOk(Int_t ns)                {return ns>=0 && ns<6;}
  Bool_t        nMdcOk(Int_t nm)                {return nm>=0 && nm<4;}
  void          testHitsInDetectors(void);
  Int_t         testMdcHits(void);
  Bool_t        addSegAtAndExpand(Int_t segNum);
  Bool_t        addSegment(void);
  Int_t         collectWires(Char_t sec, Char_t mod, Char_t lay, Float_t atof);
  Bool_t        isGeantBug1(Float_t momLay);
  Bool_t        isGeantBug2(Char_t sec,Char_t dMod,Char_t dLay,Char_t hitDir,
                            Float_t dTof);
  Bool_t        isGeantBug3(void);
  Bool_t        mayBeGeantBug(Float_t dMom);
  void          printDebug(Float_t dMom,Char_t dMod);
  Char_t        dirDLayer(Char_t dir)           {return dir == 0 ? 0 :
                                                               (dir<0) ? -1:+1;}
  void          calcNSectors(void);
  void          setStatusFlags(void);
  static Int_t  calcLay14(Int_t m, Int_t l);
  Bool_t        testBit(UChar_t bit)            {return (trackStatus&bit)==bit;}
...
public:
  ...
  void    fillClusModTrackInf(HMdcClusSim* pClusSim,HMdcList12GroupCells* wrLst,
                           Int_t modi);
  void    fillSegTrackInf(HMdcSegSim* pSegSim,HMdcList24GroupCells* wrLst);
  ...
  void    testIOMatching(Int_t ind,HMdcClusSim* pClusSim,Int_t bin,
...
public:
  ...
  static HMdcGeantEvent*  getObject(void);
  ...
  Bool_t isMdcActive(Int_t s, Int_t m) const {return
                                              HMdcGeantTrack::isMdcActive(s,m);}
  void   setDebugPrintFlag(Bool_t fg=kTRUE)  {debugPrintFlag=fg;}
  ...
  Bool_t isGeantBug(void)                    {return geantBugFlag;}
  void   print(void);
