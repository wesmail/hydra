...
Bool_t  HMdcEvtDisplay::init() {
  ...
  mdcGeomPar=(HMdcGeomPar*)(rtdb->getContainer("MdcGeomPar"));
  mdcLayGeomPar=
    (HMdcLayerGeomPar*)(rtdb->getContainer("MdcLayerGeomPar"));
  if (mdcGeomPar && mdcLayGeomPar) shapes=mdcGeomPar->getShapes();
  else return kFALSE;
    ...
    if (refMod) {
      ...
      for(Int_t j=0;j<maxLay;j++) {
        ...
        if (lRef)  copyWires(nm,nWir,nSec,i,j);
          ...
          p3[1]=0.-pitch/2.;
          p5[1]=pitch/2.;
          p3[2]=layer->getPoint(0)->getZ();
          p5[2]=layer->getPoint(4)->getZ();
