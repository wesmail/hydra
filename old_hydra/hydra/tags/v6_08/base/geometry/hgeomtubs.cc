...
// HGeomTubs
//
// class for the ROOT shape TTUBS which is identical with the
// GEANT shape TUBS
// 
...
void HGeomTubs::calcVoluParam(HGeomVolume* volu) {
  // calculates the parameters needed to create the ROOT shape TTUBS 
  Double_t fac=10.;
...
TShape* HGeomTubs::createRootVolume(HGeomVolume* volu) {
  // creates a volume with ROOT shape TTUBS
  calcVoluParam(volu);
  Text_t* name=(char*)volu->GetName();
  TArrayD& p=*param;
  TShape* s=new TTUBS(name,name,"",(Float_t)(p[0]),(Float_t)(p[1]),
