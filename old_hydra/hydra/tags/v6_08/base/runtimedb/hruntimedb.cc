  ...
  if (getOutput() && output->check() && output->isAutoWritable()) {
    ...
    if (isRootFileOutput) {
      ...
      if (cont->hasChanged()) {
        ...
        if (cv>0) gHades->getMsg()->info(10,DET_ALL,"%s written to output",c);
...
Bool_t HRuntimeDb::initContainers(void) {
  ...
  if (currentFileName.IsNull())
    ...
    gHades->getMsg()->info(10,DET_ALL,this->GetName(),"     run id %d",currentRun->GetName());
...
void HRuntimeDb::print() {
  ...
  SEPERATOR_msg("-",40);
  gHades->getMsg()->info(10,DET_RUNTIMEDB,this->GetName(),"%-25s%10s%10s%10s","container","1st-input","2nd-input","output");
  ...
  //cout<<"---------------------  input/output  ----------------------"<<'\n';
  SEPERATOR_msg("-",80);
  INFO_msg(10,DET_RUNTIMEDB,"input/outpu");
  SEPERATOR_msg("-",40);
  //cout<<"first Input:"<<'\n';
  if (firstInput) 
      {
