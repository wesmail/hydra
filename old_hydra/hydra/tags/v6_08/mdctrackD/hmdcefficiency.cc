...
Bool_t HMdcEfficiency::reinit(void) {
  ...
  for (Short_t sec = 0; sec < 6; sec++) {
    ...
    for(Int_t i=0;i<3;i++) fsec->target[i]=
      (fLUTbSec.getTargetF()(i)+fLUTbSec.getTargetL()(i))*0.5;
  }
  isCreated=kTRUE;
  return kTRUE;
}
 	...
 	for(Short_t layer=0; layer<6; layer++) {
 	  ...
 	  //----- N. fired cells in plane = 0
            ...
            fSCLayer.calcIntersection(
                fsec.target[0],fsec.target[1],fsec.target[2],
                clst.getX(),clst.getY(),clst.getZ(), xcoor,ycoor,zcoor);
