...
Int_t HKickDetDigi::digiTof(void) {
  ...
  Float_t x,y,en,tof,tofResol=0.6; //100 ps
  HLocation loc;
  HGeantKine *geKine=0;
  Int_t trackN,pid;
...
Int_t HKickDetDigi::digiMdc2(void) {
  ...
  const Float_t resolX[4]={0.16,0.16,.16,.16};//{0.9,0.9,1.4,1.4};
  const Float_t resolY[4]={0.08,0.08,.08,.08}; //{0.457,0.457,0.724,0.724};
  ...
  while ( (geMdc=(HGeantMdc *)fMdcIter->Next()) != 0) {
	...
	if (!isMdcPerfect()) {
	  x+=gRandom->Gaus(0,resolX[geMdc->getModule()]);
	  y+=gRandom->Gaus(0,resolY[geMdc->getModule()]);
	  x2+=gRandom->Gaus(0,resolX[geMdc2->getModule()]);
	  y2+=gRandom->Gaus(0,resolY[geMdc2->getModule()]);
	}
...
void HKickDetDigi::fillHit(HGeantMdc *geMdc, Float_t x, Float_t y, Float_t p) {
  ...
  Float_t theta,phi;
  Float_t resTheta = .02 ; // Res theta = 1 degree
  Float_t resPhi = .04 ; // Res phi = 2 degrees
...
Bool_t HKickDetDigi::init(void) {
  ...
  if (gHades) {
	...
	if (fFillMdc) {
	  ...
	  if (!fMdcHitOut) {
	    Int_t sizes[]={6,4,100};
	    fMdcHitOut=new HMatrixCategory("HMdcSimHit",3,sizes,0.5);
