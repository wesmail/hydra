...
HHypReconstructor::HHypReconstructor(Text_t * name, Text_t * title, TString opt)
...
Bool_t HHypReconstructor::finalize()
{
  ...
  if (skip_event_enable) {
    cout << "The Micro-HYP-DST has been filled with " <<  num_accepted_events << " events" << endl;
  }
...
Bool_t HHypReconstructor::orderBeforeExecute()
{
  ...
  for (i = 0; i < (numOfAlgorithms); i++)       //Loop 1 on the inputs
  {
    ...
    if (input_counter == 0)     //no input
    {
      ...
      if (has_build_cat) {
        std::
          cout << "HHypReconstructor::orderBeforeExecute(): Hyp list from tree required, but no category found...(maybe you misspelled the input channel name?)" <<
          std::endl;
        exit(1);
      }
      ...
      std::cout << "HHypReconstructor::orderBeforeExecute(): More than one input list... (Maybe two algos feeding same output channel?)" << std::endl;
      exit(1);
    }
  }
...
Int_t HHypReconstructor::execute()
{
  ...
  if (kReturnValue != kSkipEvent) num_accepted_events++;
  //cout << "exit" << endl;
  return kReturnValue;
