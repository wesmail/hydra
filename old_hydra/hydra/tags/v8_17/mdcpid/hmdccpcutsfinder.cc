...
void HMdcCPCutsFinder::findPidCuts(Char_t *outputname){
  ...
  for (Int_t i=0;i<sing->GetEntries();i++){
    ...
    if(calculateMdcThetaBin(values[0])>=0&&calculateMdcPhiBin(values[1])>=0) {
      ...
      //&&values[12]==0 means that is is Rich single -Mdc single and ==3 means Rich Double_t -Mdc single
 ...
 printf("filling doubles to  histograms\n");
  for (Int_t i=0;i<doub->GetEntries();i++){
    ...
    if(calculateMdcThetaBin(values[0])>=0&&calculateMdcPhiBin(values[1])>=0){
      //&&values[12]==2 means that is is Rich Double_t -Mdc Double_t and ==1 means Rich single -Mdc Double_t       //mdc module =0
...
void HMdcCPCutsFinder::findPidCutsFromMdc(Char_t *outputname){
  Char_t name_s[200];
  Char_t name_d[200];
  Int_t module=-1;
