...
Double_t HMdcSizesCellsLayer::calcWire(Double_t x, Double_t y) const{
  // return point on the layer plane in wire Float_t number scale!
...
Bool_t HMdcSizesCells::fillModCont(Int_t sec, Int_t mod, 
  ...
  for(Int_t layer=0; layer<6; layer++) {
    ...
    Float_t shift;
    Int_t   wire;
    if(fLayerCorrPar->getLayerCorrPar(sec,mod,layer,wire,shift))
          fSCellsMod[layer].addSecondPart(wire,shift);
  }
  return kTRUE;
}
...
Bool_t HMdcSizesCellsLayer::calcSecondPart(void) {
  Int_t nPoint=pGeomVol->getNumPoints();
  if(nPoint != 8) {
    Error("calcSecondPart","NumPoints for layer volume not eg. 8.");
    return kFALSE;
  }  
  Double_t x[4],y[4];
  for(nPoint=0; nPoint<4;nPoint++) {
  ...
  for(Int_t cell=fCellSecondPart; cell<nCells; cell++) {
//Old:    Double_t b=(cell*pitch-wireOffSet)/cosWireOr;
    Double_t b = -calcWireY(cell)/cosWireOr;
    HMdcSizesCellsCell& fSizesCellsCell=(*this)[cell];
    HGeomVector& lineP1=fSizesCellsCell.wirePnt1;
    HGeomVector& lineP2=fSizesCellsCell.wirePnt2;
    ...
    if(nLine1<0 && nLine2 <0) {
      if(cell<100) {
        Warning("calcSecondPart","S%i M%i L%i: Cell %i is out of MDC.",
        sector+1,module+1,layer+1,cell+1);
        continue;
      } else {
        Warning("calcSecondPart","S%i M%i L%i: Cells %i-%i are out of MDC.",
        sector+1,module+1,layer+1,cell+1,nCells);
        break;
      }
    }
...
void HMdcSizesCellsLayer::addSecondPart(Int_t c,Float_t shift) {
  //call it after init only !!!!!!!!!!!!!
  if(c<=0 || c>=nCells || nCells<0) return;
  fCellSecondPart = c;
...
Bool_t HMdcSizesCells::getCellVol(Int_t sec,Int_t mod,Int_t lay,Int_t cell,
  ...
  for(Int_t i=0;i<4;i++) {
    ...
    if(dif.length()<0.01) {
      ...
      break;               // not more then one Double_t point
    }
  }
