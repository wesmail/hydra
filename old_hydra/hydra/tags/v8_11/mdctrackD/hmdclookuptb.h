...
inline void HMdcLookUpTbSec::initCluster(Int_t nBin) {
  clus->clear();
  stack->init(nBin);
  Int_t ny=nBin/nBinX;
  nLMinCl=nLMaxCl=ny;
...
inline void HMdcLookUpTbSec::initClusterT2(Int_t nBin) {
  initCluster(nBin);
...
inline void HMdcLookUpTbSec::addBinInCluster(Int_t nBin,Char_t wt) {
  Int_t nx=nBin%nBinX;
  Int_t ny=nBin/nBinX;
