#define projectPidHisto_cxx
#include <TStyle.h>
#include <TCanvas.h>

#include "projectPidHisto.h"

#ifdef projectPidHisto_cxx
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      tree = (TTree*)gDirectory->Get("pTrackCandidate");
...
void projectPidHisto::Init(TTree *tree, TFile *outFile)
{
   ...
   // output file
   //outFile = new TFile("outHistFile.root","recreate");
   outFile->cd();
   ...
   for(Int_t iAlg = 0; iAlg < MAX_ALG; iAlg++) // tracking algorithms
   {
     for(Int_t iSec = 0; iSec < 6; iSec++) // sectors
     {
       ...
       pShowSumDiffVsMom[iAlg][iSec] = new TH2F(name,name,200,0,2000,1000,0,300);
     }
   } 
}
...
#endif // #ifdef projectPidHisto_cxx


void projectPidHisto::Loop(Int_t events)
{
