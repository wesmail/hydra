...
void TGo4MdcOnline::DrawColors(Int_t size,Int_t symbol,HMonHistAddon* His)
{
    ...
    {
	HHistArray*  h=(HHistArray*)His;
	if(colorsSec[0]==0&&size==6&&symbol==0)for(int i=0;i<h->getSize2();i++){colorsSec[i]=h->getP(0,i)->GetLineColor();}
	if(colorsMod[0]==0&&size==4&&symbol==1)for(int i=0;i<h->getSize2();i++){colorsMod[i]=h->getP(0,i)->GetFillColor();}
	if(colorsSec[0]==0&&size==6&&symbol==0)for(int i=0;i<h->getSize2();i++){colorsSec[i]=h->getP(0,i)->GetLineColor();}
	if(colorsMod[0]==0&&size==4&&symbol==1)for(int i=0;i<h->getSize2();i++){colorsMod[i]=h->getP(0,i)->GetFillColor();}
    }
