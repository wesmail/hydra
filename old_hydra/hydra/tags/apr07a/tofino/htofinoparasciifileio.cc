...
Bool_t HTofinoParAsciiFileIo::init(HParSet* pPar,Int_t* set) {
  ...
  if (pFile) {
    ...
    if (!strncmp(name,"TofinoDigitPar",strlen("TofinoDigitPar"))) {
...
Int_t HTofinoParAsciiFileIo::write(HParSet* pPar) {
  ...
  if (pFile) {
    ...
    if (!strcmp(name,"TofinoGeomPar")) return HDetParAsciiFileIo::writeFile((HDetGeomPar*)pPar);
    if (!strncmp(name,"TofinoDigitPar",strlen("TofinoDigitPar"))) return HDetParAsciiFileIo::writeCond((HParCond*)pPar);
    return -1;
  }
