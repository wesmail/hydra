...
//
// Features:
// 1. DeltaTOF works only with HSplineTrack. to retrieve PathLegth
//  --> Now should work with RK: NOT TESTED.
// 2. TimeOfFlight from HSpline has already been recalculated  due to the 
...
// 3. It will only work with less than 10 particles in an event. Anyhow this
//    is harcoded somewhere else, it should be changed top something more
//    flexible.
// 4. We should pass DEBUG flag as a execution parameter. 
//    -> On "init" or "constructor" --  debugFlag=0 default.
// 5. Many variables can be declared earlier and the reused. to save time (?).
// 6. Can we have to differnt TrackingAlgos in the same file for 2 different 
...
//    Tiago Perez.
//
//  Added new method getTrackInfo(). This method handels all that complicated
...
HHypDeltaTofAlg::HHypDeltaTofAlg(char *name_i, Option_t par[])
...
Bool_t HHypDeltaTofAlg::execute()
{
  ...
  while (mylist->CombIterator()) {
    ...
    for (i = 0; i < (mylist->getNpart() - 1); i++) {
      ...
      hyppid1 = mylist->getPid(i);
      particle1 = (HPidParticle *) mylist->getPidParticle(i);

      hyppid2 = mylist->getPid(i + 1);
      particle2 = (HPidParticle *) mylist->getPidParticle(i + 1);

      // making DeltaTof here
      //get mom algo
      momIndex1 = particle1->getMomAlg();
      momIndex2 = particle2->getMomAlg();

      getTrackInfo(momIndex1, momIndex2);

      // Claculate Theoretical Values Here.
      //part 1
      mass1 = HPhysicsConstants::mass(hyppid1);
      partHyp1 = (TLorentzVector) (*particle1);
      partHyp1mom = partHyp1.Vect();
      ...
      //part 2
      mass2 = HPhysicsConstants::mass(hyppid2);
      partHyp2 = (TLorentzVector) (*particle2);
      partHyp2mom = partHyp2.Vect();
...
Bool_t HHypDeltaTofAlg::getTrackInfo(Int_t momIndex1, Int_t momIndex2)
{
  ...
  if (momIndex1 == ALG_SPLINE) {
    ...
    // set TRacking values to nonsense.
    tof1_exp = -1;              // Exp TOF
