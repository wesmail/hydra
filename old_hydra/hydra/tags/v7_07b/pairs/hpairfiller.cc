// $Id: hpairfiller.cc,v 1.9 2005-06-23 17:17:05 jacek Exp $
// Author: Thomas.Eberl@ph.tum.de, last modified : 2005-05-12 16:52:55
//
// ---------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
//                                                                           
...
// ---------------------------------------------------------------------------
HPairFiller::HPairFiller(Text_t *name,Text_t *title) :
...
Bool_t HPairFiller::init() 
{
        if (gHades) 
	{
		...
		{//build category
		    ...
		    if ((pCatPair=HPairFL::buildLinearCategory(catPairs,pCatName)))
...
// 	if ( cNTupleFileName.Length()!=0 && !createNTuple() ) return kFALSE;
// 	if ( cTTreeFileName.Length()!=0  && !createTTree() ) return kFALSE;
	return kTRUE;
}
...
Bool_t HPairFiller::fillPairCategory(TObjArray* pPairs)
{
	...
	for (Int_t iPairIdx=0; iPairIdx<pPairs->GetLast()+1; iPairIdx++)
	{
	    ...
	    //cout<<"number of pairs in TObjArray "<<pPairs->GetLast()+1<<endl;
	    if (kIsReal) 
	    {
...
HPairSim *pPairSim = NULL;
	...
	{
// 	    HPidParticle* p1 = (HPidParticle*)pCatPart->getObject(iParticle1);
// 	    HPidParticle* p2 = (HPidParticle*)pCatPart->getObject(iParticle2);
	    if (t)
	    {
		if (kIsReal) 
		{
		  pPair = new HPair((HPidParticle*)pCatPart->getObject(iParticle1),(HPidParticle*)pCatPart->getObject(iParticle2));
		  t->Add(pPair);
	    }
		else 
		{
		  pPairSim = new HPairSim((HPidParticleSim*)pCatPart->getObject(iParticle1),
		                       (HPidParticleSim*)pCatPart->getObject(iParticle2));
		  t->Add(pPairSim);
		}
		}
	    else 
...
void HPairFiller::selectPairs(TObjArray* pPairs)
{
    ...
    for (Int_t i=0;i<pPairs->GetLast()+1;i++) 
    {
        ...
        nRing1 = pPart1->getPidCandidate()->getTrackCandidate()->getHitData()->hasRingCorrelation[nMomInd];
        nRing2 = pPart2->getPidCandidate()->getTrackCandidate()->getHitData()->hasRingCorrelation[nMomInd];
        //Do not make pairs with lepton without ring.
        if(((nPid1==2||nPid1==3)&&!nRing1) || ((nPid2==2||nPid2==3)&&!nRing2)){
            p -> setIsCutFlag(1);
        }
        #warning FIX_ME
        //if at leats one partcile is lepton pair is OK
