// $Id: hrichcut.cc,v 1.13 2009-07-15 11:39:21 halo Exp $
// Last update by Thomas Eberl: 03/06/21 17:17:09
//
using namespace std;
#include "hrichcut.h"
...
Bool_t HRichCut::isRMPhiDiff(HHitMatch* track,Float_t limit)
{
    Bool_t k = kFALSE;
    Float_t richphi = track->getRichPhi();
    Float_t mdcphi  = track->getMdcPhi();
    if ((TMath::Abs(richphi-mdcphi))<limit) 
    {
	//cout<<TMath::Abs(richtheta-mdctheta)<<"   "<<limit<<endl;
	k=kTRUE;
    }
...
Bool_t HRichCut::isGEANTPair(HDiHitMatchSim* pair, HLinearCategory* cat, const Char_t* opt)
{
...
Bool_t HRichCut::isGoodRing(HRichHitSim *r)
{
    ...
    // avCharge>4 padNr>4 pmQual>200  localMax>3  centroid<2.5
    // gives 93% good and 23% fakes in simulation
    // 70% of those corr to a kicktrack survive !
     Bool_t kTEST = kFALSE;
