...
void HMdcCalibrater1::calcTimes(Float_t* testTime1,Float_t* testTime2,Float_t starttime,Int_t nHits,HMdcCalParTdc* tdc)
{
  ...
  //
    Float_t offset = tdc->getOffset();
    Float_t slope  = tdc->getSlope();
    ...
    if(NoStartandCal)
    {
	*testTime1 = (offset - ((t1 + rnd1) * slope)) - globalOffset[loc[1]];
	if (abs(nHits) >= 2) {
	    *testTime2 = (offset - ((t2 + rnd2) * slope)) - globalOffset[loc[1]];
	}
    }
    if(StartandCal)
    {
	*testTime1 = (offset - ((t1 + rnd1) * slope) + starttime) - globalOffset[loc[1]];
	if (abs(nHits) >= 2) {
	    *testTime2 = (offset - ((t2 + rnd2) * slope) + starttime) - globalOffset[loc[1]];
	}
    }
    if(NoStartandNoCal)
    {
	...
	break;
	default :
	    *testTime1 = (2048 - (t1 + rnd1)) * globalSlope - globalOffset[loc[1]];
	    break;
	}
	if (abs(nHits) >= 2){
	    switch(t2){
	    ...
	    break;
	    default:   *testTime2 = (2048 - (t2 + rnd2)) * globalSlope - globalOffset[loc[1]] ;
	    break;
	    }
	}
    }
