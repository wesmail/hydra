...
HRtMatchingPar::HRtMatchingPar(const Char_t* name, const Char_t *title,
...
void HRtMatchingPar::printTo(ostream &out) {
  ...
  out << "Cells: ";
  for (Int_t i=0;i<fTotalBins;i++) { out << int(fCells[i]); }
  out << endl;
  out << "Binning: \n";
...
Char_t &HRtMatchingPar::bin(Int_t idx[]) {
  Int_t address = idx[0];
  Int_t *bins = fBins.GetArray();

  if (idx[0]<fBins[0]) {
    for (Int_t i=1;i<fBins.GetSize();i++) {
      if (!(idx[i]<bins[i])) {
	address = fTotalBins;
	break;
      }
...
Char_t &HRtMatchingPar::bin(Float_t a[]) {
  Int_t *idx = fIdx.GetArray();

  for (Int_t i=0;i<fMin.GetSize();i++) {
...
void HRtMatchingPar::setRange(Int_t i,Float_t min, Float_t max) {
    fMin[i] = min;
    fMax[i] = max;
