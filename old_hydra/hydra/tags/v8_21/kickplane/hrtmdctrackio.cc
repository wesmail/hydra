...
Int_t HRtMdcTrackIo::readOuterHits (TClonesArray &segments) {
  ...
  if (fOutHitIter) {
	...
	//for (Int_t ii=0;ii<4;ii++) {
	  //for (Int_t jj=0;jj<5;jj++) derivatives[ii*5+jj]*=0.00001; //printf("%11.4g ",derivatives[ii*5+jj]);
	  //printf("\n");
	//}
...
HRtMdcTrackIo::init (HEvent * ev, HRuntimeDb * rtdb)
{
  ...
  if (!fCatTrk)
    {
      Int_t sizes[2] = { 6, 100 };
      fCatTrk = new HMatrixCategory (const_cast<Char_t *>(fOutputClassName.Data()), 2, sizes, .3);
      if (fCatTrk)
