...
public:
  inline HRtMatrix(Int_t rows,Int_t cols);
  inline HRtMatrix(const HRtMatrix &m);
  inline ~HRtMatrix(void);
  ...
  inline Double_t convolute(HRtVector &v);
  inline void identity(void);
  inline void print(const Text_t name[]="");
};
...
class HRtVector {
 public:
  ...
  inline HRtVector &operator=(HRtVector &v);
  inline void operator-=(HRtVector &v);
  inline void operator+=(HRtVector &v);
  inline void operator/=(Double_t f);
  Double_t &operator()(Int_t i) { return fData[i]; }
  const Double_t at(Int_t i) const { return fData[i]; }
  Int_t getSize(void) const { return fSize; }
  inline void multiply(HRtMatrix &a,HRtVector &v);
  void zero(void) { for(Int_t i=0;i<fSize;i++) fData[i]=0.; }
  inline void resizeTo(Int_t n); 
  void print(void) { 
    for(Int_t i=0;i<fSize;i++) std::cout << fData[i] << "\t"; 
    std::cout << std::endl; 
  }
 private:
  Double_t *fData;
...
inline void HRtVector::operator-=(HRtVector &v) {
  for (Int_t i=0;i<fSize;i++) fData[i]-=v(i);
}
...
inline void HRtVector::operator/=(Double_t f) {
  for (Int_t i=0;i<fSize;i++) fData[i]/=f;
}

inline HRtVector &HRtVector::operator=(HRtVector &v) {
  HRtVector &r = *this;
  r.resizeTo(v.getSize());
  for (Int_t i=0;i<getSize();i++) fData[i]=v(i);
  return v;
}
...
inline void HRtVector::multiply(HRtMatrix &a,HRtVector &v) {
  HRtVector &r = *this;
  for (Int_t i=0;i<a.getRows();i++) {
    r(i) = 0.;
    for (Int_t j=0;j<a.getCols();j++) r(i) += a(i,j) * v(j);
  }
}
...
inline void HRtMatrix::print(const Text_t name[]) {
  HRtMatrix &a=*this;
