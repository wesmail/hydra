...
HMdcDeDx::HMdcDeDx(const Char_t* name,const Char_t* title,
...
void HMdcDeDx::putParams(HParamList* l)
{
    ...
    l->add("shift"      ,&shift[0][0][0][0],size);
    l->add("shiftsigma" ,&shiftsigma[0][0][0][0],size);
    l->add("ref_dedx"   ,ref_dedx);
    l->add("shift_dedx" ,&shift_dedx[0][0],24);
    l->add("scale_dedx" ,&scale_dedx[0][0],24);
    l->add("hefr"       ,hefr);
...
Bool_t HMdcDeDx::getParams(HParamList* l)
{
    if (!l) return kFALSE;
    Int_t size=6*4*18*100;
    if (!( l->fill("shift"      ,&shift[0][0][0][0],size      ))) return kFALSE;
    if (!( l->fill("shiftsigma" ,&shiftsigma[0][0][0][0],size ))) return kFALSE;
    if (!( l->fill("ref_dedx"   ,&ref_dedx                    ))) return kFALSE;
    if (!( l->fill("shift_dedx" ,&shift_dedx[0][0],24         ))) return kFALSE;
    if (!( l->fill("scale_dedx" ,&scale_dedx[0][0],24         ))) return kFALSE;
