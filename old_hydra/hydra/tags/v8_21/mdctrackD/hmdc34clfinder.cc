...
void HMdcProjPlot::setEdges(Double_t iyMin, Double_t ixMinD, Double_t ixMaxD,
  ...
  xFirstBin = 0.5*stX+xMin;
  yFirstBin = 0.5*stY+yMin;
...
TH2C* HMdcProjPlot::getPlot(const Char_t* name,const Char_t* title){
...
TH2C* HMdcProjPlot::getPlotV2(const Char_t* name,const Char_t* title){
...
Int_t HMdc34ClFinderSec::findClustersSeg2(HMdcSeg* fSeg, HMdcClus* pClus,
    Int_t *mBin){
...
Int_t HMdc34ClFinderSec::findClustersSeg2(HMdcClus* pClus, Int_t *mBin){
  ...
  if(mBin!=0) setMinBin(mBin);
  if(notEnoughWrs) return 0;
  Double_t dirX    = pClus->getX();
  Double_t dirY    = pClus->getY();
  Double_t dirZ    = pClus->getZ();
  Double_t errDirX = pClus->getErrX();
  Double_t errDirY = pClus->getErrY();
  // Double_t errDirZ=pClus->getZ();
  Double_t x0      = pClus->getXTarg();
  Double_t y0      = pClus->getYTarg();
  Double_t z0      = pClus->getZTarg();
  Double_t errX0   = pClus->getErrXTarg();
  Double_t errY0   = pClus->getErrYTarg();
  Double_t errZ0   = pClus->getErrZTarg();
  // segOnkick[4]:
  //    [0]---[1]  ^ y
  //     |     |   |
  //     |     |   |
  ...
  dirSeg1[0].setXYZ(dirX+errDirX,dirY+errDirY,
                    pClus->getZOnPrPlane(dirX+errDirX,dirY+errDirY));
  seg1[1].setXYZ(x0+errX0,y0-errY0,z0-errZ0);
...
Int_t module = mod;
  cPlMod=(module==2) ? prPlotSeg2->plModF:prPlotSeg2->plModS;
  if(cPlMod==0) return;
  cFMod=&((*this)[module]);
  cXMinM = xCMin[module];
  cXMaxM = xCMax[module];
  ...
  UChar_t* weights   = prPlotSeg2->weights;
...
TH2C* HMdc34ClFinderSec::getPlot(const Char_t* name,const Char_t* title,Int_t ver) {
  for(Int_t mod=2; mod<4; mod++) {
    cPlMod=(mod==2) ? prPlotSeg2->plModF:prPlotSeg2->plModS;
    if(cPlMod==0) continue;
    cFMod = &((*this)[mod]);
    cXMinM = xCMin[mod];
    cXMaxM = xCMax[mod];
    for(Int_t lay=0; lay<6; lay++) if(calcLayerProjVar(lay)) makeLayProjV1(lay);
  }
  TH2C* plt;
...
//???optimizaciya???        if(dY>100.) break;
        ...
        if(Abs(cls1.getX()-cls2.getX()) > 100.) continue;    // 100. mm !???
...
void HMdc34ClFinderSec::initCluster(Int_t nBin,UChar_t amp) {
  ...
  cluster->init(1,prPlotSeg2->xFirstBin, prPlotSeg2->yFirstBin,
...
Bool_t HMdc34ClFinderSec::calcClusterSeg2FloatLevel(Int_t nBin) {
  ...
  UChar_t amp = prPlotSeg2->weights[nBin];
  ...
  while (kTRUE) {
    for(Int_t ib=0; ib<8; ib++) {
      ...
      UChar_t ampN = prPlotSeg2->weights[nb];
...
Bool_t HMdc34ClFinderSec::fillClusterSeg2(void) {
  ...
  for(Int_t module=2; module<4; module++) {
    if(!lMods[module]) continue;
    HMdc34ClFinderMod& fMod=(*this)[module];
...
//          Int_t nYbin = line+nDownLine;
    ...
    //if (dY1b==0 || dY2b==0) - optimizaciya, proverit' na time !!!!???
          ...
          Int_t nDeleted =
              cluster->getLCells2().setTime(layerSeg,cell,cells->getTime(cell));
...
Bool_t          HMdc34ClFinder::useFixedLevel  = kFALSE;
...
Bool_t HMdc34ClFinder::calcProjPlaneSeg2(Int_t sec){
  ...
  if(nL<-7||nL>7 || (nL<0 && mSeg[1]==2) || (nL>0 && mSeg[1]==1) ||
    //(one mdc exist only)
      (nL==0 && mSeg[1]!=3)) {
    nL  = 0;
    par = 0.1;
...
Bool_t HMdc34ClFinder::calcSizePlotSeg2(Int_t sec){
  ...
  for(Int_t mod=2; mod<4; mod++) {
    ...
    for(Int_t point=0; point<4; point++) {
      ...
      fsec.getPlotSeg2()->calcIntersection(pKick[point],vect[indx][point],
          vect[indx][point]);
...
Bool_t HMdc34ClFinder::calcWiresProj(Int_t sec) {
  ...
  for(Int_t mod=2; mod<4; mod++) {
    ...
    for(Int_t lay=0; lay<6; lay++) {
      ...
      Double_t x       = (prPlotSeg2->nBinX-1) * prPlotSeg2->stX +
                         prPlotSeg2->xMin;
      Double_t yLeft   = al*(x - point[1](0))+point[1](1); // (x-x2)*a+y2
      Double_t yRight  = al*(prPlotSeg2->xMin - point[1](0))+point[1](1);
      Short_t  nYLeft  = Short_t((yLeft  - prPlotSeg2->yMin) /
                                 prPlotSeg2->stY);
      Short_t  nYRight = Short_t((yRight - prPlotSeg2->yMin) /
                                 prPlotSeg2->stY);
...
void HMdc34ClFinder::setPrPlotSize(Int_t nx,Int_t ny) {
  // Should be called before "initContainer" calling
  nBinX = nx;
  nBinY = ny<=1 ? (nx*277)/100 : ny;
}
