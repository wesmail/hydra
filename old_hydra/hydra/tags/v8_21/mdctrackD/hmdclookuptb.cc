...
HMdcLookUpTbSec::HMdcLookUpTbSec(Int_t sec, Int_t nSegs,
  ...
  clusArrSize   = pClustersArrs->createAllArrays(1000); // Clusters array size!
...
Int_t module = mod;
  ...
  for(Int_t indLine=0; indLine<6; indLine++) {
    ...
    if(maxAmp[module]-nFiredLay+1>=minAm) {  // determination min(max)Bin[y]
      if(nFiredLay<minAm) makeLayProjV1(module,layer);  // filling pr.plot
      else {                                         // ...+ amp.checking
...
//?W          if(hitsDel[indLay][n]<0) continue;
      ...
      if(nFiredLay<minAm) makeLayProjV2(module,layer);  // filling pr.plot
      else {                                                // ...+ amp.checking
        UChar_t add=1<<layer;
        HMdcLookUpTbLayer& fLayer=(*cFMod)[layer];
        Int_t cell = -1;
...
//???optimizaciya???        if(dY>100.) break;  //????????? biylo zakomentirovano ???
        if(TMath::Abs(dY) > 30.) continue;                         // 10. mm !???
        if(TMath::Abs(cls1.getX()-cls2.getX()) > 100.) continue;   // 40. mm !???
...
void HMdcLookUpTbSec::testClusMod12toSeg(void) {
  ...
  for(Int_t cl=0; cl<nClsArr; cl++) {
    ...
    for(Int_t cl1=0; cl1<nClsArrM1; cl1++) {
      ...
      if(TMath::Abs(cls.getY()-cls1.getY()) > 30.) continue;         // 10. mm !???
      if(TMath::Abs(cls.getX()-cls1.getX()) > 100.) continue;        // 40. mm !???
    ...
    for(Int_t cl2=0; cl2<nClsArrM2; cl2++) {
      ...
      if(TMath::Abs(cls.getY()-cls2.getY()) > 30.) continue;         // 10. mm !???
      if(TMath::Abs(cls.getX()-cls2.getX()) > 100.) continue;        // 40. mm !???
