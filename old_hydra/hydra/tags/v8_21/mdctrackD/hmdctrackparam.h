...
class HMdcTrackParam : public HMdcLineParam {
  ...
  protected:
    ...
    Double_t timeOffset[8];     // time offsets for each MDC in sec.
    Double_t errTimeOffset[8];  // errors of time offsets for each MDC in sec.
    Short_t  isTmoffFixed;      // if bit 0-7 eq.1 timeOffset for mdc is fixed

    // Variable data mambers:
    Double_t sDev[8];           // For timeOffset calculation.
    Double_t sWht[8];           // [4] - for each module.
    Double_t dTdPar[4][8];      // [nParam!][nMod]
  ...
  public:
    ...
    void      setOneSecData(void)                  {nMods = 4;}
    void      setTwoSecData(void)                  {nMods = 8;}
    void      copyNMods(HMdcTrackParam& p)         {nMods = p.nMods;}
    ...
    Short_t   isMdcTimeOffsetFixed(Int_t m) const  {return (isTmoffFixed>>m)&1;}
    Short_t   isSegTimeOffsetFixed(Int_t s) const  {return (isTmoffFixed>>(s*2))&3;}
    Short_t   isTrackTimeOffsetFixed(void) const   {return isTmoffFixed ? 1:0;}
...
inline void HMdcTrackParam::copyTimeOffsets(const HMdcTrackParam& tp) {
  for(Int_t m=0;m<8;m++) timeOffset[m]=tp.timeOffset[m];
