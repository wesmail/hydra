...
//
// Example:
//    hyp->AddAlgorithm("empty",new HHypAllFillerInclusiveAlg("Test of All Filler",
...
// expected (needed for kinematic fitting). If there are several missing
...
// Number of particles is limited to 10, this is limited by Filler
//
////////////////////////////////////////////////////////////////////////
...
#include <stdlib.h>
#include <iostream>
#include "hhypAllFillerInclusiveAlg.h"
#include "hypinfodef.h"

ClassImp(HHypAllFillerInclusiveAlg)

HHypAllFillerInclusiveAlg::HHypAllFillerInclusiveAlg(Char_t *name_i, Option_t par[])
  :HHypBaseAlgorithm(name_i,par)
{
	for(Int_t i=0; i<HHypAllFillerInclusiveAlg_ANZ_FILLER; i++) filler[i] = NULL;
}

HHypAllFillerInclusiveAlg::~HHypAllFillerInclusiveAlg()
{
	for(Int_t i=0; i<HHypAllFillerInclusiveAlg_ANZ_FILLER; i++){
		if( filler[i]) delete filler[i];
	}
}

Bool_t HHypAllFillerInclusiveAlg::execute()
{
...
// 	cout << "Filler Exec: ";
	Int_t nrs[HHypAllFillerInclusiveAlg_ANZ_FILLER], nr=0;
	for(Int_t i=0; i<HHypAllFillerInclusiveAlg_ANZ_FILLER; i++){
		if( filler[i]){
...
Bool_t HHypAllFillerInclusiveAlg::InitFillers(Int_t &fnr,Int_t particles_anz,Int_t *particles,Int_t missing_anz,Int_t *missing)
{
	Bool_t flag=false;
	if(particles_anz>0){
		cout << "HypAllFillerInclusiveAlg "<<fnr<<": Parsed as:";
		filler[fnr] = new HHypListFillerInclusive("filler", "filler");
		...
		for(Int_t mi=0; mi<missing_anz; mi++){
			cout << " " << missing[mi];
			cerr << "The Inclusive Filler does NOT support missing particles!!!"<<endl;
			cerr << "Better use the HHypAllFiller instead!!!"<<endl;
//			filler[fnr]->AddOneMissing(missing[mi]);
		...
		cerr << "HypAllFillerInclusiveAlg: No Particles defined!"<<endl;
	}
	return(flag);
}

Bool_t HHypAllFillerInclusiveAlg::init()
{
	Bool_t flag=false;
	Int_t fnr=0;
  ...
  TString *abc, def;
   ...
   def=abc->Data();
	if(abc==NULL){
		Error("HHypAllFillerInclusiveAlg","No Reactions defined in Option String by Reactions=()()()");
		return(false);
	}
	cout << "HypAllFillerInclusiveAlg ("<<algoName<<"): "<<def.Data()<<endl;
	TObjArray *allreac_arr;
	allreac_arr=def.Tokenize("()");
	...
	for(Int_t j=0; j<arr_nr; j++){
		...
		if(partmissdiv_arr->GetEntries()>2){
			cerr<<"HypAllFillerInclusiveAlg: Parse Error in Reaction >>"<<reac_str<< "<< !!!"<<endl;
			delete partmissdiv_arr;
			break;
		}
		cout << "HypAllFillerInclusiveAlg ("<<algoName<<"): Reaction String "<<j<<": <<"<<reac_str<<">>"<<endl;
		...
		if(partmissdiv_arr->GetEntries()>=1){
			{
				...
				for(Int_t k=0; k<part_arr->GetEntries(); k++){
					...
					if(particles[particles_anz]!=0){
						particles_anz++;
						if(particles_anz==11){
							cerr << "HypAllFillerInclusiveAlg: number of particles exceeds 10!"<<endl;
							break;
						}
					}
				}
			...
			if(partmissdiv_arr->GetEntries()==2){
				...
				for(Int_t k=0; k<miss_arr->GetEntries(); k++){
					...
					if(missing[missing_anz]!=0 || miss_part=="dummy"){
						missing_anz++;
						if(missing_anz==11){
							cerr << "HypAllFillerInclusiveAlg: number of missing particles exceeds 10!"<<endl;
							break;
						}
					}
				}
			...
			if(fnr==HHypAllFillerInclusiveAlg_ANZ_FILLER) break;
		}else{
			cerr << "HypAllFillerInclusiveAlg: Problem: Empty string!"<<endl;
		}
		delete partmissdiv_arr;
	}
	delete allreac_arr;
...
Bool_t HHypAllFillerInclusiveAlg::reinit()
{
  return kTRUE;
}

Bool_t HHypAllFillerInclusiveAlg::finalize()
{
  return kTRUE;
}
