...
HTriggerHistBooker::HTriggerHistBooker(const Char_t* name,const Char_t* title,
      ...
      if (io->getDetParIo("HGo4ParIo") == 0)
				{
					if (strcmp(io->IsA()->GetName(),"HParAsciiFileIo")==0)
						{
							HGo4ParAsciiFileIo* p = new HGo4ParAsciiFileIo(((HParAsciiFileIo*)io)->getFile());
							io->setDetParIo(p);
							io->setInputNumber(1);
						}
					if (strcmp(io->IsA()->GetName(),"HParRootFileIo")==0)
						{
							HGo4ParRootFileIo* p = new HGo4ParRootFileIo(((HParRootFileIo*)io)->getParRootFile());
							io->setDetParIo(p);
							io->setInputNumber(1);
						}
  ...
  if( gSystem->AccessPathName( "./rootFileTrigger", (EAccessMode)F_OK ) == 1 ) // checks if ./triggerfile exists. (true if not)
    {
      ...
      if( gSystem->AccessPathName( ".", (EAccessMode)W_OK ) == 0 ) // checks if current dir (exists...) and is writable (true if it is)
				...
				{
					cout << "---trigger: current directory does exist (surprise!) but you have not the permission to write." << endl;
					return(1);
				}
    }
  else
  ...
  if( option == "active" )
    {
      ...
      if(myiter)
				...
				{
					cout << "---trigger: could not create iterator -> no hists written to .root file" << endl;
				}
    }
