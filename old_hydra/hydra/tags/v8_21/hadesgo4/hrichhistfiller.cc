...
HRichHistFiller::HRichHistFiller(const Char_t* name,const Char_t* title)
    : TNamed(name,title)
{
  colorsSector.Set(6);
  colorsCuts.Set(4);
...
void   HRichHistFiller::putRichParams(HParamList* l,TList* histpool)
{
    ...
    // HParamList (which ist used by the io);
       ...
       l->add("hRichPulseDist"         ,mytemp);

       hRichMultiplicityAddon->getText(mytemp);
       l->add("hRichMultiplicity"      , mytemp);
       hRichPulseheightAddon->getText(mytemp);
       l->add("hRichPulseheight"       ,mytemp);
       hRichChargedParticlesAddon->getText(mytemp);
       l->add("hRichChargedParticles"  ,mytemp);
       sprintf(mytemp,"%s","s");
       l->add("hRichFancy"             ,mytemp);
       sprintf(mytemp,"%s","s");
       l->add("hRichPattern"           ,mytemp);
       hRichmult_rawA->getText(mytemp);
       l->add("hRichmult_raw"          ,mytemp);
       hRichmulttemp_rawA->getText(mytemp);
       l->add("hRichmulttemp_raw"      ,mytemp);
       hRichmulttrend_rawA->getText(mytemp);
       l->add("hRichmulttrend_raw"     ,mytemp);
