...
HRichPedCalNew::HRichPedCalNew(const Text_t *name,const Text_t *title,const Char_t *
				       path ,const Char_t * pString): HReconstructor(name,title) {
...
Int_t HRichPedCalNew::getCol(Int_t hardwarenumber){
  fMappingPar = gHades->getRuntimeDb()->getContainer("RichMappingParameters");
  //WRONG! This should have been removed Long_t since (T.C. 16.03.03)
...
Int_t HRichPedCalNew::getRow(Int_t hardwarenumber){
  fMappingPar=gHades->getRuntimeDb()->getContainer("RichMappingParameters");
  //WRONG! This should have been removed Long_t since (T.C. 16.03.03)
...
void HRichPedCalNew::meanAndSigma(Int_t sIx, Int_t col, Int_t row, Float_t charge){
...
void HRichPedCalNew::initMappingPar() {
