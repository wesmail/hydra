...
HRichUnpackerRaw::HRichUnpackerRaw(Int_t rId) {
    richId   = rId;
    pRawCat  = NULL;
    fEventNr = -1; 
}
...
Int_t HRichUnpackerRaw::getSubEvtId(void) const {
    return richId; 
}

Int_t HRichUnpackerRaw::execute() {
    HLocation loc;
    loc.set(3,0,0,0);
    fEventNr++;
    //  cout<<fEventNr<<endl;
...
Bool_t HRichUnpackerRaw::padReadOut(Int_t h_col,Int_t h_row){
...
Bool_t HRichUnpackerRaw::diagnose(Int_t h_charge,Int_t h_col,Int_t h_row,Int_t h_sec, Int_t h_fEventNr) {
