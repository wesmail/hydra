...
//reading and initializing pads table from ascii file (HAsciiKey format)
  ...
  try {
     ...
     Int_t nrPads = mydata.ReadInt("Pads");
     Int_t nrPadX = mydata.ReadInt("PadsX");
     Int_t nrPadY = mydata.ReadInt("PadsY");
     Int_t nr;
...
// writing pads table from ascii file (HAsciiKey format)
  ...
  try {
     ...
     mydata.WriteKeyword("Pads Data");

     for(Int_t i=0; i<nPads; i++){
        HShowerPad *pad = getPad(i);

        mydata.WriteInt(pad->getPadNr());
        ...
        mydata.WriteFloat(pad->getLinkedNr());
        mydata.WriteFloat(i);// always linked to itself, Feature, not a bug 
				mydata.WriteString("\n\\ ");
    }
  }
  catch (Bool_t ret) {
...
TObject*& HShowerPadTab::getSlot(Int_t nIndx) {
//access to object in TClonesArray 
  return (m_pPadArr->operator[](nIndx));
}

TObject* HShowerPadTab::getObject(Int_t nIndx) {
