...
{
  if(strcmp(His->GetName(),"hTriggerTrendTime")==0)
    {
      fxQtCanvasTime->getCanvas()->cd();
      His ->getP()->Draw() ;
      fxQtCanvasTime->getCanvas()->Update();
    }
}
...
void TGo4TriggerOnline::initCanvasColor( short color )
{
      ...
      for(Int_t sec=0;sec<7;sec++)
 {
  ...
  // --- Sub tab correlation ----------------------------------------------
  // -- TOF
  CanvasArrayCorr[0][0] = TQRootCanvasTOFCorrSec0phi;
  CanvasArrayCorr[0][1] = TQRootCanvasTOFCorrSec1phi;
  CanvasArrayCorr[0][2] = TQRootCanvasTOFCorrSec2phi;
  CanvasArrayCorr[0][3] = TQRootCanvasTOFCorrSec3phi;
  CanvasArrayCorr[0][4] = TQRootCanvasTOFCorrSec4phi;
  CanvasArrayCorr[0][5] = TQRootCanvasTOFCorrSec5phi;
 
  CanvasArrayCorr[1][0] = TQRootCanvasTOFCorrSec0theta;
  CanvasArrayCorr[1][1] = TQRootCanvasTOFCorrSec1theta;
  CanvasArrayCorr[1][2] = TQRootCanvasTOFCorrSec2theta;
  CanvasArrayCorr[1][3] = TQRootCanvasTOFCorrSec3theta;
  CanvasArrayCorr[1][4] = TQRootCanvasTOFCorrSec4theta;
  CanvasArrayCorr[1][5] = TQRootCanvasTOFCorrSec5theta;
   // --- SHOWER
  CanvasArrayCorr[2][0] = TQRootCanvasSchowerCorrSec0phi;
  CanvasArrayCorr[2][1] = TQRootCanvasSchowerCorrSec1phi;
  CanvasArrayCorr[2][2] = TQRootCanvasSchowerCorrSec2phi;
  CanvasArrayCorr[2][3] = TQRootCanvasSchowerCorrSec3phi;
  CanvasArrayCorr[2][4] = TQRootCanvasSchowerCorrSec4phi;
  CanvasArrayCorr[2][5] = TQRootCanvasSchowerCorrSec5phi;
 
  CanvasArrayCorr[3][0] = TQRootCanvasSchowerCorrSec0theta;
  CanvasArrayCorr[3][1] = TQRootCanvasSchowerCorrSec1theta;
  CanvasArrayCorr[3][2] = TQRootCanvasSchowerCorrSec2theta;
  CanvasArrayCorr[3][3] = TQRootCanvasSchowerCorrSec3theta;
  CanvasArrayCorr[3][4] = TQRootCanvasSchowerCorrSec4theta;
  CanvasArrayCorr[3][5] = TQRootCanvasSchowerCorrSec5theta;
    // --- global settings
   for(Int_t ipu=0;ipu<4;ipu++)
    {
      for(Int_t sec=0;sec<6;sec++)
 {
   CanvasArrayCorr[ipu][sec]->getCanvas()->SetFillColor(canvasColor); 
   CanvasArrayCorr[ipu][sec]->getCanvas()->SetTickx();
   CanvasArrayCorr[ipu][sec]->getCanvas()->SetTicky(); 
           }
   }
  // --- Overview (main) Tab --------------------------------------------
  // --- Main Canvases
  ...
  // --- Time Tab -------------------------------------------------------
  fxQtCanvasTime->getCanvas()->SetFillColor(canvasColor);
  fxQtCanvasTime->getCanvas()->SetTickx();
  fxQtCanvasTime->getCanvas()->SetTicky();
}
...
{
  Int_t ipu = -1;
  if       ((strcmp(TabWidgetTof->currentPage()->name(),"Tof_corr_phi")==0) && (strcmp(His->GetName(),"hTriggerTofCorrPhi" )==0 )) { ipu=0;}
  else if((strcmp(TabWidgetTof->currentPage()->name(),"Tof_corr_theta")==0) && (strcmp(His->GetName(),"hTriggerTofCorrTheta")==0)) {ipu=1;}
  else if((strcmp(TabWidgetShower->currentPage()->name(),"Shower_corr_phi")==0) && (strcmp(His->GetName(),"hTriggerShowerCorrPhi")==0)) {ipu=2;}
 else if((strcmp(TabWidgetShower->currentPage()->name(),"Shower_corr_theta")==0) && (strcmp(His->GetName(),"hTriggerShowerCorrTheta")==0)) {ipu=3;}  
  else return;
  if((ipu==0) || (ipu==1) || (ipu==2) || (ipu==3))
  {
      for(Int_t sec=0;sec<6;sec++)
      {
	  CanvasArrayCorr[ipu][sec]->getCanvas()->cd();
	  His ->getP(0,sec)->Draw();
	  CanvasArrayCorr[ipu][sec]->getCanvas()->Update();
      }
  }
}
