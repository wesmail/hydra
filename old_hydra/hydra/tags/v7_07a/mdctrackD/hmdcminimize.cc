...
//   Double_t dpar[25] = {5.0, 5.0, 5.0, 5.0, 5.0, 5.0,5.0,5.0,5.0,5.0,5.0,5.0};
  ...
  for(int i = 0; i < nPar; i++) pari[i] = par0[i];
...
//     for(int i = 0; i < nPar; i++) pari[i] = par0[i];
    ...
    for(int i = 0; i < nPar; i++) pari[i] = parOut[i];
//     f0 = (*fFCN)(fObjectFit,nPar,pari);
    f0 = (*fFCN)(fObjectFit,nPar,parOut);
    cout << " ========================================================== " << endl;
    if(f0 < fbest) {
       fbest = f0;
       for(int i = 0; i < nPar; i++) parbest[i] = parOut[i];
    }
//     else break;
       
  }
  for(int i = 0; i < nPar; i++) parOut[i] = parbest[i];
  
  for(int i = 0; i < nPar; i++) cout << parbest[i] << " , ";
  cout << fbest << endl;
...
Int_t HMdcMinimize::minbar(Int_t maxIter, Int_t nParIn,
  ...
  for(int i = 0; i < numOfParam; i++) parameter[i] = par0[i];
...
//   Double_t step[24] = {1.0, 1.0, 2.0, 1.0, 1.0, 2.0,1.0,1.0,2.0,1.0,1.0,2.0};
  ...
  for(int i = 0; i < numOfParam; i++) parOut[i] = parameter[i];
...
Int_t HMdcMinimize::scanXYZ(Int_t maxIter, Int_t nParIn,
  ...
  Double_t functi, pari[200], funmin = 1000000000., parmin[200];
...
Int_t HMdcMinimize::cog(Int_t maxIter, Int_t nParIn,
  ...
  Double_t functi, pari[200], par[200], alpha[200], alphamin[200], funmin = 1000000000., funstart, parsum[200];
