...
//*-- Version: $Id: hrichanalysisIPUSim.cc,v 1.7 2005-07-15 09:02:50 wuestenf Exp $
      ...
      if (hit!=NULL)
        {
          ...
          HRichHitIPUSim &ringIn = *((HRichHitIPUSim*)pRings->At(i));
          HRichHitIPUSim *ring = new HRichHitIPUSim(ringIn);

          *hit = ringIn;
          hit->setSector(nSec);

          // mapping according to RICH
          HRichPad * pad = ((HRichGeometryPar*)fpGeomPar)->getPadsPar()->getPad(ring->getX(),ring->getY());

          if(!pad)
          {
            gHades->getMsg()->error(10,HMessageMgr::DET_TRIGGER,GetName(),"RICH ring  %d coordinate out of sector (%d %d)!",i, ring->getX(),ring->getY());
            continue;
          }
          ...
          Int_t k = 0;
          while(ring->getRingTrack(k) && ( k < 256))
            {

              if (hit->getTrack1()==0)
                {
                  hit->setTrack1(ring->getRingTrack(k));
                  hit->setFlag1(ring->getRingFlag(k));
                  hit->setWeightTrack1(hit->getWeightTrack1()+1);
                }
              else
                {
                  if(ring->getRingTrack(k)== hit->getTrack1())
                    {
                      hit->setWeightTrack1(hit->getWeightTrack1()+1);
                    }
                  else
                    {
                      if(hit->getTrack2()==0)
                        {
                          hit->setTrack2(ring->getRingTrack(k));
                          hit->setFlag2(ring->getRingFlag(k));
                          hit->setWeightTrack2(hit->getWeightTrack2()+1);
                        }
                      else
                        {
                          if(ring->getRingTrack(k)== hit->getTrack2())
                            {
                              hit->setWeightTrack2(hit->getWeightTrack2()+1);
                            }
                          else
                            {
                              if (hit->getTrack3()==0)
                                {
                                  hit->setTrack3(ring->getRingTrack(k));
                                  hit->setFlag3(ring->getRingFlag(k));
                                  hit->setWeightTrack3(hit->getWeightTrack3()+1);
                                }
                              else
                                {
                                  hit->setWeightTrack3(hit->getWeightTrack3()+1);
