...
void add_hyp(HHypReconstructor * hyp) {
  ...
  if (hypLevel == 0) {
    ...
    hyp->AddAlgorithm("empty",new HHypPPEpEmXAlg("inclusive lepton filler",""),"PPEpEm");
//    hyp->AddAlgorithm("empty",new HHypPPEpEmXXAlg("inclusive leptons+X filler",""),"PPEpEm");
    ...
    hyp->AddAlgorithm("PPEpEm_dtof_Gamma",new HHypPPEpEmProjector("exclusive lepton projector"),"ntuple", hfile);
    ...
    //Hi Marco, add your filler, selectors and projectors here....
     ...
     hyp->AddAlgorithm("empty",new HHypPPXAlg("inclusive pi0 filler mw",""),"PP");
     ...
     hyp->AddAlgorithm("PP",pppi0,"ntuple", hfile);
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt_long (myargc, myargv, "s1:2:l:MHi:d:o:n:#:$:Sc:r:q:B", long_options, &option_index)) != -1) {
