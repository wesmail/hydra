...
ClassImp(HPidPdfMaker)
  ...
  // ---------------------------------------------------------------------------
  HPidPdfMaker::HPidPdfMaker(const char*cOutFileName, Bool_t analyseSim)
    :HSUDummyRec(kFALSE){
...
Int_t HPidPdfMaker::execute(void) {
  ...
  //reset iterators
	...
	//Check for ring
	  ...
	  if(pHitData->hasRingCorrelation[ALG_RUNGEKUTTA]) 
	    {
	...
	aPatternInstance[b_kicktrack]= (pTrackData->nKickTrackInd>=0 && pTrackData->bIsAccepted[ALG_KICK])? kTRUE : kFALSE;
	
	aPatternInstance[b_splinetrack]= (pTrackData->nSplineTrackInd>=0 && pTrackData->bIsAccepted[ALG_SPLINE])? kTRUE : kFALSE;
	
	aPatternInstance[b_rktrack]= (pTrackData->nRKTrackInd>=0  && pTrackData->bIsAccepted[ALG_RUNGEKUTTA])? kTRUE : kFALSE;
	...
	if(pHitData->iIndTOF>=0)
	  {
	    ...
	    aPatternInstance[n_tof_clss]  = pHitData->nTofClsSize;
	    aPatternInstance[n_meta_cell]  = pHitData->nMetaCell;
	  }
	aPatternInstance[n_tofino_mult] = pHitData->iTofinoMult;	
...
Bool_t HPidPdfMaker::getDownscalingFlag()
{
  if(analyseSimData)
    {
      return kTRUE;
...
{  
  aPatternInstance[b_is_cp_cand]=pTrackData->bIsClosePairCandidate;
  aPatternInstance[f_angle_with_cp_candidate]=pTrackData->fAngleWithClosePairCandidate;
  aPatternInstance[n_charge_rk]=pTrackData->nPolarity[ALG_RUNGEKUTTA];
  aPatternInstance[n_charge_kick]=pTrackData->nPolarity[ALG_KICK];
  aPatternInstance[n_charge_spline]=pTrackData->nPolarity[ALG_SPLINE];
  aPatternInstance[n_sector]=(Float_t)(pHitData->nSector);
  aAuxPatternInstance[aux_n_sector]=(Float_t)(pHitData->nSector);
  ...
  if(aPatternInstance[b_ring] && hasATrack())
    {
      ...
      aAuxPatternInstance[aux_f_deltatheta]=pHitData->fRichTheta-pHitData->fMdcTheta;

      aPatternInstance[f_deltaphi]=(pHitData->fRichPhi - pHitData->fMdcPhi)
	* TMath::Sin(TMath::DegToRad()*pHitData->fMdcTheta);
      aAuxPatternInstance[aux_f_deltaphi]=(pHitData->fRichPhi - pHitData->fMdcPhi)
	* TMath::Sin(TMath::DegToRad()*pHitData->fMdcTheta);
    }
...
Bool_t HPidPdfMaker::bookVertexInformation()
{
  ...
  aPatternInstance[f_distance_to_vertex]=pHitData->fDistanceToVertex[ALG_SPLINE];

  if(pTrackData->bIsAccepted[ALG_SPLINE]){
  ...
  if(pTrackData->bIsAccepted[ALG_KICK]){
    aPatternInstance[f_track_r_kick]=pTrackData->fTrackR[ALG_KICK];
    aPatternInstance[f_track_z_kick]=pTrackData->fTrackZ[ALG_KICK];
  }
  if(pTrackData->bIsAccepted[ALG_RUNGEKUTTA]){
    aPatternInstance[f_track_r_rk]=pTrackData->fTrackR[ALG_RUNGEKUTTA];
    aPatternInstance[f_track_z_rk]=pTrackData->fTrackZ[ALG_RUNGEKUTTA];
  }
...
{
  if(aPatternInstance[i_tracktype]>=0 && analyseSimData)
    {
  ...
  aAuxPatternInstance[aux_f_track_theta]=pHitData->fMdcTheta;

  aPatternInstance[f_track_phi]=pHitData->fMdcPhi;

  aPatternInstance[f_kick_mom_exp]=pTrackData->fMomenta[ALG_KICK];
  aPatternInstance[f_kick_beta]=pTrackData->fCorrectedBeta[ALG_KICK];
  aAuxPatternInstance[aux_f_kick_mom_exp]=pTrackData->fMomenta[ALG_KICK];

  aPatternInstance[f_spline_mom_exp]=pTrackData->fMomenta[ALG_SPLINE];
  aPatternInstance[f_spline_beta]=pTrackData->fCorrectedBeta[ALG_SPLINE];
  aAuxPatternInstance[aux_f_spline_mom_exp]=pTrackData->fMomenta[ALG_SPLINE];

  aPatternInstance[f_kick123_mom_exp]=pTrackData->fMomenta[ALG_KICK123];
  aPatternInstance[f_kick123_beta]=pTrackData->fCorrectedBeta[ALG_KICK123];
  aPatternInstance[f_reft_mom_exp]=pTrackData->fMomenta[ALG_REFT];
  aPatternInstance[f_reft_beta]=pTrackData->fCorrectedBeta[ALG_REFT];
  aPatternInstance[f_rk_mom_exp]=pTrackData->fMomenta[ALG_RUNGEKUTTA];
  aPatternInstance[f_rk_beta]=pTrackData->fCorrectedBeta[ALG_RUNGEKUTTA];
  aPatternInstance[f_kick_pull]=pTrackData->fPull;
  aPatternInstance[f_spline_quality]=pTrackData->fSplineChiSquare;
  aPatternInstance[f_rk_quality]=pTrackData->fRKChiSquare;
  
  aPatternInstance[f_kick_mass]=pTrackData->fMassSquared[ALG_KICK];
  aPatternInstance[f_spline_mass]=pTrackData->fMassSquared[ALG_SPLINE];
  aPatternInstance[f_kick123_mass]=pTrackData->fMassSquared[ALG_KICK123];
  aPatternInstance[f_reft_mass]=pTrackData->fMassSquared[ALG_REFT];
  aPatternInstance[f_rk_mass]=pTrackData->fMassSquared[ALG_RUNGEKUTTA];
...
Float_t HPidPdfMaker::getTrackType(HPidTrackCandSim* pTrackCand)
{
  ...
  /*
  if((!pGeantTrackSet->wasSeenInDetector(pGeantTrackSet->getCorrelationFlag(0), kSeenInInnerMDC))) 
  {
    //Most common particle
    //was not seen in inner mdc
    //cout << "Ghost in mdc1/2: " << pGeantTrackSet->getCorrelationFlag(0) << endl;
    if(pGeantTrackSet->getCorrelationFlag(0)==70 || pGeantTrackSet->getCorrelationFlag(0)==68)
      {
	cout << "INCOMPLETE:----------------------------------------------" << endl;
	cout << "Most common: "<<pGeantTrackSet->getCorrelationFlag(0)<< endl;
	cout << "Track number: " << pGeantTrackSet->getGeantTrackID(0)<< endl;
	cout << "PID number: " << pGeantTrackSet->getGeantPID(0)<< endl;
	cout << "Mother id: " << pGeantTrackSet->getGeantParentID(0)<< endl;
	cout << endl;
	cout << "Second most common: " << pGeantTrackSet->getCorrelationFlag(1) << endl;
	cout << "Track number: " << pGeantTrackSet->getGeantTrackID(1)<< endl;
	cout << "PID number: " << pGeantTrackSet->getGeantPID(1)<< endl;
	cout << "Mother id: " << pGeantTrackSet->getGeantParentID(1)<< endl;

      }
  }
  else
    {
      if(pGeantTrackSet->getCorrelationFlag(0)>=76)
	{
	  cout << "FULLY TRACKED:----------------------------------------------" << endl;
	  cout << "Most common: "<<pGeantTrackSet->getCorrelationFlag(0)<< endl;
	  cout << "Track number: " << pGeantTrackSet->getGeantTrackID(0)<< endl;
	  cout << "PID number: " << pGeantTrackSet->getGeantPID(0)<< endl;
	  cout << "Mother id: " << pGeantTrackSet->getGeantParentID(0)<< endl;
      ...
      //cout << "OK: " << pGeantTrackSet->getCorrelationFlag(0)<< endl;
    }
  */
