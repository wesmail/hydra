...
//      if (invmass>150.) continue;                  // low mass cut
//      if (invmass<150. || invmass>600.) continue;  // medium mass cut
//      if (invmass<600.) continue;                  // high mass cut
	...
	if (isCutNb==0 && iscpcandidate1==0 && iscpcandidate2==0 && 
	    pt>100.){
	  fillHistograms(32,weightTemp);
	  fillHistograms(33,weight);
	}
	if (isCutNb==0 && iscpcandidate1==0 && iscpcandidate2==0 &&
	    pt>100. && invmass>140.){
	  fillHistograms(34,weightTemp);
	  fillHistograms(35,weight);
	}
	//////////// C U T S /////////////////////////////////////////////
    ...
    // Float_t evt = 110582360.;    // URQMD SIMULATION LVL1 EQUIVALENT
    Float_t piEvt = 1.12;           //pion multiplicity per event
...
Float_t pairsE::getEfficiencyFactor()
{
  if(mom1>2000 || mom2>2000) return 0.;
    ...
    //    cout << " phi1_tmp " << phi1_tmp  <<" theta "<< theta1 <<"  sec " << sec1 <<"  mom1  "<<mom1<< " eff1  " << fEff1 << endl;
    
    // second leg of the pair
    if (chrg2==1) // positron
    {
	
	if (p3DEffPosi) fEff2 = p3DEffPosi->
	...
	if (p3DEffEle) fEff2 = p3DEffEle->GetBinContent(p3DEffEle->FindBin(d2r*phi2_tmp,d2r*theta2,mom2));
	else fEff2=1.;
	
    }
    //    cout << " phi2_tmp " << phi2_tmp  <<" theta "<< theta2<<" sec " <<sec2 <<"  mom2  "<<mom2<< " eff2  " << fEff2 << endl;
	...
	//{
	cout<<" d2r*phi2_tmp "<< d2r*phi2_tmp<<"  d2r*theta2  "<< d2r*theta2<<"   mom2  "<<mom2<<endl;
	cout<<" d2r*phi1_tmp "<< d2r*phi1_tmp<<"  d2r*theta1  "<< d2r*theta1<<"   mom1  "<<mom1<<endl;
	    //}
	return 0.;
    }
}
