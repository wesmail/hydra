  ...
  //see HEvent::makeBranch 
   ...
   if (sl==0) {
     ...
     for(i=0;i<=fPartialEvs->GetLast();i++) {
       par=(HPartialEvent **)&vector[i];
       if ((*par)!=NULL) {
	 b=gHades->getTree()->Branch( (*par)->GetName(),
				      (*par)->ClassName(),par,4000,0);
       }
     }
	 ...
	 b=gHades->getTree()->Branch( buffer,
				      (*par)->ClassName(),par,4000,99);
	 (*par)->makeBranch(b);
       }
     }
     gHades->getTree()->Branch("EventHeader",fHeader->ClassName(),&fHeader,4000,3);
   }
#if DEBUG_LEVEL>2
   gDebuger->leaveFunc("HRecEvent::activateBranch");
#endif
