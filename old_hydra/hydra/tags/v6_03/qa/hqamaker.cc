...
// Build Postscript file name, either from user-supplied name or from DST filename
     ...
     if(fDir.IsNull() && (!psFile.Contains("/")))
        Warning("makePS","dir not specified, QA output written to working dir");
...
void HQAMaker::fillHistShower() {
  ...
  if(iterShoHit) {
    ...
    while((shoHit=(HShowerHit*) iterShoHit->Next())!=0) {
      ...
      if(mod==0) hists->shoHit_rowVsCol_m0->Fill(row,col);
      else if(mod==1) hists->shoHit_rowVsCol_m1->Fill(row,col);
      else if(mod==2) hists->shoHit_rowVsCol_m2->Fill(row,col);

      charge = shoHit->getCharge();
      if(mod==0) hists->shoHit_avgCharge_m0->Fill(row,col,charge);
      else if(mod==1) hists->shoHit_avgCharge_m1->Fill(row,col,charge);
      else if(mod==2) hists->shoHit_avgCharge_m2->Fill(row,col,charge);
...
void HQAMaker::finalizeHistShower() {
  ...
  hists->shoHit_avgCharge_m1->Divide(hists->shoHit_rowVsCol_m1);
  hists->shoHit_avgCharge_m2->Divide(hists->shoHit_rowVsCol_m2);

  hists->shoHit_rowVsCol_m0->Scale(1./nProcessed);
  hists->shoHit_rowVsCol_m1->Scale(1./nProcessed);
  hists->shoHit_rowVsCol_m2->Scale(1./nProcessed);
}
