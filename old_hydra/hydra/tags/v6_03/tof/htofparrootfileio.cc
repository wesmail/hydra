...
Bool_t HTofParRootFileIo::init(HParSet* pPar,Int_t* set) {
  ...
  if (pFile) {
    ...
    if (strcmp(name,"TofClusterFPar")==0) return read((HTofClusterFPar*)pPar);
...
Bool_t HTofParRootFileIo::read(HTofClusterFPar* pPar) {
  // reads and fills the container "TofClusterFPar"
  Text_t* name=(char*)pPar->GetName();
  ...
  if (pPar->getInputVersion(inputNumber)==version
        && pPar->getInputVersion(inputNumber)!=-1) return kTRUE;
  // needs reinitialization
  HTofClusterFPar* r=(HTofClusterFPar*)findContainer(name,version);
  Bool_t allFound=kTRUE;
