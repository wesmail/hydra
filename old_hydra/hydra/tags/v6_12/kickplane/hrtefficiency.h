...
public:
  ...
  UInt_t getBinsTotal(void) { return fBinsTotal; }
  UInt_t getBinsTheta(void) { return fBinsTheta; }
  UInt_t getBinsPhi(void) { return fBinsPhi; }
  UInt_t getBinsP(void) { return fBinsP; }
  inline Float_t getP(UInt_t binP);
  inline Float_t getTheta(UInt_t binTheta);
  inline Float_t getPhi(UInt_t binPhi);
  inline Int_t getBinP(Float_t p);
  inline Int_t getBinTheta(Float_t theta);
  inline Int_t getBinPhi(Float_t phi);
protected:
  UInt_t fSize;
...
Float_t HRtEfficiencyMatrix::getP(UInt_t binP) { 
  return fMinP + binP*fBinSizeP + fBinSizeP/2.;
}

Float_t HRtEfficiencyMatrix::getTheta(UInt_t binTheta) { 
  return fMinTheta + binTheta*fBinSizeTheta + fBinSizeTheta/2.;
}

Float_t HRtEfficiencyMatrix::getPhi(UInt_t binPhi) { 
  return fMinPhi + binPhi*fBinSizePhi + fBinSizePhi/2.;
}

Int_t HRtEfficiencyMatrix::getBinP(Float_t p) { 
  return int(floor((p - fMinP) /  fBinSizeP)); 
}

Int_t HRtEfficiencyMatrix::getBinTheta(Float_t theta) { 
  return int(floor((theta - fMinTheta)) / fBinSizeTheta); 
}

Int_t HRtEfficiencyMatrix::getBinPhi(Float_t phi) { 
