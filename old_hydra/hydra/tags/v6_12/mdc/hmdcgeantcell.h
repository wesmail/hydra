...
private:
  ...
  Float_t fMinDist[15];     // Shortest distance from particle trajectory to the sense wire in mm
  Float_t fTimeFlight[15];  // Time of flight in ns
  Float_t fImpactAngle[15]; // impact angle in Coordinate system of the cell in degree
  Int_t nTrack[15];         // Track number
  Bool_t flagCutEdge[15];   // flag for cell where the minimum distance was recalculated
  Float_t wireOffset[15];   // offset of signal due to propagation of the signal on the wire in ns
public:
   HMdcGeantCell() {clear();}
  ~HMdcGeantCell() {}
   void clear();
