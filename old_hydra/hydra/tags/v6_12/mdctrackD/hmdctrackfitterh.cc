...
void HMdcTrackFitterH::setParam(void) {
  ...
  if(mdc>=0) {
    ...
    fitPar->getWeights(sector,mdc,weightPar);
    stepNorm =  fitPar->getStepNorm(sector,mdc);
    ...
    if(segment==0) {
      ...
      fitPar->getWeightsSeg1(sector,weightPar);
      stepNorm =  fitPar->getStepNormSeg1(sector);
      ...
      fitPar->getWeightsSeg2(sector,weightPar);
      stepNorm =  fitPar->getStepNormSeg2(sector);
