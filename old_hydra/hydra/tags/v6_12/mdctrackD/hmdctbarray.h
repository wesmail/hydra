...
Int_t HMdcTBArray::getNSet(const UChar_t *pF, const UChar_t *pL) {
  Int_t nTBits=0;
...
Int_t HMdcTBArray::position(const UChar_t *pF, const UChar_t *pL, Int_t idx) {
  // return: >=0 - ok.!;
  if(idx<0) return -1;
...
Int_t HMdcTBArray::first(const UChar_t *pF, const UChar_t *pL) {
...
Int_t HMdcTBArray::last(const UChar_t *pF, const UChar_t *pL) {
...
UChar_t HMdcTBArray::get(const UChar_t *pF, Int_t pos) {
...
Int_t HMdcBArray::getNSet(const UChar_t *pF, const UChar_t *pL) {
  Int_t nBits=0;
  while(pF <= pL) nBits+=nBitLookUp[*(pF++)];
  return nBits;
}

Int_t HMdcBArray::getNSet(const UChar_t *pF) {
  return nBitLookUp[*pF];
}

Int_t HMdcBArray::getNSet(const UChar_t byte) {
  return nBitLookUp[byte];
}

Int_t HMdcBArray::getNSet2B(const UChar_t *pF) {
...
Int_t HMdcBArray::position(const UChar_t *pF, const UChar_t *pL, Int_t idx) {
  // return: >=0 - ok.!;
  if(idx<0) return -1;
  for(const UChar_t *pB=pF; pB<=pL; pB++) {
...
Int_t HMdcBArray::first(const UChar_t *pF, const UChar_t *pL) {
  // return: -1 - num.cells=0
  for(const UChar_t *pB=pF; pB<=pL; pB++) {
...
Int_t HMdcBArray::last(const UChar_t *pF, const UChar_t *pL) {
    // return: >=0 - ok.!; -2 - num. of cells =0
  for(const UChar_t *pB=pL; pB>=pF; pB--) {
...
Int_t HMdcBArray::next(const UChar_t *pF, const UChar_t *pL, Int_t prPos) {
  // prPos <0 eq. first
  // return: -1 next is absent
  const UChar_t *pB=pF;
  UChar_t next=*pB;
...
Int_t HMdcBArray::previous(const UChar_t *pF, const UChar_t *pL, Int_t prPos) {
...
Int_t HMdcBArray::prevAndUnset(UChar_t *pF, UChar_t *pL, Int_t prPos) {
...
UChar_t HMdcBArray::get(const UChar_t *pF, Int_t pos) {
