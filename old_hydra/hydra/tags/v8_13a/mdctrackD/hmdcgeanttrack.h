...
protected:
  ...
  Char_t     ioseg;           // ...
  Char_t     mod;             // mod = -1 for two mdc segment
...
public:
  ...
  Bool_t getLayerHitPosLab(Int_t m,Int_t l, HGeomVector& hit,
                           Bool_t extrFlag=kTRUE);
  Bool_t geantClustPos(HMdcClusSim* pMdcClusSim, HMdcList12GroupCells* listIden,
                       Int_t indtr,Float_t& x, Float_t& y);
private:
...
protected:
  ...
  Int_t           arrOffset;      // offset of the first this track segment in array
...
//  void   setMdcEvntListCells(HMdcEvntListCells* pt=0);
  ...
  HMdcGeantSeg* getSegment(Int_t ns) {
    return nSegOk(ns) ? (HMdcGeantSeg*)segments->At(ns+arrOffset):0;}
  ...
  Bool_t  getLayerHitPosLab(Int_t ns, Int_t m, Int_t l, HGeomVector& hit,
                            Bool_t extrFlag=kTRUE);
//  Bool_t  geantClustPos(Int_t ns, HMdcClusSim* pMdcClusSim, Int_t indtr,
//                        Float_t& x, Float_t& y);
  void    print(void);
...
public:
  ...
  UChar_t getNumLayers(Int_t trInd, Int_t modi=-1) const;
  HMdcList12GroupCells* getListCells(Int_t trInd);
  HMdcGeantSeg*         getMdcGeantSeg(Int_t trInd);
  
private:
  Bool_t isTrIndOk(Int_t ind) {return (ind>=0 && ind<numTracks) ? kTRUE:kFALSE;}
