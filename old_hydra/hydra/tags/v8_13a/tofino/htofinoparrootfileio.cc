...
Bool_t HTofinoParRootFileIo::init(HParSet* pPar,Int_t* set) {
  ...
  if (pFile) {
//    if (strncmp(name,"TofinoCalPar",strlen("TofinoCalPar"))==0) return read((HTofinoCalPar*)pPar,set);
    ...
    if (strcmp(name,"TofinoGeomPar")==0) return HDetParRootFileIo::read((HTofinoGeomPar*)pPar,set);
    if (strncmp(name,"TofinoDigitPar",strlen("TofinoDigitPar"))==0) return HDetParRootFileIo::read(pPar);
  }
...
Bool_t HTofinoParRootFileIo::read(HTofinoCalPar* pPar,Int_t* set) {
  printf("Container TofinoCalPar initialized from the %d ROOT input\n",inputNumber); 
...
Bool_t HTofinoParRootFileIo::read(HTofinoShowerMap* pPar,Int_t* set) {
  if (!readObject(pPar)) return kFALSE;
  printf("Container TofinoShowerMap initialized from the %d ROOT input\n",inputNumber); 
  return kTRUE;
}
...
Bool_t HTofinoParRootFileIo::read(HTofinoLookup* pPar,Int_t* set) {
  ...
  if (version<=0) 
  { 
   printf("Container %s not found in RuntimeDB in the %d input\n",name,inputNumber);
   pPar->setInputVersion(-1,inputNumber);  
   return kFALSE;}    // not in ROOT file
  if (pPar->getInputVersion(inputNumber)==version
        && pPar->getInputVersion(inputNumber)!=-1) return kTRUE;
  ...
  pPar->setInputVersion(version,inputNumber);
  pPar->setChanged();
  //printInfo("TofinoLookup: module(s) initialized from ROOT input");
  printf("TofinoLookup: module(s) initialized from the %d ROOT input",inputNumber);
  delete rTofino;
