...
protected:
  class HGeomCopyNode : public TNamed {
    public:
      HGeomNode* pNode;
      HGeomCopyNode(const char* name,HGeomNode* node) {
        SetName(name);
        pNode=node;
      }
      ~HGeomCopyNode() {}
...
public :
  ...
  HGeomNode* getVolume(const char* name) {return (HGeomNode*)volumes->FindObject(name);}
  HGeomNode* getMasterNode(const char* name) {return (HGeomNode*)masterNodes->FindObject(name);}
