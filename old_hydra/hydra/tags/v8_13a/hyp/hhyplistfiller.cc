...
Bool_t HHypListFiller::init()
{
  ...
  flagDoFilter = true;          // this should be a option...
  ...
  if (m_pContItPart == NULL) {
    Error("init", "Cannot get catPidPart Iterator");
    return kFALSE;
  }
  //-----------------------------------------------------------------------
  return kTRUE;
...
Int_t HHypListFiller::execute()
{
  ...
  while (((pidPart = (HPidParticle *) m_pContItPart->Next()) != NULL)
         && (counter < HYP_MAX_PARTICLES) && (iCount < HYP_MAX_PARTICLES)) {
    if (!is_valid[iCount++])
      continue;
      ...
      if (pidPart->getCharge() == -1)   // set Idx for negative particle
      {
        for (Int_t i = 0; i < numberOfCombinations; i++) {
          if (pid_full_array[i][(numberOfParticles - 1) - iPosMinusF[i]] !=
            ...
            for (Int_t k = 0; k < numberOfPossibleMissing; k++) {
              hypcomb->setIdxPidPart(i + k * numberOfCombinations,
      ...
      } else if (pidPart->getCharge() == 1)     // set Idx for positive particle
      {
        for (Int_t i = 0; i < numberOfCombinations; i++) {
          if (pid_full_array[i][iPosPlusF[i]] !=
            ...
            for (Int_t k = 0; k < numberOfPossibleMissing; k++) {
              hypcomb->setIdxPidPart(i + k * numberOfCombinations, iPosPlus[i],
                                     idx);
            }
            iPosPlus[i]++;
          }
...
{
//  HPidTrackCand  *pTrack = NULL;        // track candidate
  HPidHitData const *pHitData = NULL;   // hit data
  HPidTrackData const *pTrackData = NULL;       // track candidate data

//  HIterator      *pitPidTrackCandIter =
//    (HIterator *) catlTrackCand->MakeIterator();

  Int_t counter, counter1 = 0, counter2 = 0;

  Int_t innerIndex[HYP_MAX_PARTICLES];
  Int_t outerIndex[HYP_MAX_PARTICLES];
  Int_t tofIndex[HYP_MAX_PARTICLES];
  Int_t showerIndex[HYP_MAX_PARTICLES];

  Bool_t metaFake[HYP_MAX_PARTICLES];
  Bool_t trackFake[HYP_MAX_PARTICLES];

  Float_t theta[HYP_MAX_PARTICLES];
  Float_t phi[HYP_MAX_PARTICLES];
  Float_t QTOF[HYP_MAX_PARTICLES];
  Float_t MomRunge[HYP_MAX_PARTICLES];
  Float_t PolRunge[HYP_MAX_PARTICLES];
  Float_t BetaRunge[HYP_MAX_PARTICLES];
  Float_t chiRunge[HYP_MAX_PARTICLES];
  Float_t tof[HYP_MAX_PARTICLES];

//  cout << "*** This is Trackfilter ***" <<endl;

  counter = m_pContCatPart->getEntries();

//  cout << counter <<" PidParticles found" <<endl;
  ...
  HPidParticle   *pidPart = NULL;
  Int_t partIndex[HYP_MAX_PARTICLES];

  Int_t iCount = 0;

  m_pContItPart->Reset();
  while ((pidPart = (HPidParticle *) m_pContItPart->Next()) != NULL) {  //iteration over pidParticle
    pHitData = pidPart->getHitData();   // hit information
    pTrackData = pidPart->getTrackData();       // track canidate
    iCount++;
    ...
    if (!pTrackData) {
      cout << "reject " << iCount -
        1 << " because of missing TrackData" << endl;
      continue;
    }

    if ((pHitData->iSystem) < 0) {
      cout << "reject " << iCount -
        1 << " because of pHitData->iSystem) < 0" << endl;
      continue;
    }
    if (!pTrackData->bIsAccepted[2]) {
      cout << "reject " << iCount -
        1 << " because of !pTrackData->bIsAccepted[2]" << endl;
      continue;
    }
    if (!pTrackData->bIsAccepted[4]) {
      cout << "reject " << iCount -
        1 << " because of !pTrackData->bIsAccepted[4]" << endl;
      continue;
    }
/*    if (pHitData->fInnerMdcChiSquare < 0 || pHitData->fOuterMdcChiSquare < 0){
    ...
    if (pTrackData->getRKChiSquare() > 1000) {
      cout << "reject " << iCount -
        1 << " because of frunge->getChiq() > 1000" << endl;
      continue;
    }
    chiRunge[counter1] = pTrackData->getRKChiSquare();

    MomRunge[counter1] = pTrackData->fMomenta[4];
    PolRunge[counter1] = pTrackData->nPolarity[4];

    BetaRunge[counter1] = pTrackData->getBeta(4);
...
//  cout << counter1 << " have valid track information" <<endl;
