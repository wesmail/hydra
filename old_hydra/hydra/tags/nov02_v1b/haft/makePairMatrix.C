...
void makePairMatrix(Text_t *inName, Text_t *outName,
  ...
  TH3F *pairAcceptance = new TH3F(outName,comment,mchan,ml,mu,ptchan,ptl,ptu,ychan,yl,yu);
  ...
  Int_t mode = 1;              // interpolation mode
  const Int_t phibins = 30;    // nb. of phi bins
  const Int_t samples = 400;   // nb. of decay samples
  const Float_t openCut = 9.0; // pair opening angle cut
  const Float_t momcut = 0.1;  // momentum cut at 100 MeV/c
...
//
  ...
  for(Int_t i=n; i<xchan-n; i++) {    // clip and copy back filtered data
    for(Int_t j=n; j<ychan-n; j++) {
      for(Int_t k=n; k<zchan-n; k++) {
        data[i*yzchan+j*zchan+k] = TMath::Max(TMath::Min(work[i][j][k],1.0),0.);
      }
    }
  }
