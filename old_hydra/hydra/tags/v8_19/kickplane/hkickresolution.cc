...
Int_t HKickResolutionMdc2::execute(void) {
  ...
  Float_t Px=0,Py=0,Pz=0;
  Double_t dist=0.;
  Int_t count[4]; //counts per module
  ...
  while ( (data=(HGeantMdc *)fIter->Next()) != 0) {
    if (data->getLayer() == 6) {
      if (count[(Int_t)data->getModule()]<1) {
        ...
        if (data->getModule() == 0 || data->getModule()==2) { //Modules 0,2
  ...
  if (count[0]==1 && count[1]==1 && count[2]==1 && count[3]==1 ) {
    ...
    case kMdcTofMode:
      ...
      //so I don't need to care if there is a Double_t hit (cluster) in TOF
...
*/
    ...
    if (fData) fData->Fill(tuple_args);
     ...
     //Warning("execute","Bad event %i %i %i %i",count[0],count[1],count[2],
     //    count[3]);
  }
  return 0;
}
