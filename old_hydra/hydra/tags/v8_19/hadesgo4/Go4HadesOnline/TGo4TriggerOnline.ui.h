...
void TGo4TriggerOnline::init()
{
  ...
  fxTGo4GUIRegistry->ReDirectObject("hTriggerMUCorrAll"                ,fxTGo4TriggerOnlineSlots ,true,fxQtCanvasMuCorrAll);
}
...
void TGo4TriggerOnline::destroy()
{
  ...
  fxTGo4GUIRegistry->RemoveRedirectEntry("hTriggerMUCorrAll"               ,fxTGo4TriggerOnlineSlots);
}
...
void TGo4TriggerOnline::checkNaming( HMonHistAddon *His )
{
  if     ( (strcmp(His->GetName(),"hTriggerIPUtrendTof"       )==0) && (tofArraySetting       ==kFALSE) ) { histNaming( His ); tofArraySetting       =kTRUE; }
   ...
   else if( (strcmp(His->GetName(),"hTriggerMUCorrAll"        )==0)&& (muCorrAll== kFALSE)) {histNaming(His); muCorrAll= kTRUE;}
...
//--------------------------------------------------------------------------------------------------------
void TGo4TriggerOnline::downscaled( Int_t index )
{
...
void TGo4TriggerOnline::drawTimeTab( HMonHistAddon *His )
{
  if(strcmp(His->GetName(),"hTriggerIPUButterfly")==0)
	...
	else if(strcmp(His->GetName(),"hTriggerMUCorrAll")==0)
    {
      fxQtCanvasMuCorrAll->getCanvas()->cd();
      His ->getP()->Draw() ;
      fxQtCanvasMuCorrAll->getCanvas()->Update();
    }
}

//--------------------------------------------------------------------------------------------------------
void TGo4TriggerOnline::drawSubTab( Int_t forceIpu, HMonHistAddon *His )
{
  Int_t ipu = -1;
...
//--------------------------------------------------------------------------------------------------------
void TGo4TriggerOnline::drawSubTabTime(  Int_t forceIpu, HMonHistAddon *His )
{
  Int_t ipu = -1;
...
void TGo4TriggerOnline::initCanvasColor( Short_t color )
{
  //------------------ Canvas color
...
void TGo4TriggerOnline::drawSubTabCorr( Int_t forceIpu, HMonHistAddon *His )
{
  ...
  {ipu_p=2;}
  else if((strcmp(TabWidgetMatchU->currentPage()->name(),"MU_corr_phi")==0) && (strcmp(His->GetName(),"hTriggerMUCorrPhi")==0))
  {ipu_p=3;}  
