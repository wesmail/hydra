...
void TGo4HadesConfig::init()
{
    ...
    //-------------------------checkboxes of hodo detector setup
    hododetSetup[0]=hodomod0;
    hododetSetup[1]=hodomod1;
    hododetSetup[2]=hodomod2;
    hododetSetup[3]=hodomod3;
}
...
void TGo4HadesConfig::UpdateParam( TGo4Parameter * par )
{
  ...
  hodorefreshspin    ->setValue(param->getHodoRefresh());
  wallrefreshspin    ->setValue(param->getWallRefresh());
  ...
  hodocheck          ->setChecked((bool)param->getHodo());
  wallcheck          ->setChecked((bool)param->getWall());
  ...
  hodobox             ->setCurrentItem(param->getHodoMaxLevel()-1);
  // start detector setup
  TArrayI* hododetsetup=param->getHodoDetSetup();
  for(Int_t m=0;m<4;m++){
      hododetSetup[m]->setChecked((bool)(*hododetsetup)[m]);
  }

  wallbox            ->setCurrentItem(param->getWallMaxLevel()-1);
  ...
  wallbox   ->removeItem(3);
  hodobox   ->removeItem(3);
...
void TGo4HadesConfig::writeParams( HGo4OnlineParam * p, HMdcSetup * p2 )
{
    ...
    p->setHodoRefresh   (hodorefreshspin->value());
    p->setWallRefresh   (wallrefreshspin->value());
    ...
    p->setStartMaxLevel (startbox->currentItem()+1);
	 ...
	 p->setHodo          (hodocheck->isChecked());

    TArrayI* hododetsetup=p->getHodoDetSetup();
    for(Int_t m=0;m<4;m++){
	(*hododetsetup)[m]=(Int_t)hododetSetup[m]->isChecked();
    }
    
	 p->setHodoMaxLevel  (hodobox->currentItem()+1);

    p->setWall          (wallcheck->isChecked());
    p->setWallMaxLevel  (wallbox->currentItem()+1);
