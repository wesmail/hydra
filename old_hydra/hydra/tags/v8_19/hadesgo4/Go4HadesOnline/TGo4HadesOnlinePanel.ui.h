    ...
    /*
      ...
      */
      ...
      "$HADDIR/lib/libHadesGo4.so",
       "$HADDIR/hadesgo4/Go4Library/libGo4HadesClient.so",
 };

  printf("\nrootlogon\nloading shared libraries...\n\n");
  for(Int_t i=0;i<4;i++) 
  {
      Char_t *path = gSystem->ExpandPathName(libFiles[i]);
...
void TGo4HadesOnlinePanel::DrawLCDEventRate( Double_t rate )
{
...
void TGo4HadesOnlinePanel::openWall()
{
    TGo4LockGuard glob;
    fxTGo4WallOnlineSlots =(TGo4WallOnlineSlots *) fxTGo4GUIRegistry->GetSlotClass("TGo4WallOnlineSlots");
    if (!fxTGo4WallOnlineSlots){
        fxTGo4WallOnline =new  TGo4WallOnline();
        fxTGo4WallOnline->show();
    }else{
        fxTGo4WallOnline= fxTGo4WallOnlineSlots->GetGUI();
        if (fxTGo4WallOnline!=0){
            fxTGo4WallOnline->show();
        }else{
            fxTGo4WallOnline =new  TGo4WallOnline();
            fxTGo4WallOnline->show();
