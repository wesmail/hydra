...
// the class HSlice.
// This is a GUI class which allows to make slicing of 2dim histogrammes.
//
//-------------------------------------------------------------------------
using namespace std;
#include "hslice.h"
...
//////////////////////////////////////////////////////////////////////////
//
// HSlice is a GUI class which allows to make slicing of 2dim histogrammes.
//
//////////////////////////////////////////////////////////////////////////
...
fSave = new TGTextButton(fF1, "Save in ascii", M_SAVE);
fSave->Associate(this);
fSave->SetToolTipText("Save this 1d histogram in ascii file");
fF1->AddFrame(fSave, new TGLayoutHints(kLHintsNormal, 50,20, 1,2));
...
//******  HSlice::saveAscii()
//
// This function saves 1d histogram currently drawn in the canvas to an
// ascii file.
//
//---------------------------------------------------------------------

void HSlice::saveAscii() {
//
// This function saves 1d histogram currently drawn in the canvas to
// an ascii file.
//

  // check if a 1d histo is in the canvas

  TIterator *it;
  TH1D *histo = 0;
  TGFileInfo fi; // to save a picture
  Char_t savefilename[100];
  const Char_t *filetypes[] = { "All",     "*.*",
			      0,               0 };
  fi.fFileTypes = (const Char_t **)filetypes;  
  
  TList *pList = fSliceCanv->GetCanvas()->GetListOfPrimitives();
  if(!pList) {
    HError::message("List of primitives not found.");
    return;
  }
  ...
  while((select = it->Next()) != NULL) {
    ...
    if(!strcmp(select->ClassName(),"TH1D")) {
      histo = (TH1D*) select;
      break;
    }
  }
  ...
  if(!histo) {
    HError::message("No 1d histogram found in the canvas.");
    return;
  }

  new TGFileDialog(fClient->GetRoot(), this, kFDSave, &fi);
  sprintf(savefilename,"%s",fi.fFilename);
  if(!strcmp(savefilename,"(null)")) return;
  sprintf(savefilename,"%s",fi.fFilename);
  ...
  for(i=1; i<=nbins; i++) {
    ...
    content = (UShort_t) histo->GetBinContent(i);
    lowbin = histo->GetXaxis()->GetBinLowEdge(i);
    upbin = histo->GetXaxis()->GetBinUpEdge(i);

    fprintf(fp,"%.2f\t%.2f\t%hu\t%hu\n",lowbin,upbin,i,content);
  }
...
//
// This function sets entries of fOptionCB. If kClear is TRUE, 
// simply clears combobox.
//
//---------------------------------------------------------------------
...
//---------------------------------------------------------------------
//******  HSlice::setCursors(const Char_t* name)
//
// This function saves histogram cursors according to the sliders positions.
//
//---------------------------------------------------------------------

void HSlice::setCursors(const Char_t* name) {
//
// This function saves histogram cursors according to the sliders positions.
//
