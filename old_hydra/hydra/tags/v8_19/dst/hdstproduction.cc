 ...
 * 
 * $Id: hdstproduction.cc,v 1.34 2009-02-04 17:41:07 jurkovic Exp $
 *
 */
  ...
  EvalOptions:
   ...
   while (1)
   {
      option = getopt_long( current_argc, current_argv,
			    "1:2:c:d:D:e:f:hi:I:n:o:O:p:q:r:s:t:",
	 ...
	 terminate( 3 );
	  ...
	  if(dstCfg.historyDate.Contains("-") != 0 || dstCfg.historyDate.CompareTo("now") == 0)
	  {
		  ...
		  Error( "setupParameterSources",
			"historyDate contains \"-\", but is no date format : %s !",dstCfg.historyDate.Data() );
		  terminate( 3 );
	      }
	  } else {
	      ...
	      {
		  Error( "setupParameterSources",
			"historyDate contains no \"-\", but is no valid param release : %s !",dstCfg.historyDate.Data() );
		  terminate( 3 );
	      }
	  }
      }
...
void HDstProduction::setupRootInput()
{
   ...
   if ( dstCfg.dstId.Contains( "EMBED", TString::kIgnoreCase ) ) 
   {
   ...
   while ((category_name = dynamic_cast<TObjString*>(element->Next())))
   {
	 ...
	 source->disableCategory( catGeantHeader );
      ...
      {
	 source->disableCategory( catGeantKine );
      }
	 ...
	 source->disableCategory( catRichGeantRaw );
      ...
      else if (name.CompareTo( "catRichGeantDirect", TString::kIgnoreCase ) == 0)
      {
	 source->disableCategory( catRichGeantRaw + 1 );
      }
      else if (name.CompareTo( "catRichGeantMirror", TString::kIgnoreCase ) == 0)
      {
	 source->disableCategory( catRichGeantRaw + 2 );
      }
	 ...
	 source->disableCategory( catMdcCal2 );
      ...
      {
	 source->disableCategory( catMdcGeantCell );
      }
	 ...
	 source->disableCategory( catShowerCal );
      ...
      {
	 source->disableCategory( catShowerGeantWire );
      }
	 ...
	 source->disableCategory( catShowerPID );
      ...
      {
	 source->disableCategory( catShowerTrack );
      }
      else if (name.CompareTo( "catShowerRawMatr", TString::kIgnoreCase ) == 0)
      {
	 source->disableCategory( catShowerRawMatr );
      }
	 ...
	 source->disableCategory( catShowerHitHdr );
      ...
      {
	 source->disableCategory( catShowerHitTrack );
      }
      else if (name.CompareTo( "catShowerPIDTrack", TString::kIgnoreCase ) == 0)
      {
	 source->disableCategory( catShowerPIDTrack );
      }
...
void HDstProduction::setupOutput()
{
   ...
   while ((category_name = dynamic_cast<TObjString*>(element->Next())))
   {
	 ...
	 category = gHades->getCurrentEvent()->getCategory( catPidPoorCandidate );
      ...
      {
	 category = gHades->getCurrentEvent()->getCategory( catPidEvtInfo );
      }
      else if (name.CompareTo( "catRichRaw", TString::kIgnoreCase ) == 0)
      {
...
void HDstProduction::printConfiguration()
{
   ...
   cout << "==> Disabled Categories .............: "
	<< (dstCfg.disabledCategories.IsNull()  ?  "none"  :
	    dstCfg.disabledCategories) << endl;
   cout << endl;
