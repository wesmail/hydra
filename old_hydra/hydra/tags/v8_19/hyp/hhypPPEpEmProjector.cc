      ...
      em_DistVertex=-1;

		{
			...
			if (pidTrackCandCat != NULL) {
				...
				pPidTrackCand =(HPidTrackCand *) pidTrackCandCat->getObject(mylist->getIdxPidTrackCand("e+", 1));
...
/*					if(ep_MdcR!=pPidTrackCand->getR() || ep_MdcZ!=pPidTrackCand->getZ() ){
					...
					Error("execute","no e+ pPidTrackCand");
				}
				pPidTrackCand =(HPidTrackCand *) pidTrackCandCat->getObject(mylist->getIdxPidTrackCand("e-", 1));
				if (pPidTrackCand != NULL) {
					HPidHitData const *hit;
...
/*					if(em_MdcR!=pPidTrackCand->getR() || em_MdcZ!=pPidTrackCand->getZ() ){
				...
				pPidTrackCand =(HPidTrackCand *) pidTrackCandCat->getObject(mylist->getIdxPidTrackCand("p", 1));
				if (pPidTrackCand != NULL) {
					HPidHitData const *hit;
					hit=pPidTrackCand->getHitData();
					if(hit){
...
/*					if(p1_MdcR!=pPidTrackCand->getR() || p1_MdcZ!=pPidTrackCand->getZ() ){
					...
					Error("execute","no p pPidTrackCand");
				}

				pPidTrackCand =(HPidTrackCand *) pidTrackCandCat->getObject(mylist->getIdxPidTrackCand("p", 2));
				if (pPidTrackCand != NULL) {
					HPidHitData const *hit;
					hit=pPidTrackCand->getHitData();
					if(hit){
...
/*					if(p2_MdcR!=pPidTrackCand->getR() || p2_MdcZ!=pPidTrackCand->getZ() ){
				...
				Error("execute","no pidTrackCandCat");
			}
		}
		...
		if (mylist->getIterStatus() == kTRUE) {
			...
			if(nt_dtof_refit){
				Float_t dtof, dtof2, kine_chi2, kine_prechi2;
				Float_t pid_tracks;

				if (!mylist->getUserValue(DELTATOF_DTOF, dtof))
					dtof = -1;
				if (!mylist->getUserValue(DELTATOF_DTOF2, dtof2))
					dtof2 = -1;
         ...
         if(nt_mdc_and_vertex){
			...
			if (simuflag != 0){
				...
				temp[tmp_cnt++]=event_genweight;// geant/pluto weight
				if( nt_full_geant){
...
Bool_t HHypPPEpEmProjector::init()
{
  ...
  TString st_dtof_kine(":dtof:dtof2:pidtr:kine_chi2:kine_prechi2:fakes:ep_in_chi2:em_in_chi2");
  ...
  if (simuflag != 0){
    ...
    TString st_base_geant(":ep_id:em_id:p1_id:p2_id"
			  ":ep_geninfo:em_geninfo:p1_geninfo:p2_geninfo:weight");
