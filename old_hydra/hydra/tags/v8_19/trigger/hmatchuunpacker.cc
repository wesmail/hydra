...
//*-- Version: $Id: hmatchuunpacker.cc,v 1.43 2008-05-09 16:18:42 halo Exp $
...
HMatchUUnpacker::HMatchUUnpacker(Int_t id, Int_t units, Bool_t compression)
  : HldUnpack()
{
  // Default constructor
  //
  ...
  : HldUnpack()
{
  ...
  INFO_msg(10,HMessageMgr::DET_TRIGGER,"Called depreciated constructor, use HMatchUUnpacker(Int_t id, Int_t units, Bool_t compression) instead!!");
  setDefaults();
  fUnit = unites;
  fLog =flog;
  isCompressed = compression;
  ...
  if(gHades)
  {
    ...
    if (event)
    {
      ...
      //-------------------------------------
        ...
        //fTrigSetup = (HTriggerSetup *)rtdb->getContainer("TriggerSetup");
// 				if(!fTrigSetup)
// 					{
...
Int_t HMatchUUnpacker::mapAngles(void)
{
  ...
  while (( pMULept = (HMULeptons *)iterLepton->Next()) != 0)
  {
    ...
    {
      gHades->getMsg()->debug(10,HMessageMgr::DET_TRIGGER,GetName(),"Bug in Meta Number! %i %i %i %i %i !!!!!",pMULept->getDetBit(), pMULept->getMetaNr(),nShower[sec],pMULept->getMetaNr(),nTof[sec]);
      continue;
    }
  }
  if (fDebugMode)
