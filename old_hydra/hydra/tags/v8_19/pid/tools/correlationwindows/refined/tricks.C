TH1F* signal(Float_t threshold,Float_t lowerp, Float_t upperp, Float_t& smallsig, Float_t& largesig, Float_t& chisquare, Float_t fixedwidth=0.0)
{
  ...
  sprintf(draw,"%s%f%s%f","f_track_theta>25 &&(i_geant_corrflag>=76 ||i_geant_corrflag==12 || i_geant_corrflag==14)&& b_corrRingKick==1 && f_kick_mom_exp>",lowerp,"&&f_kick_mom_exp<",upperp);
...
TH1F* background(Float_t threshold,Float_t lowerp, Float_t upperp)
{
  TH1F* dAngle_bg = new TH1F("dAngle_bg","dAngle_bg",200,-10,10);
  Char_t draw[256];
  sprintf(draw,"%s%f%s%f"," f_track_theta>25 &&!(i_geant_corrflag==68 || i_geant_corrflag==70 ||i_geant_corrflag>=76 ||i_geant_corrflag==12 || i_geant_corrflag==14)&& b_corrRingKick==1 && f_kick_mom_exp>",lowerp,"&&f_kick_mom_exp<",upperp);
  TString* Draw= new TString(draw);
...
TH1F* total(Float_t threshold,Float_t lowerp, Float_t upperp, Float_t& smallsig, Float_t& largesig,Float_t& chisquare, Float_t fixedwidth=0.0,Int_t sector=-1)
{
  ...
  TNtuple* N = (TNtuple*)gDirectory->FindObjectAny("N");
  Char_t draw[256];
  sprintf(draw,"%s%f%s%f"," f_track_theta>25 &&b_corrRingKick==1 && f_kick_mom_exp>",lowerp,"&&f_kick_mom_exp<",upperp);
  TString* Draw= new TString(draw);
