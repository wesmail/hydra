...
Int_t HMdcClusterToHit::execute(void) {
  ...
  while ((fClst=(HMdcClus*)iter->Next())!=0) {
    clusIndex = -1; //fClustCat->getIndex(fClst);
...
void HMdcClusterToHit::printStatus(void) {
  ...
  Char_t* hFSeg1a ="-1 = don't fill HMdcSeg for segment 1,";
  Char_t* hFSeg1b =" 0 = fill HMdcSeg by cluster in MDC1 if typeClFinder=1";
  Char_t* hFSeg1c ="     and sector has mdc1&2, else by cluster in existing MDC,";
  Char_t* hFSeg1d =" 1 =  -/- in mdc2";
  Char_t* hFSeg2a ="-1 = don't fill HMdcSeg for segment 2,";
  Char_t* hFSeg2b =" 2 = fill HMdcSeg by cluster in MDC3 if typeClFinder=1";
  Char_t* hFSeg2c ="     and sector has mdc3&4 and MagnetOff,";
  Char_t* hFSeg2d ="     else by cluster in existing MDC,";
  Char_t* hFSeg2e =" 3 =  -/- in MDC4";
  printf("--------------------------------------------------------------------------------------------\n");
