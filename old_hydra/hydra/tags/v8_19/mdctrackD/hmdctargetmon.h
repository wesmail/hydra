...
class HMdcTargetMon : public HReconstructor {
  protected:
    ...
    HGeomVector        target;                 // target middle point
    Bool_t             useRtdbTarger;          // target param from Rtdb
    ...
    Double_t           phi[24][500];           // phi(deg.) for each hit in lab.
    Double_t           x[24][500];             // x(mm) hits positions in lab.
    Double_t           y[24][500];             // y(mm) hits positions in lab.
    Double_t           z[24][500];             // z(mm) hits positions in lab.
    ...
    Double_t           leftTargetPoint;        // Z region of target
  ...
  public:
    ...
    void   addToCanvasName(Char_t* add)       {addCnName = add;}
    TH1F*  setTargetZRegion(Double_t lpoint,Double_t rpoint,Int_t nRuns=100);
    void   setCanvasSize(Int_t ww, Int_t wh);
    void   setPathOneRunHits(Char_t* p)       {pathHistOneRun = p;}
    void   setPathSumRunsHits(Char_t* p)      {pathHistSumRun = p;}
    void   setConvertFormat(Char_t* p,Bool_t delEps=kFALSE);
