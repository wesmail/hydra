// @(#)$Id: hpidtrackfiller.cc,v 1.39 2007-10-10 08:18:00 kempter Exp $
...
// SKIPNOLEPTON - (default kFALSE) skip the event when there is no matchig RICH with track
// SKIPTOFINOMULT - (default kFALSE) skip the track when tofino mult. > 1
// NOCHI2SEG0 -  (default kFALSE) do not remove track with chi^2<0 from seg0
// NOCHI2SEG1 -  (default kFALSE) do not remove track with chi^2<0 from seg1
// SKIPNOOUTERSEGANDNOMETA (default kFALSE) - do not create candidate if outer seg and Meta hit are missing (-> only inner seg)
...
Int_t HPidTrackFiller::execute(void)
{
  ...
  while((pMatch = (HMetaMatch *)pitMatch->Next()) != NULL)
      ...
      for(Int_t i=0;i<MAX_MOM_ALGS;i++)
	{
      ...
      //-----------------------------------------------
      //-----------------------------------------------
      // skip/do not skip candidates which have
      // no outer segment and no meta hit (close pair candidates)
	  ...
	  if(NULL != pTrkCand){
	      // check  for no outer segment and
	      // no meta hit (close pair candidates)
      ...
      // select candidates where at least one
      // tracking alg has been successful. For Spline/RK this could
      // be inner/outer seg + meta where the tof clusterfinder selected one
      // out of two tof hits (the other one is marked !XXXAcceped() )
      if(pMatch->isKickAccepted() || pMatch->isSplineAccepted() || pMatch->isRungeKuttaAccepted()){
	   isMomAccepted = kTRUE;
      }
      // skip the not tracked candidates but
      // keep single inner MDC segs (close pair candidates)
      if(!isCPCand && !isMomAccepted )          { continue; }

      // if selected skip the close pair candates
      // (default == do not skip)
...
// set default values inside the construtors
  ...
  bSkipNoOuterSegAndNoMeta = kFALSE; // By default we create candidates if outer seg and meta hit are missing
