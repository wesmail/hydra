...
Int_t HPidTofRec::execute(void){
    ...
    Bool_t  bIsRichRec [MAX_MOM_ALGS];
    Bool_t  bIsMinusRec[MAX_MOM_ALGS];
    Float_t fStartTime [MAX_MOM_ALGS];
    Float_t fRecTof    [MAX_MOM_ALGS];
    Float_t fRecStart  [MAX_MOM_ALGS];
    Short_t nIndex     [MAX_MOM_ALGS];
    Short_t nRecSys    [MAX_MOM_ALGS];
    Short_t nRecPol    [MAX_MOM_ALGS];
    Short_t nRecSec    [MAX_MOM_ALGS];
    Short_t nRecIndex  [MAX_MOM_ALGS];
    Bool_t bIsRec = kFALSE;

    for (Short_t nAlg = 0; nAlg < MAX_MOM_ALGS; nAlg ++)
    {
	...
	for (Int_t nAlg = 0; nAlg < MAX_MOM_ALGS; nAlg ++)           // loop over tracking algorythms
	{
		   ...
		   )
		{
			...
			if (bGoodRing) {
				...
				|| ( bIsRichRec[nAlg]                                                   &&   // priority on sectors with more MDC chambers
				    (nMdcSeg[pCand->getHitData()->nSector]   >  nMdcSeg[nRecSec[nAlg]])
				   )
				|| (  bIsRichRec[nAlg]                                                  &&   //  or if new one TOF, old one TOFINO
				     (nMdcSeg[pCand->getHitData()->nSector]  == nMdcSeg[nRecSec[nAlg]]) &&
				     (pCand->getHitData()->iSystem           == 1                       &&
				      nRecSys[nAlg]                          == 0
				     ) 
				   )
				|| (  bIsRichRec[nAlg] &&                                                    // or if new one pol==1 and old  pol ==1
				     (nMdcSeg[pCand->getHitData()->nSector]  == nMdcSeg[nRecSec[nAlg]]) &&
				     (pCand->getHitData()->iSystem           == nRecSys[nAlg]         ) &&
				     (pCand->getTrackData()->nPolarity[nAlg] == -1                      &&
				      nRecPol[nAlg]                          ==  1
				     )
				   )
				|| ( bIsRichRec[nAlg]                                                   &&   // faster particle
				    (nMdcSeg[pCand->getHitData()->nSector]   == nMdcSeg[nRecSec[nAlg]]) &&
				    (pCand->getHitData()->iSystem            == nRecSys[nAlg])          &&
				    (pCand->getTrackData()->nPolarity[nAlg]  == nRecPol[nAlg])          &&
				    (pCand->getHitData()->getTof()<fRecTof[nAlg])
				   ) 
			       )
			    {
				bIsRec           = kTRUE;
				...
				fRecStart [nAlg] = pCand->getHitData()->getTof()
			...
			if ( (  !bIsMinusRec[nAlg])                                                   // if there are no other negative charged trackss
			    || ( bIsMinusRec[nAlg]                                                 && // priority on sectors with more MDC chambers
				(nMdcSeg[pCand->getHitData()->nSector] > nMdcSeg[nRecSec[nAlg]])) 
			    || ( bIsMinusRec[nAlg]                                                 && //  or if new one TOF, old one TOFINO
				(nMdcSeg[pCand->getHitData()->nSector] == nMdcSeg[nRecSec[nAlg]])  &&
				(pCand->getHitData()->iSystem          == 1                        &&
				 nRecSys[nAlg]                         == 0)
			       ) 
			    || (bIsMinusRec[nAlg]                                                  && // faster particle
				(nMdcSeg[pCand->getHitData()->nSector] == nMdcSeg[nRecSec[nAlg]])  &&
				(pCand->getHitData()->iSystem          == nRecSys[nAlg])           &&
				(pCand->getHitData()->getTof()         <  fRecTof[nAlg]))
			   )  
	...
	for (Int_t nAlg = 0; nAlg < MAX_MOM_ALGS; nAlg ++)           // loop over tracking algorythms
	{
		   ...
		   )  
		{
			...
			if (bIsSimulation)
			{
				...
				(Float_t)(nRecIndex[nAlg]==nIndex[nAlg]),
	...
	HStartHit* pHit = (HStartHit*)pStartHitCat->getNewSlot(loc);
	if(pHit){
	    pHit = new (pHit) HStartHit;
	    pHit->setModule(0);
	    pHit->setStrip(0);
	    if(bIsRec){
		pHit->setTime(-fRecStart[nStartAlg]);
	    } else {
		pHit->setTime(0);
	    }
	    pHit->setStartMult(1);
