...
HMdcGarCal2Maker::HMdcGarCal2Maker(const char* name,const char* title)
    : TNamed(name,title)
{
...
void HMdcGarCal2Maker::findMax(Int_t m)
{
    ...
    for(Int_t a=0;a<18;a++)
    {
        // start finding spikes at 40% of the drift
        // cell size (10 bins = 1 mm)
        calcBounderies(m,a,&cellEdge,&maxDist);
	startBin=Int_t(cellEdge*0.4*10);
	...
	// Max2
	for(Int_t i=startBin;i<100;i++)
	{   // find max2 and max2bin
	    if(mtime1_err_corr[m][a][i]>max2[m][a] &&
	       mtime1_err_corr[m][a][i]<max1[m][a])
	    {
		max2   [m][a]=mtime1_err_corr[m][a][i];
		max2bin[m][a]=i;
	    }
	}
	for(Int_t i=max2bin[m][a];i<100;i++)
	{   // replace bins >max2bin with const value
...
void HMdcGarCal2Maker::smoothHist(TH1F* h,Int_t type)
{
    ...
    if(type==0) h->Smooth(getNTimes(type));
    if(type==1) h->Smooth(getNTimes(type));
    if(type==2) h->Smooth(getNTimes(type));
    //if(type==3) h->Smooth(getNTimes(type));
}
void HMdcGarCal2Maker::replaceBins(TH1F* h,Int_t m,Int_t a,Int_t type)
{
	...
	for(Int_t i=0;i<getMaxCopy();i++){// first bins
        ...
        for(Int_t i=max2bin[m][a];i<100;i++){// bins after max
