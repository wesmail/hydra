...
//*--- $Id: hmdc12fit.cc,v 1.62 2007-10-10 07:59:01 kempter Exp $
...
//   for(Int_t m=0;m<4;m++) if(fitter->nTimesInFit[m]>0) nmods++;
  ...
  if(fitSt) {
    fClusInf->setNLayersFitted(
      fitter->getWiresArr().getOutputListCells().getNLayersMod(
        fHit->getModule()));
    fClusInf->setNDrTimesFitted(
      fitter->getWiresArr().getOutputListCells().getNDrTimesMod(
        fHit->getModule()));
  }
  if(isGeant) {
