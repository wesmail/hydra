...
HParamFileGenerator::HParamFileGenerator(const char* beamtime,
...
HParRootFileIo* HParamFileGenerator::openParameterFile(const char* filename) {
  paramFilename=filename;
  if (!paramFilename.IsNull()) {
    HParRootFileIo* f=new HParRootFileIo;
    Bool_t rc=f->open((char*)filename,"CREATE");
    if (rc) {
      ...
      closeLogFile();
      logFile=new fstream();
      logFile->open((char*)logFilename.Data(),ios::out);
