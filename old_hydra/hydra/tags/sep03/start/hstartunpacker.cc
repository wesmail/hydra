            ...
            if( ((*data >>24) & 0x6)==0 ){
                ...
                if(uBoardType==1){
                    if( uStruck == SCALER ){
                        ...
                        data = fillScaler(data,&uTrigbits,kFALSE);
                    }
...
UInt_t* HStartUnpacker::fillScaler(UInt_t *pData,UInt_t *uTrigbits,Bool_t bOld){
...
#warning FIXME scaler trailer !!!!
        if( !bOld && (*(pData+uBlockSize-1)>>24)!=0x8){
            ...
            if(uWordCount!=(uBlockSize-2) && !bOld){
                Error("fillScaler",
...
#warning FIXME  latch trailer !!!!
        //if( (*(pData+uBlockSize-1))!=0x8000008 ){
