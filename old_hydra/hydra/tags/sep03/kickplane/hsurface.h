...
Int_t HSurfPlaneTriV::getParams(TArrayD& array) {
  array.AddAt(fDx,0);
  for (int i=0;i<3;i++) array.AddAt(fDz[i],i+1);
  array.AddAt(fC[0],4);
  return 5;
}
...
Bool_t HSurfPlaneTriV::setParams(TArrayD& array) {
  if (array.GetSize() == 5) {
    fDx = array.At(0);
    for (int i=0;i<3;i++) fDz[i] = array.At(i+1);
    fC[0] = array.At(4);
    calcC();
    return kTRUE;
  }
