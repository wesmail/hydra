...
Bool_t HKickTrackFSim::init(void) {
  ...
  if (gHades->getOutputFile() && fDebug) {
    ...
    fControl=new TNtuple("trackfsim_con","Control","p:preal:errP:valid:mass:xPull:x:y:z:qf:theta:phi:pid:den:s");
  }
  return kTRUE;
}
...
void HKickTrackFSim::fillControl(HKickCandidate &candidate, HKickImpact *imp) {
  ...
  fControl->Fill(candidate.getMomentum(),p,
		 ...
		 candidate.getMass(),
		 candidate.getXPull(),
		 parentTrack, //candidate.getPos1X(),
		 pmdc, //candidate.getPos1Y(),
		 candidate.getPos1Z(),
		 candidate.isAssigned()?1:0,
		 theta,
		 phi,
