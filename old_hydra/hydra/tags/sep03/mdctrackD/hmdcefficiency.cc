...
void HMdcEfficiencyLayer::creatCanvas(void) {
  ...
  if(nPads==0) return;
  canvEff=new TCanvas(nameCanv,titleCanv,1,1,
      700,(nPads>3) ? 900 : nPads*300);
  setPads();
}
...
void HMdcEfficiencyMod::setNameCanv(void){
  ...
  canvasMb=0;
  sprintf(buf,"%s_S%iM%iEffMb",fileName.Data(),sector+1,module+1);
  nameCanvMb=buf;
  sprintf(buf,"%s S%i M%i Cells efficiency",
      fileName.Data(),sector+1,module+1);
  titleCanvMb=buf;
  sprintf(nameSliderMb,"Sl_S%iM%iEffMb",sector+1,module+1);
  sprintf(buf,"%s_S%iM%iTimes",fileName.Data(),sector+1,module+1);
  nameCanvTimes=buf;
...
void HMdcEfficiencyMod::drawCanvTimes(Int_t bining) {
...
HMdcEfficiencySec::HMdcEfficiencySec(Short_t sec) {
  ...
  Char_t buf[200];
  sprintf(buf,"%s_S%iProjPl",fileName.Data(),sector+1);
  nameCanvProj=buf;
  sprintf(buf,"%s S%i Cells projections",fileName.Data(),sector+1);
  titleCanvProj=buf;
  sprintf(buf,"%s_S%iHitsPl",fileName.Data(),sector+1);
  nameCanvHits=buf;
...
Bool_t HMdcEfficiency::init(void) {
  ...
  if( !fLookUpTb || !fSizesCells || !fMdcDetector) return kFALSE;
...
Int_t HMdcEfficiency::execute(void) {
  ...
  while((fClus=(HMdcClus*)iterClus->Next())) {
    ...
    fsec.getPlotAllHits()->Fill(xCl,yCl);
    for(Int_t m=0; m<2; m++) {
      Bool_t qual=kFALSE;
