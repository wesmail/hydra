...
public:
  ...
  Bool_t isFake(void) const {return (nTrWiresClus<4) ? kTRUE:kFALSE;}
  Float_t getPurity(void) const  {return nTrWires ? 
      Float_t(nTrWires)/Float_t(numOfWires) : 0.;}
  Float_t getEfficiency(void) const {return nTrWires ? 
      Float_t(nTrWires)/Float_t(nTrWiresClus) : 0.;}
  virtual void print(void) const;
  virtual Bool_t isGeant() const {return kTRUE;}

  ClassDef(HMdcClusFitSim,2) // geant data for HMdcClusFit
};
