...
protected: 
 ...
 public:
  ...
  void setHitInd(Int_t i,Int_t idx) {
    if(idx > 8190 || i > 1){
    Warning("setHitInd","max hit= 8190, ind= %i mdc=0,1 ind= %i",idx,i);
    }
    else{
    ind = (((idx+1) & 0x1FFF) << (i*13)) | (ind & (~(0x1FFF << (i*13)))); 
    }
  }
  ...
  Float_t getZprime(void) {return zPrime;}
  Float_t getRprime(void) {return rPrime;}
  ...
  inline Int_t getHitInd(Int_t i); // Hit index (-1 --> No hit)

  
  ClassDef(HMdcSeg,2) // MDC segment
};
