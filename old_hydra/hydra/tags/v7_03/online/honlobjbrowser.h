...
class HOnlObjBrowser : public TGMainFrame {
 ...
 public:
  ...
  //add counters of all local conditions which have pSet and are present in
  //the list of pGC
  void addCounterPar(TGComboBox *p, HLocalCondition *pLC); //add counters
  // of local condition pLC
  const char* getBeforeUnderline(const char *word);
  const char* getAfterUnderline(const char *word);
 ...
 public:
  ...
  ClassDef(HOnlObjBrowser,0) //Browser and manager of online objects
