// @(#)$Id: hpidphysicsconstants.cc,v 1.4 2003-03-06 15:24:32 jaskula Exp $
//*-- Author : Dan Magestro
//*-- Created: 03/09/01
...
const int nPidPart = 53;  // number of particles stored permanently

// -----------------------------------------------------------------------------

// particle naming convention (adopted from Pluto++)
const char *cPidName[nPidPart] = {
...
const int   nArtPidPart = 2;
const char *cArtPidName[nArtPidPart]   = { "fake-", "fake+" };
const int   fArtPidCharge[nArtPidPart] = { -1,  1 };
const int   fArtPidMass[nArtPidPart]   = { -1, -1 };
...
const int   nArtPidPartNew = 4;
const char *cArtPidNameNew[nArtPidPartNew]   = { "artificial+", "artificial-",
                                                    "fake+", "fake-" };
const int   fArtPidChargeNew[nArtPidPartNew] = {  1, -1,  1, -1 };
const int   fArtPidMassNew[nArtPidPartNew]   = { -1, -1, -1, -1 };
...
Int_t HPidPhysicsConstants::pid(const char *n)
{
...
Int_t i;
    ...
    for(i = 0; i < nPidPart && strcmp(n, (char*)cPidName[i]); i++)
        ;

    if(i < nPidPart)
        return i;

    for(i = 0; i < nArtPidPartNew && strcmp(n, (char*)cArtPidNameNew[i]); i++)
        ;

    if(i < nArtPidPartNew)
        return i - nArtPidPartNew;  // negative ids

    for(i = 0; i < nArtPidPart && strcmp(n, (char*)cArtPidName[i]); i++)
        ;
...
const char *HPidPhysicsConstants::pid(Short_t pid)
{
