...
HMatchUUnpacker::HMatchUUnpacker(Int_t unites, Bool_t compression, Int_t flog,TString vers) {
  ...
  else if(vers.CompareTo("sep03")==0)
  {
      version=2;
  }
  else if(vers.CompareTo("nov02")==0)
  {
      version=1;
  }
  else if(vers.CompareTo("old")==0)
  {
      version=0;
  }
  else
...
Int_t  HMatchUUnpacker::decodeMUData(UInt_t *pData,Int_t maxLength) {
  ...
  if ( (gHades->isCalibration()) && (length >2)){  
    ...
    scaler_flag =1;
    if (fDebugMode) printf("Scaler event, corrected length to %i\n",length);
  }
   muDataSize =  muData->decode(ptr,length); 
   if (fDebugMode) printf ("MU-Data length :%i %i \n", length, r );
   if (fDebugMode) cout << "mu data size " << muDataSize << endl;
   if ( muDataSize < 0 ) { 
   ...
   printf("MU-Data decoding %i leptons \n", muData->getNLeptons()); 
	...
	if (fDebugMode)  printf("Lepton dataword: 0x%x\n",dataw);
    ...
    for (Int_t nl=1; nl<(1+(3*(Int_t)muData->getNDiLeptons())); nl+=3){
      ...
      if (fDebugMode)   printf("DILepton dataword: 0x%x\n",dataw);
      nn1= dataw & 0xFF; 
      nn2= (dataw >> 8 ) & 0xFF; 
...
Int_t HMatchUUnpacker::decodeIpuRich(UInt_t *pData,Int_t maxLength) {
  ...
  for(Int_t i=0;i<7;i++){Rich_Sector_Map[i]=0;}
  if(version==0)      {for(Int_t i=0;i<6;i++){Rich_Sector_Map[i]=Rich_Sector_Map_old  [i];}}
  else if(version==1) {for(Int_t i=0;i<6;i++){Rich_Sector_Map[i]=Rich_Sector_Map_nov02[i];}}
  else if(version==2) {for(Int_t i=0;i<7;i++){Rich_Sector_Map[i]=Rich_Sector_Map_sep03[i];}}
  else if(version==3) {for(Int_t i=0;i<7;i++){Rich_Sector_Map[i]=Rich_Sector_Map_sep03[i];}}

  Int_t Row_Offset;
  if (version<3) Row_Offset=23;
  else Row_Offset=26;
  ...
  if ( Rich_Sector_Map[sec_header-1]!=sec_count ) {
    cout << "event " << evt <<" HMUUnpack: Mismatch in RichIPU!!! Sector_Count " << sec_count << " != Sector_Header " 
  ...
  if (length>maxLength) {
	...
	if ((hardsegId < 1) || (hardsegId >6) ) {
	  cout << "event " << evt << " HMUUnpack: Mismatch in RichIPU!!! HardsegId = " << hardsegId << endl; 
...
segId:%i theta:%f phi:%f \n  ",   row-Row_Offset, 95 - fifo * 8 - bc,fifo,bc,segId,theta,phi);
	  ...
	  if (muRich) {
	    ...
	    if (row < row_old) {
	      cout << "event " << evt << " HMUUnpack: Mismatch in RichIPU!!! in sec " << sec_count << "  X " << row 
		   << "  after X_old " << row_old << endl; 
	    } else if (row == row_old) {
	      if ( col > col_old ) { 
		cout << "event " << evt << " HMUUnpack: Mismatch in RichIPU!!! in sec " << sec_count << "  in X " << row << "  Y " << col 
	    ...
	    if(theta>0.1) { // valid range of mapped rings
	      if(ReducedCount>99)cout<<"HMatchUUnpacker::decodeIpuRich():array out of bounds"<<endl;
...
Int_t HMatchUUnpacker::decodeTof(UInt_t *pData,Int_t maxLength) {
  ...
  if ( (muData->getSecHitPatternRich()>0 )&&  ( MU_sec_pattern_Tof != TofSecPatternMask ) ) {
    cout << "event " << evt << " ERROR: Tof Hit Pattern Wrong!!! " << MU_sec_pattern_Tof << "  " << TofSecPatternMask << endl;
  }
...
Int_t HMatchUUnpacker::fillTofData( HMatchUTof* mtof, UInt_t dataword){
  ...
  if (sec > 5) {     // Invalid sector number
    sec = 6;
    cout << "event " << evt << " *********** MU-Tof Invalid sector: " << hex << dataword << dec << "  IPU Nr " << TofIpuNr << endl;
  }
  ...
  if (PID ==1) {
    if(tofLep>99)cout<<"HMatchUUnpacker::fillTofData():array out of bounds"<<endl;
...
cout << "row " <<  bc+nRowOffset << "  col " << col << "  theta " << muShower->getTheta() << "  phi " << muShower->getPhi() << endl;
	...
	if(showerLep>99)cout<<"HMatchUUnpacker::storeShowElements(():array out of bounds"<<endl;
...
Int_t HMatchUUnpacker::decodeShower(UInt_t *pData,Int_t maxLength) {
  ...
  if ( (muData->getSecHitPatternRich()>0 )&&  ( MU_sec_pattern_Shower != ShowerSecPatternMask ) ) {
    cout << "event " << evt << " ERROR: Tof Hit Pattern Wrong!!! " << MU_sec_pattern_Shower << "  " << ShowerSecPatternMask << endl;
  }
