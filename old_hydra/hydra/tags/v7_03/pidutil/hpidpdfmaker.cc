...
Int_t HPidPdfMaker::execute(void) {
  ...
  //reset iterators
	...
	//Check for ring
	  ...
	  if(pHitData->hasRingCorrelation[ALG_SPLINE]) aPatternInstance[b_corrRingSpline]=kTRUE;
	  if(pHitData->hasRingCorrelation[ALG_KICK]) aPatternInstance[b_corrRingKick]=kTRUE;
	  bookRingObservables();
	}
