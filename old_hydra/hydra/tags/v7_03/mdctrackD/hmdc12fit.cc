...
Int_t HMdc12Fit::execute(void) {
  ...
  else if(clusFitAlg==3) fitAlgorithm3();   // magnet off + sector clusters    
...
Int_t HMdc12Fit::findSeg2Clusters(HMdcTrackFitter* fitter, Bool_t flag) {  
  ...
  if(!fitter->fitStatus || flag) return f34CFSc.findClustersSeg2(fitter->fClst1);
  HMdcSeg* fSeg=(HMdcSeg*)fSegCat->getObject(fitter->segIndex);
  if(fSeg==0) return 0;
  return f34CFSc.findClustersSeg2(fSeg,fitter->fClst1);
}
...
Bool_t HMdc12Fit::fillArrays(HMdcClus * fClst) {
  if(HMdcTrackDSet::fprint) fClst->print();
  fitter->fClst1=fClst;
  fitter->setAddress(fClst->getSec(),fClst->getIOSeg());
  if(!fitter->fillListHits((HMdcList12GroupCells*)fClst)) return kFALSE;
...
Bool_t HMdc12Fit::fitMod(HMdcClus* fClst, Int_t arrInd) {
  ...
  if(!fillArrays(fClst)) return kFALSE;
...
Bool_t HMdc12Fit::fitCombClusIndFit(HMdcClus* fClst, Int_t arrInd) {
  ...
  for(Int_t iMod=0; iMod<2; iMod++) {
    ...
    fitter->setFittingTimesList(mod);
    fstIndex[iMod]=-1;
    lstIndex[iMod]=-2;
    Bool_t isFitted=fitter->fitCluster(threshold,mod);  // (threshold ???)
    if(!isFitted) {
      hitSegFiller.setClusParam(fClst,HMdcTrackDSet::isCoilOff());
      ...
      //???   nFitedTimes=fitter->unsetHits();
      //???   isFitted=fitter->fitCluster(threshold,mod);
    }
  }
...
void HMdc12Fit::fillClusInf(HMdcHit* fHit, Int_t iMod, Char_t fitSt,
  ...
  fClusInf->setLevelClFinding(fitter->fClst1->getMinCl(iMod));
  Int_t nmods=fitter->fClst1->getMod();
  ...
  fClusInf->setClusIndex(fClusCat->getIndex(fitter->fClst1));
  fClusInf->setClusFitIndex(fitter->indClusFit);
  if(iMod==0) {
    fClusInf->setClusSize(fitter->fClst1->getClusSizeM1());
    fClusInf->setNMergClust(fitter->fClst1->getNMergClustM1());
    fClusInf->setNDrTimes(fitter->fClst1->getNDrTimesM1());
    fClusInf->setSigma1(fitter->fClst1->getSigma1M1());
    fClusInf->setSigma2(fitter->fClst1->getSigma2M1());
    fClusInf->setAlpha(fitter->fClst1->getAlphaM1());
  } else {
    fClusInf->setClusSize(fitter->fClst1->getClusSizeM2());
    fClusInf->setNMergClust(fitter->fClst1->getNMergClustM2());
    fClusInf->setNDrTimes(fitter->fClst1->getNDrTimesM2());
    fClusInf->setSigma1(fitter->fClst1->getSigma1M2());
    fClusInf->setSigma2(fitter->fClst1->getSigma2M2());
    fClusInf->setAlpha(fitter->fClst1->getAlphaM2());
  }
  if(isGeant) {
    HMdcClusSim* fClSim=(HMdcClusSim*)fitter->fClst1;
...
void HMdc12Fit::fillClusInf(HMdcSeg* fSeg, Bool_t fitSt) {
  Int_t nMrCl = fitter->fClst1->getNMergClust();
    if(nMrCl>99) nMrCl=99;
    Int_t flag=(fitter->fClst1->getNBins()&32767) <<16;
    flag += (fitter->fClst1->getNDrTimes()*100 + nMrCl) & 65535;
    fSeg->setFlag(flag);
...
Int_t HMdc12Fit::fillHitByFit(Int_t iMod) {
  ...
  Int_t modClus=fitter->fClst1->getMod();
  ...
  setGroup(&(fitter->listCells), fHit,mod);
...
Int_t HMdc12Fit::fillHitByClus(Int_t iMod) {
  ...
  setGroup(&(fitter->listCellsOrg), fHit,mod);
  if(isGeant) ((HMdcHitSim*)fHit)->calcNTracks();
...
void HMdc12Fit::setGroup(HMdcList24GroupCells* lCl, HMdcSeg* lHit) {
  // Filling list of cells in HMdcSeg
  Int_t lay1=fitter->segment*12;
  Int_t lay2=lay1+12;
  Int_t t[4];
...
void HMdc12Fit::setGroup(HMdcList24GroupCells* lCl, HMdcHit* lHit, Int_t mod) {
