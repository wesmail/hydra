...
HWallHistFiller::HWallHistFiller(const char* name,const char* title)
: TNamed(name,title)
{
	colorsSector.Set(6);
	colorsModule.Set(4);
...
void   HWallHistFiller::putWallParams(HParamList* l,TList* histpool)
{
	...
	l->add("hWallProf1"                  ,mytemp,500);
	
	hProf2A->getText(mytemp);
	l->add("hWallProf2"                  ,mytemp,500);
	
	hProf3A->getText(mytemp);
	l->add("hWallProf3"                  ,mytemp,500);
	
	hRawTime1A->getText(mytemp);
	l->add("hWallRawTime1"                   ,mytemp,500);
	
	hRawWidth1A->getText(mytemp);
	l->add("hWallRawWidth1"                   ,mytemp,500);
	
	hHitTimeA->getText(mytemp);
	l->add("hWallHitTime"                   ,mytemp,500);
	
	hHitChargeA->getText(mytemp);
	l->add("hWallHitCharge"                   ,mytemp,500);
	
	hHitXYA->getText(mytemp);
	l->add("hWallHitXY"                   ,mytemp,500);

	hFib1MultA->getText(mytemp);
	l->add("hWallFib1Mult"                   ,mytemp,500);

	hRawMultA->getText(mytemp);
	l->add("hWallRawMult"                   ,mytemp,500);
}
