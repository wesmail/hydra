...
Int_t HMdcTrackDSet::fitAuthor=1;     // 1 - Alexander's fit, 2 - tuned Alexander's fit
...
Int_t HMdcTrackDSet::useWireOffset=1; // remove signal time on wire from dr.time
Int_t HMdcTrackDSet::mixFitCut=2;     // mixed cluster finder cut
Int_t HMdcTrackDSet::mixClusCut=4;    // mixed cluster finder cut
...
void HMdcTrackDSet::setClFinderType(Int_t type) {
  if(isSetCall) return;
  if(type<0 || type>2) exitSetup("setClFinderType(Int_t):",1,"0,1 or 2",type);
  clFinderType=type;
}
