...
public:
	...
	indClusFit(-1), dedx(0), dedxSigma(0), dedxNWire(0), dedxNWireCut(0) {;}
    ~HMdcClusInf(void) {;}
    void clear(void) {
	clusSize=nMergClus=nDrTimes=0;
	indClus=indClusFit=-1;
	dedx=dedxSigma=dedxNWire=dedxNWireCut=0;
    }
  ...
  ClassDef(HMdcClusInf,2) // information about clusters in MDC
};
