...
HMdcCutStat::HMdcCutStat(const char* name,const char* title,
...
{   // returns values of different cut statistics to int*
    // cT1    : cut in T1
    // cT2    : cut in T2
    // cT12   : cut in T2-T1
...
void HMdcCutStat::putAsciiHeader(TString& b) {
	...
	"# Format:\n"
	"# sector mod ct1 ct2 ct12 cwires ct1t12 call all \n"
	"#######################################################################\n";
}
Bool_t HMdcCutStat::writeline(char*buf, Int_t s, Int_t m) {
    Bool_t r = kTRUE;
    if (fMdc)
