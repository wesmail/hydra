...
void process_filename_lst(TString &inputFile, TString &outputFile, TString prefix,TString outputpath="")
{
	...
	char buffer[1000], *c;
	strncpy(buffer,(const char *)inputFile,999);
	c=strrchr(buffer,'/');
	if(c){
		*c=0;
		file=c+1;
	...
	strncpy(buffer,(const char *)file,999);
	c=strrchr(buffer,'.');
...
int reduction(TString filename,Int_t nr_events,Bool_t verbose)
{
	//  gROOT->Reset();
	TString outFile="";
...
int main(int argc, char** argv)
{
	...
	switch( argc-cur_arg)
	{
		...
		default:
         ...
         cout <<  " full path of the dst-files must be given int the list file" << endl;
