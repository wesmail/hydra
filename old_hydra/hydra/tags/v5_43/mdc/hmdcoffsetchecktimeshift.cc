...
Bool_t HMdcOffsetCheckTimeShift::init(void)
{
    ...
    if (!lookupgeom)
    {
        lookupgeom=new HMdcLookupGeom;
        gHades->getRuntimeDb()->addContainer(lookupgeom);
    }
...
/*
	...
	for(Int_t mo=0; mo<4; mo++) {  //loop over modules
	    for(Int_t step=0;i<10;i++){  //loop over steps in file
		for(Int_t i=0;i<100;i++){  //loop over files

		 sprintf(title,"%s%i%s%i%s%02i%s%02i%s","hmdctime1[",s,"][",mo,"][",step,"][",i,"]");
		 sprintf(name, "%s%i%s%i%s%02i%s%02i%s","hmdcTime1[",s,"][",mo,"][",step,"][",i,"]");
		 hmdc[s][mo][step][i]     = new TH1F(name,title,nbin,-nbin+0.5,0.5);
		 sprintf(title,"%s%i%s%i%s%02i%s%02i%s","hmdctime1int[",s,"][",mo,"][",step,"][",i,"]");
		 sprintf(name, "%s%i%s%i%s%02i%s%02i%s","hmdcTime1Int[",s,"][",mo,"][",step,"][",i,"]");
		 hmdcInt[s][mo][step][i]     = new TH1F(name,title,nbin,-nbin+0.5,0.5);
		}
	    }
	}
    }
...
/*
    for(Int_t i=0;i<100;i++){
        sprintf(title,"%s%02i%s","hOffset[",i,"]");
	sprintf(name ,"%s%02i%s","hOffset[",i,"]");
	hOffset[i]     = new TH1F(title,name,2048, 0, 2048);
	sprintf(title,"%s%02i%s","hOffsetCorr[",i,"]");
	sprintf(name ,"%s%02i%s","hOffsetCorr[",i,"]");
	hOffsetcorr[i] = new TH1F(title,name,2048, 0, 2048);
        sprintf(title,"%s%02i%s","hmdcAll[",i,"]");
	sprintf(name ,"%s%02i%s","hmdcAll[",i,"]");
	hmdcAll[i] = new TH1F(title,name,nbin,-nbin+0.5,0.5);
        sprintf(title,"%s%02i%s","hmdcAllInt[",i,"]");
	sprintf(name ,"%s%02i%s","hmdcAllInt[",i,"]");
	hmdcAllInt[i] = new TH1F(title,name,nbin,-nbin+0.5,0.5);
    }
*/
    Int_t err;
	...
	for(Int_t s=0; s<6; s++) {  //loop over sectors
		...
		if (dirMod)
		{
			...
			if ((err=fitHist(s, mo,step,f))) crosspointX = fitpar1 = -err;
		       // hOffset[step][f]->Fill(crosspointX);
...
Float_t myerror=0;
 ...
 timer.Stop();
/*
 for(Int_t step=0;step<10;step++){
     for(Int_t f=0;f<filecounter;f++){

	 hOffset[step][f]->Write();
     }
 }
*/
...
TDirectory *HMdcOffsetCheckTimeShift::Mkdir(TDirectory *dirOld, Char_t *c, Int_t i, Int_t p=1)    //! Makes new Dir, changes to Dir, returns pointer to new Dir
{
...
Int_t HMdcOffsetCheckTimeShift::execute()
{
	...
	while ((raw=(HMdcRaw*)iter->Next()))
	{
		...
		// we have to use Float_ts here
                //cout<<step<<" "<<filecounter <<" "<<skipcount<<endl;
	...
	while ((raw=(HMdcRaw*)iter->Next()))
	{
	    ...
	    if(timecut->cutTime1(s,mo,testTime1)&&
	       timecut->cutTime2(s,mo,testTime2)&&
	       timecut->cutTimesDif(s,mo,testTime1,testTime2))
	    {
