...
HCategory *HTofDetector::buildCategory(Cat_t cat) {
  switch (cat) {
  case catTofRaw : return buildMatrixCat("HTofRaw",0.5);
  case catTofCal : return buildCategorySplit("HTofCal",0.5);
  case catTofHit : return buildMatrixCat("HTofHit",0.5);
  case catTofGeantRaw : return buildMatrixCat("HGeantTof",0.1);
  default : return NULL;
  }
}
