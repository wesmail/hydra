...
HShowerCalParCalculator::HShowerCalParCalculator()
{
  m_pCalPar = NULL;
  m_pCalParHist = NULL;
  fIter=NULL;
...
Bool_t HShowerCalParCalculator::finalize(void) {
   ((HShowerHist*)getCalParHist())->calculate((HShowerCalPar*)getCalPar());
...
Int_t HShowerCalParCalculator::execute()
{
  ...
  while((pRaw = (HShowerRaw*)fIter->Next()))
  {
     ...
     ((HShowerHist*)getCalParHist())->fill(loc, pRaw->getCharge());
     n++;
  }
