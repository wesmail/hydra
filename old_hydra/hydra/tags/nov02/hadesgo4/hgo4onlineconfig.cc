...
Bool_t HGo4OnlineParam::isEqual(HGo4OnlineParam* p,Int_t det=0)
{
    if(det==0)
    {
    ...
    if(mdcRaw        !=p->getMdcRaw())return kFALSE;
    if(mdcCal        !=p->getMdcCal())return kFALSE;
    if(mdcHit        !=p->getMdcHit())return kFALSE;
...
void HGo4OnlineParam::printParam(void)
{
    ...
    printf("refresh     : trigger %i, start %i, rich %i, richOD %i, mdc %i, tof %i, tofino %i, shower %i \n"
	   ,triggerrefresh,startrefresh,richrefresh,richODrefresh,mdcrefresh,tofrefresh,tofinorefresh,showerrefresh);
