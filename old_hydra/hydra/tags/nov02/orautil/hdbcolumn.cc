...
HDbColumn::HDbColumn(const char* cName) {
...
void HDbColumn::setColType(const char* tName, Int_t dlen, Int_t prec=0, Int_t scale=0) {
  ...
  t.ToUpper();
  char ta[7][10]={"VARCHAR","CHAR","NUMBER","DATE","LONG","ROWID","RAW"};
  Int_t i=0;
  for(;i<7;i++) {
...
void HDbColumn::show() {
  ...
  char c='Y';
  if (colNullable) c='N';
  cout<<setiosflags(2)<<setw(30)<<GetName()<<c<<"   ";
  if (colType) cout<<setiosflags(2)
