...
Bool_t HShowerTofPIDTrackMatcher::init() {
  // creates the HShowerTofPID, HShowerTrack and HShowerTofPIDTrack categories 
  // and adds them to the current event,
  // creates an iterator which loops over all pid local maxima
  // and an iterator which loops over track number information
    ...
    if (!m_pHitCat) {
      //m_pHitCat=pShowerDet->buildCategory(catShowerTofPID);
