  ...
  while ((pTrackCand=(HPidTrackCand *)iterPidTrackCand->Next()) != 0) 
    {
      ...
      //hists->hDThetaPerSec->Fill(pHitData->)
      hists->hNTrackCandPerSec->Fill(pHitData->getSector());

      if(pHitData->getFlagRICH())
	{
	  nLepCand++;
	  hists->hNTrackCandLepPerSec->Fill(pHitData->getSector());
	}
    }
  varHists->pidTrackCandLep_n_Var->Fill(nEvent,nLepCand);
}
