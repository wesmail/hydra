...
HRungeKutta::HRungeKutta() {
  ...
  sig[0]=0.140*2.00;   sig[1]=0.140*1.00;  // MDC1 intrinsic resolution
  sig[2]=0.140*2.00;   sig[3]=0.140*1.00;  // MDC2
  sig[4]=0.140*2.00;   sig[5]=0.140*1.00;  // MDC3
  sig[6]=0.140*2.00;   sig[7]=0.140*1.00;  // MDC4
}
...
void HRungeKutta::setMdcPosition(Int_t s, Int_t m, HGeomTransform& gtrMod2Sec) {
  ...
  if (m==0) {
    dzfacMdc1[s]=tan(acos(norm.getZ()));
    HGeomVector v(0.0, 0.0, -300.0);
    ...
    HGeomVector vs1(0.0, 0.0, -301.0);
    HGeomVector n=gtrMod2Sec.transFrom(vs1) - vs;
  }
}
...
//    if (finite(pfit)==0 || fabsf(pfit)>1.e+5 || (chi2old>1.e+06&&chi2>10.*chi2old)) return kFALSE;
    ...
    if ((TMath::Abs(chi2-chi2old)/chi2)<0.005||iter==10) break;
    ...
    fit[0][0]+=fitpar[3];
    fit[0][1]+=fitpar[4];
    fit[0][2]-=fitpar[4]*dzfacMdc1[sector];
    if (pfit==0.) pfit=10.;
  }
  return rc;
}
...
void HRungeKutta::parSetNew0(Float_t uh,Float_t uv,Int_t iter) {
  ...
  if (iter>0) {
    ...
    findMdcIntersectionPoint(upos,udir,0,&fit[0][0]);
    for(Int_t i=0;i<3;i++) { upos[i]=fit[0][i]; }
  }
  for(Int_t i=0;i<3;i++) {
    dirAtMDC1[i]=udir[i];  // direction of track on MDC1
  }
  trackLength=0.;
...
void HRungeKutta::parSetNewVar(Float_t uh,Float_t uv) {
  ...
  upos[2]=fit[0][2]-dpar[4]*dzfacMdc1[sector]; // change in the MDC1 plane
  cosines(uh,uv,udir);
...
Bool_t HRungeKutta::linSys() {
  ...
  Double_t tol=0.;
  Int_t nrZeros;
  if(!decompose(test,sign,tol,nrZeros)){return kFALSE;}
  //-------------------------------------------
...
void HRungeKutta::gentrkNew(Float_t p, Double_t* upos, Double_t* udir, Double_t* hitxyz, Int_t kind ) {
  ...
  for(Int_t nMod=1;nMod<nMaxMod;nMod++) {
    ...
    do {	
      ...
      findMdcIntersectionPoint(upos,udir,nMod,pHit);
    ...
    } while (d>=maxDist&&upos[2]<pHit[2]&&jstep<maxNumSteps);
  }
  if (kind==1) {
...
void HRungeKutta::traceToVertex(HMdcSizesCells* pMSizesCells) {
  // propogates the track backwards from MDC1 till Z=zVertex plane
  // should be called only after RK-minimisation is done
  Float_t step=initialStepSize;
  Float_t stepFac=1.;
  Int_t jstep=0;
  Double_t dist=0.;
  Double_t* normplane=&normVecMdc[0][sector][0];
  Double_t* pplane=&pointForVertexRec[sector][0];
  Double_t upos[3], udir[3], xyz[3];
  for(Int_t i=0;i<3;i++) {
    upos[i]=fit[0][i];     // track position at MDC1
    udir[i]=-dirAtMDC1[i]; // direction (reversed to go back from MDC1 to target)
  }
  do {
    ...
    jstep++;
    stepFac=rkgtrk(step,pfit,upos,udir); // we use pfit as value for momentum to propagate
    trackLength+=step;
    step*=stepFac;
  } while (jstep<maxNumSteps);
...
void HRungeKutta::traceToMETA(HGeomVector& metaHit,HGeomVector& metaNormVec,
  ...
  Double_t dist=0.;
  do {
    findIntersectionPoint(upos,udir,pointOnMETA,normVectorOfMETA,&trackPosOnMETA[0]);
    dist=distance(upos,trackPosOnMETA);
    if (upos[2]>trackPosOnMETA[2]) {
      trackLength-=dist;
      break;
    } else if (dist<maxDist) {
      trackLength+=dist;
      break;
    } else if (fabs(upos[1])>=2475.) {
      trackLength=0.;
      break;
    }
    if (dist<step) { step=dist; }
  ...
  if (pathCorrPos!=0&&pathCorrNorm!=0) {
