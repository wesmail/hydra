...
Float_t HMdcTrackGSpline::calcIOMatching(HMdcSeg *segments[])
{
    HGeomVector innerInt=calcKickIntersection(segments[0]);
    HGeomVector outerInt=calcKickIntersection(segments[1]);
    Float_t qIOMatching=10.*sqrt((innerInt.getX()-outerInt.getX())*(innerInt.getX()-outerInt.getX())/4+
			                 (innerInt.getY()-outerInt.getY())*(innerInt.getY()-outerInt.getY()));
    return qIOMatching;
}
