  ...
  if(plot[nBin] > 0) {
   ...
   rootPlot->Fill((Float_t)nx*stX+stX*0.5+xMin,
              (Float_t)ny*stY+stY*0.5+yMin,
               HMdcBArray::getNSet2B(plMod1+nBin*2));
  }
 }
 return rootPlot;
}
...
// rootPlotV2->SetMinimum(0.);
 ...
 for (Int_t nBin=0; nBin<size; nBin++) {
  if(plot[nBin] > 0) {
   Int_t ny=nBin/nBinX;
   Int_t nx=nBin%nBinX;
   Int_t color=HMdcBArray::getNSet(plMod1[nBin*2]);
   Int_t col4=HMdcBArray::getNSet(plMod2[nBin*2]);
...
Int_t  HMdc34ClFinderSec::stackSizeSeg2=0;
...
void HMdc34ClFinderSec::makePlotSeg2() {
  ...
  Int_t maxB4Sc=-1;
  Int_t minB4Sc=prPlotSeg2->sizeBAr4b*32;
  Int_t nLayers=0;
  for(Int_t mod=2; mod<4; mod++) {
    ...
    for(Int_t lay=0; lay<6; lay++) {
      ...
      if(cell<0) continue;
      nLayers++;   // num. of layers with fired wires
...
// Mozhno optimizirovat'!
          for(Int_t nx=0; nx<prPlotSeg2->nBinX; nx++) {
            ...
            for(; ny<=nyU; ny++) {
              ...
              if(nLayers>=minBinS && lay1>=minAm &&
                  HMdcBArray::get(prPlotSeg2->plotBArSc,binNum)==0) {
                Int_t b1=binNum*2;
                if( HMdcBArray::getNSet(prPlotSeg2->plMod2[b1])>=mBm4 &&
                    HMdcBArray::getNSet(prPlotSeg2->plMod1[b1])>=mBm3 ) {
...
void HMdc34ClFinderSec::calcClusterSeg2(Int_t nBin) {
  Int_t stackT,stackB;
  stackT=stackB=0;
  Int_t nx,ny;
//  Float_t x,y;
...
//  Float_t binsInCl=0.;
  ...
  while(stackT != stackB) {
    ...
    Double_t wt=
        (Double_t)(HMdcBArray::getNSet2B(prPlotSeg2->plMod1+nBin*2)-minBinS);
    //region of cluster ???
...
//     rmsY+=y*y*wt;
    ...
    if(stackB>stackSizeSeg2){
      Warning("calcClusterSeg2","Sec.%i Stack is increased on 50\% ",sector+1);
