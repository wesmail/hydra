...
void HQAHistograms::bookHistMdc() {
  ...
  Float_t t1taty[2] = { 0.,350.};
  Int_t   t1tatn[2] = { 250 ,350 };
  //
  ...
  for(Int_t im=0;im<4; im++){
    for(Int_t is=0;is<6; is++){
       ...
       //else{
         if(is==0){ histList->Add(mdcCal1_t1[is][im],"mdc--");}
         else{      histList->Add(mdcCal1_t1[is][im],"mdc--same");}
       //}
    }
  }
  ...
  for(Int_t im=0;im<4; im++){
    for(Int_t is=0;is<6; is++){
       ...
       if(is==0 && im==0){histList->Add(mdcCal1_t2mt1[is][im],"mdc--newpage");}
         ...
         if(is==0){ histList->Add(mdcCal1_t2mt1[is][im],"mdc--");}
         else{      histList->Add(mdcCal1_t2mt1[is][im],"mdc--same");}
       }
    }
  }
  //***************************************************
