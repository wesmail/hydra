...
//*-- Modified : 10/29/2002 L.Fabbietti
...
Int_t HRichAnalysisSim::execute() {
 ...
 HRichCal *pCal;
  ...
  while((pCal = (HRichCal *)fIter->Next())) {
    ...
    //     if ( (ampl = (Int_t)pCal->getCharge()) > 0 && (pCal->getCol() + pCal->getRow()*maxCols)<maxPads ){
    if ( (ampl = (Int_t)pCal->getCharge()) > 0){
	  sec = pCal->getSector();
	  fPadFired[sec]++;
	  padnr = pCal->getCol() + pCal->getRow()*maxCols;
	  Int_t maxPads = ((HRichGeometryPar*)fpGeomPar)->getPadsNr();
