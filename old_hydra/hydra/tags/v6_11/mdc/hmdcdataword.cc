...
// Modified: 09/09/02 by Peter Zumbruch <P.Zumbruch@gsi.de>
...
Bool_t HMdcDataWord::decode( const UInt_t data, UInt_t version, Bool_t consistencyCheck)
{
  switch(version)
    ...
    case 1:
      ...
      break;
    default:
      Error("decode","decoding version %i not a valid number",version);
      return kFALSE;
    }
  
  if (consistencyCheck) return checkConsistency(version);
  return kTRUE;
}
...
Bool_t HMdcDataWord::subHeader( const UInt_t* data, UInt_t version, Bool_t consistencyCheck)
{
  ...
  switch(version)
    ...
    case 0:
      ...
      break;
    default:
      Error("subHeader","decoding version %i not a valid number",version);
      return kFALSE;
    }
