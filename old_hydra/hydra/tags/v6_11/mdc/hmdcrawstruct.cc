...
HMdcRawStruct::HMdcRawStruct(const char* name,const char* title,
...
void HMdcRawStruct::printParam() {
  ...
  Char_t buf[255];
  SEPERATOR_msg("-",60);
  INFO_msg(10,DET_MDC,"Position identifier of motherboards");
  INFO_msg(10,DET_MDC,"number of TDCs");
    ...
    for(Int_t i=0;i<6;i++) {
      ...
      for(Int_t j=0;j<4;j++) {
	    ...
	    gHades->getMsg()->info(10,DET_MDC,GetName()
          ...
          for(Int_t k=0;k<16;k++) {
	    strcat(buf,mod[k].GetName());
	    gHades->getMsg()->info(10,DET_MDC,GetName(),"%s",buf);
          }
	  INFO_msg(10,DET_MDC,buf);
          for(Int_t k=0;k<16;k++) {
            Int_t nt=mod[k].getNTdcs();
	    gHades->getMsg()->info(10,DET_MDC,GetName(),"%3i ",nt);
          }
        }
      }
    }
...
void HMdcRawStruct::readline(const char *buf, Int_t *set) {
...
Bool_t HMdcRawStruct::writeline(char *buf, Int_t sec, Int_t mod, Int_t mbo) {
