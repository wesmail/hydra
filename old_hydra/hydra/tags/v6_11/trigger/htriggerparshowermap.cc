...
void HTriggerParShowerMap::readline(const char *buf, Int_t *set) {
  ...
  HTriggerParShowerMap& par=(*this);

  sscanf(buf,"%i%i%f%f",&col,&row,&phi,&theta);
 
  if (phi>300.0) {
    cout << "phi = " << phi << " > 300 !!!*******************************" << endl;
    phi-=330.0;
    cout << "phi = " << phi << endl;
  }
  else if(phi<40) {
    cout << "phi = " << phi << " < 40 !!!*******************************" << endl;
    phi+=30; 
    cout << "phi = " << phi << endl;
  }
    ...
    if( (row > -1) && (row < 32 ) 
	&& (col > -1) && (col < 32 ) ) {
      par.setShowerTheta(row,col,theta);
      par.setShowerPhi(row,col,phi);
    }
    else {
...
Bool_t HTriggerParShowerMap::writeline(char *buf,Int_t s, Int_t m, Int_t c) {
  // writes one line to the buffer used by ascii file I/O
  HTriggerParShowerMap& par=(*this);
  for (Int_t xx=0; xx < 32; xx++) {
    for (Int_t yy=0; yy < 32; yy++) {
      sprintf(buf,"%i%i%f%f\n",
	      xx,yy,par.getShowerPhi(xx,yy),par.getShowerTheta(xx,yy));
...
void HTriggerParShowerMap::printParam(void) {
  ...
  printf("==============================================================\n");
   ...
   for (Int_t xx=0; xx < 32; xx++) {
      for (Int_t yy=0; yy< 32; yy++) {
	cout << xx << "   " << yy << "   " << par.getShowerPhi(xx,yy) << "   " << par.getShowerTheta(xx,yy) << endl;
        }
      }
