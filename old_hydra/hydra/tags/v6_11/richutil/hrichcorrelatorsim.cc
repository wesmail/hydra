// $Id: hrichcorrelatorsim.cc,v 1.14 2002-10-21 10:41:09 eberl Exp $
// Last update by Thomas Eberl: 02/10/15 17:01:44
//
...
// 		->MakeIterator("native");
		...
		if (!pRichHitFitCat){
		    Error("init()","no Rich hit fit category in input");
		}
		pRichHitFitIter = (HIterator*) pRichHitFitCat->MakeIterator();
...
//  	    cout<<"*********************************"<<endl;
//  	    cout<<"xmirr: "<<xMirrPhotCM<<" ymirr: "<<yMirrPhotCM<<" sec: "<<sec<<" phot: "<<phot<<endl;
//  	    cout<<"xring: "<<xRing<<" yring: "<<yRing<<" sec: "<<sec<<endl;
//  	    cout<<"*********************************"<<endl;
	    if (TMath::Abs(xMirrPhotCM-xRing)<2 && TMath::Abs(yMirrPhotCM-yRing)<2 && sec==secRing)
	    {
		// now check if trk nb in ring is that of the photon parent
		Int_t t1,t2,t3;
		t1=t2=t3=-1;
		...
		if (t3>-1) p3=HRichUtilFunc::getParID(t3,(HLinearCategory*)
						      getGeantKineCat());
		if ((t1==aTrk && aID==p1) || (t2==aTrk && aID==p2) || (t3==aTrk && aID==p3))
		{

		    // *** diagnostic output ***
//    		    cout<<"*************trk nbmatched *****************"<<endl;
//    		    cout<<"xmirr: "<<xMirrPhotCM<<" ymirr: "<<yMirrPhotCM
//  			<<" sec: "<<sec<<" phot: "<<phot<<" "<<aTrk<<" "<<aID<<endl;
//    		    cout<<"xring: "<<xRing<<" yring: "<<yRing
//  			<<" sec: "<<sec<<" "
//  			<<t1<<" "<<t2<<" "<<t3<<" "<<p1<<" "<<p2<<" "<<p3<<endl;
//  		    cout<<"*********************************"<<endl;

		    RingCanBeInCorrelation=aTrk;
		    // ring in tracklet was made by this lepton whiched passed
		    // the mirror
		    h->setNumPhot(phot);//number of photons reflected originating from this lep
		    HTrackInfo* t=(HTrackInfo*) h->getTrackInfoObj();
		    //h->dumpToStdoutSim();
		    //t->dumpToStdout();
		    // check if this lepton is in GEANT correlation
		    for (Int_t i=0;i<MAXPARTICLES;i++)
		    {
...
Int_t HRichCorrelatorSim::isLepOnMDC(HHitMatchSim* h,Int_t trknb)
  {
      ...
      if (h->getNumPhot()>0) //has Rich ring made of lepton photons on mirr
      {
  	HGeantMdc *gmdc = 0;
  	iter_mdcgeant->Reset();
