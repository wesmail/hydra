...
//*-- Modified : 07/11/01 by R. Holzmann
...
void HMatrixCategory::setup(Text_t *className,Int_t nSizes,Int_t *sizes,Float_t fillRate) {
...
void HMatrixCategory::activateBranch(TTree *tree,Int_t splitLevel) {
  ...
  if (splitLevel) {
    ...
    //tree->SetBranchStatus(name,1); Not needed from ROOT 2.23-11
...
TIterator *HMatrixCategory::MakeIterator(Option_t* opt,Bool_t dir) {
...
void HMatrixCategory::Clear(Option_t *opt) {
  // see HCategory
