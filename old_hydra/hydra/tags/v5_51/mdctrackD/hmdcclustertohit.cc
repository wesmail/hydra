...
HMdcClusterToHit::HMdcClusterToHit(Int_t fSeg1, Int_t fSeg2) {
  tFillSeg[0]=(fSeg1<-1 || fSeg1>1) ? 1:fSeg1;
  tFillSeg[1]=(fSeg2!=-1 && fSeg2!=2 && fSeg2!=3) ? 3:fSeg2;
}
...
Int_t HMdcClusterToHit::execute(void) {
  ...
  while ((fClst=(HMdcClus *)iter->Next())!=0) {
    ...
    if(isGeant) ((HMdcClusSim*)fClst)->getNTracks();
    ...
    if(tFillSeg[seg]!=-1 && (typeClFn==0 || nMods==1 || modCl==tFillSeg[seg])) {
      ...
      // NDriftTimes in clust. and num. of merged clusters --> user flag in seg.
      fSeg->setFlag(fClst->getNDrTimes()*100 + nMrCls);
      fillSeg(fClst,fSeg);
    }          
    for(Int_t imod=0; imod<2; imod++) {
      ...
      for(Int_t layer=0; layer<6; layer++) {
        ...
        if(lastCell-firstCell >3 ) nMax=3;
        flag+=fClst->getNDrTimes(iLayer);
        Int_t t[4]={0,0,0,0};
