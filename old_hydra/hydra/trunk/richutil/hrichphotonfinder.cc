// $Id: hrichphotonfinder.cc,v 1.9 2009-09-09 14:11:39 jurkovic Exp $
...
HRichPhotonFinder::HRichPhotonFinder(const Text_t *name,const Text_t *title) :
	    ...
	    if (pCal && nHitSector == pCal->getSector())
	   {
		  ...
		  TMath::Abs(nCalCol-nHitX) <= fs)
		   ...
		   fSigCh[nPadCounter] = pCal->getCharge() -
		       3*(((HRichCalParCell*)((HRichCalPar*)getCalPar())->getObject(cloc))->getSigma());
...
// # warning "findLocMax():passing floats through Int_t arguments. fix me."
