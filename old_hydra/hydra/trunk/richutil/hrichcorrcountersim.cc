// $Id: hrichcorrcountersim.cc,v 1.12 2009-07-23 14:52:20 halo Exp $
...
HRichCorrCounterSim::HRichCorrCounterSim(const Text_t *name,const Text_t *title) :
...
HRichCorrCounterSim::HRichCorrCounterSim(const Text_t *name,const Text_t *title,const Char_t* filename) :
  HRichCorrCounter(name,title)
{
...
Int_t HRichCorrCounterSim::fillGeantMdcShowerdTheta(HHitMatchSim* h,Int_t evtsize,Int_t* m,Int_t* s)
{
  ...
  Int_t mi,si;
    ...
    Bool_t isRealTracklet=kFALSE;
    Int_t nPartInd=-1;
    //Int_t maxNbGPart = trk->getPartNr(); // unused
    //    cout<<maxNbGPart<<endl;
    for (Int_t k=0;k<MAXPARTICLES;k++)
    {
	//	if (trk->getMatchedRM(k) > 0 && trk->getMatchedMT_S(k) > 0) 
