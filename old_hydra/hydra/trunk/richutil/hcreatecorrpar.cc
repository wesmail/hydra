...
HCreateCorrPar::HCreateCorrPar(const Text_t *name,const Text_t *title) :
  HReconstructor(name,title)
{

}
HCreateCorrPar::HCreateCorrPar(const Text_t *name,const Text_t *title,const Char_t* filename,Int_t iRichQual) :
  HReconstructor(name,title)
{
...
Int_t HCreateCorrPar::execute(){
	...
	if(nRInd>-1 && nMInd>-1&&isGoodRing(pHM)){
		...
		//cout<<" Double_t tof for one rm "<<tToftofRej[m]<<" at " <<m<<endl;
		m++;
	    }
	    if(nTInd>-1){
    ...
    if(m>-1){
    for(Int_t j= 0 ;j<nNbCorrObjs;j++){
	for(Int_t i= j ;i<nNbCorrObjs;i++){
...
void HCreateCorrPar::fitGausHistos(TH1F* histo,TH1F *histoFit){
    ...
    //    name+="FitFc";
    const Char_t* name = histo->GetName();
    Char_t file1[40] ;
    strcpy(file1,name);
    strcat(file1,"_left");
    Char_t file2[40] ;
