...
Int_t HMdc12Fit::findSeg2Clusters(HMdcTrackFitter* fitter, Bool_t flag) {
  ...
  if(!fitter->getFitStatus() || flag) 
    return f34CFSc.findClustersSeg2(wiresArr.getClust1());
...
void HMdc12Fit::fitAlgorithm4(void) {
  ...
  while((fClstSeg=(HMdcClus*)iterClus->Next())) {
    ...
    if(nMdcsSeg==1)           flag = fitMod(fClstSeg); // One mdc per segment
      ...
      if(fitType==0)          flag = fitSeg(fClstSeg);
      else if(fitType==1)     flag = fitCombClusIndFit(fClstSeg);
...
Bool_t HMdc12Fit::fit2Sectors(HMdcClus* fClst1, HMdcClus* fClst2,
  ...
  if(!isFitted) {
    //???????????!!!!!!!!!!!!!!!!!!+++
    hitSegFiller.setClusParam(fClst1,HMdcTrackDSet::isCoilOff());
    if(fillHitSegByClus(-1) && fClst1) fClst1->setSegIndex(fitter->getSegIndex());
    if(fillHitSegByClus(-2) && fClst2) fClst2->setSegIndex(fitter->getSegIndex());
    ...
    if(fillHitSegByClus(-3) && fClst3) fClst3->setSegIndex(fitter->getSegIndex());
    if(fillHitSegByClus(-4) && fClst4) fClst4->setSegIndex(fitter->getSegIndex());
  } else /*while(isFitted)*/ {
    hitSegFiller.setFitParam(*(fitter->getFinalParam())); //!
    if(fillHitSegByFit(-1) && fClst1) fClst1->setSegIndex(fitter->getSegIndex());
    if(fillHitSegByFit(-2) && fClst2) fClst2->setSegIndex(fitter->getSegIndex());
    if(fitter->getWiresArr().getOutputListCells2().getNDrTimes() > 0) {
      ...
      if(fillHitSegByFit(-3) && fClst3) fClst3->setSegIndex(fitter->getSegIndex());
      if(fillHitSegByFit(-4) && fClst4) fClst4->setSegIndex(fitter->getSegIndex());
    }
    //???   nFitedTimes=fitter->getWiresArr().unsetHits();
    //???   isFitted=fitter->fitCluster();
  }
...
Bool_t HMdc12Fit::fitMixedClus(HMdcClus* fClst, Int_t arrInd) {
  ...
  Bool_t isFitted=fitter->fitCluster();
  if(!isFitted) {
    if(fClst->getNLayersMod(iMod^1) >= HMdcTrackDSet::getMixClusCut()) {
      hitSegFiller.setClusParam(fClst,HMdcTrackDSet::isCoilOff());
      if(fillHitSegByClus(iMod)) fClst->setSegIndex(fitter->getSegIndex());
    }
  } else /*while(isFitted)*/ {
    if(fitter->getWiresArr().getOutputListCells().getNLayersMod(addMod) >=
        HMdcTrackDSet::getMixFitCut()) {
...
HMdcTrkCand* HMdc12Fit::fillTrkCandISeg(void) {
  ...
  return new(fTrkCandNew) HMdcTrkCand(fitter->getWiresArr().getSector(),
      fitter->getSegIndex(),index);
}
