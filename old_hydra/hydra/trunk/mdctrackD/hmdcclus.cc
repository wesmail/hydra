...
void HMdcClus::printPos(void) const {
  ...
  printf("Cluster: Sec.%i Seg.%i Mod.%i Clust.type=%i %sLevel Index=%i.",
      sec+1,seg+1,modp,getTypeClFinder(),isFixedLevel() ? "Fixed":"Float",
      index);
  ...
  if( !isSegmentAmpCut() && (seg==0 || (typeClFinder&127)!=2) )
       printf(" %i:%i,",minCl1,minCl2);
...
void HMdcClus::Streamer(TBuffer &R__b)
{
   ...
   if (R__b.IsReading()) {
      ...
      R__b >> parD;
      R__b >> status;
      if(R__v > 3) R__b >> fakeFlag;
      else         fakeFlag = 0;
      if(R__v < 2) clFnLevelM1 = minCl1;
      else         R__b >> clFnLevelM1;
      R__b >> clusSizeM1;
      R__b >> nDrTimesM1;
      R__b >> nMergClusM1;
      R__b >> sigma1M1;
