...
using namespace std;
ClassImp(HHypKineFitPPPipPimPi0);

// First the Function which are neede for the minuit stuff...

// PipPimPi0
void fcnPipPimPi0(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
	HHypKineFitPPPipPimPi0 *myObject=(HHypKineFitPPPipPimPi0*)(gMinuit->GetObjectFit());
	Float_t F;
	Float_t fInput[12];
	...
	{// Quadratic distance normalized to Error
		if(i==3 || i==6 || i==9 || i==0){
	...
	// Function consist of Quadratic distance and difference in missing mass to Pi0 Mass
	// The factor is "hand made"
	funct+=10.*(F-myObject->getM2Pion0())*(F-myObject->getM2Pion0())/errF;
	f=funct;
}

Int_t HHypKineFitPPPipPimPi0::minuitFit(void)
{// for PPPipPimPi0
	Int_t result, ierflg = 0;
	
	if(!gMinuit) gMinuit=new TMinuit(13);// 12 PLUS 1, or memory corruption, Internal error in Minuit?
	
	gMinuit->SetFCN(fcnPipPimPi0);
...
void HHypKineFitPPPipPimPi0::CalcDerNum(Float_t *DER11,Double_t *Output)
{//for PPPipPimPi0
	Double_t out[12][12];
	Double_t out1[12][12];
	for(Int_t i=0; i<12; i++)
	...
	for(Int_t i=0; i<12; i++)
	{
		...
		// eta -> pi+ pi- pi0, missing pi0
		DER11[i]=((missing4(out[i])-m2Pion0)-(missing4(out1[i])-m2Pion0))/(out[i][i]-out1[i][i])/2.;
	}
}

void HHypKineFitPPPipPimPi0::secondIter(Float_t *input,Double_t *foutput,Float_t *errInput,Double_t *out,Float_t *derOut)
{// Only for PipPimPi0
	Float_t der[12];
	CalcDerNum(der,foutput);
	Float_t fT=missing4(output)-m2Pion0;
	Float_t sum=0.;
	Float_t sum1=0.;
	Float_t lambda;
	for(Int_t i=0; i<12; i++)
...
Float_t HHypKineFitPPPipPimPi0::missing4(HGeomVector &data3,HGeomVector &data4,HGeomVector &data5,HGeomVector &data6)
{// for _PipPimPi0
	Float_t P1=pInput;
	Float_t P1_2=P1*P1;
	Float_t P2_2=0.;
...
*/
	// if this would be a TVector3, one could use Mag2()
	...
	Float_t E5=sqrt(P5_2+m2PionC);
	Float_t E6=sqrt(P6_2+m2PionC);
	Float_t totalESq=(E1+E2-E3-E4-E5-E6)*(E1+E2-E3-E4-E5-E6);
	Float_t totalPSq=P1_2+P2_2+P3_2+P4_2+P5_2+P6_2

		// as dataa1 only has z component, this is much simpler... todo?
...
Float_t HHypKineFitPPPipPimPi0::missing4(Double_t *input)
{
	HGeomVector data[4];
	for(Int_t i=0; i<4; i++)
	{
...
Float_t HHypKineFitPPPipPimPi0::missing4(Float_t *input)
{
	HGeomVector data[4];
	for(Int_t i=0; i<4; i++)
	{
