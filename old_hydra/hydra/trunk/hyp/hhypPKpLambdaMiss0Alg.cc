...
ClassImp(HHypPKpLambdaMiss0Alg)
  ...
  HHypPKpLambdaMiss0Alg::HHypPKpLambdaMiss0Alg(Char_t *name_i , Option_t par[]) : HHypBaseAlgorithm(name_i, par)
{
}
...
Bool_t HHypPKpLambdaMiss0Alg::execute()
{
  ...
  while (mylist->CombIterator()) {
    ...
    HRKTrackB *track_p1  = NULL;
    HRKTrackB *track_p2  = NULL;
    HRKTrackB *track_kp  = NULL;
    HRKTrackB *track_pim = NULL;
    HCategory *pidtrackcandCat = gHades->getCurrentEvent()->getCategory(catPidTrackCand);

    track_p1  = (HRKTrackB *)( (HPidTrackCand *) pidtrackcandCat->getObject(idx_p1))->getTrackData()->getBaseTrack(4);
    track_p2  = (HRKTrackB *)( (HPidTrackCand *) pidtrackcandCat->getObject(idx_p2))->getTrackData()->getBaseTrack(4);
    track_kp  = (HRKTrackB *)( (HPidTrackCand *) pidtrackcandCat->getObject(idx_kp))->getTrackData()->getBaseTrack(4);
    track_pim = (HRKTrackB *)( (HPidTrackCand *) pidtrackcandCat->getObject(idx_pim))->getTrackData()->getBaseTrack(4);

//  HPidTrackCand *PidPart = NULL;
//   HCategory *pidtrackcandCat = gHades->getCurrentEvent()->getCategory(catPidTrackCand);

//   PidPart = (HPidTrackCand *) pidtrackcandCat->getObject(idx);
