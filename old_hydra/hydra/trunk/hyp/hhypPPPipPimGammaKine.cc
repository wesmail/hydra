...
Double_t HHypPPPipPimGammaKine::getMomError(Double_t theta, Double_t phi, Double_t mass, Double_t p,Int_t sec,Bool_t simu)
{
	Double_t res;
...
Bool_t HHypPPPipPimGammaKine::DoTheRefit(TVector3 *momentum,Int_t idx_p1,Int_t idx_p2,Int_t idx_pim,Int_t idx_pip,Float_t &prechi2,Float_t &chi2,Float_t *pulls)
{
...
/*	Float_t FakErrMom=2.2;
	...
	{
		prechi2+=(input[ii]-output[ii])*(input[ii]-output[ii])/errInput[ii]/errInput[ii];
	}
	...
	for(Int_t ii=0; ii<12; ii++)
	{
		if(ii==3 || ii==6 || ii==9 || ii==0){
			chi2+=(1/input[ii]-1/output[ii])*(1/input[ii]-1/output[ii])/errInput[ii]/errInput[ii]*input[ii]*input[ii]*input[ii]*input[ii];
		}else{
