...
void HPartialEvent::makeBranch(TBranch *parent) { 
  ...
  if (fCategories) {
	...
	if ( (*cat)!=NULL && ((*cat)->IsPersistent())) {
	  if ( (*cat)->isSelfSplitable()) {
	    b=gHades->getTree()->Branch((*cat)->getClassName(),
					    (*cat)->ClassName(),
...
//       cout << fName->Data() << " " << fHeader << endl;
//       if (fHeader) {
// 	sprintf(name,"%s_fHeader.",fName->Data());
// 	gHades->getTree()->Branch(name,fHeader->ClassName(),&fHeader,4000);
//       }
    }
  }
  //  if (fHeader && gHades->getTree()) {
  //    sprintf(name,"%s.Header",GetName());
  //    b=gHades->getTree()->Branch(name,fHeader->IsA()->GetName(),&fHeader,4000,1);
