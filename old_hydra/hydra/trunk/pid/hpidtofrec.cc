...
HPidTofRec::HPidTofRec(const Text_t *name,const Text_t *title, const Char_t *select, const Char_t *track)
...
Int_t HPidTofRec::execute(void){
	...
	while ((pCand = (HPidTrackCandSim *)iterTrackCand->Next()) != 0) // begin of PidTrackCand iterator
	{
		...
		if(bReset){
		    //-------------------------------------------------------------------------
		    // reset to old values
		    pCand->getTrackData()->nTofRecFlag       [nAlg] = 0;
		    pCand->getTrackData()->fTofRecTof        [nAlg] = pCand->getHitData()->getTof();
		    pCand->getTrackData()->fTofRecBeta       [nAlg] = pCand->getTrackData()->fCorrectedBeta[nAlg];
		    pCand->getTrackData()->fTofRecMassSquared[nAlg] = pCand->getTrackData()->fMassSquared  [nAlg];
		    //-------------------------------------------------------------------------
		}
			...
			{  // check of RICH correlation
				...
				if (// positrons
				    ( (pCand->getHitData()->nRingPatMat   >  richParam->getValue(2, 0, mdc_sec, 0)) &&
				      ...
				      (pCand->getHitData()->nRingAmplitude / pCand->getHitData()->nRingPadNr >= richParam->getValue(2, 0, mdc_sec, 3))
				    )
				    ...
				    ( (pCand->getHitData()->nRingPatMat    >  richParam->getValue(3, 0, mdc_sec, 0)) &&
				      ...
				      (pCand->getHitData()->nRingAmplitude / pCand->getHitData()->nRingPadNr >= richParam->getValue(3, 0, mdc_sec, 3))
				    )
