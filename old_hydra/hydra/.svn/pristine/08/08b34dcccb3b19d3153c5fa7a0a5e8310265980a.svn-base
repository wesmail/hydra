...
Bool_t HMdc12Fit::fitMod(HMdcClus* fClst, Int_t arrInd) {
  ...
  else { isFitted=fitter->fitCluster(threshold);}  // (threshold ???)
  if(!isFitted) {
  ...
  for(Int_t iMod=0; iMod<2; iMod++) {
    ...
    lstIndex[iMod]=-2;
     Bool_t isFitted=(fClst->getNLayersMod(iMod)<5) ? kFALSE:
         fitter->fitCluster(threshold,mod);  // (threshold ???)
    if(!isFitted) {
      hitSegFiller.setClusParam(fClst,HMdcTrackDSet::isCoilOff());
...
Bool_t HMdc12Fit::fillHitSegByFit(Int_t iMod) {
  ...
  if(!fSeg) return kFALSE;
  Int_t indx1,indx2;
  if(iMod!=1) {indx1=fillHitByFit(segment*2);  }else{indx1=-1;}
  if(iMod!=0) {indx2=fillHitByFit(segment*2+1);}else{indx2=-1;}
  ...
  if(&fSCSec){targ=&(fSCSec.getTargetMiddlePoint());}else{targ=0;}
...
void HMdc12Fit::setGroup(HMdcList24GroupCells* lCl, HMdcSeg* lHit, Int_t seg) {
