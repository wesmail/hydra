...
Int_t HConversion::countConvert;
...
fMenuTools->AddEntry("TOF Index conversion", M__CONVERT);
...
switch(GET_MSG(msg)) {
 case kC_COMMAND:
  ...
  case kCM_MENU:
    ...
    case M__CONVERT:
      if(HConversion::countConvert) {
	HError::message("The requested panel is already open.");
	break;
      }
      fConversion = new HConversion(fClient->GetRoot(),1,1);
      break;
...
//
// Returns the canvas preferences.
//
//---------------------------------------------------------------------
...
//
// This function scans the histogram container and creates the list of cursors
