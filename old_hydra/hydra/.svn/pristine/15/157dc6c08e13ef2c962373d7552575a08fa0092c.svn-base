...
HGo4OnlineParam::HGo4OnlineParam(const Char_t* name,const Char_t* title)
    : TNamed(name,title)
{
    mdcdetsetup.Set(24);
    startdetsetup.Set(6);
...
void HGo4OnlineParam::copySet(HGo4OnlineParam* p)
{
    ...
    hodoCal       =p->getHodoCal();
    hodoHit       =p->getHodoHit();
    hododetsetup  =*p->getHodoDetSetup();
    hodohitf      =*p->getHodoHitf();
    hodohit2start =p->getHodoHit2Start();
    wall          =p->getWall();
    wallRaw       =p->getWallRaw();
    wallCal       =p->getWallCal();
    wallHit       =p->getWallHit();
    rich          =p->getRich();
...
Bool_t HGo4OnlineParam::isEqual(HGo4OnlineParam* p,Int_t det)
{
	...
	if(wallrefresh   !=p->getWallRefresh())  return kFALSE;
    ...
    if(hodoCal      !=p->getHodoCal())    return kFALSE;
    if(hodoHit      !=p->getHodoHit())    return kFALSE;
    ...
    TArrayI* set3=p->getHodoHitf();
    for(Int_t i=0;i<10;i++)
    {
	if(hodohitf[i]!=(*set3)[i])     return kFALSE;
    }
    
	 if(hodohit2start !=p->getHodoHit2Start())    return kFALSE;

    if(wall         !=p->getWall())       return kFALSE;
    if(wallRaw      !=p->getWallRaw())    return kFALSE;
    if(wallCal      !=p->getWallCal())    return kFALSE;
    if(wallHit      !=p->getWallHit())    return kFALSE;
...
void HGo4OnlineParam::putParams(HParamList* l)
{
    ...
    l->add("hodoCal"     ,hodoCal);
    l->add("hodoHit"     ,hodoHit);
    l->add("hododetsetup",hododetsetup);
    l->add("hodohitf"    ,hodohitf);
    l->add("hodohit2start",hodohit2start);
    l->add("wall"        ,wall);
    l->add("wallRaw"     ,wallRaw);
    l->add("wallCal"     ,wallCal);
    l->add("wallHit"     ,wallHit);
    l->add("rich"        ,rich);
...
Bool_t HGo4OnlineParam::getParams(HParamList* l)
{
    ...
    if(!(l->fill("wallrefresh"    ,&wallrefresh    ))) return kFALSE;
    ...
    if(!(l->fill("hodoCal"  ,&hodoCal  ))) return kFALSE;
    if(!(l->fill("hodoHit"  ,&hodoHit  ))) return kFALSE;
    if(!(l->fill("hododetsetup",&hododetsetup ))) return kFALSE;
    if(!(l->fill("hodohitf" ,&hodohitf ))) return kFALSE;
    if(!(l->fill("hodohit2start",&hodohit2start ))) return kFALSE;
    if(!(l->fill("wall"     ,&wall     ))) return kFALSE;
    if(!(l->fill("wallRaw"  ,&wallRaw  ))) return kFALSE;
    if(!(l->fill("wallCal"  ,&wallCal  ))) return kFALSE;
    if(!(l->fill("wallHit"  ,&wallHit  ))) return kFALSE;
    ...
    if(!(l->fill("wallunp"   ,&wallunp  ))) return kFALSE;
...
void HGo4OnlineParam::printParam(void)
{
    ...
    printf("refresh       : trigger %i, start %i, rich %i, richOD %i, mdc %i, tof %i, tofino %i, shower %i physics %i hodo %i wall %i\n",
    ...
    printf("hodo          : det %i, Raw %i, Cal %i, Hit %i, Hit2Start %i\n",
	   hodo,hodoRaw,hodoCal,hodoHit,hodohit2start);
    printf("hodohitf      : mod  %i %i %i %i %i %i %i %i %i %i\n",
	   hodohitf[0],hodohitf[1],hodohitf[2],hodohitf[3],hodohitf[4],
      hodohitf[5],hodohitf[6],hodohitf[7],hodohitf[8],hodohitf[9]);
    printf("hododetsetup  : mod  %i %i %i %i %i %i %i %i %i %i\n",
	   hododetsetup[0],hododetsetup[1],hododetsetup[2],hododetsetup[3],hododetsetup[4],
      hododetsetup[5],hododetsetup[6],hododetsetup[7],hododetsetup[8],hododetsetup[9]);
    printf("wall          : det %i, Raw %i, Cal %i, Hit %i\n",
	   wall,wallRaw,wallCal,wallHit);
...
HGo4OnlineConfig::HGo4OnlineConfig(const Char_t* name,const Char_t* title,
