...
void HMetaMatchF::makeOuterSegMatch(HMdcTrkCand* pTrkCand) {
  ...
  while((trkCandIndex=pTrkCand->getNextCandInd())>=0) {
    ...
    for(Int_t nTof=0; nTof<nTofHitsSec;nTof++) {
      Int_t& nTrCndForT=nTrCndForTHit[nTof];
      if(nTrCndForT>=256) continue;
      qualTof=qualityTof(nTof);
      if(qualTof>qualityTOFCut[sector]) continue;
    ...
    for(Int_t nSh=0; nSh<nShowerHitsSec;nSh++) {
      Int_t& nTrCndForS=nTrCndForSHit[nSh];
      if(nTrCndForS>=256) continue;
      qualShower=qualityShower(nSh);
      if(qualShower>qualitySHOWERCut[sector]) continue;
...
void HMetaMatchF::makeRichMatching(void) {
  ...
  if(fCatRich) {
    ...
    while((pRichHit=(HRichHit*)(IterRich->Next()))!=0) {
      ...
      if(qualTheta>richThetaMinCut[sector]&&qualTheta<richThetaMaxCut[sector])
      {
      qualityTheta=kFALSE;
      }
      else qualityTheta=kTRUE;
      
      if(quality>qualityRichCut[sector] || qualityTheta) continue;
  ...
  if(fCatRichIPU) {
    ...
    while((pRichHitIPU=(HRichHitIPU*)(IterRichIPU->Next()))!=0) {
      ...
      if(qualTheta>richIPUThetaMinCut[sector]&&qualTheta<richIPUThetaMaxCut[sector])
        {
	qualityTheta=kFALSE;
        }
    else qualityTheta=kTRUE;
      if(quality>qualityRichIPUCut[sector] || qualityTheta) continue;
...
Float_t HMetaMatchF::qualityRich(Float_t mdcTheta, Float_t mdcPhi,
  ...
  // mdcTheta - in rad.!
  // mdcPhi - in deg.!
  Double_t dPhi=pRich->getPhi() - mdcPhi - dPhRichOff[sector];
  dPhi *= TMath::Sin(mdcTheta)/dPhRich[sector];
  Double_t dTheta=pRich->getTheta()-mdcTheta*rad2deg-dThRichOff[sector];
 // dTheta=(dTheta-dThRichOff[sector])/dThRich[sector];
  ...
  // mdcTheta - in rad.!
  // mdcPhi - in deg.!
  Double_t dPhi = pRich->getPhi() - mdcPhi - dPhRichIPUOff[sector];
  dPhi *= TMath::Sin(mdcTheta)/dPhRichIPU[sector];
  Double_t dTheta=pRich->getTheta()-mdcTheta*rad2deg-dThRichIPUOff[sector];
