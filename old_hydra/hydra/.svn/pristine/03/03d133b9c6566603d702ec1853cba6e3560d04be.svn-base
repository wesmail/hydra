...
void HMdc12Fit::setParContainers() {
  ...
  if(HMdcTrackDSet::fitAuthor != 1)
       trackFitter=(HMdcTrackFitter*) new HMdcTrackFitterH();
  else trackFitter=(HMdcTrackFitter*) new HMdcTrackFitterA();
  trackFitter->setPrintFlag(HMdcTrackDSet::fprint);
  trackFitter->setVersion(HMdcTrackDSet::distTimeVer);
  useWireOffset=HMdcTrackDSet::useWireOffset;
  fCalCat=0;
  fSegCat=0;
  fSizesCells=0;
  fDigitPar=0;
...
void HMdc12Fit::connectArrays() {
  ...
  drTime=trackFitter->getDriftTimeArr();
}
...
Int_t mixFitCut=2;   // mixed cut
Int_t mixClusCut=4;  // mixed cut
...
//      iterClus->Reset();
      ...
      while((fClst=(HMdcClus*)iterClus->Next())) {
        //-----------------------------------------------------
        modClus=fClst->getMod();
        if(typeClFn==2 && modClus>=0 && modClus<2 && 
            fClst->getNLayersMod(modClus^1)<mixFitCut) continue; // mixed cut
        HMdcList12GroupCells listCellsOrg=*fClst;
        if(HMdcTrackDSet::fprint) fClst->print();
        if(!fillArrays(listCellsOrg)) continue;
        clusIndex=fClusCat->getIndex(fClst);
        typeClFn=fClst->getTypeClFinder();
        ...
        for(; fittingMod<=endLoop; fittingMod++) {
          ...
          trackFitter->setFLayerPlane(lParA[ind1],lParB[ind1],lParD[ind1]);
          trackFitter->setLLayerPlane(lParA[ind2],lParB[ind2],lParD[ind2]);
          trackFitter->setXVertex(xPlane[ind1]);
          trackFitter->setYVertex(yPlane[ind1]);
          trackFitter->setZVertex(zPlane[ind1]);
          trackFitter->setXInitPlane(xPlane[ind2]);
          trackFitter->setYInitPlane(yPlane[ind2]);
          trackFitter->setZInitPlane(zPlane[ind2]);
          //                     MDC1(3)                      MDC2(4)  ???
          Int_t layf=(fittingMod!=1) ? 0:6;
          Int_t layl=(fittingMod==0) ? 5:11;
          while(kTRUE) {
            Int_t ncells=listCellsOrg.getNCells(layf,layl);
            listCells=listCellsOrg;
            //??? calcMinFunctional();  // Utochnenie nachal'nogo priblizheniya
            if(ncells>4 && fitCluster(300.)) { // (threshold???)
              if(mod<0) {
                if(typeClFn==2 && modClus>=0 && modClus<2) { 
                  if(listCells.getNLayersMod(modClus^1)>=mixFitCut) {
                    if(fillHitByFit(modClus)) fillSegByModFit(modClus);
                  }
                } else fillHitSegBySegFit();     // Segment fit
              } else {                              // Module fit
                if(nModsSeg[sector][segment]==2) {
                  if(typeClFn==1 && mod!=modForSeg[segment]) break;
                  if(typeClFn==2 && mod<2 && mixFitCut>0) break;
                }
                if(fillHitByFit(fittingMod)) fillSegByModFit(fittingMod);
              }
              //???   unsetHits(listCellsOrg);
              break;
            } else {
              if(notFillByClus) break;
              if(mod<0) {   // Seg. fit
                if(nMdcHits[mod1]>=0 || nMdcHits[mod2]>=0) break;
                if(typeClFn==2 && modClus>=0 && modClus<2) { 
                  if(fClst->getNLayersMod(modClus^1)>=mixClusCut) {
                    if(fillHitByClus(modClus)) fillSegByModClus(modClus);
                  }
                } else fillHitSegByClus(); 
              } else {      // Mod. fit
                if(nMdcHits[mod]>=0) break;
                if(nModsSeg[sector][segment]==2) {
                  if(typeClFn==1 && mod!=modForSeg[segment]) break;
                  if(typeClFn==2 && mod<2 && mixClusCut>0) break;
                }
                if(fillHitByClus(fittingMod)) fillSegByModClus(fittingMod);
              }
              break;
            }
          }
...
Bool_t HMdc12Fit::fillArrays(HMdcList12GroupCells& lCells) {
  ...
  for(Int_t mod=0; mod<2; mod++) {       // relative number of module in segment
    ...
    fSCMod->getPlanePar(parA[mod],parB[mod],parD[mod]);
    for(Int_t lay=0;lay<6;lay++) {      // nl - Num. of layers in MDC1+MDC2
      HMdcSizesCellsLayer& fSCLay=(*fSCMod)[lay];
      if(lay==0 || lay==5) {
        Int_t ind=mod*2+lay/5;       
        fSCLay.getPlanePar(lParA[ind],lParB[ind],lParD[ind]);
        fSCLay.calcIntersection(xVertex,yVertex,zVertex,
        xPlane[4],yPlane[4],zPlane[4],xPlane[ind],yPlane[ind],zPlane[ind]);
      }
      Int_t nLayer=mod*6+lay;
      ...
      while((cell=lCells.next(nLayer,cell)) > -1) {
        loc[3]=cell;
	Int_t nTimes=lCells.getTime(nLayer,cell); // num. times for nWire
	HMdcCal1* cal=(HMdcCal1*)fCalCat->getObject(loc);
	if(!cal) {
          Error("fillArrays","S.%i M.%i L.%i  Cell %i is not fired",
              loc[0]+1,loc[1]+1,lay+1,cell+1);
          continue;
        }
        Float_t outside=0;
//         if(useWireOffset) wireOffset=fSCLay.getSignPath(xVertex,yVertex,zVertex,
//             xPlane[4],yPlane[4],zPlane[4],cell,outside) * signalSpeed;
        if(useWireOffset) {
          wireOffset=fSCLay.getSignPath(xVertex,yVertex,zVertex,
            xPlane[4],yPlane[4],zPlane[4],cell,outside) * signalSpeed;
          if(!isGeant)    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              wireOffset -= fSCLay[cell].getWireLength()*0.5*signalSpeed;
        }
        
//if(outside!=0.) Printf("S%iM%iL%iC%i outside=%f",sector,loc[1],lay,cell,outside);
        
//        HMdcSizesCellsCell& fSCCell=fSCLay[cell];
...
//---------------------------------------------------------------
        for(Int_t time=1; time<3; time++) {
          ...
          else tdcTime[nClTimes] = cal->getTime2();
          tdcTime[nClTimes] -= wireOffset;
          nMdcTime[mod]++;
          fSizeCellsLayer[nClTimes]=&fSCLay;
          modIndx[nClTimes]=loc[1];
          layIndx[nClTimes]=lay;
          cellIndx[nClTimes]=cell;
          timeIndx[nClTimes]=time;
          hitIndex[mod*6+lay][cell][time-1]=nClTimes;
...
Bool_t HMdc12Fit::fitCluster(Double_t threshold) {
  ...
  while(kTRUE) {
    ...
    if( delW<5.0 || (nCellM1<5 && nCellM2<5) ) break;  // 5.0 ??? (mnogo?)
...
Bool_t HMdc12Fit::fillHitByFit(Int_t iMod) {
  ...
  if(modClus>=0 && modClus!=mod) return kFALSE;
...
//       x,eX, y,eY, xDir,eXDir, yDir,eYDir);
  ...
  fillClusInf(fHit,iMod,1,indClusFit); // Cluster inform.
  return kTRUE;
}
...
//Printf("indClusFit=%i",indClusFit);
  ...
  for(Int_t hit=firstHit; hit<lastHit; hit++) {
    ...
    fWireFit->setAddress(sector, modIndx[hit], layIndx[hit], cellIndx[hit]);
    ...
    fWireFit->setFullTime(drTime[hit]+arrTm);
    ...
    fWireFit->setTdcTimeErr((Float_t)trackFitter->getTdcTimeErr(hit));
  }
