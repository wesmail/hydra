...
class HPidTrackData : public TObject
{
 ...
 public:
  ...
  Float_t     fAngleWithClosestLeptonCandidate;  //What is the angular distance with the closest lepton candidate? 
  Float_t     fAngleWithClosestHadronCandidate;  //What is the angular distance with the closest hadron candidate?
  Bool_t      closestHadronCandidateIsFitted;    // Is the inner MDC segment of the closest hadron candidate >0 ?
  Bool_t      closestLeptonCandidateIsFitted;    // Is the inner MDC segment of the closest lepton candidate >0 ?
 
  UShort_t    unInnerMdcNumber;               // number of MDC segments in the set of neighbours - not filled at present


  Float_t     fPull, fSplineChiSquare,fRKChiSquare;    //Quality parameters of KickPlane, Spline and RK-tracking

  Short_t     aInnerMdcIds[MAX_MDC_SEG];      // Array of Indices of inner MDC segments in the neighbourhood of the current particle's
                                              // segment - not filled at present

  Short_t     aInnerMdcCounts[MAX_MDC_SEG];   // How many times the segments in the neighbourhood were used for any track in this event
  ...
  */
  ...
  Float_t getAngleWithClosestLeptonCandidate() const {return fAngleWithClosestLeptonCandidate;} 
  Float_t getAngleWithClosestHadronCandidate() const {return fAngleWithClosestHadronCandidate;} 
  Bool_t  getClosestHadronCandidateIsFitted() const {return closestHadronCandidateIsFitted;}   
  Bool_t  getClosestLeptonCandidateIsFitted() const {return closestLeptonCandidateIsFitted;}   
  ...
  Short_t getInnerMdcId(Int_t index) const {return aInnerMdcIds[index];}     
  Short_t getInnerMdcCount(Int_t index) const {return aInnerMdcCounts[index];}  
  ...
  void setAngleWithClosestLeptonCandidate(Float_t o_AngleWithClosestLeptonCandidate)
    {fAngleWithClosestLeptonCandidate = o_AngleWithClosestLeptonCandidate;} 
  void setAngleWithClosestHadronCandidate(Float_t o_AngleWithClosestHadronCandidate)
    {fAngleWithClosestHadronCandidate = o_AngleWithClosestHadronCandidate;} 

  void setclosestHadronCandidateIsFitted(Bool_t o_closestHadronCandidateIsFitted)
    {closestHadronCandidateIsFitted = o_closestHadronCandidateIsFitted;}   
  void setclosestLeptonCandidateIsFitted(Bool_t o_closestLeptonCandidateIsFitted)
    {closestLeptonCandidateIsFitted = o_closestLeptonCandidateIsFitted;}   
  ...
  void     seterrZMeta(Float_t o_errZMeta) {errZMeta = o_errZMeta;}
    ...
    //Float_t setAngleWithClosePairCandidate( o_AngleWithClosePairCandidate){fAngleWithClosePairCandidate = o_AngleWithClosePairCandidate;} 

  
  ClassDef(HPidTrackData, 9)  // Track data for PID

    //obsolete variables:
    //Bool_t      bIsClosePairCandidate;         //Did this have a rejected (chi-square cut) trajcetory in its neighbourhood?
    //Float_t     fAngleWithClosePairCandidate;  //What is the angular distance with a possible close-pair candidate  
                                                 //(chi-square cut rejected trajcetory)?
