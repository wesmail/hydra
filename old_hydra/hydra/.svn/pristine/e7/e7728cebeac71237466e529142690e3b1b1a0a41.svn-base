...
#include "hhypXKineBeam.h"

// In principle, this number can be increased w/o problem (if needed)
#define HYP_FITTER_PARTICLES	6

using namespace std;

ClassImp(HHypXKineBeam)

HHypXKineBeam::HHypXKineBeam(Char_t *name_i, Option_t par[])
  :HHypBaseAlgorithm(name_i,par)
{
	fitter=new HLagrangeSolmitzBeamFit();
	kinetest=NULL;
}

HHypXKineBeam::~HHypXKineBeam()
{
	delete fitter;
}

Double_t HHypXKineBeam::getMomError(Double_t theta, Double_t phi, Double_t mass, Double_t p,Int_t sec,Bool_t simu)
	{
	Double_t res;
...
#define KINEDEBUG	0
Bool_t HHypXKineBeam::execute()
{
 	Int_t nMaxPart;
 	nMaxPart=mylist->getNpart();

	if (nMaxPart > HYP_FITTER_PARTICLES){
		cerr << "HHypXKineBeam::execute(): more than HYP_FITTER_PARTICLES not possible yet"<<endl;
		return kFALSE;
	}
...
//				cerr << algoName << " Theta or Phi Error<=0.. does not make sense to fit! " << endl;
			...
			/*{
				...
				if( hit){
					...
					cerr << algoName << "HHypXKineBeam::execute(): Not Hit data! " << endl;
					exit(0);
				}
			}*/
...
//			cout << ":: " << i <<": "<<pidofPart[i] <<" , "<< momentum_ini[i].Mag()*1e-3 << " , "<< theta <<" , " << phi<< endl;
		...
		if (icnt!=nPart){
			// this is the case, too, if invalid_input is set,
			// therefor this check has to be done after!
			Error("HypXKineBeam","BIG BIG error: valid particles!=getNValid()");
...
//			momentum_fitted[i].SetXYZ(p*sin(theta)*cos(phi),p*sin(theta)*sin(phi),p*cos(theta));
		...
		if (mylist->getIterStatus() == kFALSE) {
			cout << "!!!!!error HHypXKineBeam execute mylist->getIterStatus() == kFALSE!!!!!! " << endl;
			exit(2);
		}
...
Bool_t HHypXKineBeam::init()
{
	...
	if (histofile){
		qa = new TNtuple(output + TString("_kinebeam_debug"), "Kine Debug ntuple",
