...
//  Double_t xmid=cls.getXmid()/cls.getNBins();
//  Double_t ymid=cls.getYmid()/cls.getNBins();
//  Double_t dXmid=sqrt(cls.getErrXmid()/cls.getNBins()-xmid*xmid);
//  Double_t dYmid=sqrt(cls.getErrYmid()/cls.getNBins()-ymid*ymid); 
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
//????  cls.setXmid(point.getX());
//????  cls.setYmid(point.getY());
//????  cls.setZmid(point.getZ());
/*  
  ...
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
   point=system.transFrom(point);
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
   cls.setErrXmid(point.getX()-cls.getXmid());
   cls.setErrYmid(point.getY()-cls.getYmid());
Printf("CalcTr: dXmid=%e-%e=%e",point.getX(),cls.getXmid(),point.getX()-cls.getXmid());  
Printf("CalcTr: dYmid=%e-%e=%e",point.getY(),cls.getYmid(),point.getY()-cls.getYmid());    
*/
   cls.calcQuality();
   }
}
...
Bool_t HMdcCluster::Test(HMdcCluster& clst, Double_t Ap, Double_t Bp,
                         Double_t Dp, Double_t shX2, Double_t shY2) {
  Int_t nIdent[2];
  nIdent[0]=nIdent[1]=0;
...
void HMdcCluster::setCluster(Short_t numhitsI[], Short_t hitsI[][40],
                                                 Short_t numTimesI[][40]) {
