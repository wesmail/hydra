...
void TGo4RichOnlineSlots::StartMonitor()
{
  ...
  // do now init+start analysis here
   //-------------------------RICH------------------------------------
  fxTGo4GUIRegistry->ReDirectObject("hRichpulsheight",this,true);
}
...
{	
	...
	fxTGo4RichOnlineStatus->OverWriteLocalObject(fxdata);
	...
	if(fxTGo4RichOnline!=0)  UpdateGUI(fxdata->GetName());
}	
void TGo4RichOnlineSlots::UpdateGUI(const char *ObjName)
{
  TGo4LockGuard Global;

  TObject *Obj = fxTGo4RichOnlineStatus->GetObjectfromLocalList(ObjName);
