...
Bool_t HRichParRootFileIo::init(HParSet* pPar,Int_t* set) {
 ...
 using namespace HadAsciiAuxiliaryFunctions;
  ...
  if (pFile) {
    ...
    if (strcmp(name,"RichCorrelatorParameters")==0) return read((HRichCorrelatorPar*)pPar,set);
  }
...
//----------------------------------------------------------------------------
Bool_t HRichParRootFileIo::read(HRichCorrelatorPar* pPar,Int_t* set) {
 using namespace HadAsciiAuxiliaryFunctions;
  if (!readObject(pPar)) return kFALSE;
  ErrorMsg(0,"HRichParRootFileIo::read",
   1,"Container \'RichCorrelatorPar\' has been read from ROOT file.");
  return kTRUE;
}
//============================================================================
