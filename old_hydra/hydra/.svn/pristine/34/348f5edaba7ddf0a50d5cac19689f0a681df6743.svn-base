...
//allocate input/output categories
   ...
   //    m_pHitCat=gHades->getCurrentEvent()->getCategory(catRichHitSim);
    m_pHitCat=gHades->getCurrentEvent()->getCategory(catRichHit);
    //    cout<<"address of cat rich hit sim"<<m_pHitCat<<endl;
    if (!m_pHitCat) {
	//	cout<<"HRichAnalysisSim::init <> Message: building new lin category"<<endl;
	...
	//cout<<"address of cat rich hit sim"<<m_pHitCat<<endl;
      ...
      if (!m_pHitCat) return kFALSE;
	       ...
	       //                     ->addCategory(catRichHitSim, m_pHitCat, "Rich");
...
//cout<<"Executing Analysis at evt =  "<<cont<<endl; 
 ...
 HRichCal *pCal;
  ...
  while((pCal = (HRichCal *)fIter->Next())) {
   ...
   // the cut-off threshold is now applied already in the digitizer
  ...
  iEventNr++;
    ...
    // cout << "Event: " << iEventNr << " Pads fired #" << (fPadFired[0]+fPadFired[1]+fPadFired[2]+fPadFired[3]+fPadFired[4]+fPadFired[5]) << endl;
  ...
  for(secCount = 0; secCount < 6; secCount++) 
      ...
      iRingNrTot+=iRingNr;
       //if (iLabelNr > 0) {
      //cout<<"Event "<<iEventNr<<"  "<<iPadFiredNr<<" pads stored, "<<"cleaned="<<iPadCleanedNr<<" labels="<<iLabelNr<<" rings="<<iRingNr<<endl;
      // if(iRingNr>0) cout<<"Event "<<iEventNr<<" rings: "<<iRingNrTot<<endl;
      //}
	  
      //cout << "Event : "<<iEventNr<< " sector : "<<secCount<<" rings : "<<iRingNr<<endl;
