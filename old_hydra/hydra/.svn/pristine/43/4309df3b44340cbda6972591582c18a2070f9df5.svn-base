...
HMdcHit* HMdcFunc1::getMdcHit(HMdcSeg *seg,Int_t module){
  ...
  if(seg) {
    ...
    if(hit_inx>-1){
      if(getMdcHitCat()) {
	//	cout<<getMdcHitCat()->getEntries()<<endl;
	hit=(HMdcHit *)getMdcHitCat()->getObject(hit_inx);
	// cout<<hit<<endl;
	return hit;
      } else {
	printf("******MdcHitCat is NULL********\n");
	printf("******use HMdcFunc1::setMdcHitCat()********\n");
      }
    } 
  }
  return hit;
...
HMdcClusInf* HMdcFunc1::getClusInfObj(Int_t clsindex){
  ...
  if(getClusInfCat()){
    ...
    printf("******use HMdcFunc1::setMdcClusInfCat()********\n");
  }
  return pObj_clusinf;
}
...
Int_t  HMdcFunc1::getGeantCommonTrack(HRichHitSim * pRichHitSim,HKickTrackSim * pKickTrack){
...
HGeantKine* HMdcFunc1::getKineObj(Int_t pTrack){
  if(fGeantKineCat!=NULL) {
    ...
    printf("HMdcFunc1::getKineObj there is no fGeantKineCat defined please use HMdcFunc1::setGeantKineCat before\n");
    return NULL;
  }
}
...
Bool_t HMdcFunc1::isConv(HGeantKine *kine){
  //here is checking only whether mother is gama and pair prod is process
  Int_t pTrack_par=-1,pMed=-1,pMech=-1,pID_par=0;
  Int_t pTrack_particle=-1,pID_particle=0;
...
Bool_t HMdcFunc1::isDouble(HMdcHitSim *hit,Int_t conv_mode){
  ...
  //Using the geant track numbers of the tracks in mdcseg (5 tracks stored) decide whether 
  //it is a double tracks from lepton or not
  //conv_more =0 seg is DOUBLE if there are 2 different leptons contributing to it and they are pair
  //conv_more =1 seg is DOUBLE if there are 2 different leptons contributing to it and leptons are conversion pair
  //Update from v6_12 for complete list of track num related to hit one has to look at MdcClusInf
  Int_t debug=0;
  Int_t clsindex=-1;
  Int_t track_mul=0;
  Int_t pID2=0,pID1=-1,pTrack_par1=-1,pTrack_par2=-2,dummy=-1;
  ...
  if(pObj_clusinf){
    ...
    if(track_mul>1&&pObj_clusinf->getNTimesInTrack(1)>2){
      ...
      if (debug==1){     
	if (track_mul==2) printf("mul=%i t0=%i(%i) t1=%i(%i)\n",track_mul,pObj_clusinf->getTrack(0),pObj_clusinf->getNTimesInTrack(0),pObj_clusinf->getTrack(1),pObj_clusinf->getNTimesInTrack(1));
	
	if (track_mul==3) printf("mul=%i t0=%i(%i) t1=%i(%i) t2=%i(%i) \n",track_mul,pObj_clusinf->getTrack(0),pObj_clusinf->getNTimesInTrack(0),pObj_clusinf->getTrack(1),pObj_clusinf->getNTimesInTrack(1),pObj_clusinf->getTrack(2),pObj_clusinf->getNTimesInTrack(2));
	
	if (track_mul==4) printf("mul=%i t0=%i(%i) t1=%i(%i) t2=%i(%i) t3=%i(%i)\n",track_mul,pObj_clusinf->getTrack(0),pObj_clusinf->getNTimesInTrack(0),pObj_clusinf->getTrack(1),pObj_clusinf->getNTimesInTrack(1),pObj_clusinf->getTrack(2),pObj_clusinf->getNTimesInTrack(2),pObj_clusinf->getTrack(3),pObj_clusinf->getNTimesInTrack(3));
	
	if (track_mul==5) printf("mul=%i t0=%i(%i) t1=%i(%i) t2=%i(%i) t3=%i(%i) t4=%i(%i)\n",track_mul,pObj_clusinf->getTrack(0),pObj_clusinf->getNTimesInTrack(0),pObj_clusinf->getTrack(1),pObj_clusinf->getNTimesInTrack(1),pObj_clusinf->getTrack(2),pObj_clusinf->getNTimesInTrack(2),pObj_clusinf->getTrack(3),pObj_clusinf->getNTimesInTrack(3),pObj_clusinf->getTrack(4),pObj_clusinf->getNTimesInTrack(4));
      }
	...
	for(Int_t j=i+1;j<track_mul;j++){
	 ...
	 if(debug==1)   printf(" Module=%i DOUBLE %i %i \n",hit->getModule(),pObj_clusinf->getTrack(i),pObj_clusinf->getTrack(j));
...
Bool_t HMdcFunc1::isMdcSegInGeant(HMdcSegSim *seg,Int_t track){
  ...
  while ((pObj_geantmdc=(HGeantMdc*)iterator_mdcgeant->Next())!= 0) { 
    ...
    printf("To Call HMdcFunc1::isMdcSegInGeant(HMdcSegSim *seg,Int_t track) one has to set GeomMdc category\n");
  }
  return kFALSE;
}
...
Int_t  HMdcFunc1::getPairTrack(Int_t track1){
  ...
  if(track1>=0){
    ...
    //printf("A tr1=%i cr1=%i cr2=%i ID2=%i tr2=%i \n",track1,pTrack_par1,pTrack_par2,aID2,aTrack2);
    
    if(pTrack_par2==pTrack_par1) return (track1+1);

    getKineObj(track1-1)->getCreator(pTrack_par2,dummy,dummy);
    // printf("B tr1=%i cr1=%i cr2=%i \n",track1,pTrack_par1,pTrack_par2); 
    if(pTrack_par2==pTrack_par1) return (track1-1);
    
    /*
    getKineObj(track1+2)->getCreator(pTrack_par2,dummy,dummy);
    getKineObj(track1+2)->getParticle(aTrack2,aID2);
    printf("C tr1=%i cr1=%i cr2=%i ID2=%i tr2=%i \n",track1,pTrack_par1,pTrack_par2,aID2,aTrack2);
  
    if(pTrack_par2==pTrack_par1) return (track1+2);


    getKineObj(track1+3)->getCreator(pTrack_par2,dummy,dummy);
    getKineObj(track1+3)->getParticle(aTrack2,aID2);
   
    printf("D tr1=%i cr1=%i cr2=%i ID2=%i tr2=%i \n",track1,pTrack_par1,pTrack_par2,aID2,aTrack2);
    if(pTrack_par2==pTrack_par1) return (track1+3);
    ...
    while (tr>=0) {
      i=i+1;
      if(getKineObj(i)){
	getKineObj(i)->getCreator(pTrack_par2,dummy,pMech2);
	getKineObj(i)->getParticle(aTrack2,aID2);
	printf("%i cr1=%i cr2=%i ID2=%i tr2=%i pMech2=%i \n",i,pTrack_par1,pTrack_par2,aID2,aTrack2,pMech2);
	if(pTrack_par2==pTrack_par1 && aTrack2!=track1) return (i);
      } else {
	tr=-1;printf("LAST track=%i\n",i-1); 
      }
    }
...
HGeantKine*  HMdcFunc1::getPairTrack(HGeantKine* kine1){
  //this method finds a pair lepton to lepton with track1  
  //I use the fact that it is either track1+1 or track1+1
  Int_t pTrack_par1=-1,pTrack_par2=-2,dummy=0,track1=-1;
  if(kine1){
...
Bool_t HMdcFunc1::isInGeantMdc(Int_t tr){
  ...
  if(getGeantMdcCat()){   
    ...
    printf("To Call HMdcFunc1::isInGeantMdc(Int tr)one has to set HMdcFunc1::GeomMdc category\n");
  }
  return kFALSE;
}
...
Bool_t HMdcFunc1::isInGeantRichMirror(Int_t tr){
  ...
  if(getGeantRichMirrorCat()){   
    ...
    printf("To Call HMdcFunc1::isInGeantRichMirr(Int tr)one has to set HMdcFunc1::GeomRichMirr category\n");
  }
  return kFALSE;
}
...
Bool_t HMdcFunc1::isInGeantTof(Int_t tr){
  ...
  if(getGeantTofCat()){   
    ...
    printf("To Call HMdcFunc1::isInGeantTof(Int tr)one has to set HMdcFunc1::GeomTof category\n");
  }
  return kFALSE;
} 
...
Bool_t HMdcFunc1::isInGeantShower(Int_t tr){
  ...
  if(getGeantShowCat()){   
    ...
    printf("To Call HMdcFunc1::isInGeantShower(Int tr)one has to set HMdcFunc1::GeomShower category\n");
  }
  return kFALSE;
} 
...
Bool_t HMdcFunc1::couldBeAccepted(HGeantKine *kine){
  //check whether this particle was created before rich mirror that it could be possible flying to HADES 
  //acceptance and should be taken in an account for acceptance studies
  ...
  if(kine){
    ...
    if(material==3||material==4||material==8||material==9||material==10||material==17||material==19) return kTRUE;
...
Bool_t HMdcFunc1::passCprPdfCuts(HPidDilepton* pDil,Float_t prob,Char_t * name_of_pdf_file,Int_t mode_rs=0,Option_t *rej_condition){ 
...
Bool_t HMdcFunc1::passCprPdfCuts(HPidParticle* pPart,Float_t prob,Char_t * name_of_pdf_file,Int_t mode_rs=0,Option_t *rej_condition){
  ...
  Int_t sec =-1,nwires_part_0=-1,nwires_part_1=-1,level_0=-1,level_1=-1,the_b=-1,phi_b=-1,level_b0=-1,level_b1=-1,cls_part_0=-1,cls_part_1=-1,charge_part=-1;
  Float_t mdc_the=-1.0, mdc_phi=-1.0;

  sec=(getPidTrackCand(pPart))->getMdcSeg()->getSec();
  //if(sec==0) {printf("sec==%i\n",sec);return kFALSE;}

  mdc_the=(getPidTrackCand(pPart))->getMdcSeg()->getTheta();
  mdc_the=getNormalMdcTheta(mdc_the);
  the_b=calculateBin(mdc_the,0);
  mdc_phi=(getPidTrackCand(pPart))->getMdcSeg()->getPhi();
  ...
  //  Float_t prop_single=par.getProbSingle(the_b,phi_b,level_b0,level_b1,cls_part_0,cls_part_1,nwires_part_0,nwires_part_1);
  ...
  if(strstr(condition.Data(),"OR")){
    // cout<<"current condition "<<condition.Data()<<endl;
  ...
  if(strstr(condition.Data(),"AND")==NULL && strstr(condition.Data(),"OR")==NULL ){
    printf("THIS condition: %s is unknown possible: AND and OR",condition.Data());
  }
...
Int_t HMdcFunc1::whatCommonHits(HPidDilepton* dil){
  ...
  if(dil){
    ...
    if(pPart1&& pPart2){
      ...
      // pTrackCand1->print();
      //   pTrackCand2->print();
      //   printf("info dilepton hits:\n");
      //   printf("sectors:%i  %i \n",pTrackCand1->getSector(),pTrackCand2->getSector());
      //   printf("RICH hits:%i  %i \n",pTrackCand1->getRingId(),pTrackCand2->getRingId());
      //   printf("MDC  hits:%i  %i \n",pTrackCand1->getInnerMdcId(0),pTrackCand2->getInnerMdcId(0));
      //   printf("META hits:%i  %i \n",pTrackCand1->getMetaId(),pTrackCand2->getMetaId());
	...
	if(pTrackCand1->getRingId()==pTrackCand2->getRingId()){ 
	  //printf ("in dilepton common RICH ind1=%i ind2=%i\n",pTrackCand1->getRingId(),pTrackCand2->getRingId());
	  common_RICH=kTRUE;
	}
	...
	if((pTrackCand1->getInnerMdcId(0)==pTrackCand2->getInnerMdcId(0))&&(pTrackCand1->getSector()==pTrackCand2->getSector())){ 
	  //printf ("in dilepton common MDC ind1=%i ind2=%i\n",pTrackCand1->getInnerMdcId(0),pTrackCand2->getInnerMdcId(0));
	  common_MDC=kTRUE;
	}
	//check META Id
	
	if(pTrackCand1->getMetaId()==pTrackCand2->getMetaId()){
	  // printf ("in dilepton common META ind1=%i ind2=%i\n",pTrackCand1->getMetaId(),pTrackCand2->getMetaId());
	  common_META=kTRUE;
	}
