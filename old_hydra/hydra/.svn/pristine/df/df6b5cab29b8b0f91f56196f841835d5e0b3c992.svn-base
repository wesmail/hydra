...
//  Bit 1, SEGS_MATH_FALSE    : =0 - fake inner-outer segments matching
...
#define SEGS_MATH_FALSE     254

HMdcGeantSeg::HMdcGeantSeg(Int_t ns) {
  clear(ns);
}
...
Int_t HMdcGeantTrack::testMdcHits(void) {
  ...
  while((pGeantMdc=(HGeantMdc*)pGeantKine->nextMdcHit()) != NULL) {
    ...
    if(modPrev<0 || !isDirOk || !segment->addHit(pGeantMdc)) {
      ...
      segment->trackNumber = trackNumber;
    }
...
void HMdcGeantTrack::setStatusFlags(void) {
  ...
  if(nSegments<1 || nSegments>2)     trackStatus &= NOT_RECONSTRUCTABLE;
    ...
    if(nIOSeg==1 &&
             (mdcSp[2] || mdcSp[3])) trackStatus &= NOT_RECONSTRUCTABLE;
    if(nIOSeg==2 &&
             (mdcSp[0] || mdcSp[1])) trackStatus &= NOT_RECONSTRUCTABLE;
  } else if(nWires >= 0) {
    // wires are collected
  ...
  if( !isWrCollected ) pClusSim->calcTrList(); // old calculation
    ...
    for(Int_t n=0; n<nTrcks; n++) {
      ...
      if(gntSeg) {
        ...
        UChar_t status = ind == embedInd ? 255 : 0;
        if(pParentClus && ind == embedInd) {
          ...
          if(indPar >= 0) {
            pClusSim->setIsIOSMatching();
            pParentClus->setIsIOSMatching();
          } else {
            status &= SEGS_MATH_FALSE;
            pClusSim->setNoIOSMatching();
            pParentClus->setNoIOSMatching();
          }
        }
...
void HMdcTrackInfSim::testIOMatching(Int_t ind,HMdcClusSim* pClusSim,Int_t bin,
  ...
  if(noMatching) {
    pClusSim->setTrackStatus(bin,
          pClusSim->getTrackStatus(bin) & SEGS_MATH_FALSE);
    pClusSim->setNoIOSMatching();
    pClusSimPar->setNoIOSMatching();
  } else {
    pClusSim->setIsIOSMatching();
    pClusSimPar->setIsIOSMatching();
  }
// printf("#################### ind=%i bin=%i track=%i ############\n",ind,bin,track);
// pClusSim->print();
// pClusSimPar->print();
// printf("##################### %s ##############################\n",noMatching ? "noMatching":"isMatching");
