...
Bool_t HMdcTrackFitterH::init() {
  Bool_t flag=HMdcTrackFitter::init();
  return flag;
}

Bool_t HMdcTrackFitterH::reinit(void) {
...
//   }
  ...
  for(int time = firstTime; time < nClTimes; time++) {
    if(hitSt[time] == 0 || (hitSt[time]<0 && !flDstTime) ) continue;
  ...
  // 	   << driftTimeMy << " " << driftTimeJM << " " << driftTimeMyJM << endl;
    ...
    fullTime[time] = arriveTime+ driftTime;
    if (iflag == 5) {
      driftTime = ( distance / driftVelMDC4 )*1000.;
      fullTime[time] = arriveTime + driftTime;
//       if(segment == 0 && fullTime[time] > 200) fullTime[time] = 200;
//       if(segment == 1 && fullTime[time] > 1000) fullTime[time] = 1000;
    }
...
//      if (iflag == 1) weight = qualityWeight[time];
    if (iflag == 1) {
      ...
      if(version==0) {
        if( (segment == 0 && fullTime[time] > 200) ||
            (segment == 1 && fullTime[time] > 1000) ) {
