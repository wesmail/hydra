...
// Modified: 27/02/02 by Peter Zumbruch
...
void HMdcDataWord::dump(){ 
 ...
 cout << "samP    : " << getSamP() << endl; 
 cout << "Roc#    : " << getRoc()  << endl;  
 cout << "RocPos  : " << getRocP() << endl;  
 cout << "Mbo#    : " << getBus()  << endl;
 cout << "TDC#    : " << getTdc()  << endl;  
 cout << "Channel : " << getChan() << endl;
 cout << "Hit     : " << getHit()  << endl;
 cout << "Time    : " << getTime() << endl;
 cout << "EvtSeqNr: " << gHades->getCurrentEvent()->getHeader()->getEventSeqNumber() << endl;
  ...
  if (pSubEvt && pReadout) 
      ...
      while( data < end ) 
	{
	  ...
	  if(w->subHeader(data))
		...
		if (w->getSam()<0 || w->getTdc() < 0)
		  {
		    Char_t tmp[160];
		    sprintf(tmp,"Negative values while decoding Data-Word %x in Event: %i",
			    (UInt_t) *deb,
			    gHades->getCurrentEvent()->getHeader()->getEventSeqNumber());
		    Error("Error in hmdcunpacker::fill()",tmp);
		    w->dump();
		    goto loop;
		  }
		...
		if (standalone) 
		  {
		      ...
		      if (w->getTrigType()==kMdc_Standalone_CalibrationEvent)
		      {
			gHades->getCurrentEvent()->getHeader()->setId(kMdc_Standalone_CalibrationEvent);
		      }
		  }
