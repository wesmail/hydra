...
Bool_t HRichParRootFileIo::read(HRichCalPar* pPar,Int_t* set) {
 using namespace HadAsciiAuxiliaryFunctions;
  if (!readObject(pPar)) return kFALSE;
  cout << "calling read func for calpars" << endl;
  cout << "Number of calparrows after reading: " << pPar->getNRows() <<endl;
  cout << "Number of calparcols after reading: " << pPar->getNColumns() <<endl;
  ErrorMsg(0,"HRichParRootFileIo::read",
...
Bool_t HRichParRootFileIo::read(HRichThresholdPar* pPar,Int_t* set) {
  ...
  cout << "Readigng threshold pars from root file not yet allowed!" << endl;
  return kTRUE;
}
//==
