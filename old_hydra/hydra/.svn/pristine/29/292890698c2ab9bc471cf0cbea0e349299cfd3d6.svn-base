...
Int_t HMdcList12GroupCells::compare(HMdcList12GroupCells* lst,
  ...
  // if lstIdent!=0 list of identical wires will stored in lstIdent
  if(l1>=12) return 0;
  setInBounds(l1,l2);
  Int_t nLay=0;
  ...
  for(Int_t lay=l1; lay<=l2; lay++) {
    ...
    if(c1>c2) continue;  // no overlap
    if(lstIdent) {
      if(HMdcTBArray::andArr(arr+lay*laySz+((c1-firstCell[lay])>>2),
         (lst->arr)+lay*laySz+((c1-lst->firstCell[lay])>>2),(c2-c1)>>2,
         (lstIdent->arr)+lay*laySz)) {
        lstIdent->firstCell[lay]=c1;
        nLay++;
      }
    } else if(HMdcTBArray::compare(arr+lay*laySz+((c1-firstCell[lay])>>2),
