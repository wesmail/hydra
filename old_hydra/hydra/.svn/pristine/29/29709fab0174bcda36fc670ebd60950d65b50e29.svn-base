...
Int_t HKickDetDigi::digiMdc2(void) {
  ...
  while ( (geMdc=(HGeantMdc *)fMdcIter->Next()) != 0) {
	...
	geMdc->getHit(x,y,tof);
	geMdc2->getHit(x2,y2,tof);
	if (!isMdcPerfect()) {
...
void HKickDetDigi::fillData(HGeomVector &r,HGeomVector &alpha,
  ...
  } else Warning("fillData","No slot available");
}
...
void HKickDetDigi::calcPosDir(HGeantMdc *data, HGeantMdc *data2,
  ...
  HModGeomPar *module=fGeometry->
    getModule(0, //Transform to sector coordinate system. not LAB system
	      data->getModule());
  ...
  HModGeomPar *module2=fGeometry->
    getModule(0,data2->getModule());
  if (!module2) Error("calcDir",
		     "Module %i:%i invalid\n",
		     data2->getSector(),data2->getModule());
  HGeomTransform &modTrans2=module2->getLabTransform();
