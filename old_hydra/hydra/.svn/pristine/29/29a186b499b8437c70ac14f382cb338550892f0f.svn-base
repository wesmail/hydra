...
protected:
    ...
    TString fAuthor;       // creator of the parameters
    TString fDescription;  // description
    TString fRunsUsed;     // runs used to create the parameters
    TString fValidity;     // validity range of the parameters

public:
    HMdcCalParRaw(const char* name="MdcCalParRaw",
                  const char* title="raw calibration parameters for Mdc",
                  const char* context="",
                  Int_t n=6);
    ~HMdcCalParRaw();
    ...
    const char* getAuthor(){return fAuthor.Data();}
    const char* getMethodDescription(){return fDescription.Data();}
    const char* getRunsUsed(){return fRunsUsed.Data();}
    const char* getValidity(){return fValidity.Data();}
    void setAuthor(const char* s) {fAuthor=s;}
    void setMethodDescription(const char* s) {fDescription=s;}
    void setRunsUsed(const char* s) {fRunsUsed=s;}
    void setValidity(const char* s) {fValidity=s;}
    ...
    void clear();
    void printParam();
    ClassDef(HMdcCalParRaw,1) // Container for the MDC calibration parameters on raw level
};
