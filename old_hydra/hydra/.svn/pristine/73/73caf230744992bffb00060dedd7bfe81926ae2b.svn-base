...
//*-- Version: $Id: muEmulation.cc,v 1.23 2009-07-15 11:34:06 halo Exp $
...
//----------------------------------------------------------------------------
HMUEmulation::HMUEmulation(const Text_t *name,const Text_t *title)
  ...
  for (Int_t i = 0; i < nEmuLeptons; i++)
    {
      ...
      if (emuLepton!=NULL)
        {
           emuLepton=new(emuLepton) HMUEMULeptons(*((HMUEMULeptons*)(pEmuLeptons->At(i))));
        }
    }
}
//============================================================================
  ...
  while ((pMUTof = (HMatchUTof *)iterTof->Next()) != 0)
    {
      ...
      if(sector<0||sector>5)
      {
	  Error("prepareInput()","tof Address out of Range in event %i sequence number %i !",
		gHades->getEventCounter(),
		gHades->getCurrentEvent()->getHeader()->getEventSeqNumber());
	  continue;
      }
  ...
  while ((pMUShower = (HMatchUShower *)iterShower->Next()) != 0)
    {
      ...
      if(sector<0||sector>5)
      {
	  Error("prepareInput()","shower Address out of Range in event %i sequence number %i !",
		gHades->getEventCounter(),
		gHades->getCurrentEvent()->getHeader()->getEventSeqNumber());
	  continue;
      }
