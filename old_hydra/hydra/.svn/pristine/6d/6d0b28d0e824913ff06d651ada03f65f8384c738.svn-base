// @(#)$Id: hpidgeanttrackset.cc,v 1.22 2006-11-28 13:25:57 halo Exp $
...
void HPidGeantTrackSet::fillFromMetaMatch(HMetaMatch* pMetaMatch) //OK
{
  ...
  if(pMetaMatch->getSystem()==0)
    {  
      ...
      if(pShwTof!=NULL)
	{
	  if(pShwTof->getTrack()>=0 || pShwTof->getTrack()==gHades->getEmbeddingRealTrackId())
	    {
	      nShowerTracks++;
	      ...
	      if(debugoutput&&sNCorrTrackIds) cout <<"Geant Track id in Shower: " << pShwTof->getTrack()<<endl;
	    }
	  else 
	    {
      ...
      else if ( nClsSize >= 1 )
	{
	  ...
	  if(NULL!=pTofClsSim)
		...
		for(Int_t i=0;i<pTofClsSim->getNParticipants()&&i<3;i++)
		{
		    ...
		    if (nTrc0 != nTrc1){
                        // double hit, add both tracks to map
  ...
  if(pTrkCand == NULL)
      ...
      if(pInnerMdcSeg != NULL) //we found an inner mdc segment
	{
	  for(i = 0; i < pInnerMdcSeg->getNTracks(); i++) //loop over all contributing tracks ...
	    {  
              if(pInnerMdcSeg->getTrack(i)==-99) continue; // skip cells which contain no valid hit
      ...
      if(pOuterMdcSeg != NULL) //There is a segment in MDC III and IV
	{
	  ...
	  {
	      if(pOuterMdcSeg->getTrack(i)==-99) continue; // skip cells which contain no valid hit
