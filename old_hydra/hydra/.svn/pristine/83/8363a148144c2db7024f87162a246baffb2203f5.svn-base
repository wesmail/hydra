...
HOraSlowChannel::HOraSlowChannel(const char* name) {
...
void HOraSlowChannel::setRunSumData(TObjArray* p) {
  // Sets the pointer to the array of summary data
  clearRunSumData();
  pRunSumData=p;
  if (p) runSumDataIter=p->MakeIterator();      
}

void HOraSlowChannel::setRawData(TObjArray* p,const char* t1,const char* t2) {
...
void HOraSlowChannel::printRunSumData(Int_t opt) {
  ...
  if (pRunSumData&&partition) {
    ...
    cout<<"          number of data, run id, filename\n";
    cout<<"---------------------------------------------------------------------\n";
    while ((p=((HOraSlowChanRunSum*)(runSumDataIter->Next())))!=0) {
      if (opt==0
        ...
        cout<<setw(5)<<i;
        p->print(maxPrecision+1,partition->getPeriod(i));
      }
      i++;
    }
    cout<<"---------------------------------------------------------------------\n";
...
void HOraSlowChannel::writeRunSumData(fstream& fout,Int_t opt) {
  ...
  if (pRunSumData&&partition) {
    ...
    fout<<"          number of data, run id, filename\n";
    fout<<"---------------------------------------------------------------------\n";
    while ((p=((HOraSlowChanRunSum*)(runSumDataIter->Next())))!=0) {
      if (opt==0
          ||(opt==1&&strlen(((HOraSlowPeriod*)(runs->At(i)))->getFilename())>4)
          ||(opt==2&&((HOraSlowPeriod*)(runs->At(i)))->getFilterFlag()==1)) {
        fout<<setw(5)<<i;
        p->write(fout,maxPrecision+1,partition->getPeriod(i));
      }
      i++;
    }
    fout<<"---------------------------------------------------------------------\n";
...
Bool_t HOraSlowChannel::readRawData(Int_t index) {
  ...
  return gHOraSlowManager->getOraReader()->readRawData(this,period->getStartTime(),period->getEndTime());
}

Bool_t HOraSlowChannel::readRawData(const char* t1, const char* t2) {
...
void HOraSlowChannel::printRawData() {
  ...
  if (pRawData) {
    ...
    cout<<"  Format:      timestamp, nano seconds, value\n";
...
void HOraSlowChannel::writeRawData(fstream& fout) {
  ...
  if (pRawData) {
    ...
    fout<<"  Format: seconds since start time, nano seconds, value\n";
