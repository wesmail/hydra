...
//*-- Author : D.Vasiliev
//*-- Modified: 05/10/2000 by M.Golubeva
...
Int_t HTofinoDigitizer::execute(void) {
  ...
  while ((pGeant=(HGeantTof *)iter->Next())!=0) {  // loop over HGeant hits
    ...
    pGeant->getHit(geaEner,geaX,geaY,geaTof); // geaX, geaY in mm, geaTof in ns
...
// Translate Geant fLoc to HTofinoCal fLoc
    ...
    Int_t nHits = pCal->getNHit(); // number of hits in this cell up till now
