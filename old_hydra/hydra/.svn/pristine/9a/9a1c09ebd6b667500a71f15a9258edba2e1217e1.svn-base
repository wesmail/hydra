...
HMdcTdcChannelMod::HMdcTdcChannelMod(Int_t sec, Int_t mod, Int_t mbo)
{
  ...
  if (pMdc)
		...
		{
			ERROR_msg(DET_MDC,"HMdcTdcChannelMod not created, container MdcRawStruct not found");
			exit(1);
		}
}
...
{ 
  ...
  Bool_t isFirst; 
	...
	if(!status)
		{
			ERROR_msg(DET_MDC,"This version is no longer valid for DAQ!");
			return kFALSE;
		}
...
void HMdcTdcChannel::printParam() {
  // prints the container
	SEPERATOR_msg("-",60);
	INFO_msg(10,DET_MDC,"HMdcTdcChannel");
	for(Int_t s=0;s<getSize();s++) {
		HMdcTdcChannelSec& sec=(*this)[s];
		for(Int_t m=0;m<sec.getSize();m++) {
	    HMdcTdcChannelMod& mod=sec[m];
