...
Double_t HTool::skew(Int_t n, Short_t* data)
{
    // calculates the skew
    //
    //   ---                     //
	...
	sum+= TMath::Power(data[i]-mean,3);
        //sum+= TMath::Power(mean-data[i],3);
    }
    Double_t skew=sum/((n)*TMath::Power(sigma,3));
    return  skew;
}
Double_t HTool::skew(Int_t n, Int_t* data)
{
    // calculates the skew
    //
    //   ---                     //
...
Double_t HTool::skew(Int_t n, Float_t* data)
{
    // calculates the skew
    //
    //   ---                     //
...
Double_t HTool::skew(Int_t n, Double_t* data)
{
    // calculates the skew
    //
    //   ---                     //
...
Bool_t HTool::fitScatter(TH2* h,TF1* f,TH1* hnorm,TString opt,Float_t xmin,Float_t xmax,Float_t ymin,Float_t ymax,Float_t window,Bool_t clean)
{
    if(h==0) return kFALSE;
    ...
    Bool_t usenorm=kFALSE;
    if(hnorm!=0)usenorm=kTRUE;
    ...
    if(max==0) return kFALSE;
    Int_t ctpoint=0;
    Double_t x,y,val,err,norm=1.;
    for(Int_t i=1;i<=nbinsx;i++){
        ...
        for(Int_t j=1;j<=nbinsy;j++){
            ...
            if(usenorm)
            {
                norm=hnorm->GetBinContent(i);
                if(norm==0)norm=1.;
            }
            err=(max-(val/norm))/max;
