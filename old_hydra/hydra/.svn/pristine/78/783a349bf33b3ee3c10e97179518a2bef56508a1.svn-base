...
void HMdcClusFit::Streamer(TBuffer &R__b) {
  ...
  if (R__b.IsReading()) {
    ...
    if (R__v > 1) {
      ...
      Float_t timeOff;
      R__b >> timeOff;
      timeOffMdc1=timeOffMdc2=timeOffMdc3=timeOffMdc4=-1000.;
      if(mod==0) timeOffMdc1=timeOff;
      else if(mod==1) timeOffMdc2=timeOff;
      else if(mod==2) timeOffMdc3=timeOff;
      else if(mod==4) timeOffMdc4=timeOff;
      else if(seg==0||seg==3) timeOffMdc1=timeOffMdc2=timeOff;
      else if(seg==1||seg==3) timeOffMdc3=timeOffMdc4=timeOff;
    }
    R__b >> numOfWires;
    R__b >> numOfLayers;
    R__b >> x1;
    ...
    R__b >>       clustIndex;
    if (R__v < 3) {
      if(clustIndex<0) clustIndex = -1;
      else             clustIndex = -clustIndex-2;
    }
    if (R__v > 1) {
      R__b >> exitFlag;
      R__b >> sigmaChi2;
