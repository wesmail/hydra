...
protected:
    ...
    Int_t mdcRaw;          ///< switch mdc raw on/off
    Int_t mdcCal;          ///< switch mdc cal on/off
    Int_t mdcHit;          ///< switch mdc hit on/off
...
public:
    ...
    {
	if(mdcHit==1)return 3;
	if(mdcCal==1)return 2;
	if(mdcRaw==1)return 1;
	else         return 0;
    }
   /// set max level of mdc
    void   setMdcMaxLevel(Int_t i)
    {
	if(i==3)     {mdcHit=mdcCal=mdcRaw=1;}
	else if(i==2){mdcHit=0;mdcCal=mdcRaw=1;}
	else if(i==1){mdcHit=mdcCal=0;mdcRaw=1;}
	else if(i==0){mdcHit=mdcCal=mdcRaw=mdc=0;}
    }
