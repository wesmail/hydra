// @(#)$Id: hpidtrackfiller.cc,v 1.12 2004-07-08 13:45:52 christ Exp $
            ...
            if(pitRich != NULL)
            {
               ...
               if(fP>1000.){
                ...
                while((pRing = (HRichHit *) pitRich->Next()) != NULL)
                {
					...
					if((pRing->getSector() != iSector)) continue;

					if((pRing->getTheta() - fKickTheta) 
						<(pParams->getLeftKickRichTheta(iSector)).fArray[nIndex]) continue;

					if((pRing->getTheta() - fKickTheta) 
						>(pParams->getRightKickRichTheta(iSector)).fArray[nIndex]) continue;
					
					if(((pRing->getPhi() - fKickPhi) * fSinTheta) 
					     <(pParams->getLeftKickRichPhi(iSector)).fArray[nIndex]) continue;

					if(((pRing->getPhi() - fKickPhi) * fSinTheta) 
					     >(pParams->getRightKickRichPhi(iSector)).fArray[nIndex]) continue;
                    ...
                    while((pMdcSeg = (HMdcSeg *) pitMdc->Next()) != NULL)
                    {
						...
						if((pRing->getTheta() - fMdcTheta) 
							<(pParams->getLeftMdcRichTheta()).fArray[iSector]) continue;

						if((pRing->getTheta() - fMdcTheta) 
							>(pParams->getRightMdcRichTheta()).fArray[iSector]) continue;
						
						if(((pRing->getPhi() - fMdcPhi) * fSinTheta) 
							 <(pParams->getLeftMdcRichPhi()).fArray[iSector]) continue;

						if(((pRing->getPhi() - fMdcPhi) * fSinTheta) 
							 >(pParams->getRightMdcRichPhi()).fArray[iSector]) continue;
