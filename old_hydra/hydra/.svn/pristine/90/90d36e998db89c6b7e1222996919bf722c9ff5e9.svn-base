...
HMdcSizesCells::HMdcSizesCells() : HParSet() {
  ...
  if(gHades) {
    ...
    if(fmdc) {
      ...
      isContainer=calc->isSet();
      if(isContainer == kFALSE) return;
      for (Int_t sec = 0; sec < 6; sec++) {
        if(!fmdc->getModule(sec,0) && !fmdc->getModule(sec,1) &&
           !fmdc->getModule(sec,2) && !fmdc->getModule(sec,3) ) continue;
...
HMdcSizesCells* HMdcSizesCells::getObject() {
  HMdcSizesCells* fSizesCells=0;
  if( gHades ) {
    HRuntimeDb* rdb=gHades->getRuntimeDb();
    if( rdb ) {
      fSizesCells=(HMdcSizesCells*)rdb->getContainer("MdcSizesCells");
      if( !fSizesCells ) fSizesCells=new HMdcSizesCells();
      if( !((HParSet*)fSizesCells)->init() ) return 0;
    }
  }
  return fSizesCells;
}
...
HMdcSizesCells* HMdcSizesCells::getExObject() {
  ...
  if( gHades ) {
    ...
    if( rdb ) {
      fSizesCells=(HMdcSizesCells*)rdb->getContainer("MdcSizesCells");
      if( fSizesCells && !((HParSet*)fSizesCells)->init() ) return 0;
    }
  }
  return fSizesCells;
}

HMdcSizesCellsCell* HMdcSizesCells::getObject(Int_t sc, Int_t md, 
                                                  Int_t lr,Int_t cl) {
  HMdcSizesCells* fSizesCells=HMdcSizesCells::getObject();
  if(!fSizesCells) return 0;
  HMdcSizesCellsSec* fSec=&((*fSizesCells)[sc]);
  if(!fSec) return 0;
  HMdcSizesCellsMod* fMod=&((*fSec)[md]);
  if(!fMod) return 0;
  HMdcSizesCellsLayer* fLayer=&((*fMod)[lr]);
  if(!fLayer) return 0;
  return &((*fLayer)[cl]);
}

HMdcSizesCellsCell* HMdcSizesCells::getExObject(Int_t sc, Int_t md, 
                                                  Int_t lr,Int_t cl) {
...
void HMdcCalcSizesCells::setParContainers() {
  ...
  if( !fMdcDet || !fLayerGeomPar || !fGeomPar ) set=kFALSE; 
}
...
Bool_t HMdcCalcSizesCells::calcVolCells(Int_t nSec, HMdcSizesCells* fSizesCells) {
  ...
  for(Int_t nmo=0; nmo<4; nmo++) {
    ...
    for(Int_t nla=0; nla<6; nla++) {
      ...
      Double_t dst=pLaGePar.getCatDist()*0.5;      // mm!
      for(nPoint=0; nPoint<8;nPoint++) {
        volSeWi[nPoint]=*(pGeomVol->getPoint(nPoint)); // mm!
      ...
      Double_t pitch=pLaGePar.getPitch();      // mm!
      ...
      for(Int_t nCell=0; nCell<nCells; nCell++) {
        ...
        if(!calcWireLine(wireP1, wireP2)) {
          if(nCell<100) {
            ...
            nSec+1,nmo+1,nla+1,nCell+1);
            continue;
          } else {
            Printf("Warning: S%i M%i L%i: Cells %i-%i are out of MDC!",
            nSec+1,nmo+1,nla+1,nCell+1,nCells);
            break;
          }
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//
// COMMENT OUT THE NEXT TWO LINES FOR SIMULATION !!!

if(nmo==0) for(Int_t i=0; i<8; i++) cell[i].setZ(cell[i].getZ()-28.);
if(nmo==1) for(Int_t i=0; i<8; i++) cell[i].setZ(cell[i].getZ()-15.);

//Double_t sh=(nSec+5)*1.0;
//if(nmo==0) for(Int_t i=0; i<8; i++) cell[i].setZ(cell[i].getZ()-5.);
//if(nmo==0) for(Int_t i=0; i<8; i++) cell[i].setY(cell[i].getY()+sh);
/*if(nmo==1) {
  for(Int_t i=0; i<8; i++) {
    cell[i].setZ(cell[i].getZ()+2.);
    cell[i].setX(cell[i].getX()+5.5);
    cell[i].setY(cell[i].getY()+sh);
  }
}*/
...
//calculation of parameters of external lines of plane
