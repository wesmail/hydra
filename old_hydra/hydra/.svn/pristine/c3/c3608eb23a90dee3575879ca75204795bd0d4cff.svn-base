// @(#)$Id: hpidtrackfiller.cc,v 1.17 2005-02-16 10:23:55 christ Exp $
...
Int_t HPidTrackFiller::execute(void)
{
  ...
  while((pMatch = (HMetaMatch *)pitMatch->Next()) != NULL)
      ...
      for(Int_t i=0;i<nMomAlgs;i++)
	{
      ...
      if(!pMatch->isKickAcceptde() && !pMatch->isSplineAccepted()){
...
Float_t HPidTrackFiller::correctRingShift(HPidTrackCand* pCand)
{
  ...
  Float_t diffbefore=pHitData->getDeltaThetaMDCRICH();
  ...
  return pHitData->getDeltaThetaMDCRICH();
