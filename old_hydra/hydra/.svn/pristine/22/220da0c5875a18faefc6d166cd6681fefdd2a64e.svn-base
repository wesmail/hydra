...
const char* HGeomHit::getDetectorName() {
...
void HGeomHit::write(fstream& fout) {
  ...
  if (nh>0) {
    fout<<&chnamh[0];
    for(Int_t i=1;i<nh;i++) fout<<"   "<<&chnamh[i*5];
    fout<<'\n'<<nbitsh[0];
    for(Int_t i=1;i<nh;i++) fout<<"   "<<nbitsh[i];
    fout<<'\n'<<orig[0];
    for(Int_t i=1;i<nh;i++) fout<<" "<<orig[i];
    fout<<'\n'<<fact[0];
    for(Int_t i=1;i<nh;i++) fout<<" "<<fact[i];
    fout<<'\n';
  }
}
...
void HGeomHit::print() {
  ...
  if (nh>0) {
    cout<<&chnamh[0];
    for(Int_t i=1;i<nh;i++) cout<<"   "<<&chnamh[i*5];
    cout<<'\n'<<nbitsh[0];
    for(Int_t i=1;i<nh;i++) cout<<"   "<<nbitsh[i];
    cout<<'\n'<<orig[0];
    for(Int_t i=1;i<nh;i++) cout<<" "<<orig[i];
    cout<<'\n'<<fact[0];
    for(Int_t i=1;i<nh;i++) cout<<" "<<fact[i];
    cout<<'\n';
  }
  if (nv>0) {
    cout<<"Hits for sensitive volume: "<<currentNode->GetName()
        <<"   idtype: "<<getIdType()<<"   nv: "<<nv<<'\n';
    for(Int_t i=0;i<nv;i++) cout<<i<<"  "<<&chnmsv[i*5]<<"  "<<nbitsv[i]<<'\n';
  }
}
...
void HGeomHit::fill(Int_t i,const char* cn,Int_t cb,Float_t co,Float_t cf) {
