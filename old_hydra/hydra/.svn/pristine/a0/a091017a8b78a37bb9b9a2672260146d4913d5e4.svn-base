...
//
///////////////////////////////////////////////////////////////////////////
void HShowerHitFinder::calcCoordWithSigma(HShowerHit* hit, HLocation &fLoc, int nRange) {
  HGeomVector vLocalCoord;

  HShowerPadTab *pPads = ((HShowerGeometry *)getGeometry())
					->getPadParam(fLoc[1]); // Access to the pad plane (module== fLoc[1])
  ...
  HShowerCal* pCell;
    ...
    for(fTmpLoc[3] = iCol1; fTmpLoc[3] <= iCol2; fTmpLoc[3]++)
    {
        ...
        ((HShowerGeometry*)getGeometry())->getLocalCoord(fTmpLoc, vLocalCoord); //position in Local Sys.
  	...
  	// Determine Sigma_X (geometry), pad_size/sqrt(12), sqrt(12)-standard deviation of a uniform density 
        float fXLD = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getXld_mm();                     
        float fXRD = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getXrd_mm();                     
        float fXLU = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getXlu_mm();                     
        float fXRU = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getXru_mm();                     
        sigma_X_tmp = ((fabs(fXLU-fXRD)>fabs(fXLD-fXRU))?
						(fabs(fXLU-fXRD)):(fabs(fXLD-fXRU)));
        ...
        float fYLD = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getYld_mm();                     
        float fYRD = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getYrd_mm();                     
        float fYLU = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getYlu_mm();                     
        float fYRU = pPads->getPad(fTmpLoc[2],fTmpLoc[3])->getYru_mm();                     
        sigma_Y_tmp = ((fabs(fYLU-fYRD)>fabs(fYLD-fYRU))?
						(fabs(fYLU-fYRD)):(fabs(fYLD-fYRU)));
...
/*      if(0){ // create control file
        int nEvt=gHades->getCurrentEvent()->getHeader()->getEventSeqNumber(); // get Event Seq. Nb.
        ofstream out1("results.txt", ios::app);

        HGeomVector vLocalCoord_Tmp;
            ((HShowerGeometry*)getGeometry())->getLocalCoord(fLoc, vLocalCoord_Tmp);
        HGeomVector2 vLabCoord;
            ((HShowerGeometry*)getGeometry())->getSphereCoord(fLoc, vLabCoord, &targetPos);
