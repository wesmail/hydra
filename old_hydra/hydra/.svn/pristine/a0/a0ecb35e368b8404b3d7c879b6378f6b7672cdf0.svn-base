...
//      The tracks will searched in all MDC of sector together to
//    respect of target (needs in a alignment of MDC's).
...
//timer.Start();
  ...
  while ((cal=(HMdcCal1 *)iter->Next())!=0) {
    ...
    if( !isGeant ) {
      ...
      nHits=1;                             // real data, in f. will ...
    } else if(((HMdcCal1Sim *)cal)->getStatus()==-1) continue;
    
    if(isCoilOff || m<2)  (*fLookUpTb)[s].setCell(m,l,c,nHits);
    else if(f34ClFinder) (*f34ClFinder)[s][m][l].setCell(c,nHits);
    numHits[s][m]++;
  }
...
void HMdcTrackFinder::fillCatMdcClus(Int_t sec) {
  ...
  errTar*=0.5;
  errTar.abs();
  const HMdcPrPlane& prPlane=flookup.getParPlane();
