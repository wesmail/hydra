...
//  Int_t indep;          // =0 - fit all hits in cluster
  ...
  Int_t typeClFn;      // type of cluster finder
  ...
  Double_t yVertex;    //
  Double_t zVertex;    //
  Double_t xPlane[5];  // [0-1] - points on the layers 0&5 in the first mdc,
  Double_t yPlane[5];  // [2-3] - points on the layers 0&5 in the second mdc,
  Double_t zPlane[5];  // [5] - point on the project plane
  Double_t parA[3];    // parameters of the planes: parA*x+parB*y+z=parD
  Double_t parB[3];    // [0]-[2] - mdc1, mdc2, pr.plane
  Double_t parD[3];    //
  Double_t lParA[4];   // param. of the layers planes: lParA*x+lParB*y+z=lParD
  Double_t lParB[4];   // [0]-[1] - layers 0 and 5 in first mdc
  Double_t lParD[4];   // [2]-[3] - layers 0 and 5 in second mdc
  ...
  Double_t* drTime;         // calcul. drift time
...
public:
  ...
  void setSignalSpeed(Float_t sp) {signalSpeed=sp;}
  void setNotFillByClus() {notFillByClus=kTRUE;}
...
//  void fillMdcHitSeg(void);
  ...
  void fillClusInf(HMdcHit* fHit,Int_t iMod,Char_t fitSt,Short_t indClusFit=-1);
