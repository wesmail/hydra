...
//
// track filter options (for hhyplistfiller):
// cleaner=none      -- take all PidTrackCand
// cleaner=builtin   -- internal cleaner
// cleaner=official  -- isUsed flag of PidTrackCleaner
// cleaner=orring    -- isUsed || HasRing
// if not set, no cleaner at all (none)
//
////////////////////////////////////////////////////////////////////////
...
Bool_t HHypAllFillerAlg::InitFillers(Int_t &fnr,Int_t particles_anz,Int_t *particles,Int_t missing_anz,Int_t *missing)
{
	...
	if(particles_anz>0){
		...
		cout << endl;
      filler[fnr]->setFilterType(typeDoFilter);
...
Bool_t HHypAllFillerAlg::init()
{
  ...
  abc=GetOpt("cleaner=");
  if(abc){
		if(abc->CompareTo("none",TString::kIgnoreCase)==0){
      typeDoFilter=0;
      cout << "Filler is using NONE Track cleaner!" << endl;
    }else if(abc->CompareTo("builtin",TString::kIgnoreCase)==0){
      typeDoFilter=1;
      cout << "Filler is using BUILTIN HypListFiller Track cleaner!" << endl;
    }else if( abc->CompareTo("official",TString::kIgnoreCase)==0){
      typeDoFilter=2;
      cout << "Filler is using OFFICIAL PidTrackCleaner!" << endl;
    }else if( abc->CompareTo("orring",TString::kIgnoreCase)==0){
      typeDoFilter=3;
      cout << "Filler is using OFFICIAL||HASRING PidTrackCleaner!" << endl;
    }else{
      cerr << "Error: HypAllFillerAlg: Cleaner type "<< abc <<" not known!"<<endl;
      exit(0);
    }
  }
   ...
   def=abc->Data();
	...
	cout << "HypAllFillerAlg ("<<algoName<<"): "<<def.Data()<<endl;
	...
	for(Int_t j=0; j<arr_nr; j++){
		...
		cout << "HypAllFillerAlg ("<<algoName<<"): Reaction String "<<j<<": <<"<<reac_str<<">>"<<endl;
