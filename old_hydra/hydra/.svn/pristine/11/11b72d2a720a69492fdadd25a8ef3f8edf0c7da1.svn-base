...
void HMdcCalibrater2::calibrate(HMdcCal1* cal1,HMdcCal2* cal2) {
  ...
  const Float_t driftV = 0.042; //FIXME: Read from parameter container
  cal2->setAddress(loc[0],loc[1],loc[2],loc[3]);
  Int_t nHits=cal1->getNHits();
  cal2->setNHits(nHits);
  cal2->setDist1(driftV*cal1->getTime1(),0.F);
  if (nHits==-2 || nHits==+2) cal2->setDist2(driftV*cal1->getTime2(),0.F);
}
