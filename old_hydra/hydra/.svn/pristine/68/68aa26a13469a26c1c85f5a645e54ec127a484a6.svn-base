...
Bool_t HPidAlgShower::calculatePDF(HPidTrackCand *pTrack,
	...
	if((pTrack->getSystem()!=0 ) || 
        ...
        if( fFactor == 0. ){ //particle didn't pass  the shower detectors
            return kFALSE;
...
#if DEBUG_LEVEL>4
            cout<<" Tof Mult=1 Value :"<<afReturn[i];
#endif
			}
				...
				afReturn[i]*=pParams->getValue(nId,nOffset,fTof,fMom,fTheta);
#if DEBUG_LEVEL>4
            cout<<" Tof Mult Value :"<<afReturn[i]<<endl;
#endif
			}
#if DEBUG_LEVEL>4
            //cout<<"\n\n **** Mom: "<<fMom<<" Theta: "<<fTheta<<" Value: "<<afReturn[i]<<"*****\n"<<endl;
#endif
		}	
	}	
	 	return kTRUE;
