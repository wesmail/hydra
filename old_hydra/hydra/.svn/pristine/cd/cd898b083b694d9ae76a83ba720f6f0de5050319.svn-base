...
void HClusterSelector::chainPads(Int_t iSec,Int_t iRow,Int_t iCol,HPhotonCluster* pClus){
    //for each fired pad it is checked if the 4 (up-down-sides) neighbours have fired, as Long_t as no fired neighbours is left. IN this way the cluster is built, while collecting the pads belongning to the clusters the pClus is updated.
