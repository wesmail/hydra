...
HMdcLookupRMod::HMdcLookupRMod(Int_t sec=0, Int_t mod=0, Int_t n=6) {
  ...
  else ERROR_msg(DET_MDC,"HMdcLookupRaw not created, container MdcGeomStruct not found");
}
...
HMdcLookupRaw::HMdcLookupRaw(const char* name,const char* title,
...
void HMdcLookupRaw::putAsciiHeader(TString& header) {
  ...
  header=
    ...
    "# Format:\n"
    "# sector  module  layer  cell  motherboard index  TDC channel \n";
}

Bool_t HMdcLookupRaw::writeline(char *buf, Int_t sec, Int_t mod, Int_t mbo,
                                           Int_t tdc) {
  // writes one line to the buffer used by ascii file I/O
    HMdcLookupCell &cell=(*this)[sec][mod][mbo][tdc];
    Int_t nTdc=cell.getNChan();
    if (nTdc==-1) return kFALSE;
    Int_t nMbo=cell.getNMoth();
    sprintf(buf,"%1i %1i %1i %3i %2i %2i\n",
            sec, mod, mbo, tdc, nMbo, nTdc);
    return kTRUE;
}
