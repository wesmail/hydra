...
void HShowerCalParCalculator::setMethod(const char* pName)
{
int i;
...
const char* HShowerCalParCalculator::getMethodName(void)
{
...
Bool_t HShowerCalParCalculator::init()
{
    ...
    fIter = (HIterator*)m_pRawCat->MakeIterator("native");
		...
		// MJ JO IK Sat Sep  1 00:57:19 CEST 2001 >>
...
HShowerGeometry *pGeom = (HShowerGeometry*)rtdb->
    ...
    if (!pGeom) {
       ...
       rtdb->addContainer(pGeom);
		...
		// << end Sat Sep  1 00:57:19 CEST 2001
...
Bool_t HShowerCalParCalculator::finalize(void)
{
   	((HShowerHist*)getCalParHist())
                ->calculate((HShowerCalPar*)getCalPar(),
	...
	if(m_pCalPar->getInputVersion(1) == -1)
	{
		...
		{
			printf("Change 2: %d ->", m_pCalPar->getInputVersion(2));
	...
	{
		printf("Change 1: %d ->", m_pCalPar->getInputVersion(1));
