...
{  
  ...
  //
  if(sector==6){ //finding target for all MDCs in a frustrum 
...
void HMdcGeomParF::initDefaults(void)
{
  ...
  else {////finding target for all MDCs in a frustrum 
...
void HMdcGeomParF::setTukeyConstant(Float_t cte)
{
  //
...
void HMdcGeomParF :: setMinimization(Int_t select=1)
{
  //
...
{  
  ...
  // trying to eliminate fails in reconstruction fue to
...
void HMdcGeomParF :: setGeomAuxPar(void)
{
  ...
  if(fNumMods>3){    
	...
	cout << "**************************************************" << endl;
	cout << "*  Target finder in a complete frustrum          *" << endl;
	cout << "**************************************************" << endl;
      }
    }
    if(GP_DEBUG>0){
...
{    
  // 
  // Filling Hits of a frustrum. Trying to find out the point where all
  // tracks point to. 
  //
...
Bool_t HMdcGeomParF::targetFinder(ofstream *fout)
{
  ...
  if(fUseInitValue) theTarget = fInitValue;
    ...
    for (Int_t i=0;i<entries;i++) { 
      ...
      if(fNumMods==0) {//target finder of frustrum 
  ...
  while(fConstTukey>0.4){
    ...
    if(fConstTukey==1) finalTarget=theTarget;
