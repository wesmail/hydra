...
// or or HSeqArr arr; arr.setArrType(HSeqArr::kChar); ...
...
Bool_t HSeqArr::setCursor(UInt_t indArr) {
...
Bool_t HSeqArrIter::next(ESeqArrType tp) {
  // Setting cursor to the next word
  if(array->type!=tp) return array->err(tp);
  return next();
}
...
Bool_t HSeqArrIter::nextAExp(ESeqArrType tp) {
  if(array->type!=tp) return array->err(tp);
  return nextAndExpand();
}

Bool_t HSeqArrIter::setCursor(UInt_t indArr) {
  // setting of block,index for element number ind
