...
void HMdcCal2ParSim::calcTimeDigitizer(Int_t s,Int_t m,Float_t a,Float_t aOld,Float_t d,Float_t*time1,Float_t*time1Err) {
    // This function calculates the drift time for a given distance
    // to sense wire.A Interpolation between the two closest angle steps
    // is performed and the drift time value returned.
    //
    ...
    timeResolution=( (1-t)*(1-u)*y1 ) + ( t*(1-u)*y2 )+ (t*u*y3) + ( (1-t)*u*y4 );
   /*
    finaltime=time+timeResolution*gauss;
    return finaltime;
   */
    *time1=time;
    *time1Err=timeResolution*gauss;
}
void HMdcCal2ParSim::calcTime2Digitizer(Int_t s,Int_t m,Float_t a,Float_t aOld,Float_t d,Float_t*time2,Float_t*time2Err) {
    // This function calculates the drift time for a given distance
    // to sense wire.A Interpolation between the two closest angle steps
    // is performed and the drift time value returned.
    //
