...
void HRichAnalysis::updateHits(Int_t nSec, Int_t nEvNr) {
  ...
  for (int i = 0; i < iRingNr; i++) {
      ...
      Int_t rotation[6] = {90,150,210,270,330,30};
	...
	hit->setTheta( ((HRichGeometryPar*)fpGeomPar)-> getPadsPar()-> getPad(pRings[i].iRingX,pRings[i].iRingY)-> getTheta() );
        ...
        Float_t phiRot = (((HRichGeometryPar*)fpGeomPar)->getPadsPar()->getPad(pRings[i].iRingX,pRings[i].iRingY)->getPhi()) + rotation[nSec];
        hit->setPhi(phiRot);
      }
  }
}      
