...
HRtTrackEvaluator *HRtTrackFind::makeEvaluator(void) {
...
Bool_t HRtTrackFind::init(void) {
  ...
  if (gHades) {
	...
	if (fMdc3Mode) {
	  Error("init","cannot work in MDC3 mode without catMdcHit");
	  return kFALSE;
	}
      }
	...
	if (fCatTrk) ev->addCategory(catMdcTrack,fCatTrk,"Mdc");
	else {
	  Error("init","Not possible to define catMdcTrack");
	  return kFALSE;
	}
      } else { //Check class name
	if (!fCatTrk->getClass()->InheritsFrom("HRtMdcTrk")) {
	  Error("init","Reft needs the catMdcTrack category to "
  ...
  fControl = new TNtuple("RtMatching","matching","d:dKick:angle:dPhi:valid:pc:pr:xc:yc:zc");
...
Bool_t HRtTrackFind::finalize(void) {
...
Int_t HRtTrackFind::readOuterHits(void) {
  ...
  if (fOutHitIter) {
    ...
    while ( (hit = (HMdcHit *)fOutHitIter->Next()) != 0) {
      if (hit->getChi2()>fMaxChi2) continue;
      ref = new (fSegmentRefs[nHits]) HRtSegmentRef;
      ...
      HModGeomPar *mod = fMdcGeometry->getModule(hit->getSector(),
						 hit->getModule());
      if (mod) { 
	HGeomTransform &transMod = mod->getLabTransform();
	HGeomTransform &transSec = 
	  fSpecGeometry->getSector(hit->getSector())->getTransform();
	...
	alpha = trans.getRotMatrix() * alphaLocal;
      } else Warning("readOuterHits","Module %i unknown",hit->getModule());
...
Int_t HRtTrackFind::readOuterSegments(void) {
  ...
  while ( (seg = (HMdcSeg *)fOutSegIter->Next()) != 0) {
    ref = new (fSegmentRefs[nSegments]) HRtSegmentRef;
    if (!ref) Error("readOuterSegments","Memory exhausted");
    nSegments++;
...
void HRtTrackFind::extractMeasurements(HMdcSeg *inner, HRtSegmentRef *outer,
...
void HRtTrackFind::acceptTrack(HRtVector &p,HRtMatrix &cov,Float_t chi2,
  ...
  if (slot) {
    ...
    track->setOuterDir(fEvaluator->getDir()); //outerSeg->getAlpha());
    track->setLength(fEvaluator->calcEvaluatedLength());
    track->setCharge(fEvaluator->getCharge());
    track->setQuality(fEvaluator->getDistanceKick(),
		      fEvaluator->getDistance());
    track->setLeverArm(fEvaluator->getLeverArm());
  }
}
...
Int_t HRtTrackFind::execute(void) {
  ...
  for (fLocSector[0]=0; fLocSector[0]<fMdc->getMaxSectors(); fLocSector[0]++) {
    fLocInner[0] = fLocOuter[0] = fLocSector[0];

    if (fMdc3Mode)
      nOuter = readOuterHits();
    else
      nOuter = readOuterSegments();
	...
	fEvaluator->evaluate(innerSeg,outerSeg,fMdc3Mode);
	fEvaluator->fillControl(fControl,fMdc3Mode);
...
// 	  fFitter.fit(m,W,p,cov,chi2); 
	  ...
	  if (chi2 < fParam->getMaxChi2()) {
	    ...
	    acceptTrack(p,cov,chi2,innerSeg,outerSeg);
	  }
...
//------------------------------- HRtTrackEvaluator ---------------------
void HRtTrackEvaluator::evaluate(HMdcSeg *inner,HRtSegmentRef *outer,
				 Bool_t onlyMdc3) {
...
//     fAngle = TMath::ACos(alpha1.scalarProduct(alpha2));
  ...
  fDistance = TMath::Abs((r1-r2).scalarProduct(alpha1.vectorProduct(alpha2)));
...
Float_t HRtTrackEvaluator::calcEvaluatedLength(void) {
  //FIXME: Do average over targets
  Float_t length;
  const HGeomVector &target = fSpecGeomPar->getTarget(0)
    ->getTransform().getTransVector();
  length = (fRClosest - target).length();
  length += (fOuterSeg->getR() - fRClosest).length();
  return length;
}

Bool_t HRtTrackEvaluator::isGoodCombination(void) {
...
void HRtTrackEvaluator::fillControl(TNtuple *control, Bool_t onlyMdc3) {
  // Fills ntuple with control variables
  if (onlyMdc3) {
    control->Fill(fDistance,fDistanceKick,fAngle,fDeltaPhi,0,fP,
		  0,
