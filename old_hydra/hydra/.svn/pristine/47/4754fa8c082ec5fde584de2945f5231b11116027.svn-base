...
#include <stdlib.h>
#include <iostream>
#include "hhypPPipPimPAlg.h"
#include "hypinfodef.h"

ClassImp(HHypPPipPimPAlg)

HHypPPipPimPAlg::HHypPPipPimPAlg(Char_t *name_i, Option_t par[])
...
HHypPPipPimPAlg::~HHypPPipPimPAlg()
{
  if (filler3)
    delete filler3;
...
Bool_t HHypPPipPimPAlg::execute()
{
...
Bool_t HHypPPipPimPAlg::init()
{
	...
	filler3->AddTrack(HPidPhysicsConstants::pid("pi+"));
	filler3->AddTrack(HPidPhysicsConstants::pid("pi-"));
	...
	// 3 + 1 part filler
	if(use_3_1){
		cout << "HHypPPipPimPAlg will use option ONEFAKE" << endl;
		...
		filler3p1->AddTrack(HPidPhysicsConstants::pid("pi+"));
		filler3p1->AddTrack(HPidPhysicsConstants::pid("pi-"));
		...
		filler3m1->AddTrack(HPidPhysicsConstants::pid("pi+"));
		filler3m1->AddTrack(HPidPhysicsConstants::pid("pi-"));
	...
	// 3 + 2 part filler
	if(use_3_2){
		cout << "HHypPPipPimPAlg will use option TWOFAKES" << endl;
		...
		filler3p2->AddTrack(HPidPhysicsConstants::pid("pi+"));
		filler3p2->AddTrack(HPidPhysicsConstants::pid("pi-"));
		...
		filler3p1m1->AddTrack(HPidPhysicsConstants::pid("pi+"));
		filler3p1m1->AddTrack(HPidPhysicsConstants::pid("pi-"));
		...
		filler3m2->AddTrack(HPidPhysicsConstants::pid("pi+"));
		filler3m2->AddTrack(HPidPhysicsConstants::pid("pi-"));
...
Bool_t HHypPPipPimPAlg::reinit()
{
  return kTRUE;
}

Bool_t HHypPPipPimPAlg::finalize()
{
  return kTRUE;
}
