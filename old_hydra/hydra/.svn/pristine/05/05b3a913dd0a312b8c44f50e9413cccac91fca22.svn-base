...
////////////////////////////////////////////////////////////////////////
 ...
 /**
    ...
    beta_e1    -  beta (v/c) of first lepton
    beta_e2    -  beta (v/c) of secound lepton
...
HHypPPEpEmGammaProjector::HHypPPEpEmGammaProjector(char *name_i, Option_t par[])
...
Bool_t HHypPPEpEmGammaProjector::execute()
{
    ...
    Float_t beta_1=0, beta_2=0, beta_e1=0, beta_e2=0;
	  ...
	  if(mylist->getProbAlg(icomb)<=0) continue;
        ...
        if (mylist->getIterStatus() == kTRUE) 
        {
            ...
            if (simuflag == 1 ) 
            {
                ...
                HPidParticleSim *my_e1 = (HPidParticleSim *) CatPartSim->
                                                getObject(mylist->getIdxPidPart(icomb, 2));
                HPidParticleSim *my_e2 = (HPidParticleSim *) CatPartSim->
                                                getObject(mylist->getIdxPidPart(icomb, 3));
            ...
            if (pidpartCat != NULL ) 
            {
                ...
                {
                    PidPart[ipar] = (HPidParticle *) pidpartCat->getObject(mylist->getIdxPidPart(icomb, ipar));
                    if (PidPart != NULL) 
                    {
                        PidData        = PidPart[ipar]->getHitData();
                        pTrack         = PidPart[ipar]->getTrackData();
                        PidTrackCand   = PidPart[ipar]->getTrackCand();
                        ...
                        Int_t   sector = PidPart[ipar]->sector();
                        Float_t beta   = PidPart[ipar]->getBetaExp();
                        Int_t q        = PidPart[ipar]->getCharge();
                        Float_t P      = pTrack->fMomenta[4];
                        Float_t Th     = D2R*pTrack->getRKTheta();
                        Float_t Ph     = D2R*pTrack->getRKPhi();
                        Float_t R      = PidPart[ipar]->getR();
                        Float_t Z      = PidPart[ipar]->getZ();
                        ...
                        switch(ipar)
                        {
                            case 0:  { P_1=P; Th_1=Th; Ph_1=Ph; proton1.SetVectM(p,P_mass); pip1.SetVectM(p,Pip_mass);
							            system_1=system; sector_1=sector; beta_1=beta; 
                                        RKchiq_p1=RKchiq; r_p1=R; z_p1=Z; InnerMDCchiq_p1=InnerMDCchiq;
                                        v1.SetXYZ(P*sin(Th)*cos(Ph),P*sin(Th)*sin(Ph),P*cos(Th)); 
	                                    OpAngleClosestLept_p1=closestLepton; OpAngleClosestHadr_p1=closestHadron; break; }
                            case 1:  { P_2=P; Th_2=Th; Ph_2=Ph; proton2.SetVectM(p,P_mass); pip2.SetVectM(p,Pip_mass);
							            system_2=system; sector_2=sector; beta_2=beta;
                                        RKchiq_p2=RKchiq; r_p2=R; z_p2=Z; InnerMDCchiq_p2=InnerMDCchiq;
                                        v2.SetXYZ(P*sin(Th)*cos(Ph),P*sin(Th)*sin(Ph),P*cos(Th)); 
	                                    OpAngleClosestLept_p2=closestLepton; OpAngleClosestHadr_p2=closestHadron; break; }
                            case 2:  { positron.SetVectM(p,E_mass); system_e1=system; sector_e1=sector; beta_e1=beta;
                                        RKchiq_e1=RKchiq; q_e1=q; r_e1=R; z_e1=Z; InnerMDCchiq_e1=InnerMDCchiq; 
	                                    OpAngleClosestLept_e1=closestLepton; OpAngleClosestHadr_e1=closestHadron; break; }
                            case 3:  { electron.SetVectM(p,E_mass); system_e2=system; sector_e2=sector; beta_e2=beta;
                                        RKchiq_e2=RKchiq; q_e2=q; r_e2=R; z_e2=Z; InnerMDCchiq_e2=InnerMDCchiq;
	                                    OpAngleClosestLept_e2=closestLepton; OpAngleClosestHadr_e2=closestHadron; break; }
                            default: cerr<<"ERROR : HHypPPEpEmGammaProjector::execute() "<<ipar<<endl;
                        }
                    }
                }
                //----------------------------- vertex calculation ------------------------------------------------
                dist_pp =  calcVertex( PidPart[0], PidPart[1], &pp_vertex, &pp_distance);
                dist_ee =  calcVertex( PidPart[2], PidPart[3], &ee_vertex, &ee_distance);
                //----------------------------- vertex calculation -------- end -----------------------------------
            }

            TLorentzVector miss2 = (*beam) - (part_1 + part_2);  // beam = beam + target
            TLorentzVector R1    = miss2 + part_1;
            TLorentzVector R2    = miss2 + part_2;

            if(part_1.E()==0 || part_2.E()==0) {cout<<"empty proton"<<endl; continue;}
...
Bool_t HHypPPEpEmGammaProjector::init()
{
    ...
    {
        pp =  new TNtuple(input + TString("_pp"),
            "sector_1:sector_2:sector_e1:sector_e2:system_1:system_2:system_e1:system_e2:P_1:Th_1:Ph_1:P_2:Th_2:Ph_2:q_e1:P_e1:Th_e1:Ph_e1:q_e2:P_e2:Th_e2:Ph_e2:r_e1:z_e1:r_e2:z_e2:r_p1:z_p1:r_p2:z_p2:xvert_pp:yvert_pp:zvert_pp:dist_pp:xvert_ee:yvert_ee:zvert_ee:dist_ee:m2_miss2:p_miss2:dphi_pp:m2_miss2e1e2:p_miss2e1e2:opAngle_e1e2:m2_inv_e1e2:tanT1tanT2:cosThCM_miss:beta_1:beta_2:beta_e1:beta_e2:RKchiq_p1:RKchiq_p2:RKchiq_e1:RKchiq_e2:ncomb:dsf:triggerBit:triggerDecision:date:time:InnerMDCchiq_p1:InnerMDCchiq_p2:InnerMDCchiq_e1:InnerMDCchiq_e2:OpAngleClosestLept_e1:OpAngleClosestHadr_e1:OpAngleClosestLept_e2:OpAngleClosestHadr_e2:OpAngleClosestLept_p1:OpAngleClosestHadr_p1:OpAngleClosestLept_p2:OpAngleClosestHadr_p2", 
            "sector_1:sector_2:sector_e1:sector_e2:system_1:system_2:system_e1:system_e2:P_1:Th_1:Ph_1:P_2:Th_2:Ph_2:q_e1:P_e1:Th_e1:Ph_e1:q_e2:P_e2:Th_e2:Ph_e2:r_e1:z_e1:r_e2:z_e2:r_p1:z_p1:r_p2:z_p2:xvert_pp:yvert_pp:zvert_pp:dist_pp:xvert_ee:yvert_ee:zvert_ee:dist_ee:m2_miss2:p_miss2:dphi_pp:m2_miss2e1e2:p_miss2e1e2:opAngle_e1e2:m2_inv_e1e2:tanT1tanT2:cosThCM_miss:beta_1:beta_2:beta_e1:beta_e2:RKchiq_p1:RKchiq_p2:RKchiq_e1:RKchiq_e2:ncomb:dsf:triggerBit:triggerDecision:date:time:InnerMDCchiq_p1:InnerMDCchiq_p2:InnerMDCchiq_e1:InnerMDCchiq_e2:OpAngleClosestLept_e1:OpAngleClosestHadr_e1:OpAngleClosestLept_e2:OpAngleClosestHadr_e2:OpAngleClosestLept_p1:OpAngleClosestHadr_p1:OpAngleClosestLept_p2:OpAngleClosestHadr_p2"); 
    }
    //f->Close();
    return kTRUE;
...
Float_t HHypPPEpEmGammaProjector::calcVertex(HPidParticle *p1, HPidParticle *p2,
