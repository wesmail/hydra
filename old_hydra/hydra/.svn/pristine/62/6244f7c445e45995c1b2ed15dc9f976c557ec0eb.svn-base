...
void TGo4MdcOnlineSlots::StartMonitor()
{
  ...
  //--------------------------MDC-------------------------------
  fxTGo4GUIRegistry->ReDirectObject("MdcConfig",this,true);

  fxTGo4GUIRegistry->ReDirectObject("hMdctime1Raw",this,true);

  fxTGo4GUIRegistry->ReDirectObject("hMdctime1Cal1",this,true);

  fxTGo4GUIRegistry->ReDirectObject("hMdctime1Cal1MeanTrend",this,true);

  fxTGo4GUIRegistry->ReDirectObject("hMdccutstat",this,true);

  fxTGo4GUIRegistry->ReDirectObject("hMdccal1hits",this,true);

  fxTGo4GUIRegistry->ReDirectObject("hMdcrawRoc_Subev",this,true);

  fxTGo4GUIRegistry->ReDirectObject("hMdcrawError_Stat",this,true);
}
...
{	
	...
	fxTGo4MdcOnlineStatus->OverWriteLocalObject(fxdata);
	...
	if(fxTGo4MdcOnline!=0)  UpdateGUI(fxdata->GetName());
}	
void TGo4MdcOnlineSlots::UpdateGUI(const char *ObjName)
{
  TGo4LockGuard Global;

  TObject *Obj = fxTGo4MdcOnlineStatus->GetObjectfromLocalList(ObjName);

  if(Obj!=0){
    if (Obj->InheritsFrom("HMonHistAddon")) {
      HMonHistAddon* hist = (HMonHistAddon*) Obj;
      fxTGo4MdcOnline->DrawHis(hist);
    }else if( strcmp (ObjName,"MdcConfig") == 0 ) {
