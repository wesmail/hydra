...
void HShowerPadDigitizer::digiPads(HShowerGeantWire* pWireHit) {
   ...
   for (int m = 0; m < 2*nPadRange + 1; m++) {
        ...
        if(pTmpPad && pTmpPad->getPadFlag())
            numericalCalc(pTmpPad, pWireHit); //charge is calculated on the  pad 
    }
...
{
//calculates charge  on the  pad numerically by integrating elements of charge density matrix
