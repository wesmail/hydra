...
void HTofCalPar::readline(const char *buf, Int_t *set) {
  ...
  Int_t s,m,c;
  Float_t lk,rk,vg,pk,tk,pl,pr,al,ga;
  sscanf(buf,"%i%i%i%f%f%f%f%f%f%f%f%f",&s,&m,&c,&lk,&rk,&vg,&pk,&tk,&pl,&pr,&al,&ga);
...
void HTofCalPar::putAsciiHeader(TString& header) {
  ...
  header=
    ...
    "#     pedestalL  pedestalR   attLen   gainAsym\n";
}

Bool_t HTofCalPar::writeline(char *buf,Int_t s, Int_t m, Int_t c) {
  // writes one line to the buffer used by ascii file I/O
  HTofCalParCell& cell=(*this)[s][m][c];
  sprintf(buf,"%i%4i%3i%9.5f%9.5f%8.3f%9.3f%9.3f%9.3f%9.3f%9.3f%9.3f\n",
              s,m,c,cell.getLeftK(),cell.getRightK(),cell.getVGroup(),
              cell.getPosK(),cell.getTimK(),cell.getPedestalL(),
              cell.getPedestalR(),cell.getAttLen(),cell.getGainAsym());
  return kTRUE;
}
...
void HTofCalPar::printParam(void) {
  ...
  printf("     pedestalL  pedestalR   attLen   gainAsym\n");
  printf("==============================================================\n");
  Text_t buf[155];
  Int_t line=0;
  Int_t n0 = getSize();
