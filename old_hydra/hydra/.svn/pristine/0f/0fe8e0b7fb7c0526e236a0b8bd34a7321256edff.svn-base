// $Id: hrichpaircreator.cc,v 1.6 2002-11-29 18:40:18 eberl Exp $
// Last update by Thomas Eberl: 02/11/29 19:25:38
//
...
void HRichPairCreator::initHistos()
{
    ...
    pHistArray->Add(hist);
    // -----------------------------------------
    hist2=HRichHistFac::getHMomvsBeta("lepton_tracks");
    hist2->Print();
    pHistArray->Add(hist2);
}
...
TObjArray* HRichPairCreator::selectTracklets()
{
    ...
    // but filtered via a dedicated filter task
...
Int_t HRichPairCreator::fillHistosFromTracklets(TObjArray* t)
{
 ...
 for (Int_t j=0;j<max;j++) tupt[j]=tups[j]=kicks[j]=-2;
	...
	// fill beta vs momentum for each lepton track
	...
	if(kNewTrack)
	{
	    ...
	    s.Append("lepton_tracks");
	    TH2F *hist = (TH2F*)pHistArray->FindObject(s.Data());
	    if(hist) {hist->Fill(beta,mom);}
	}
    }
    delete [] tupt;
    delete [] tups;
