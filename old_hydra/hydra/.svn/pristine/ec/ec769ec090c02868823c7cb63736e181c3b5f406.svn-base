...
// ... ???
//
// HMdcCustFilter is an HFilter to reduce the size of HMdcClus
//   catregory. The flag "status" in HMdcClus is used for filtering.
//   If  status=kFALSE the object will deleted by category->filter(...)
//
////////////////////////////////////////////////////////////////
...
void HMdcClus::clear() {
  ...
  status=kTRUE;
  indexPar=indexChild[0]=indexChild[1]=-1;
  typeClFinder=0;
  mod=-1;
}
...
void HMdcClus::printPos() {
  Printf("Sec.%i Seg.%i Cluster:  Level %i:%i, %i bins/cluster, %i hits/cluster",
      sec+1,seg+1,minCl[0],minCl[1],nBins,getNCells());
  ...
  if(!status) Printf("  Custer was deleted !.");
}
