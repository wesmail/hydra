...
public:
  ...
  ~HOraSimRunStorage();
  void setProjectName(const Char_t* s) { project=s; }
  void setSubProjectName(const Char_t* s) { subProject=s; }
  void setGeneration(const Int_t n) { generation=n; }
  const Char_t* getProjectName() { return project.Data(); }
  const Char_t* getSubProjectName() { return subProject.Data(); }
  Int_t getGeneration() { return generation; }
  Int_t getSimRefRunId() { return simRefRunId; }
  Bool_t readAllFromFile(const Char_t*);
  Bool_t store();
  void print();
private:
  Bool_t readRunFromFile(const Char_t* s,TString&,TString&,TString&,
