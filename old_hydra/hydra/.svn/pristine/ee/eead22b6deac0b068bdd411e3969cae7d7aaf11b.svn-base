...
void HMdcHitF ::  setParContainer(void){
  ...
  if(target){
    ...
    if(!targetGeo){
      targetGeo=new HMdcTargetGeometry;
      gHades->getRuntimeDb()->addContainer(targetGeo);
    }
  }
...
Int_t HMdcHitF :: execute(void){
  ...
  for(nWantedHits=nMdcPlanes; nWantedHits>=minNumHits;nWantedHits--){
	...
	nPlanesScanned--){
      ...
      for(firstScannedPlane; firstScannedPlane<=lastFirstScannedPlane; firstScannedPlane++){
  ...
  delete filtering;
    ...
    //makeSlopeCorrection();           // slope correction function
...
HMdcHitAux* seg;
Float_t pos[nlayers];
Float_t newpos, newSlopeY;
Float_t originalpos[nlayers];
Float_t correction, lastsumcorrection;
Float_t sumcorrection = 0;
Float_t itercriteria = 0.01; //Sum of 10 microns as convergence criteria FIXME
Int_t itercounter = 0;
...
while((seg=(HMdcHitAux*)iter->Next())!=NULL){
  do {
    lastsumcorrection = sumcorrection;
    for(Int_t i=0; i<nlayers; i++){
      hit = seg->getCal(i);
      if(hit!=NULL){
	pos[i] = hit->getPos();
	
	if(itercounter == 0) originalpos[i] = pos[i];
	
	wirepos = (hit->getCell() - centralWire[i]) * pitch[i];
	if(i == 0 || i == 4) wirepos = -wirepos;
	wiredist = originalpos[i] - wirepos;
	
	newSlopeY = seg->getSlopeY() * cosn[i] - seg->getSlopeX() * sinn[i];
	correction = (wiredist/cos(atan(newSlopeY)))-wiredist;
	
	newpos = originalpos[i] + correction;
    ...
    //cout << "." << itercounter << "->" << sumcorrection - lastsumcorrection; 

  } while(sumcorrection - lastsumcorrection > itercriteria);

  //cout << "OUT." << itercounter << "->" << sumcorrection - lastsumcorrection << endl;  
}
