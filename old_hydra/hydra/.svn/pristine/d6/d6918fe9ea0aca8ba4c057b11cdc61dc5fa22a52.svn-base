...
public:
  inline HMdcClus(void);
  inline HMdcClus(HMdcList12GroupCells& lcells);
  ~HMdcClus(void) {}
  ...
  void    setAddress(Int_t sc, Int_t sg,
                     Int_t ind)            {sec=sc; seg=sg; index=ind;}
  void    setAddress(HLocation &loc)       {sec=loc[0]; seg=loc[1];
                                            index=loc[2];}
  ...
  Int_t   getTypeClFinder(void) const      {return typeClFinder;}
  UChar_t getSec(void) const               {return sec;}
  UChar_t getIOSeg(void) const             {return seg;}
  Short_t getIndex(void) const             {return index;}
  void    getAddress(Int_t& sc,Int_t& sg,
                     Int_t& in) const      {sc=sec;sg=seg;in=index;}
...
inline HMdcClus::HMdcClus(HMdcList12GroupCells& lcells) :
    HMdcList12GroupCells(lcells) {
  status=1;
  indexPar=indCh1=-1;
  indCh2=-2;
  typeClFinder=0;
