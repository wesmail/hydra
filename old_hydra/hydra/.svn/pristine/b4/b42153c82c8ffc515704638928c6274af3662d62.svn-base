...
Bool_t HMdcParFileIo::read(HMdcMboReadout* pPar,Int_t* set) {
...
Bool_t HMdcParFileIo::read(HMdcDigitPar* pDigitPar,Int_t* set) {
...
Bool_t HMdcParFileIo::read(HMdcDigitGeomPar* pDigitGeomPar,Int_t* set) {
  // reads and fills the container "MdcDigitGeomPar" for digitisation parameters
  Text_t* name=(char*)pDigitGeomPar->GetName();
  Int_t version=findInputVersion(name);
  if (version==-1) return kFALSE;    // not in ROOT file
  if (pDigitGeomPar->getInputVersion(inputNumber)==version
      && pDigitGeomPar->getInputVersion(inputNumber)!=-1) return kTRUE;
  // needs reinitialization
  HMdcDigitGeomPar* r=(HMdcDigitGeomPar*)findContainer(name,version);
  Bool_t allFound=kTRUE;
  Int_t pos;
  initModules->Reset();
  HMdcDigitGeomPar& rMdc=*r;
  HMdcDigitGeomPar& pMdc=*pDigitGeomPar;
  for(Int_t s=0;s<6;s++) {
    HMdcDigitGeomParSec& rSec=rMdc[s];
    HMdcDigitGeomParSec& pSec=pMdc[s];
    for(Int_t m=0;m<4;m++) {
      HMdcDigitGeomParMod& rMod=rSec[m];
      HMdcDigitGeomParMod& pMod=pSec[m];
      pos=s*4+m;
      if (set[pos]) {
        if (modulesFound->At(pos)) {
          for(Int_t l=0;l<pMod.getSize();l++) {
  ...
  pDigitGeomPar->setInputVersion(version,inputNumber);
  pDigitGeomPar->setChanged();
  printInfo("MdcDigitGeomPar: module(s) initialized from Root file ");
  delete r;
  return allFound;
}
