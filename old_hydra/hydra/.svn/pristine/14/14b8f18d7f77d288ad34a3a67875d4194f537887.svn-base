...
protected:
  ...
  Char_t  ioSMatchStatus;  // =1 - at list one track has IOseg matching
                           // =0 - no matching
...
public:
  ...
  Int_t   addTrack(Int_t tr,Short_t nTm,
                   UChar_t lM1,UChar_t lM2);
  Bool_t  resetTrack(Int_t ind,Int_t tr,Short_t nTm,
                   UChar_t lM1,UChar_t lM2);
  void    setNTracksM(Int_t modi,Int_t nTrs);
  void    setTrackM(Int_t modi,Int_t ind,Int_t tr,
                    Short_t nTm);
  ...
  void    setIOSMatchStatus(Char_t fl)            {ioSMatchStatus = fl;}
  void    setIsIOSMatching(void)                  {ioSMatchStatus = 1;}
  void    setNoIOSMatching(void)                  {if(ioSMatchStatus<0)
                                                   ioSMatchStatus=0;}
  void    calcTrList(void);
  void    calcTrListMod(HMdcList12GroupCells& list,
                        Int_t m);
  ...
  Int_t   getNNotFakeTracks(void) const           {return nNotFakeTracks;}
  Int_t   getTrack(Int_t indx) const;
  Short_t getNTimesInTrack(Int_t indx) const;
  Int_t   getNLayersInTrack(Int_t indx,
                            Int_t m=-1) const;
  ...
  Bool_t  getXYGeant(Int_t i,Float_t& xg,
                             Float_t& yg) const;
  Float_t getXGeant(Int_t i) const                {return indOk(i) ?
                                                          xGeant[i]:-10000.;}
  Float_t getYGeant(Int_t i) const                {return indOk(i) ?
                                                          yGeant[i]:-10000.;}
  Float_t dX(Int_t i) const                       {return indOk(i) ?
                                                          xGeant[i]-x:-10000.;}
  Float_t dY(Int_t i) const                       {return indOk(i) ?
                                                          yGeant[i]-y:-10000.;}
  
  Bool_t  isGntTrackOk(Int_t i)                   {return testBit(i,255);}
  Bool_t  isFakeIOSegMatch(Int_t i)               {return !testBit(i,1);}
  Bool_t  isFakeContribution(Int_t i)             {return !testBit(i,2);}
  Bool_t  isClFnLevelTooHigh(Int_t i)             {return !testBit(i,4);}
  Bool_t  is1or2HitsNotRec(Int_t i)               {return !testBit(i,8);}
  Bool_t  isSegNotRec(Int_t i)                    {return !testBit(i,16);}
  Bool_t  isTrackNotRec(Int_t i)                  {return !testBit(i,32);}
  Bool_t  isNoMeta(Int_t i)                       {return !testBit(i,64);}
  Bool_t  isGEANTBug(Int_t i)                     {return !testBit(i,128);}
  Char_t  getIOSegMatchingStatus(void)            {return ioSMatchStatus;}
...
private:
  ...
  Bool_t  testBit(Int_t i,UChar_t bit) const 
                                           {return (trackBits(i)&bit) == bit;}
  void     setDefForOldVer(Int_t ind);
  ClassDef(HMdcClusSim,3)
};
