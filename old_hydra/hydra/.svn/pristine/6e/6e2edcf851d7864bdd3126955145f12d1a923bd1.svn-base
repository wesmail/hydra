...
Bool_t HMdcRichCorr::init(void) {
  ...
  if(pListCells == 0)  pListCells = HMdcEvntListCells::getObject();
...
void HMdcRichCorr::setHistFile(Char_t* dir,Char_t* suf,Char_t* option) {
  if(dir>0) {
    histFileDir=dir;
...
void HMdcRichCorr::printStatus(void) const{
  ...
  Char_t* hType ="0 = comb.chamb.clust., 1 = single chamb.clust., 2 = 0+1";
  Char_t* hLevS1="max. occupancy of MDC for cl.find. level 4 & 5 layers";
