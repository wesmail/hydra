...
void HShowerHist::setCellClassName(Char_t* pszName)
{
...
void HShowerHist::calculate(HShowerCalPar* pCalPar,  Int_t nPeaksNumber) {
    ...
    for(loc[1] = 0; loc[1] < m_nModules; loc[1]++) {
      ...
      for(loc[2] = 0; loc[2] < m_nRows; loc[2]++)
         for(loc[3] = 0; loc[3] < m_nColumns; loc[3]++) {
            if (nPeaksNumber==2)
              pHistCell->calculate2(loc[2], loc[3], &slope, &offset);
            else
              pHistCell->calculate1(loc[2], loc[3], &slope, &offset);
 
            pCalPar->setSlope(loc, slope);
            pCalPar->setOffset(loc, offset);
         }
...
void HShowerHist::Write(const Text_t* name, Int_t option, Int_t bufsize) {
  HShowerParSet::Write(name, option, bufsize);
...
Bool_t HShowerHist::initAscii(HParHadAsciiFileIo* pHadAsciiFile) {
  if (!pHadAsciiFile) return kFALSE;

  HAsciiKey* pHAscii = pHadAsciiFile->GetKeyAscii();
  HAsciiKey &mydata = *pHAscii;

  if (!mydata.SetActiveSection("Shower Lookup Asic Setup"))
     return kFALSE;

  return kTRUE;
}

Bool_t HShowerHist::writeAscii(HParHadAsciiFileIo* pHadAsciiFile) {
  if (!pHadAsciiFile) return kFALSE;
