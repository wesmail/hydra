...
// more than 50 microm.
      ...
      fCal->setTime1(dTime[0]/*-offsets[loc[1]]*/);
      ...
      if (nHits==1) fCal->setNHits(1);
        ...
        if (!second) fCal->setNHits(1);
          ...
          fCal->setNHits(2);
          fCal->setTime2(dTime[second]/*-offsets[loc[1]]*/);
...
//
{
	...
	for(Int_t m=0; m<4; m++) {
       ...
       // cout<<"sector "<<s<<" module "<<m<<" "<<setup[s][m]<<endl;
	}
    }
}
