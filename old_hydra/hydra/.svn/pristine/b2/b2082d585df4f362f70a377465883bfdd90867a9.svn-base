...
//    maxCl=maxLocal;
    ...
    while (stackT != stackB) {
      ...
      if(stackB>stackSize){
        Printf("HMdcLookUpTbSec: Stack is increased on 50\%");
    ...
    for(Int_t layer=0; layer<6; layer++) {
	  ...
	  if(nx2i > nx2t) nx2t=nx2i;
          if(nx1i<1 || nx1i>fsec.nBinX-2) {
            Warning("calcLookUpTb","S%iM%iL%iC%1  nx1i=%i isn't in 1 - %i !",
              sec+1,mod+1,layer+1,cell+1,nx1i,fsec.nBinX-2);
            nx1i=(nx1i<1) ? 1:fsec.nBinX-2;
          }
          if(nx2i<1 || nx2i>fsec.nBinX-2) {
            Warning("calcLookUpTb","S%iM%iL%iC%1  nx2i=%i isn't in 1 - %i !",
...
Bool_t HMdcLookUpTb::calcPlotSize(Int_t sec){
  ...
  for(Int_t mod=0; mod<4; mod++) {
    ...
    if(!fComVol) {
      printf("Error: GeomCompositeVolume for MDC%i Sec.%i is absent!\n",
          mod+1,sec+1);
      return kFALSE;
    }
...
Bool_t HMdcLookUpTb::calcCellProj(Int_t sec,HMdcTrap& cellSize,
  ...
  if(!indb) {
    ...
    if(y<0. || y>10000.) {
      Printf("sec.%i p0,3  x=%f y=%f",sec+1,x,y);
      return kFALSE;
    }
    cellProj[0].set(x,y);
    cellProj[3].set(x,y);
  ...
  if(!indu) {
    ...
    if(y<0. || y>10000.) {
      Printf("Error: sec.%i p1,2  x=%f y=%f",sec+1,x,y);
      return kFALSE;
    }
    cellProj[1].set(x,y);
    cellProj[2].set(x,y);
