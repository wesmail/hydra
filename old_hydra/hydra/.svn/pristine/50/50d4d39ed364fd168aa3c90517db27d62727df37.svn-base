...
Bool_t HMdcTrackFitter::fillListHits(HMdcList12GroupCells* lCells1,
        HMdcList12GroupCells* lCells2) {
//  filler=kNoFill;
  segIndex=-1;
  HMdcSizesCellsMod** fSCModAr=fitInOut->fSCModAr[sector];
  indClusFit=-1; //??? mozhet v drugoe mesto?
  nMdcTimes[0] = nMdcTimes[1] = nMdcTimes[2] = nMdcTimes[3] = 0;
  nClTimes = 0;
  lastTime=0;
  if(segment==0) {
    listCells.setSeg1(lCells1);
    listCells.clear(1);          // cleaning another segment
  } else if(segment==1) {
    listCells.setSeg2(lCells1);
    listCells.clear(0);          // cleaning another segment
  } else listCells.set(lCells1,lCells2);
  listCellsOrg=listCells;
  Int_t nDrTimes=listCells.getNDrTimes();
  ...
  for(Int_t layer=0;layer<24;layer++) {
    Int_t nDrTimes = listCells.getNDrTimes(layer);
    if(nDrTimes<=0) {
      indFWireInLay[layer]=-1;
      indLWireInLay[layer]=-2;
      continue;
    }
    indFWireInLay[layer]=nClTimes;
    indLWireInLay[layer]=nClTimes+nDrTimes-1;
    Int_t modr = layer/6;
    Int_t lay=layer%6;
    fitInOut->loc[1]=modr;
    fitInOut->loc[2]=lay;
    ...
    while((cell=listCells.next(layer,cell)) > -1) {
      Int_t nTimes=listCells.getTime(layer,cell);
      fitInOut->loc[3]=cell;
      ...
      for(Int_t time=1; time<3; time++) {
        ...
        if(tdcTimeCal1[nClTimes]<0.) tdcTimeCal1[nClTimes]=0.;
        nClTimes++;
      }
    }
  }
...
//  filler=kNoFill;
  ...
  for(Int_t layer=0;layer<24;layer++) {
    ...
    while((cell=listCells.next(layer,cell)) > -1) {
      ...
      hitStatus[nClTimes] = 1;
      fSizeCellsLayer[nClTimes]=&fSCLay;
      tdcTimeCal1[nClTimes] = storeLay.getTimeValue(cell);
      nClTimes++;
    }
  }
  firstPl=&((*(fSCModAr[lModule[0]]))[0]);
...
Bool_t HMdcTrackFitter::fillClusFitCont(void) {
  ...
  if(!fClusFit) {
    Warning("fillClusFitCont","No slot HMdcClusFit available");
    return kFALSE;
  }
  ...
  fClusFit->setClustIndex(fClst1->getIndex()); // ????
  ...
  for(Int_t hit=firstTime; hit<lastTime; hit++) {
    ...
    if(!fWireFit) {
      Warning("fillFitCat","No slot HMdcWireFit available");
      break;
    }
...
Double_t HMdcTrackFitter::testFitResult(void) {
  ...
  Int_t l1=lLayer[firstTime]+lModule[firstTime]*6;
  Int_t l2=lLayer[lastTime-1]+lModule[lastTime-1]*6;
...
Int_t HMdcTrackFitter::unsetHits(void) {
  ...
  for(Int_t hit=firstTime;hit<lastTime;hit++) {
    if(hitStatus[hit]<1) continue;
    listCellsOrg.delTime(lModule[hit]*6+lLayer[hit],lCell[hit],timeIndx[hit]);
    nFitedTimes++;
  }
  return nFitedTimes;
}
...
Bool_t HMdcTrackFitter::fitCluster(Double_t threshold, Int_t fittingMod) {
  ...
  while(kTRUE) {
    ...
    nCells=(fittingMod<0 || fittingMod>3) ? listCells.getNCells() :
        listCells.getNCells(fittingMod*6,fittingMod*6+5);
    if( delW<0.5 || nCells<6 ) break;  // 5.0 ??? (mnogo?)
    if(HMdcTrackDSet::fPrint()) printf(
...
void HMdcTrackFitter::setFittingTimesList(Int_t mod) {
  firstTime=0;
  if(mod<0 || mod>3) {
    lastTime=nClTimes;
    for(Int_t m=0;m<4;m++) nTimesInFit[m]=nMdcTimes[m];
  } else {
    for(Int_t m=1;m<=mod;m++) firstTime += nMdcTimes[m-1];
    lastTime=firstTime+nMdcTimes[mod];
    nTimesInFit[0]=nTimesInFit[1]=nTimesInFit[2]=nTimesInFit[3]=0;
    nTimesInFit[mod]=nMdcTimes[mod];
  }
}
