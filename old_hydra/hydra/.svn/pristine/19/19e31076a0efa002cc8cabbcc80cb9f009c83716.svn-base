...
HOraSlowPartition::HOraSlowPartition(const char* name) {
...
void HOraSlowPartition::clearRates() {
  // Clears the array of archiver rates
  deleteGraph();
  if (ratesIter) {
    delete ratesIter;
    ratesIter=0;
...
HOraSlowPeriod* HOraSlowPartition::getRun(Int_t runId) {
  // Returns the pointer to the period corresponding to run with id runId 
  HOraSlowPeriod* p=0;
  if (periodIter) {
    periodIter->Reset();
...
HOraSlowPeriod* HOraSlowPartition::getRun(const char* fname) {
  // Returns the pointer to the period corresponding to run with hld-filename fname 
  HOraSlowPeriod* p=0;
  if (periodIter) {
    periodIter->Reset();
    while ((p=((HOraSlowPeriod*)(periodIter->Next())))!=0) {
