...
Bool_t HMdc12Fit::fitMod(HMdcClus* fClst, Int_t arrInd) {
  ...
  if(!isFitted) fillHitSegByClus(iMod);
  else while(isFitted) {
...
Bool_t HMdc12Fit::fitCombClusIndFit(HMdcClus* fClst, Int_t arrInd) {
  ...
  for(Int_t iMod=0; iMod<2; iMod++) {
    ...
    if(!isFitted) fstIndex[iMod]=lstIndex[iMod]=fillHitByClus(iMod);
    else /*while(isFitted)*/ {
...
//   eX=eY=eXDir=eYDir=0;
//   fSCMod->calcMdcHit(fitter->getXOutVertex(),fitter->getYOutVertex(),
//       fitter->getZOutVertex(),fitter->getXOutPlane(),
//       fitter->getYOutPlane(),fitter->getZOutPlane(), x,y,xDir,yDir);
  fSCMod->calcMdcHit(fitter->xOutVertex,fitter->yOutVertex,
      fitter->zOutVertex,fitter->xOutPlane,fitter->yOutPlane,fitter->zOutPlane,
      fitter->getParErr(0),fitter->getParErr(1),0.,
      fitter->getParErr(2),fitter->getParErr(3),
      -fitter->firstPl->A(), -fitter->firstPl->B(),   // =dZ1dX1,dZ1dY1, 
      -fitter->secondPl->A(), -fitter->secondPl->B(), // =dZ2dX2,dZ2dY2,
      x,eX, y,eY, xDir,eXDir, yDir,eYDir);

  fHit->setXY(x,eX,y,eY);
  fHit->setXYDir(xDir,eXDir,yDir,eYDir);
  fHit->setChi2(fitter->getFunMin());   // /deg.of_fr.num. ???
  fHit->setOff(fitter->getArriveTime(),0.);  // Error ?
...
//   HMdcSizesCells::calcMdcSeg(fitter->getXOutVertex(),
//       fitter->getYOutVertex(),fitter->getZOutVertex(),
//       fitter->getXOutPlane(),fitter->getYOutPlane(),
//       fitter->getZOutPlane(),zm,r0,theta,phi);
  HMdcSizesCells::calcMdcSeg(fitter->xOutVertex,fitter->yOutVertex,
      fitter->zOutVertex,fitter->xOutPlane,fitter->yOutPlane,fitter->zOutPlane,
      fitter->getParErr(0),fitter->getParErr(1),0.,
      fitter->getParErr(2),fitter->getParErr(3),
      -fitter->firstPl->A(), -fitter->firstPl->B(),   // =dZ1dX1,dZ1dY1, 
...
// Float_t rtd=180./acos(-1.);
// Int_t seg=fitter->fClst->getIOSeg();
// Printf("=== %iSec %iSeg %iMod: Theta=%6.1f/%6.1f  Phi=%6.1f/%6.1f dTh,dPhi=%6.1f %6.1f",
// fitter->fClst->getSec(),seg,fitter->fClst->getMod(),
// fitter->fClst->getTheta()*rtd,theta*rtd,
// fitter->fClst->getPhi()*rtd,phi*rtd,
// (fitter->fClst->getTheta()-theta)*rtd,(fitter->fClst->getPhi()-phi)*rtd);
// if(seg==1) {
// HMdcTrackFitter* seg1Fitter=fittersArr[0].getFitter(0); //fittersArr[0][0];
// seg1Fitter->fClst->print(kFALSE);
// }
// fitter->fClst->print(kFALSE);
...
Int_t HMdc12Fit::fillHitByClus(Int_t iMod) {
  ...
  if(mod<2 || HMdcTrackDSet::isCoiloff) zErrTag=fitter->fClst->getErrZTarg();
  else {           // kick plane
    dZ1dX1=0.;
    dZ1dY1=fitter->fClst->getErrYTarg();
    dZ1dY1=(dZ1dY1>0.0001) ? -fitter->fClst->getErrZTarg()/dZ1dY1 : -1.274;
  }
  Double_t x,eX,y,eY,xDir,eXDir,yDir,eYDir;
  fSCMod->calcMdcHit(fitter->fClst->getXTarg(),fitter->fClst->getYTarg(),
      fitter->fClst->getZTarg(),fitter->fClst->getX(),fitter->fClst->getY(),
      fitter->fClst->getZ(),fitter->fClst->getErrXTarg(),
      fitter->fClst->getErrYTarg(),zErrTag,fitter->fClst->getErrX(),
      fitter->fClst->getErrY(),dZ1dX1, dZ1dY1, -fitter->fClst->A(),
      -fitter->fClst->B(),x,eX, y,eY, xDir,eXDir, yDir,eYDir);
  fHit->setXY(x,eX,y,eY);
  fHit->setXYDir(xDir,eXDir,yDir,eYDir);
  if(isGeant) ((HMdcHitSim*)fHit)->calcNTracks();
...
Bool_t HMdc12Fit::fillHitSegByClus(Int_t iMod) {
  ...
  Double_t dZ1dX1=0., dZ1dY1=0.,zErrTag=0.;
  if(fitter->segment==0 || HMdcTrackDSet::isCoiloff)
      zErrTag=fitter->fClst->getErrZTarg();
  else {                                  // kick plane
    dZ1dX1=0.;
    dZ1dY1=fitter->fClst->getErrYTarg();
    dZ1dY1=(dZ1dY1>0.0001) ? -fitter->fClst->getErrZTarg()/dZ1dY1 : -1.274;
  ...
  HMdcSizesCells::calcMdcSeg(fitter->fClst->getXTarg(),fitter->fClst->getYTarg(),
      fitter->fClst->getZTarg(),fitter->fClst->getX(),fitter->fClst->getY(),
      fitter->fClst->getZ(),fitter->fClst->getErrXTarg(),
      fitter->fClst->getErrYTarg(),zErrTag,fitter->fClst->getErrX(),
      fitter->fClst->getErrY(),dZ1dX1, dZ1dY1, -fitter->fClst->A(),
      -fitter->fClst->B(),zm,eZm, r0,eR0,theta,eTheta, phi,ePhi);
  fSeg->setPar(zm,eZm, r0,eR0, theta,eTheta, phi,ePhi);
  if(isGeant) ((HMdcSegSim*)fSeg)->calcNTracks();
  // Cluster param:
