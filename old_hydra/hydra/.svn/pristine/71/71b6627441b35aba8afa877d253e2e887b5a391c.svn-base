// @(#)$Id: hshowercalibraterIPU.cc,v 1.6 2009-07-15 11:34:10 halo Exp $
...
HShowerCalibraterIPU::HShowerCalibraterIPU(const Text_t *name,const Text_t *title)
...
// Creates an iterator which loops over all fired cells
  ...
  HTriggerDetector * pTriggerDet = (HTriggerDetector*)gHades->getSetup()->getDetector("Trigger");
    ...
    if( ! m_pCalCat)
    {
        m_pCalCat = pTriggerDet->buildCategory(catShowerCalIPU);
        ...
        {
            gHades->getCurrentEvent()
                        ->addCategory(catShowerCalIPU, m_pCalCat, "Trigger");
        }
    }
