// @(#)$Id: hpidcandidate.cc,v 1.1 2002-10-30 10:55:33 halo Exp $
...
// id
//
// Ids of used algorithms are stored in pAlogrithms vector constisting
// values from EnumPidAlgorithm_t (piddef.h).
...
Int_t HPidCandidate::print(void)
{
// Print conteins of the container
...
HPidTrackCand* HPidCandidate::getCandidate(HCategory *pCat)
{
...
// Works when pCat is set or gHades->getCurrentEvent() is accessable
    ...
    HEvent *pEvent;
        ...
        {
            Error("getCandidatePtr", "Cannot access current event");
            return NULL;
        }

        if((pCat = pEvent->getCategory(catPidTrackCand)) == NULL)
            Error("getCandidatePtr", "No catPidTrackCand category");
    }

    return (HPidTrackCand*)pCat->getObject(nCandidate);
}
...
{
// Set algorithm Id eAlg at uiPos. Array bounds checked
...
// Existing of a propper algorithms array is checked
...
HCategory* buildPidCandidateCategory(const char sClassName[])
{
// global function for making the category for the specified number
