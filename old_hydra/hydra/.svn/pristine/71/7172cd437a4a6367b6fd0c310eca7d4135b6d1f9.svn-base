// @(#)$Id: hpidreconstructor.cc,v 1.21 2008-01-06 20:09:17 jurkovic Exp $
...
HPidReconstructor::HPidReconstructor(Option_t par[],Float_t HadronBias)
...
HPidReconstructor::HPidReconstructor(Text_t name[], Text_t title[],
                                    Option_t par[], Float_t HadronBias)
...
Int_t HPidReconstructor::execute(void)
{
  ...
  while((pTrack = (HPidTrackCand *)pitInput->Next()) != NULL)
      ...
      if((pCand = getNextSlot()) == NULL)
	{
      ...
      pCand->setFlagField(pTrack->getFlagField());      // copy flags set to HPidTrackCand
					 ...
					 HPidCandidate *pCand)
{
  ...
  if(bCalcRelints)
	  ...
	  iPart++)
	{
	  ...
	  /*
	  Float_t scale_protons = 2.0;
	  Float_t scale_piplus = 2.0;
	  Float_t scale_piminus = 2.0;
	  */
