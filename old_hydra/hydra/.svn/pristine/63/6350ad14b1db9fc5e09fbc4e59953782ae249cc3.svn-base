  ...
  while ((cal=(HMdcCal1 *)iter->Next())!=0) {
    ...
    if(s==2 && m==1) {
     if(cal->getNHits() == 2) {
      ...
      hist1[23]->Fill(cal->getTime2()-cal->getTime1(),1.);
     }
...
//  if(doPlots) drawPlot(sec, 1);
  ...
  for(Int_t nc=0; nc<nclst; nc++) {
   ...
   hist1[31]->Fill(clst.getErrX(),1.);
   hist1[32]->Fill(clst.getErrY(),1.);
  }
  }
 //-------------------------------------------------------------------
...
//  plotF[11]=new TH2F("pF11","N.hits | N.clusters",40,-0.,40.,10,0.,10.);
  ...
  hist1[21]=new TH1F("time1","time1",400,-1000.,1000.);
  hist1[22]=new TH1F("time2","time2",400,-1000.,1000.);
  hist1[23]=new TH1F("time1-2","time1-2",500,0.,500.);
...
// Calculeting a cross of line (vc1-vc2) with plane HGeomTransform
  ...
  A=(mt(3)*mt(7)-mt(6)*mt(4));
  B=(mt(6)*mt(1)-mt(0)*mt(7));
  C=(mt(0)*mt(4)-mt(3)*mt(1));
  D=-(A*tr(0)+B*tr(1)+C*tr(2));
  Double_t point[3];
