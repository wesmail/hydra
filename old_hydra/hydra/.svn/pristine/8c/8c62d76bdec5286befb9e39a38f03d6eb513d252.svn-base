...
Bool_t HMdcClusterToHit::init(void) {
  ...
  fmdc=parCont->getMdcDetector();
  fGeomPar=parCont->getMdcGeomPar();
  fLookUpTb=HMdcLookUpTb::getExObject();
  if( !fmdc || !fGeomPar || !fLookUpTb) return kFALSE;
  ...
  for(Int_t ns=0; ns<6; ns++) {
    ...
    const HGeomRotation& mt=prPlane->getRotMatrix();
    const HGeomVector& tr=prPlane->getTransVector();
    Double_t pC=(mt(0)*mt(4)-mt(3)*mt(1));
    pA[ns]=(mt(3)*mt(7)-mt(6)*mt(4))/pC;
    pB[ns]=(mt(6)*mt(1)-mt(0)*mt(7))/pC;
    pD[ns]=pA[ns]*tr(0)+pB[ns]*tr(1)+tr(2);
  }
  locHit.set(3,0,0,0);
  locSeg.set(3,0,0,0);
  return kTRUE;
...
Int_t HMdcClusterToHit::execute(void) {
  ...
  for(Int_t ns=0; ns<6; ns++) {
    ...
    for(Int_t nc=0; nc<nclst; nc++) {
      ...
      locSeg[2]=nc;
      HMdcSeg *seg = (HMdcSeg*)fSegCat->getSlot(locSeg);
      if(!seg) {
        Error("execute","S.%i Cluster %i No slot HMdcSeg available",ns+1,nc+1);
        continue;
      }
      seg = new(seg) HMdcSeg;
      seg->setSec(ns);
...
//Printf("theta=%g+/-%g, phi=%g+/-%g, zm=%g+/-%g, r0=%g+/-%g",theta,errTheta, phi,errPhi, zm,errZm, r0,errR0);
      ...
      for(Int_t mod=0; mod<2; mod++) {
        ...
        locHit[1]=mod;
        locHit[2]=nc;
        HMdcHit *hit = (HMdcHit*)fHitCat->getSlot(locHit);
        if(!hit) {
          Error("execute","S.%i No slot HMdcHit available",ns+1);
          continue;
        }
        hit = new(hit) HMdcHit;
