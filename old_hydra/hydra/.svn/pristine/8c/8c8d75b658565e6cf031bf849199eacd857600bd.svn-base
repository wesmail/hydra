...
ClassImp(HPidPdfMaker)
  ...
  // ---------------------------------------------------------------------------
  HPidPdfMaker::HPidPdfMaker(const char*cOutFileName, Bool_t analyseSim) //OK
    :HSUDummyRec(kFALSE){
...
Int_t HPidPdfMaker::execute(void) {
  ...
  while((pTrackCand=(HPidTrackCandSim *)pItTrackCand->Next())!=NULL)
	...
	aPatternInstance[i_tracktype]=getTrackType(pTrackCand);
      ...
      if(pTrackData->getInnerMdcNumber()>0){
	aPatternInstance[b_segments]= pTrackData->getInnerMdcNumber();
      }
...
{  
  ...
  aPatternInstance[f_angle_to_closest_lep]=pTrackData->getAngleWithClosestLeptonCandidate();
  aPatternInstance[f_angle_to_closest_had]=pTrackData->getAngleWithClosestHadronCandidate();

  aPatternInstance[b_closest_lep_isfitted]=pTrackData->getClosestLeptonCandidateIsFitted();
  aPatternInstance[b_closest_had_isfitted]=pTrackData->getClosestHadronCandidateIsFitted();
...
Bool_t HPidPdfMaker::bookSegmentObservables() //OK
{
  ...
  if(pHitData->iIndInnerSeg >= 0) {  //!!!!!!!!!!!!!!!!!!
    aPatternInstance[n_mdc_nw0]  = pHitData->getMdcNWires0();  
    aPatternInstance[n_mdc_cls0] = pHitData->getMdcClsSize0(); 
    aPatternInstance[n_mdc_cfl0] = pHitData->getMdcLevelCls0();
    aPatternInstance[n_mdc_nw1]  = pHitData->getMdcNWires1();  
    aPatternInstance[n_mdc_cls1] = pHitData->getMdcClsSize1(); 
    aPatternInstance[n_mdc_cfl1] = pHitData->getMdcLevelCls1();
