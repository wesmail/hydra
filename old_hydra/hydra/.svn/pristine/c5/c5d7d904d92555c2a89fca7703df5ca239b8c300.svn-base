...
HPidPDFBuilderSim::HPidPDFBuilderSim(Option_t options[], Char_t* OutputFile)
{
  ...
  N_Cand_Ring_Seg = 0;
  N_Cand_Hadron_KT = 0;
  N_Cand_RichIneffSingle = 0;
  N_Cand_RichIneffDouble = 0;
  ...
  LeafList = 
    new TString("dPhi/F:dTheta/F:pmq/F:padnr/F:centroid/F:locmax/F:avcharge/F:theta/F:phi/F:sector/F:neighbour/F:openingAngle/F:dalitz/F:gamma/F:ringtype/F:corrtype/F:mdcchisquare/F:iscorrgeantlep/F:nsegments/F:kicktype/F:nwiresmdc0/F:nwiresmdc1/F:clussizemdc0/F:clussizemdc1/F");
...
Int_t HPidPDFBuilderSim::execute(void)
{
  ...
  while((theCandidate = (HPidTrackCand *)pIteratorInput->Next()) != NULL)
	...
	leptonsInKickTrack=HPidFL::countLeptonsInKickTrack((HKickTrackSim*) (theCandidate->getKickTrack()));
        if(leptonsInKickTrack==0) N_Cand_Hadron_KT++;
  ...
  if(!(evcount%5000))
    {
      ...
      fprintf(fp,"Single leptons missed :%d\n",N_Cand_RichIneffSingle);
      fprintf(fp,"Double leptons missed :%d\n",N_Cand_RichIneffDouble);
      fprintf(fp,"Hadronic candidates without ring :%d\n",N_Cand_Hadron_KT);
      ...
      cout << "Single leptons missed:" << N_Cand_RichIneffSingle << endl;
      cout << "Double leptons missed:" << N_Cand_RichIneffDouble << endl;
      cout << "Hadronic Candidiates without ring:" << N_Cand_Hadron_KT << endl;
 ...
 if(!strcmp(mode,"withring"))
  {
    ...
    getGeantMatches(theCandidate, nCommonLeptonsRichMdc,nMatchesMdcMeta,nMatchesFull,nCommonLeptonsFull, nCommonLeptonsMdcMeta);
 ...
 if(!strcmp(mode,"noring"))
   {
     ...
     getGeantMatches(theCandidate,nMatchesMdcMeta,nMatchesFull, nCommonLeptonsMdcMeta);
...
Bool_t HPidPDFBuilderSim::findLeptonicTrackCandidate(Int_t ringcounter)
{
  ...
  while((currentCandidate = (HPidTrackCand*) pIteratorInput->Next())!=NULL)
	...
	{
	  getGeantMatches(currentCandidate, nCommonLeptonsRichMdc,nMatchesMdcMeta,nMatchesFull,
