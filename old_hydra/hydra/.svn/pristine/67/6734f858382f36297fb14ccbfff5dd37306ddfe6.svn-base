...
void HRichDigitizer::digitiseDirectHits(HGeantRichDirect *pDirHit) {
  ...
  if (pDirHit->getEnergyLoss() > 0.0F &&
    ...
    fStepsNr = (Int_t)(1+((HRichDigitisationPar*)fDigitisationPar)
		       ->getElectronsNr() * 1000 * pDirHit->getEnergyLoss());
    if (fStepsNr > 5000) {
      fStepsNr = 5000;
...
void HRichDigitizer::makeNoiseOnPads() {
  ...
  for (Int_t i = 0; i < 6; i++){
	...
	// centred in 0, hence we have to add the Float_t part of the 
	//mean. Hence we cast the charge to an integer, then we 
	// subtract the Float_t part of the mean value and finally
...
Int_t HRichDigitizer::checkPad(HRichCalSim *calSim){
  ...
  // The Float_t part of the mean value of the noise distribution
  // is added to the charge then
  // the charge value is casted to an integer and is 
  // compared to the threshold.
  // If the charge is above threshold, the Float_t part of the mean is
...
Float_t HRichDigitizer::calcIndCharge(Float_t yCharge,Float_t q4,Int_t iPdaIndex,Int_t iWireNr){
...
Float_t HRichDigitizer::q4Calc(Float_t charge,Float_t pos,Float_t par1,Float_t par2){
