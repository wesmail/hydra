...
class HMdcTrackParam : public HMdcLineParam {
  ...
  public:
    ...
    inline void copy(HMdcTrackParam& tp);
    inline void copyTimeOffsets(const HMdcTrackParam& tp);
    inline void copyNewParam(const HMdcTrackParam& tp);
    ...
    Double_t getTimeOffset(Int_t mod) {return timeOffset[mod];}
    void clearTimeOffsetErr(void);
    void addToTOffsetErr(Int_t m, Double_t* dTdA, Double_t wtNorm);
    void addToTOffsetErr(HMdcWireData& wire, Double_t* dTdA);
    void addToTOffsetErr(HMdcWireData& wire);
...
inline void HMdcTrackParam::copyNewParam(const HMdcTrackParam& tp) {
  copyParam(tp);
  copyTimeOffsets(tp);
  funct=tp.funct;
...
inline void HMdcTrackParam::copy(HMdcTrackParam& tp) {
  point1=tp.point1;
  point2=tp.point2;
  nParam=tp.nParam;
}
