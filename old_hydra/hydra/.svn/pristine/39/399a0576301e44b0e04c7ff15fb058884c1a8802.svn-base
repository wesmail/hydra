  ...
  if (pFile) {
    ...
    for(Int_t m=0;m<pPar->getNumRefModules();m++) {
      ...
      if (mod) {
        ...
        pFile->write(buf,strlen(buf));
        sprintf(buf,"%s  %s  %s\n",mod->GetName(),
            (const char*)mod->getShape(),(const char*)mod->getMother());
        pFile->write(buf,strlen(buf));
        pPar->getShapes()->writePoints(pFile,mod);
        writeTransform(mod->getTransform());
        for(Int_t c=0;c<mod->getNumComponents();c++) {
          ...
          sprintf(buf,"%s  %s  %s\n",vol->GetName(),
              (const char*)vol->getShape(),(const char*)vol->getMother());
          pFile->write(buf,strlen(buf));
          pPar->getShapes()->writePoints(pFile,vol);
          writeTransform(vol->getTransform());
        }
      }
    }
    sprintf(buf,"########################################################\n");
...
Bool_t HDetParAsciiFileIo::readLabPositions(const Text_t* keyName,
  ...
  // In general the code is written in such a way that it could be used
  // for all detectors. It could be a function in a base class which
...
Bool_t HDetParAsciiFileIo::readVolumes(const Text_t* keyName,HDetGeomPar* pPar) {
        ...
        while (!all && !pFile->eof()) {             
          ...
          if (buf[0]=='#') all=kTRUE;   
            ...
            if (buf[0]!='/') {
              ...
              Int_t nPoints=pPar->getShapes()->readPoints(pFile,&volume);
              ...
              if (volu) {
                if (l==-1 && volu->getNumPoints()==0)
                    volu->setVolumePar(volume);
                else volu->getComponent(l)->setVolumePar(volume);
              }
            } 
          }
        }
  ...
  for(Int_t i=0;i<pPar->getNumRefModules();i++) {
    if ((v=pPar->getRefVolume(i)) && v->getNumPoints()==0) allFound=kFALSE;
  }
  return allFound;
}
