...
Bool_t HMdcCalibrater1::init(void) {
  ...
  if (!rawCat) {
    rawCat=(HCategory*)((HMdcDetector*)(((HSpectrometer*)(gHades->getSetup()))->getDetector("Mdc"))->buildCategory(catMdcRaw));
    if (!rawCat) return kFALSE;
    else ((HEvent*)(gHades->getCurrentEvent()))->addCategory(catMdcRaw,rawCat,"Mdc");
  }
  ...
  if(StartandCal){
      ...
      if (!startHitCat) {
	  Error("HMdcCalibrater1:init()","StartHit Category not available,\n but Option StartAndCal used!");
          exit(1);
      }
  }
...
Bool_t HMdcCalibrater1::testTimeCuts(Float_t testTime1,Float_t testTime2){
  ...
  //
    ...
    if(timecut->cutTime1(loc[0],loc[1],testTime1)&&
       ...
       countNrWiresPerMod[(Int_t)loc[0]][(Int_t)loc[1]]>3)
    {
	return kTRUE;
    }
    else
...
void HMdcCalibrater1::fillCal1(Float_t testTime1,Float_t testTime2,Int_t nHits)
{
  ...
  //
	...
	cal->setAddress(loc[0],loc[1],loc[2],loc[3]);
        ...
        if(StartandCal||NoStartandCal)
	{
	    cal->setTime1(testTime1+globalOffset[loc[1]]);
	    cal->setTime2(testTime2+globalOffset[loc[1]]);
	}
	if(NoStartandNoCal&&(!setTimeCut))
	{
	    ...
	    break;
	    default :
		cal->setTime1(2048+globalOffset[loc[1]] -
		...
		break;
		default:   cal->setTime2(2048+globalOffset[loc[1]] -
...
Int_t HMdcCalibrater1::execute(void) {
	...
	{
	    gHades->getMsg()->debug(10,DET_MDC,GetName()
