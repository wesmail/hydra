...
protected:
     ...
     Bool_t debug;                      //! kTRUE print infos of trun mean
...
public:
    HMdcDeDx2(const char* name   ="MdcDeDx2",
	      const char* title  ="Mdc lookup for MDC dEdX calculation",
	      const char* context="MdcDeDx2Production");
    ~HMdcDeDx2();
    ...
    void      setDebugMode(Bool_t dodebug)   {debug=dodebug;}
    ...
    Double_t         scaledTimeAboveThreshold(HGeantKine* kine=0,Double_t p=-1,
					      ...
					      Int_t s=0,Int_t m=0,Double_t alpha=0,Double_t mindist=0);
    ClassDef(HMdcDeDx2,1) //  MDC lookup table (dedx2 normalization parameters)
};
#endif  /*!HMDCDEDX2_H*/
