...
//*-- Version: $Id: hrichanalysisIPU.cc,v 1.10 2005-07-15 09:02:50 wuestenf Exp $
...
void HRichAnalysisIPU::DataWordCut(Int_t nSec)
{
  ...
  for(Int_t jj=0; jj<1000;jj++)
    {
      psortRings[jj].setX(0);
      psortRings[jj].setY(0);
    }
  if(iRingNr > 999)
    {
  ...
  for(Int_t jj=0; jj<(iRingNr<1000)?iRingNr:1000;jj++)
    {
      kk=0;
      ...
      if ( jj == 0 )
        {
          psortRings[kk]=*((HRichHitIPU*)pRings->At(jj));
        }
      else
        {
          while  ( (((HRichHitIPU*)pRings->At(jj))->getY() > psortRings[kk].getY()) && (kk<jj ))
            {
              kk++;
            }
  ...
  for (Int_t jj = 0;  jj <iRingNr; jj++)
    {
      newX = psortRings[jj].getX();
      newY = (Int_t) psortRings[jj].getY()/100;
