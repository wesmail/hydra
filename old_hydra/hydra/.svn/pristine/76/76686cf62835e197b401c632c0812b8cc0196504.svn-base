...
HTask* HShowerTofinoTaskSet::make(const char *select, Option_t *option) {
  ...
  { // this algorithm work only with magnetic filed switched on
  	...
  	if(!isSimulation) {
  		if(analysisLevel ==1 )  tasks->add( new HShowerTofPIDFinder("shotof.pid","shotof.pid") );
  	}
  	else 
    {
  		if(analysisLevel  == 1)  
		{ 
			...
			tasks->add( new HShowerTofPIDTrackMatcher("shotof.trmatch","shotof.trmatch") );
		}
  	}
  } 
  else 
  {	  
	//cerr("HShowerTofinoTaskSet::make(): %s","This task does not work without magnetic field");	    
...
Ssiz_t len=s1.Length();
  ...
  if(len!=0) {
	...
	while ((stemp=(TObjString*)myiter->Next())!= 0) {
		...
		if(argument.CompareTo("leprecognition") == 0) 
		{
          ianalysisLevel++; 		
		  if(ianalysisLevel>1) 
		  {
