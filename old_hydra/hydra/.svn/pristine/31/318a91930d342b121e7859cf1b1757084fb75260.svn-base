...
HShowerFrameCorner *pCorner;
    ...
    for(fY = fYmin; fY <= fYmax; fY += fBoxSize)
    {
	    ...
	    {
		Error("numericalCalc()",
		      "outside1 matrix : max %i, ind %i, fX %7.4f, fXmin %7.4f, fBoxSize %5.3f",
		      2*iMatrixRange,i,fX,fXmin,fBoxSize);
		continue;
	    }
	    ...
	    {
		Error("numericalCalc()",
		      "outside2 matrix : max %i, ind %i, fX %7.4f, fXmin %7.4f, fBoxSize %5.3f",
		      2*iMatrixRange,i,fX,fXmin,fBoxSize);
		continue;
	    }
