...
void HMdcCluster::calcClusParam(Double_t xSt2, Double_t ySt2) {
  ...
  if(nBins<2) {
    ...
    if(sigma2D<10e-5) sigma2=0.;
    else sigma2=sqrt(sigma2D)/nBn;
...
HMdcLookUpTbSec::HMdcLookUpTbSec(Int_t sec, Int_t nSegs,
  ...
  if(size>hPlModsSize) {
    if(hPlMod[0]) delete [] hPlMod[0];
    if(hPlMod[1]) delete [] hPlMod[1];
    hPlMod[0]=new UChar_t [size];
    hPlMod[1]=new UChar_t [size];
    if(nSegs==2 || hPlMod[2] || hPlMod[3]) {
      if(hPlMod[2]) delete [] hPlMod[2];
      if(hPlMod[3]) delete [] hPlMod[3];
      hPlMod[2]=new UChar_t [size];
      hPlMod[3]=new UChar_t [size];
    }
    hPlModsSize=size;
    clearPrArrs();
  }
...
void HMdcLookUpTbSec::clearPrArrs(void) {
  memset(hPlMod[0],0,size);
  memset(hPlMod[1],0,size);
  if(hPlMod[2]) memset(hPlMod[2],0,size);
  if(hPlMod[3]) memset(hPlMod[3],0,size);
}

void HMdcLookUpTbSec::clearPrMod(Int_t mod) {
  Int_t* hPlModM4=(Int_t*)(hPlMod[mod]);
  if(!hPlModM4) return;
  Int_t* xMaxM=xMax[mod];
  Int_t* xMinM=xMin[mod];
  Int_t* xMinMe=xMinM+nBinY;
  for(; xMinM<xMinMe; xMinM++) {
    if(*xMinM<=*xMaxM) {
      Int_t* eb=hPlModM4+((*xMaxM)>>2);
      for(Int_t* bt=hPlModM4+((*xMinM)>>2); bt<=eb; bt++) *bt=0;
      *xMinM=size;
      *xMaxM=-1;
    }
    xMaxM++;
...
void HMdcLookUpTbSec::makeSPlot(void) {
  ...
  UChar_t addm=1;  // !!!
  Int_t nmods=-1;
  if(minAmp[3]>0) nmods=3;
  else if(minAmp[2]>0) nmods=2;
  else if(minAmp[1]>0) nmods=1;
  else if(minAmp[0]>0) nmods=0;
  ...
  for(Int_t mod=0; mod<=nmods; mod++) {
    ...
    Int_t iLay=mod*6;
    Int_t nLay=(mod<nmods) ? 6:minAmp[mod]-1;
    // ??????????????????? eto poka staraya versiya
    for(Int_t layer=0; layer<nLay; layer++) {
      Int_t indLay=iLay+layer;
      makeLayProjV1(layer,indLay);
    }

    Char_t minAm=minAmp[mod];
    for(Int_t layer=nLay; layer<6; layer++) {
      Int_t i=iLay+layer;
        ...
        for(Int_t ln=0; ln<fCell.line; ln++) {
          ...
          for(Int_t nb=nbF; nb<=nbL; nb++) {
            *bt |= add;
            ...
            if(mod>0) {
              if(HMdcBArray::getNSet(hPlMod[0][nb])<minAm0) continue;
              if(mod>1) {
...
void HMdcLookUpTbSec::makeModPlot(Int_t mod,Int_t minAm) {
  ...
  Int_t iLay=mod*6;
  Int_t indMeth=minAm-2;  
  Int_t lay[6]={2,3,1,4,0,5};   // order of layers at the pr.plot filling
  
  Int_t meth1[5]={1,2,3,2,1};   // if layer<meth1[minAm-2]      - makeLayProjV1
  Int_t meth2[5]={5,4,0,0,0};   // else if layer<meth2[minAm-2] - firs {}
  Int_t meth3[5]={5,4,3,4,5};   // else if layer<meth3[minAm-2] - makeLayProjV2
                                // else                         - second {}
  
//  Int_t mth1=meth1[indMeth];
//  Int_t mth2=meth2[indMeth];
//  Int_t mth3=meth3[indMeth];
  
//  Int_t* nHt=nHits+iLay;
  
//   if(minAm<6) {
//     if(minAm==5 && maxAmp[mod]==5/*???*/) {
//       // one layer can have 0 wiers only
//       if(nHt[lay[0]]>0) mth1=1;
//       else if(nHt[lay[5]]>0) mth3=5;
//     } else if(minAm==4) {
//       if(maxAmp[mod]==5) {
//         // one layer can have 0 wiers only
//         if(nHt[lay[3]]==0 || nHt[lay[4]]==0 || nHt[lay[5]]==0) meth1=2;
//         else mth3=4;
//       } else if(maxAmp[mod]==4) {
//       }
//     }
//     
//   }
  ...
  for(Int_t indLine=0; indLine<6; indLine++) {
    ...
    if(indLine<meth1[indMeth]) makeLayProjV1(layer,indLay);
    else if(indLine<meth2[indMeth]) {
    ...
    } else if(indLine<meth3[indMeth]) makeLayProjV2(layer,indLay);
    else {
      UChar_t add=1<<layer;
      HMdcLookUpTbLayer& fLayer=(*cFMod)[layer];
      for(Int_t n=0; n<nHits[indLay]; n++) {
...
//        if( nLMaxCl < yBinMin ) break;            Hits not sorted !
...
void HMdcLookUpTbSec::findClusInSeg1(void){
  ...
  scanPlotInMod(0,clusArrM1,nClsArrM1);
  if(nClsArrM1>1) mergeClusInMod(0,nClsArrM1,clusArrM1);

  scanPlotInMod(1,clusArrM2,nClsArrM2);
  if(nClsArrM2>1) mergeClusInMod(1,nClsArrM2,clusArrM2);

  scanPlotInSeg1(0,plotBArSc,clusArr,nClsArr);
...
void HMdcLookUpTbSec::scanPlotInMod(Int_t mod,
  ...
  for(Int_t n4=maxBinBAr4M[mod]; n4>=minBin; n4--) {
    ...
    while((nLBin=HMdcBArray::prevAndUnset(b1,b2,nLBin))>=0) {
      ...
      nClArr++;
      if(nClArr >= clusArrSize) break;
    }
    if(nClArr >= clusArrSize) {
