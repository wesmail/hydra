...
Bool_t HMdcParRootFileIo::init(HParSet* pPar,Int_t* set) {
  ...
  if (pFile) {
    ...
    if (strcmp(name,"MdcCal2Par")==0) return read((HMdcCal2Par*)pPar,set);
    if (strcmp(name,"MdcCal2ParSim")==0) return read((HMdcCal2ParSim*)pPar,set);
    if (strcmp(name,"MdcCellEff")==0) return read((HMdcCellEff*)pPar,set);
    if (strcmp(name,"MdcTimeCut")==0) return read((HMdcTimeCut*)pPar,set);
    ...
    if (strcmp(name,"MdcTrackFitPar")==0) return read((HMdcTrackFitPar*)pPar,set);
...
Bool_t HMdcParRootFileIo::read(HMdcCal2Par* pCalPar,Int_t* set) {
  // reads and fills the container "MdcCal2Par" for calibration parameters
  Text_t* name=(char*)pCalPar->GetName();
  Int_t version=findInputVersion(name);
   if (version<=0)
  {
  ...
  if (pCalPar->getInputVersion(inputNumber)==version
        && pCalPar->getInputVersion(inputNumber)!=-1) return kTRUE;
  // needs reinitialization
  HMdcCal2Par* r=(HMdcCal2Par*)findContainer(name,version);
  Bool_t allFound=kTRUE;
  Int_t pos;
  initModules->Reset();
  HMdcCal2Par& rMdc=*r;
  HMdcCal2Par& pMdc=*pCalPar;
  for(Int_t s=0;s<pMdc.getSize();s++) {
    HMdcCal2ParSec& rSec=rMdc[s];
    HMdcCal2ParSec& pSec=pMdc[s];
    for(Int_t m=0;m<pSec.getSize();m++) {
      HMdcCal2ParMod& rMod=rSec[m];
      HMdcCal2ParMod& pMod=pSec[m];
      pos=s*4+m;
      if (set[pos]) {
        for(Int_t l=0;l<18;l++) {
          HMdcCal2ParAngle& rAngle=rMod[l];
          HMdcCal2ParAngle& pAngle=pMod[l];
            pAngle.fill(rAngle);
            set[pos]=0;
            initModules->AddAt(pos+1,pos);
        }
  ...
  pCalPar->setInputVersion(version,inputNumber);
  pCalPar->setChanged();
  printInfo("MdcCal2Par: module(s) initialized from Root file ");
  delete r;
  return allFound;
}
...
Bool_t HMdcParRootFileIo::read(HMdcTimeCut* pTimeCut,Int_t* set) {
  ...
  pTimeCut->setChanged();
  printInfo("TimeCut: module(s) initialized from Root file ");
  delete r;
  return allFound;
}
Bool_t HMdcParRootFileIo::read(HMdcTrackFitPar* pTrackFitPar,Int_t* set) {
  // reads and fills the container "MdcCal2Par" for calibration parameters
  Text_t* name=(char*)pTrackFitPar->GetName();
  Int_t version=findInputVersion(name);
   if (version<=0)
  {
      pTrackFitPar->setInputVersion(-1,inputNumber);
      return kFALSE;
  }    // not in ROOT file
  if (pTrackFitPar->getInputVersion(inputNumber)==version
        && pTrackFitPar->getInputVersion(inputNumber)!=-1) return kTRUE;
  // needs reinitialization
  HMdcTrackFitPar* r=(HMdcTrackFitPar*)findContainer(name,version);
  Bool_t allFound=kTRUE;
  Int_t pos;
  initModules->Reset();

  HMdcTrackFitPar& rMdc=*r;
  HMdcTrackFitPar& pMdc=*pTrackFitPar;

  for(Int_t s=0;s<pMdc.getSize();s++) {
    HMdcTrackFitParSec& rSec=rMdc[s];
    HMdcTrackFitParSec& pSec=pMdc[s];
    for(Int_t m=0;m<pSec.getSize();m++) {
      HMdcTrackFitParMod& rMod=rSec[m];
      HMdcTrackFitParMod& pMod=pSec[m];
      pos=s*4+m;
      if (set[pos]) {
	    pMod.fill(rMod);
            set[pos]=0;
  ...
  pTrackFitPar->setInputVersion(version,inputNumber);
  pTrackFitPar->setChanged();
  printInfo("TimeCut: module(s) initialized from Root file ");
  delete r;
  return allFound;
}
...
Bool_t HMdcParRootFileIo::read(HMdcCal2ParSim* pCalPar,Int_t* set) {
  ...
  pCalPar->setInputVersion(version,inputNumber);
  pCalPar->setChanged();
  printInfo("MdcCal2ParSim: module(s) initialized from Root file ");
  delete r;
  return allFound;
}
...
Bool_t HMdcParRootFileIo::read(HMdcCellEff* pCellEff,Int_t* set) {
  ...
  pCellEff->setChanged();
  cout<<"MdcCellEff: module types initialized from Root file: 0 1 2 3"<<endl;
  //printInfo("MdcCellEff: module(s) initialized from Root file ");
  delete r;
  return allFound;
}
...
Bool_t HMdcParRootFileIo::read(HMdcDigitPar* pDigitPar,Int_t* set) {
  ...
  Int_t version=findInputVersion(name);
   if (version<=0)
  {
      pDigitPar->setInputVersion(-1,inputNumber);
      return kFALSE;
  }    // not in ROOT file
