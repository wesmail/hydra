...
// Last update by Laura Fabbietti: 00/08/09 14:12:22
//
...
void HRichRingFindSim::CalcRingParameters(HRichAnalysisSim *showMe) {
  ...
  // this  method calculates Ring parameters and stores first and second indexes
  // for all the pads beloging to the ring Aerea ( 13 x 13 Mask centred in the 
  // ring middle)in 2 integer arrays.( *pRings.iRingPadNTrack1/2). These indexes
  // correspond to the first and to the last position of the pad tracks in the
  // catRichTrack container. 
...
//   iClusterLMax4.Set(50);
  ...
  //L.F. has increased the size of the TArrayI to be able to set the cut-off threshold to 0
  iClusterSize.Set(70);
  iClusterLMax4.Set(70);
  iClusterLMax8.Set(70);
  ...
  if (pAnalysisParams->iHowManyFitMatrixRings > 0)
    pAlgorithm1Ring = new HRichHitSim[pAnalysisParams->iHowManyFitMatrixRings];
  else pAlgorithm1Ring = NULL;
  if (pAnalysisParams->iHowManyHoughTransfRings > 0)
    pAlgorithm2Ring = new HRichHitSim[pAnalysisParams->iHowManyHoughTransfRings];
...
void HRichRingFindSim::LookForTrack(HRichAnalysisSim *showMe){
  ...
  // the catRichTrack array are stored. 
   ...
   for (n = 0; n < iRingNr; n++) {
     j = k = z = 0;
       while(pRings[n].iRingPadNTrack1[j]>=0) {

  

	 pRings[n].iRingTrack[k]=showMe->getTrack(pRings[n].iRingPadNTrack1[j]);
	 if ( pRings[n].iRingTrack[k]==0 ) cout<<"track == 0 in LooForTrack"<<endl;
	 pRings[n].iRingFlag[k]=showMe->getFlag(pRings[n].iRingPadNTrack1[j]);
       ...
       // INt his case even the second track is stored.
