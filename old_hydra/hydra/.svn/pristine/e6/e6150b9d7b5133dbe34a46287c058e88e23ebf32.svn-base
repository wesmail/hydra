...
{
  Float_t fakes;
#warning "Calculation of Fakes is wrong, if calculated in this way it should always be zero! Change it by introducing a new UserValue"
  // fakes is "valid PID tracks" minus "valid" particles in list
  if (getUserValue(FILLER_VALID_PIDTRACKS, fakes)){
    Int_t np;
    np=getNpart();
    for (Int_t j = 0; j < np; j++){
      if(isValidPart(j)) fakes--;
    }
  }else{
