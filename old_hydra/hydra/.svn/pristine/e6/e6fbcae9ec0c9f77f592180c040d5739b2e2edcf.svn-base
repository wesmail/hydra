...
//    nochicut   : switch off the chi2>-1 cut for MDc segments
...
// Usage:
//
//   TofRec *tofrec = new HTofRec("tofreconstructor","tofreconstructor",option);
  ...
  HReconstructor(name,title)
{
  ...
  if (sel.Contains("nochicut"))   fChiCutOn        = kFALSE;
...
Bool_t HTofRec::init(void){
  ...
  cout << " Rich Mode: "<<fRichMode<<" Minus Mode: " << fMinusMode << " Plus Mode: "<<fPlusMode << " ChiCutMode " << fChiCutOn << " Common Mode: " << isCommon << endl;
...
Int_t HTofRec::execute(void){
  ...
  if (fSpline||fKickLowresB||fKickHires||fRK)
	...
	{
	  if ((!fChiCutOn) || ((mdcI = (HMdcSeg *)fMdcSegCat->getObject(((HMdcTrkCand *)fMdcTrkCandCat->getObject(metamatch->getTrkCandInd()))->getSeg1Ind()))->getChi2()>-1))
	    // cut on chi2> -1 for inner segment
	    { 
	      isRich = kFALSE;
	      if ((fRichMode)&&(isRichCorr(mdcI))) isRich = kTRUE;
	      if ((!fChiCutOn) || ((mdcO = (HMdcSeg *)fMdcSegCat->getObject(((HMdcTrkCand *)fMdcTrkCandCat->getObject(metamatch->getTrkCandInd()))->getSeg1Ind()))->getChi2()>-1))
		  ...
		  if ((fSpline) && (metamatch->getSplineInd()!=-1) && metamatch->isSplineAccepted()) // if a Spline track is present
		    {
		      ...
		      if (spline->getQSpline()!=-1 && spline_sec_on[spline->getSector()] &&
			  ((spline->getSystem()==0 && (((HShowerHitTof *)fTofinoCat->getObject(spline->getShowerHitInd()))->getTofinoMult()==1)) ||
			   (spline->getSystem()==1 && metamatch->getTofClusterSize()!=2))) 
			  ...
			  if (isRich)  // if the track is RICh correlated
			    {
				  ...
				  ((isRichSpline) && (mdc_sec[spline->getSector()]>mdc_sec[spline_sec])) ||  // priority on sectoprs with more MDC chambers
				  ((isRichSpline) && (mdc_sec[spline->getSector()]==mdc_sec[spline_sec]) && (spline->getSystem()==1&&spline_sys==0)) ||   // or if new one TOF, old one TOFINO
				  ((isRichSpline) && (mdc_sec[spline->getSector()]==mdc_sec[spline_sec]) && (spline->getSystem()==spline_sys) && (spline->getPolarity()==-1 && spline_pol==1)) ||   // or if new one pol==1 and old  pol ==1
				  ((isRichSpline) && (mdc_sec[spline->getSector()]==mdc_sec[spline_sec]) && (spline->getSystem()==spline_sys) && (spline->getPolarity()==spline_pol==1) && spline->getTof()<spline_tof)) // faster particle
			  ...
			  else if (!isRichSpline&&spline->getPolarity()==-1)
			    {
				  ...
				  ((isMinusSpline) && (mdc_sec[spline->getSector()]>mdc_sec[spline_sec])) ||
				  ((isMinusSpline) && (mdc_sec[spline->getSector()]==mdc_sec[spline_sec]) && (spline->getSystem()==1&&spline_sys==0)) ||   // or if new one TOF,  old one TOFINO
				  ((isMinusSpline) && (mdc_sec[spline->getSector()]==mdc_sec[spline_sec]) && (spline->getSystem()==spline_sys) && (spline->getTof()<spline_tof))) // faster particle
				{
				  isMinusSpline = kTRUE;
				  spline_tof = spline->getTof();
				  spline_sys = spline->getSystem();
				  spline_ind = metamatch->getSplineInd();
				  spline_sec = spline->getSector();
				  spline_start = spline->getTofDist()*sqrt(spline->getP()*spline_sec_field[spline->getSector()]*spline->getP()*spline_sec_field[spline->getSector()]+HPidPhysicsConstants::mass(8)*HPidPhysicsConstants::mass(8))/(C*spline->getP()*spline_sec_field[spline->getSector()])-spline->getTof(); // new start
		  ...
		  if ((fKickHires) && (metamatch->getKick123Ind()!=-1)) // if a Spline track is present
		    {
		      ...
		      if (kick123_sec_on[kick123->getSector()] &&
			  ((kick123->getSystem()==0 && (((HShowerHitTof *)fTofinoCat->getObject(kick123->getShowerHitInd()))->getTofinoMult()==1)) ||
			   (kick123->getSystem()==1 && metamatch->getTofClusterSize()!=2))) 
			// no TOF clusters or TOFINO double hits, & sector ON
			{
			  if (isRich)  // if the track is RICH correlated
			    {
			      if ((!isRichKick123) ||   // if there are no other kick123 tracks rich correlated
				  ((isRichKick123) && (mdc_sec[kick123->getSector()]>mdc_sec[kick123_sec])) ||  // priority on sectoprs with more MDC chambers
				  ((isRichKick123) && (mdc_sec[kick123->getSector()]==mdc_sec[kick123_sec]) && (kick123->getSystem()==1&&kick123_sys==0)) ||   // or if new one TOF, old one TOFINO
				  ((isRichKick123) && (mdc_sec[kick123->getSector()]==mdc_sec[kick123_sec]) && (kick123->getSystem()==kick123_sys) && (kick123->getPolarity()==-1 && kick123_pol==1)) ||   // or if new one pol==1 and old  pol ==1
				  ((isRichKick123) && (mdc_sec[kick123->getSector()]==mdc_sec[kick123_sec]) && (kick123->getSystem()==kick123_sys) && (kick123->getPolarity()==kick123_pol==1) && kick123->getTof()<kick123_tof)) // faster particle
			  ...
			  else if (!isRichKick123&&kick123->getPolarity()==-1)
			    {
				  ...
				  ((isMinusKick123) && (mdc_sec[kick123->getSector()]>mdc_sec[kick123_sec])) ||
				  ((isMinusKick123) && (mdc_sec[kick123->getSector()]==mdc_sec[kick123_sec]) && (kick123->getSystem()==1&&kick123_sys==0)) ||   // or if new one TOF,  old one TOFINO
				  ((isMinusKick123) && (mdc_sec[kick123->getSector()]==mdc_sec[kick123_sec]) && (kick123->getSystem()==kick123_sys) && (kick123->getTof()<kick123_tof))) // faster particle
				{
			  ...
			  if (isRich)  // if the track is RICH correlated
			    {
				  ...
				  ((isRichRK) && (mdc_sec[runge->getSector()]>mdc_sec[rk_sec])) ||  // priority on sectoprs with more MDC chambers
				  ((isRichRK) && (mdc_sec[runge->getSector()]==mdc_sec[rk_sec]) && (runge->getSystem()==1&&rk_sys==0)) ||   // or if new one TOF, old one TOFINO
				  ((isRichSpline) && (mdc_sec[runge->getSector()]==mdc_sec[rk_sec]) && (runge->getSystem()==rk_sys) && (runge->getPolarity()==-1 && rk_pol==1)) ||   // or if new one pol==1 and old  pol ==1
				  ((isRichSpline) && (mdc_sec[runge->getSector()]==mdc_sec[rk_sec]) && (runge->getSystem()==rk_sys) && (runge->getPolarity()==rk_pol==1) && runge->getTof()<rk_tof)) // faster particle
			  ...
			  else if (!isRichRK&&runge->getPolarity()==-1)
			    {
				  ...
				  ((isMinusRK) && (mdc_sec[runge->getSector()]>mdc_sec[rk_sec])) ||
				  ((isMinusRK) && (mdc_sec[runge->getSector()]==mdc_sec[rk_sec]) && (runge->getSystem()==1&&rk_sys==0)) ||   // or if new one TOF,  old one TOFINO
				  ((isMinusRK) && (mdc_sec[runge->getSector()]==mdc_sec[rk_sec]) && (runge->getSystem()==rk_sys) && (runge->getTof()<rk_tof))) // faster particle
				{
				  isMinusRK = kTRUE;
				  rk_tof = runge->getTof();
				  rk_sys = runge->getSystem();
	...
	{ 
	  ...
	  if (spline_ch->getSystem()!=-1)
	      ...
	      if(fillNTuple)
		{
	      ...
	      p = spline_ch->getP()*spline_sec_field[spline_ch->getSector()];
	      ...
	      if(fillNTuple)
		{
		  ...
		  after->Fill(spline_ch->getP()*spline_sec_field[spline_ch->getSector()],
			      spline_ch->getPolarity(),
			      spline_ch->getBeta(),
			      spline_ch->getTof(),
			      spline_ch->getMass2(),
