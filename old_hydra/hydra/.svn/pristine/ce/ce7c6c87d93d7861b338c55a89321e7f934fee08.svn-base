...
// Last update by Thomas Eberl: 02/04/18 18:44:56
//
///////////////////////////////////////////////////////////
// HRichCorrelator
//
...
//  	    }
		...
		fMdcHit=event->getCategory(catMdcHit);
		if (!fMdcHit) {
		  Error("init","No MDC hit category defined");
		  return kFALSE;
		}
		else fMdcHitIter=(HIterator *)fMdcHit->MakeIterator();
...
void HRichCorrelator::matchKickTracks()
{
	...
	// INDEX MATCHING DOES NOT WORK
	// AS THE KICK TRACK DATA MEMBERS
	// HAVE A DIFFERENT (UNKNOWN TO ME) MEANING
...
//  	HKickTrack *t = 0;
//  	//cout<<iterTracks<<endl;
//  	iterTracks->Reset();
//    	while ((t=(HKickTrack*) iterTracks->Next())!=0) 
//    	{
//  	    Int_t mt    = t->getSegment();
//  	    Int_t rt    = t->getRingId();
//  	    Int_t sys   = t->getSystem();//0=tof,1=shower
//  	    Int_t metat = t->getOuterHitId();
//  	    Int_t tsec  = t->getSector();
//  	    Int_t metah=-1;
	  
//  	    if (sys==0) metah=th;
//  	    else if (sys==1) metah=sh;
//  	    //	    if (hsec==tsec && mh==mt && metah==metat)
//  	    if (hsec==tsec)
//  	    {
//  		cout<<"----------------------------------------"<<endl;
//  		cout<<mt<<" "<<rt<<" "<<sys<<" "<<metat<<" "<<tsec<<endl;
//  		cout<<mh<<" "<<rh<<" "<<sys<<" "<<metah<<" "<<hsec<<endl;
//  		cout<<"----------------------------------------"<<endl;
//  		//		cout<<rt<<" "<<rh<<" "<<endl;
//  	    }
...
void HRichCorrelator::rotateTrackToLab(Int_t s,Float_t tin,Float_t pin,Float_t &tout,Float_t &pout)
{
    // this should be done using the HSpecGeomPar
    // in the moment I do it more simple ?
...
void HRichCorrelator::setMdc(HHitMatch* m,HMdcSeg* mdc){
    ...
    //begin jaro:
    // //flag in mdcseg is overwritten in a case kicktrack task is in task list; one has to look in mdchit
    
    Int_t hit_inx,hit_inx0,hit_inx1,hit_inx2,hit_inx3;
    hit_inx=-1;
    hit_inx0=mdc->getHitInd(0);
    hit_inx1=mdc->getHitInd(1);
    ...
    if(hit_inx>-1){
      HMdcHit* mdchit=(HMdcHit *)getMdcHitCat()->getObject(hit_inx);
      m->setMdcClusterNWires(mdchit->getFlag()/100);
      m->setMdcClusterNMerged(mdchit->getFlag()%100);
