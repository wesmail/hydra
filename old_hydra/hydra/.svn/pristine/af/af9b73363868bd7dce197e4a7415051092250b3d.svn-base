...
// Last update by Thomas Eberl: 01/09/18 17:36:50
//
...
Bool_t HRichPairAna::init() {
	...
	iniCounters();
	iniHistos();
	pFileOut = new TFile("hist.root","RECREATE");
    return kTRUE;
}
...
{
pHistRTMopenAngle = new TH1F("RM_opening_angle","RM_opening_angle",90,0,90);
...
void HRichPairAna::calcOpeningAngle(TObjArray *pArr)
{
	...
	for (Int_t j=nCnt;j<nMaxArr;j++)
	{
	    ...
	    if (opangle>0.) pHistRTMopenAngle->Fill(opangle);
...
Int_t HRichPairAna::execute()
{
    ...
    if (nRTM>1)
    {
 	...
 	if (nbObjs>1) calcOpeningAngle(pRTM);
    }
    delete pRTM;
    return 0;
}
