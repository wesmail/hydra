...
HSplineTaskSet::HSplineTaskSet(){isSplinePar=kFALSE;}
HSplineTaskSet::HSplineTaskSet(Text_t name[],Text_t title[]):HTaskSet(name,title){isSplinePar=kFALSE;}
...
HTask* HSplineTaskSet::make(const Char_t* select,Option_t *options)
{
...
//    {
	  ...
	  if(isSplinePar) pSplineTrackF->makeSplinePar();
