  ...
  if(pMetaMatch->getKickInd()>-1)
    {
      ...
      fPathLength[ALG_KICK] = pKickTrack->getTof()*pKickTrack->getBeta()*TMath::C()*1e-6;
    }
  ...
  if(pMetaMatch->getSplineInd()>-1)
    {
      ...
      fPathLength[ALG_SPLINE] = HPidFL::getSplineTrack(pMetaMatch->getSplineInd())->getTofDist();      
    }
    if( nRKTrackInd >-1){
        ...
        fPathLength[ALG_RUNGEKUTTA] = HPidFL::getRKTrack(nRKTrackInd)->getTofDist();
    }
}
