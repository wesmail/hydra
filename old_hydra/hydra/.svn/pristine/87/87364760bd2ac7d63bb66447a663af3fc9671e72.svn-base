...
HMdcProjPlot::HMdcProjPlot(Int_t inBinX, Int_t inBinY) {
  ...
  plMod1=new UChar_t [size];
  plMod2=new UChar_t [size];
  sizeBAr=size/8;
...
TH2C* HMdcProjPlot::getPlot(Char_t* name, Char_t* title){
  ...
  for (Int_t nBin=0; nBin<size; nBin++) {
    if(plMod1[nBin]==0 && plMod2[nBin]==0) continue;
    Int_t ny=nBin/nBinX;
    Int_t nx=nBin%nBinX;
    rootPlot->Fill((Float_t)nx*stX+stX*0.5+xMin,
               (Float_t)ny*stY+stY*0.5+yMin,
                HMdcBArray::getNSet(plMod1[nBin])+HMdcBArray::getNSet(plMod2[nBin]));
  }
  return rootPlot;
}
...
TH2C* HMdcProjPlot::getPlotV2(Char_t* name, Char_t* title){
  ...
  for (Int_t nBin=0; nBin<size; nBin++) {
    ...
    Int_t ny=nBin/nBinX;
    Int_t nx=nBin%nBinX;
    Int_t color=HMdcBArray::getNSet(plMod1[nBin]);
    Int_t col4=HMdcBArray::getNSet(plMod2[nBin]);
...
void HMdc34ClFinderSec::makePlotSeg2() {
  ...
  Int_t nmods=-1;
  if(minBin[3]>0) nmods=3;
  else if(minBin[2]>0) nmods=2;
  if(nmods<0) return; 
  UChar_t addm=1;
  Short_t* xMinB=prPlotSeg2->xMinL;
  Short_t* xMaxB=prPlotSeg2->xMaxL;
  for(Int_t mod=2; mod<=nmods; mod++) {
    if(!lMods[mod]) continue;
    HMdc34ClFinderMod& fMod=(*this)[mod];
    Int_t minAm=minBin[mod];
    UChar_t* plMod=(mod==2) ? prPlotSeg2->plMod1:prPlotSeg2->plMod2;
    Int_t nLay=(mod<nmods) ? 6:minAm-1;
    for(Int_t lay=0; lay<nLay; lay++) {
      ...
      if(cell >= fLay.nCells) {
        Warning("makePlotSeg2","Cell %i >= num.cells(=%i)",cell,fLay.nCells);
        continue;
      }
      UChar_t add=addm<<lay;
...
// Mozhno optimizirovat'!
          for(Int_t nx=0; nx<prPlotSeg2->nBinX; nx++) {
            ...
            for(; ny<=nyU; ny++) {
              if(nx>=xMinB[ny] && nx<=xMaxB[ny]) plMod[binNum] |= add;
              binNum += prPlotSeg2->nBinX;
            }
          }
          shDown=shDownN;
        }
...
// Mozhno optimizirovat'!
          for(Int_t nx=0; nx<prPlotSeg2->nBinX; nx++) {
            ...
            Int_t binNum=ny*prPlotSeg2->nBinX+nx;
            for(; ny<=nyU; ny++) {
              if(nx>=xMinB[ny] && nx<=xMaxB[ny]) {
                plMod[binNum] |= add;
                if( HMdcBArray::getNSet(plMod[binNum])>=minAm ) {
                  if( HMdcBArray::get(prPlotSeg2->plotBArSc,binNum )==0 &&
                  (mod==2 || HMdcBArray::getNSet(prPlotSeg2->plMod1[binNum])>=mBm3) ) {
                    HMdcBArray::set(prPlotSeg2->plotBArSc,binNum);
                    if(binNum<minB4Sc) minB4Sc=binNum;
                    if(binNum>maxB4Sc) maxB4Sc=binNum;
                  }
                }
              }
              binNum += prPlotSeg2->nBinX;
...
//  Int_t loc2FirstCl=counter;
  ...
  while(nClus>1) {
    ...
    for(Int_t cl1=0; cl1<nClsArr-1; cl1++) {
      ...
      for(Int_t cl2=cl1+1; cl2<nClsArr; cl2++) {
        ...
        if(minBin[2]>0 && cLCSeg2.compare(cLCSeg2s,0, 5)<4) continue;
        if(minBin[3]>0 && cLCSeg2.compare(cLCSeg2s,6,11)<4) continue;
  ...
  for(Int_t cl=0; cl<nClsArr; cl++) {
    ...
    if(!clus) {
      Warning("fillClusCut2","S.%i No slot HMdcClus available",sector+1);
      return cl-1;
    }
    if(isGeant) clus=(HMdcClus*)(new(clus) HMdcClusSim(cLCells2[cl]));
    else clus=new(clus) HMdcClus(cLCells2[cl]);
    counter++;
    nClusters++;
    clus->setAddress(locClus);
    clus->setMod((mSeg[1]==3) ? -2 : (mSeg[1]+1) );  // mSeg[1]+1=mSeg[1]-1+2
    clus->setTypeClFinder(0);
    clus->setMinCl(minBin[2],minBin[3]);
    ...
    if(clus->isGeant()) ((HMdcClusSim*)clus)->calcTrList();
  }  
  return nClusters;
}
...
void HMdc34ClFinderSec::calcClusterSeg2(Int_t nBinF) {
  ...
  while((nBin=stack->pop()) >= 0) {
    ...
    Double_t wt=(Double_t)(HMdcBArray::getNSet(prPlotSeg2->plMod1[nBin])+
                           HMdcBArray::getNSet(prPlotSeg2->plMod2[nBin])-minBinS);
    //region of cluster ???
    Double_t x=(nx+0.5)*prPlotSeg2->stX+prPlotSeg2->xMin;
    Double_t y=(ny+0.5)*prPlotSeg2->stY+prPlotSeg2->yMin;
    cSumWtI += wt;
    cMeanXWtI += x*wt;
    cMeanYWtI += y*wt;
    cMeanXXWtI += x*x*wt;
  ...
  for(Int_t mod=2; mod<4; mod++) {
    ...
    //if (dY1b==0 || dY2b==0) - optimizaciya, proverit' na time !!!!???
          ...
          if(nDeleted) Warning("calcClusterSeg2",
              "%i cells was removed (size of cluster is very big)",nDeleted);
          break;
        }
      }
    }
