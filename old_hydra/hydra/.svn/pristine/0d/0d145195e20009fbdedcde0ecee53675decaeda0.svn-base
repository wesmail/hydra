...
Bool_t HMdcOffset::finalize(void)
{
    ...
    TFile *file = NULL;
    TNtuple* offsetTuple=NULL;
    if (fNameRootOffset)
    {
	file = new TFile(fNameRootOffset,"RECREATE");
	file->cd();
	offsetTuple=new TNtuple("offset","offset","s:m:mb:tdc:slope:slopeErr:off:offErr:integral:off1");
	...
	if (!mdcDet)
		...
		for(Int_t mo=0; mo<4; mo++) {  //loop over modules
			...
			for(Int_t t=0; t<nTdc; t++) {  //loop over cells
			    ...
			    integral[s][mo][mb][t]=((*hintegral)[s][mo][mb][t][nbin-2]);
