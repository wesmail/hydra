...
//!?????????????? est' ne ponyatnoe vliyanie iflag na rezul'tat !!!
  if(hitStatus != 0) {
    distance=pSCLayer->getImpact(par,cell,alpha);
//!    driftTime=pDrTimePar->calcTime(sector,module,alpha,distance);
    if(distance>5e4)distance=5e4; // hot fix ..... vladimir....help!!!!!
...
//!?????????????? est' ne ponyatnoe vliyanie iflag na rezul'tat !!!
  if(hitStatus != 0) {
    inCell=pSCLayer->getImpact(par,cell,alpha,distance);
//!    driftTime=pDrTimePar->calcTime(sector,module,alpha,distance);
    pDrTimeParBin=pDrTimePar->getBin(sector,module,alpha,distance);
    driftTime=pDrTimeParBin->calcTime(alpha,distance);
    dev0 = driftTime - tdcTime;
...
void HMdcWireData::calcDriftTimeForDeriv(HMdcTrackParam& par) {
  ...
  if(hitStatus==1) {
    Double_t alphaD;
    Double_t distD=pSCLayer->getImpact(par,cell,alphaD);
