...
Bool_t HShowerUnpacker::init(void) {
  ...
  if (!pParam) {
     pParam = new HShowerUnpackParam;
     pParam->defaultInit();
//     rtdb->addContainer(pParam);
  } else
     printf("OK\n");
...
//  if ((nCharge>=53) && (nCharge<70)) return 1;
...
//      if (!module) column = 31 - column;
      ...
      if (chargeH >= threshold) {
        ...
        m_loc[2]=row + 4;	
        m_loc[3]=column;	
        if (fillData(m_loc, chargeH, nEvtId)) ii++;
      }
      ...
      if (chargeL >= threshold) {
        ...
        m_loc[2]=row;
        m_loc[3]=column;
        if (fillData(m_loc, chargeL, nEvtId)) ii++;
      }
