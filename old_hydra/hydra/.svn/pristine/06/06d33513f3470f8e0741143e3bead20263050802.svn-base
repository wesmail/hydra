...
Int_t  HMatchUUnpacker::decodeMUData(UInt_t *pData,Int_t maxLength) {
  ...
  if ( (triggerCode != 1) && (length >2)){  
    // for calibration events (triggerCode != 1) the length of MU data would be 2
    // if it is > than 2, it means that tehre are scalers
...
Int_t HMatchUUnpacker::decodeRich(UInt_t *pData,Int_t maxLength) {
  ...
  MU_sec_pattern_Rich = muData->getSecHitPatternRich();
   ...
   if(  (MU_sec_pattern_Rich != RichSecPatternMask)  &&  (triggerCode==1) 
       && ( (muData->getNHitsShower() + muData->getNHitsTof()) > 1 ) ) {
     //HEventHeader *fHead ;
     //cout << "***** event: " << evt << "     ******* seq nr: " << fHead->getEventSeqNumber() << endl;
     cout << " ERROR: Rich Hit Pattern Wrong!!! " << MU_sec_pattern_Rich << "  " << RichSecPatternMask << " TRIGGER CODE: " << triggerCode << endl;
   }
...
Int_t HMatchUUnpacker::decodeIpuRich(UInt_t *pData,Int_t maxLength) {
  ...
  UInt_t fifo,colPat,hardsegId, segId,row,col, segId_old;
  //Int_t Rich_Sector_Map[6]={0,5,1,3,2,4}; // before 309oct02
  Int_t Rich_Sector_Map[6]={1,5,0,3,2,4}; //311oct02 configuration
  Float_t theta, phi;
  ...
  bc = 0;
  fifo = colPat = hardsegId = segId = row = col =0;
  theta = phi = 0.;
...
segId:%i theta:%f phi:%f \n  ",   row-23, 95 - fifo * 8 - bc,fifo,bc,segId,theta,phi);
	  ...
	  if (muRich) {
	    ...
	    if (segId != segId_old) {
	      FullCount=ReducedCount=0;
	      if ( segId < segId_old )
		cout << "---- PLEASE change your code!!!! --- No proper sector order of hits!" << endl;
	    } 
	    segId_old = segId;
