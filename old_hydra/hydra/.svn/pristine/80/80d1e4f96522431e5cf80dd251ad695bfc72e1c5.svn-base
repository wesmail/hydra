...
//*-- Modified : 30/10/2002 by R. Holzmann
...
Hades::Hades(void) {
  ...
  TTree::SetMaxTreeSize(200000000); // set >fOutputSizeLimit to avoid conflict
	 ...
	 if (evtId < (UInt_t)fTaskList->GetSize()) {
	   ...
	   if (fEvTask) {
		 ...
		 if(!quiet) Warning("eventLoop",
		   "Exiting tasklist with errCode: %i at event: %i \n",err,evN);
		 if (fTree) fTree->Fill();
	       }
	     } else {
	       if (fTree) fTree->Fill();
	     ...
	     Warning("eventLoop","No tasks defined for event: sub event id 0x%x\n", evtId);
	   }
