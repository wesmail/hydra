...
// Last update by Laura Fabbietti: 01/09/12 16:35:54
//
/////////////////////////////////////////////////////////////////////////////
...
Bool_t HRichDigitizer::reinit(){
    ...
    isActiveNoise = ((HRichDigitisationPar*)fDigitisationPar)->getActiveNoise();
    ...
    a1 = ((HRichDigitisationPar*)fDigitisationPar)->getExp1Nor();
    a2 = ((HRichDigitisationPar*)fDigitisationPar)->getExp1exp();
    b1 = ((HRichDigitisationPar*)fDigitisationPar)->getExp2Nor();
    b2 = ((HRichDigitisationPar*)fDigitisationPar)->getExp2exp();
    cout<<  fFactor1<<" fFactor1  "<<fFactor2<<" fFactor2  "<<fParam1<<"  fParam1  "<<fParam2<<" fParam2  "<< a1<<" a1  "<<a2<<" a2  "<<b1<<" b1 "<<b2 <<"  b2 "<<endl;
    return kTRUE;
}
//----------------------------------------------------------------------------
...
Int_t HRichDigitizer::checkPad(HRichCalSim *calSim){
    ...
    Int_t iThreshold =   (Int_t) ( 3*fSigmaValue/fChargePerChannel + fDraw + 0.5);
...
void HRichDigitizer::calcChargeOnWire(Int_t sector, Float_t xhit, Float_t yhit, Float_t nTrack, Float_t nFlag ,Float_t ene) {
 ...
 HRichFrame *pFrame = ((HRichGeometryPar*)fGeometryPar)->getFramePar();
  ...
  while(!cTRUE){
	  ...
	  charge = 1/a2*(log(fDraw1*( exp(a2*q1)-1 ) + 1));
	  if(charge<=q1) {
	  ...
	  charge = -q1 + 1/b2*log(-fDraw2*(exp(b2*qMax) - exp(b2*q1) ) );
	  if(charge>q1) {
...
void HRichDigitizer::digitisePads() {
 ...
 // ------------------------ central pad (0,0)
  ...
  if (pTmpPad->getPadFlag() == 2) {
      ...
      param11 =  GaussFun(fDigitPadMatrix[4]*fFactor1,fDigitPadMatrix[4]*fFactor1/2.5)/fDigitPadMatrix[4];
      param21 =  GaussFun(fDigitPadMatrix[4]*fFactor2,fDigitPadMatrix[4]*fFactor2/2.5)/fDigitPadMatrix[4];
  ...
  for (Int_t n =-1;n<2;n++){
	  ...
	  if(m!=0 || n!=0 ){
		  ...
		  if (pTmpPad->getPadFlag() == 2){
		      fDigitPadMatrix[iPadInd] = calcIndCharge(yRel,fDigitPadMatrix[4],iPadInd,pCharge->getWireNr()); 
		      fDigitPadMatrix[iPadInd] = GaussFun(fDigitPadMatrix[iPadInd],fDigitPadMatrix[iPadInd]/2.5);
		      if (fDigitPadMatrix[iPadInd] < 0.) fDigitPadMatrix[iPadInd] = 0.;
		      else{
...
Float_t HRichDigitizer::calcIndCharge(float yCharge,float q4,int iPdaIndex,int iWireNr){
    ...
    case 3: {
	if(iWireNr%2==0) return (fFactor2 * q4);
	else return  (fFactor1* q4);
    }
    case 5:{
	if(iWireNr%2==0) return (fFactor1 * q4);
	else return  (fFactor2* q4);
    }
    case 2:{
	if(iWireNr%2==0) return (2*fFactor1 * q4 * qX(1-yCharge));
	else return  (2*fFactor2 * q4 * qX(1-yCharge));
    }
    case 8:{
	if(iWireNr%2==0) return (2*fFactor1 * q4 * qX(yCharge));
	else return  (2*fFactor2 * q4 * qX(yCharge));
    }
    case 6:{
	if(iWireNr%2==0) return (2*fFactor2 * q4 * qX(yCharge));
	else return  (2*fFactor1 * q4 * qX(yCharge));
    }
    case 0:{
	if(iWireNr%2==0) return  (2*fFactor2 * q4 * qX(1-yCharge));
	else return  (2*fFactor1 * q4 * qX(1-yCharge));
    }
    }
    return 0;
...
Float_t HRichDigitizer::qX(Float_t pos){
   ...
   charge =(1/fParam1)*((fParam2)*exp(fParam1*pos) - fParam2);
 
   return charge;
}
Float_t HRichDigitizer::q4Calc(float charge,float pos,int par1,int par2){
