...
// HHypPKpKmAlg
//
// HHypPKpKmAlg is an algorithm which selects PP (+X) events.
...
#include <stdlib.h>
#include <iostream>
#include "hhypPKpKmAlg.h"
#include "hypinfodef.h"

ClassImp(HHypPKpKmAlg)

HHypPKpKmAlg::HHypPKpKmAlg(Char_t *name_i, Option_t par[])
...
Bool_t HHypPKpKmAlg::execute()
{
	...
	if( use_InclusiveFiller){
		...
		else cerr << " HHypPKpKmAlg::execute() : exitIdx = "<< ERROR_IDX;
...
Bool_t HHypPKpKmAlg::init()
{
	...
	// 2 + 1 part filler
	if(use_2_1){
		cout << "HHypPKpKmAlg will use option ONEFAKE" << endl;
	...
	// 2 + 2 part filler
	if(use_2_2){
		cout << "HHypPKpKmAlg will use option TWOFAKES" << endl;
...
Bool_t HHypPKpKmAlg::reinit()
{
  return kTRUE;
}

Bool_t HHypPKpKmAlg::finalize()
{
  return kTRUE;
}
