...
Bool_t HMdcGetContainers::isInited(HParSet *par) {
 if( !par ) return kFALSE;
 return (par->getInputVersion(0)>=0 || par->getInputVersion(1)>=0
     || par->getInputVersion(2)>=0) ? kTRUE:kFALSE;
}
...
HSpecGeomPar* HMdcGetContainers::getSpecGeomPar(Bool_t init) {
  if( !fSpecGeomPar && isRtdb ) {
    ...
    if(!fSpecGeomPar) fSpecGeomPar=(HSpecGeomPar*)(spectr->createSpecGeomPar());
  }
...
HKickPlane* HMdcGetContainers::getKickPlane(Bool_t init) {
  if( !fKickPlane ) fKickPlane = HKickPlane::getMDC3();
