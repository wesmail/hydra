...
//                                   2=keep Geant data
//  setSignalSpeed(0.005) sets the speed of the signal on the wire used to calulate the time offset
...
void HMdcDigitizer::printStatus()
{
  ...
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"tdcModeDigi     =  %i :  1 = two leading edges, 2 = leading and trailing edge",modetdc);
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"NtupleDigi      =  %i :  0 = noNtuple, 1 = digitizer.root",(Int_t)fntuple);
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"useTofDigi      =  %i :  0 = NoTof in cal1, 1 = Tof in cal1 \n",(Int_t)useTof);
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"useErrorDigi    =  %i :  0 = NoErr in cal1, 1 = Err in cal1 \n",(Int_t)useError);
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"offsetsOnDigi   =  %i :  0 = global offsets off, 1 = global offsets on",(Int_t)useOffsets);
  gHades->getMsg()->info(10,DET_MDC,GetName()
  ...
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"noiseModeDigi   =  %i :  1 = override geant by noise, 2 = keep geant cells",noisemode);
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"noiseOnDigi     =  %i :  0 = noise off, 1 = noise on",(Int_t)useNoise);
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"noiseLevelDigi  = %4.1f%% %4.1f%%  %4.1f%% %4.1f%% noise level per plane"
			 ,100-(100*getNoiseLevel(0)),100-(100*getNoiseLevel(1)),100-(100*getNoiseLevel(2)),100-(100*getNoiseLevel(3)));
  gHades->getMsg()->info(10,DET_MDC,GetName()
  ...
  gHades->getMsg()->info(10,DET_MDC,GetName()
			 ,"cellEffOnDigi   =  %i :  0 = cellEff off, 1 = cellEff",(Int_t)useCellEff);
  gHades->getMsg()->info(10,DET_MDC,GetName()
...
Int_t HMdcDigitizer::execute(void) {
  ...
  while ((fCell=(HMdcGeantCell *)itercell->Next()) != NULL) {
	...
	if(getNoiseUse() && getNoiseMode()==1 && ( gRandom->Rndm()>getNoiseLevel(loc[1])) )
	{
...
void HMdcDigitizer::setEfficiencyFlags(Int_t ihit,Int_t sec,Int_t mod,Int_t lay)
{
	...
	setStatus(ihit,fCellEff->calcEfficiency(mod,getMinimumDist(ihit),getAngle(ihit),getCellEffLevel(mod)));
        setFractionOfmaxCharge(ihit,fCellEff->calcEffval(mod,getMinimumDist(ihit),getAngle(ihit),getCellEffLevel(mod)));
    }
    else
    {
	setStatus(ihit,1);
