...
////////////////////////////////////////////////////////////////
//
//  HTofDigitizer digitizes data, puts output values to 
//  raw data category
//  
////////////////////////////////////////////////////////////////
...
Int_t HTofDigitizer::execute(void) {
  ...
  while ((geant=(HGeantTof *)iter->Next())!=0) {
    ...
    raw = (HTofRaw*) fRawCat->getObject(fLoc);   // test if cell already in use
    if(raw) {
       raw->incNHit();  // increment counter for additional hits
       continue;
    }

    raw = (HTofRaw*) fRawCat->getSlot(fLoc);
    if(raw) {
      raw = new(raw) HTofRaw;
      raw->setNHit(1);
	...
	if(timeCh > 4094)  timeL = 4095.;
	else               timeL = (Float_t) (timeCh);
	timeCh = (Int_t) (timeR/channelTDC);
	if(timeCh > 4094)  timeR = 4095.;
	else               timeR = (Float_t) (timeCh);

	chargeCh = (Int_t) ((chargeL/chargeRef)*1024);
	if(chargeCh > 4094)  chargeL = 4095.;
	else                 chargeL = (Float_t) (chargeCh);
	chargeCh = (Int_t) ((chargeR/chargeRef)*1024);
	if(chargeCh > 4094)  chargeR = 4095.;
	else                 chargeR = (Float_t) (chargeCh);
      }
      else {
	timeL = 4095.;
	timeR = 4095.;
