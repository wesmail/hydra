...
//      if ((fEventHead)->getId () == 1)
  if(1)
  {
    ...
    for (Int_t sec = 0; sec < 6; sec++)
      ...
      {		//1000 arbitrary value,1600 would be consistent
        diffList[k][0] = 0;
      }
...
void HTriggerHistFiller::fillMatchUCorrelation(HIterator *iterrich, HIterator* itermuhit)
{
}
...
void   HTriggerHistFiller::putTriggerParams(HParamList* l,TList* histpool)
{
  ...
  l->add("hrichCorrPhi"                  ,mytemp,500);

  hrichCorrThetaA->getText(mytemp);
  l->add("hrichCorrTheta"                ,mytemp,500);
  ...
  l->add("hmuCorrPhi"                    ,mytemp,500);

  hmuCorrThetaA->getText(mytemp);
  l->add("hmuCorrTheta"                  ,mytemp,500);
