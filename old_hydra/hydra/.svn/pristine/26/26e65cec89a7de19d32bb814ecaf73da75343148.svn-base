...
// HHypPipPimAlg
//
// HHypPipPim is an algorithm which selects PP (+X) events.
...
#include <stdlib.h>
#include <iostream>
#include "hhypPipPimAlg.h"
#include "hypinfodef.h"

ClassImp(HHypPipPimAlg)

HHypPipPimAlg::HHypPipPimAlg(Char_t *name_i, Option_t par[])
...
Bool_t HHypPipPimAlg::execute()
{
	...
	if( use_InclusiveFiller){
		...
		else cerr << " HHypPipPimAlg::execute() : exitIdx = "<< ERROR_IDX;
...
Bool_t HHypPipPimAlg::init()
{
	...
	// 2 + 1 part filler
	if(use_2_1){
		cout << "HHypPipPimAlg will use option ONEFAKE" << endl;
	...
	// 2 + 2 part filler
	if(use_2_2){
		cout << "HHypPipPimAlg will use option TWOFAKES" << endl;
...
Bool_t HHypPipPimAlg::reinit()
{
  return kTRUE;
}

Bool_t HHypPipPimAlg::finalize()
{
  return kTRUE;
}
