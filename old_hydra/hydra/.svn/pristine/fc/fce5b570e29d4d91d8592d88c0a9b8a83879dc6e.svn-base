// File: hrichtrackletfiltersim.cc
//
// Author: Thomas Eberl <Thomas.Eberl@physik.tu-muenchen.de>
// Last update by Thomas Eberl: 02/02/09 00:24:49
//
...
HRichTrackletFilterSim::HRichTrackletFilterSim(Text_t *name,Text_t *title) :
...
Bool_t HRichTrackletFilterSim::isDoubleRingEvt()
{
	...
	if ( nRInd[idx] > -1 ) // not a default value
	{
		...
		hasPair=kTRUE;
		break;
		//((HHitMatchSim*)getHitMatchCat()->getObject(nHMInd[idx]))
		//->dumpToStdout();
	    }
	}
    }
...
Bool_t HRichTrackletFilterSim::removeSingleRing()
{
    HHitMatchSim *pHM=0;
    pIterMatchHit->Reset();
    Int_t nHM = ((HLinearCategory*)getHitMatchCat())->getEntries();
...
{
   //   if (HRichTrackletFilter::execute())
//      {

//      } //endif HRichTrackletFilter::execute
   
    filterTrackletIndividual(2);
    Int_t kReturnValue=0;
    if (isFilteredEvt(1)) kReturnValue = kSkipEvent;
    else if (isSkipNextTask(0)) kReturnValue = 1;
    return kReturnValue;
}
Bool_t HRichTrackletFilterSim::isFilteredEvt(Int_t swt)
{
...
Bool_t HRichTrackletFilterSim::isEmptyEvt()
{
    Bool_t r=kTRUE;
    HHitMatch *pHM=0;
...
Bool_t HRichTrackletFilterSim::isSkipNextTask(Int_t swt)
{
    Bool_t kSwitch=kFALSE;
